<?php
/**
 * @version			$Id: j32standard_install.php 432 2014-10-23 16:12:50Z BrianWade $
 * @name			Joomla! 3.2 Standard Code Template (Release 1.0.4)
 * @author			Component Architect (www.componentarchitect.com)
 * @package			componentarchitect
 * @subpackage		componentarchitect.cacodetemplates.j32standard
...
/**
 * Script file of Component Architect code templates J32 Standard plugin
 */
class plgCACodeTemplatesj32standardInstallerScript
{
    /**
    ...
    function install($parent) 
    {
		...
		$query->set($db->quoteName('name').' = '.$db->quote('J 3.2 Standard'));
		$query->set($db->quoteName('catid').' = '.(int) $category_id);
		$query->set($db->quoteName('description').' = '.$db->quote(JText::_('PLG_CACODETEMPLATES_J32STANDARD_CODETEMPLATE_DESCRIPTION')));
		...
		$query->set($db->quoteName('source_path').' = '.$db->quote('j_3_2_standard'));
		...
		$query->set($db->quoteName('platform_version').' = '.$db->quote('3.2'));
		...
		{
			JFactory::getApplication()->enqueueMessage(JText::sprintf('PLG_CACODETEMPLATES_J32STANDARD_ERROR_CODETEMPLATE_INSERT', $e->getMessage()), 'warning');
			return false;
		}
		
          
        // Store the record id and folder location for this code template plugin 
        $param_array = array('codetemplate_record_id' => $db->insertid(), 'codetemplate_folder_path' => 'administrator/components/com_componentarchitect/codetemplates/j_3_2_standard');   
    ...
    function uninstall($parent) 
    {
		...
		{
			JFactory::getApplication()->enqueueMessage(JText::sprintf('PLG_CACODETEMPLATES_J32STANDARD_ERROR_CODETEMPLATE_DELETE', $e->getMessage()), 'warning');
			$return = false;
		}		
		
		try
		{
			JFolder::delete(JPATH_SITE.'/administrator/components/com_componentarchitect/codetemplates/j_3_2_standard');
		}
		catch (RuntimeException $e)
		{
			JFactory::getApplication()->enqueueMessage(JText::_('PLG_CACODETEMPLATES_J32STANDARD_ERROR_CODETEMPLATE_FILE_DELETE'), 'warning');
			$return = false;
		}	
    ...
    function update($parent) 
    {
		...
		{
			JFactory::getApplication()->enqueueMessage(JText::sprintf('PLG_CACODETEMPLATES_J32STANDARD_ERROR_CODETEMPLATE_UPDATE', $e->getMessage()), 'warning');
			return false;
		}
   ...
   /**
    ...
    function postflight($type, $parent) 
    {
		...
		{
			case 'install' :
				if (!JFolder::exists(JPATH_SITE.'/administrator/components/com_componentarchitect/codetemplates/j_3_2_standard'))
				{
					// Now need to copy the code template files to the com_componentarchitect/codetemplates folder
					$result = JFolder::create(JPATH_SITE.'/administrator/components/com_componentarchitect/codetemplates/j_3_2_standard');
					if ($result !== true) 
					{
						JFactory::getApplication()->enqueueMessage(JText::_('PLG_CACODETEMPLATES_J32STANDARD_ERROR_CODETEMPLATE_FILE_COPY').' - '.$result, 'warning');
						return false;
					}
				}
				...
				{
					JFile::copy('plugins/cacodetemplates/j32standard/index.html','plugins/cacodetemplates/index.html', JPATH_SITE);
				}
				JFile::copy('plugins/cacodetemplates/j32standard/index.html','administrator/components/com_componentarchitect/codetemplates/j_3_2_standard/index.html', JPATH_SITE);
				$zip_adapter = JArchive::getAdapter('zip');
				$result = $zip_adapter->extract(JPATH_SITE.'/plugins/cacodetemplates/j32standard/com_architectcomp_j32standard.zip', JPATH_SITE.'\administrator\components\com_componentarchitect\codetemplates\j_3_2_standard\com_architectcomp_j32standard');
	...
	private function getManifest()
	{
		...
		$query->where($db->quoteName('name').' = '.$db->quote('plg_cacodetemplates_j32standard'));		
