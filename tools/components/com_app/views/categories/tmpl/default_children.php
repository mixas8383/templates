...
<?php foreach($this->children[$this->parent->id] as $id => $child) : ?>
	...
	if ($this->params->get('show_empty_categories') OR $child->getNumItems(true) OR count($child->getChildren())) :
		...
		<li<?php echo $class; ?>>
		<?php $class = ''; ?>
			<span class="item-title"><a href="<?php echo JRoute::_(AppHelperRoute::getCategoryRoute($child->id, $this->params->get('keep_item_itemid'), $child->language));?>">
				<?php echo $this->escape($child->title); ?></a>
			</span>
			<?php if ($this->params->get('show_subcategories_desc') == 1) :?>
				<?php if ($child->description) : ?>
					<div class="category-desc">
						<?php echo JHtml::_('content.prepare', $child->description, '', 'com_app.categories'); ?>
					</div>
				<?php endif; ?>
			<?php endif; ?>
			<?php if ($this->params->get('items_to_display') AND 
				...
				<dl>
					<dt>
						<?php echo JText::_('COM_APP_'.JString::strtoupper(str_replace(' ','',$this->params->get('items_to_display'))).'_NUM_ITEMS') ; ?>
					</dt>
					<dd>
