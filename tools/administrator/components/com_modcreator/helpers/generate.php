...
class ModcreatorGenerateHelper
{
	...
	*/ 
	...
	{
		$this->_progress = new ModcreatorGenerateProgressHelper();
	}		
	/**
	 * Generate component
	 *
	...
	{	
		...
		$this->_search_replace_helper = new ModcreatorSearchReplaceHelper($this->_progress);
		...
		{
			$code_template_model = JModelLegacy::getInstance('codetemplate', 'ModcreatorModel', array('ignore_request' => true));//ERROR TODO TODO++
		...
		if ($component_id > 0)
		{
			...
			$component_model = JModelLegacy::getInstance('component', 'ModcreatorModel', array('ignore_request' => true));
		...
		*/
		...
		if ($this->_component->icon_48px != '')
		{
			...
			ModcreatorHelper::createThumb(JPATH_SITE.'/'.$this->_component->icon_48px, $dst_path.'/media/images/icon-32-'.$image_name, 32, 32);
			ModcreatorHelper::createThumb(JPATH_SITE.'/'.$this->_component->icon_48px, $dst_path.'/media/images/icon-24-'.$image_name, 24, 24);
		}
		...
		if ($this->_search_replace_helper->getComponentConditions('generate_categories'))
		{
			...
			if ($this->_component->categories_icon_48px != '')
			{
				...
				ModcreatorHelper::createThumb(JPATH_SITE.'/'.$this->_component->categories_icon_48px, $dst_path.'/media/images/icon-32-categories.png', 32, 32);
				ModcreatorHelper::createThumb(JPATH_SITE.'/'.$this->_component->categories_icon_48px, $dst_path.'/media/images/icon-24-categories.png', 24, 24);
			}
		}
		...
		foreach ($this->_component_objects as $component_object)
		{
			...
			if ($component_object->icon_48px != '')
			{
				...
				ModcreatorHelper::createThumb(JPATH_SITE.'/'.$component_object->icon_48px, $dst_path.'/media/images/icon-32-'.$image_name, 32, 32);
				ModcreatorHelper::createThumb(JPATH_SITE.'/'.$component_object->icon_48px, $dst_path.'/media/images/icon-24-'.$image_name, 24, 24);
			}		
		}
	...
	protected function _getComponentObjects($component_id, $default_object_id, $template_component_name, $template_object_name)
	{
		...
		$component_objects_model = JModelLegacy::getInstance('componentobjects', 'ModcreatorModel', array('ignore_request' => true));
	...
	protected function _getChildComponentObjects(&$component_object, $default_object_id, $template_component_name, $template_object_name)
	{
		...
		$fields_model = JModelLegacy::getInstance('fields', 'ModcreatorModel', array('ignore_request' => true));
		...
		if (count($cascade_fields) > 0)
		{
			foreach ($cascade_fields as $cascade_field)
			{
				...
				$child_component_object_model = JModelLegacy::getInstance('componentobject', 'ModcreatorModel', array('ignore_request' => true));
	...
	protected function _populateComponentObject(&$component_object, $default_object_id, $template_component_name, $template_object_name)		
	{
		...
		if ($component_object->plural_name == '')
		{
			...
			$component_object->plural_name = ModcreatorPluraliseHelper::pluralise($component_object->name);
		}
		else
		{
	...
	{	
		...
		$fieldsets_model = JModelLegacy::getInstance('fieldsets', 'ModcreatorModel', array('ignore_request' => true));
		...
		foreach ($fields as $field)
		{
			...
			{		
				...
				if (isset($field->foreign_object_id) AND $field->foreign_object_id != 0)
				{
					...
					$foreignobjectmodel = JModelLegacy::getInstance('componentobject', 'ModcreatorModel', array('ignore_request' => true));
	...
	{	
		...
		$field_types_model = JModelLegacy::getInstance('fieldtypes', 'ModcreatorModel', array('ignore_request' => true));
