...
class [%%ArchitectComp%%]View[%%CompObjectPlural%%] extends JView
{
	...
	function display($tpl = null)
	{
		...
		// Compute the [%%compobject%%] slugs and se the trigger events.
		foreach ($items as $i => &$item)
		{
			...
			$results = $dispatcher->trigger('on[%%CompObject%%]Prepare', array ('[%%com_architectcomp%%].[%%compobject%%]', &$item, &$item->params,$i));
		...
		{			
			...
			$num_leading = $params->def('[%%compobject%%]_num_leading', 1);
			$num_intro = $params->def('[%%compobject%%]_num_intro', 4);
			$num_links = $params->def('[%%compobject%%]_num_links', 4);
