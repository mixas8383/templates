 ...
 * 
 * @version			$Id: finder.php 418 2014-10-22 14:42:36Z BrianWade $
...
[%%FOREACH COMPONENT_OBJECT%%]
	...
	/**
	 * Finder after save [%%compobject_name%%] method
	 ...
	 * @param	object		A JTable[%%CompObject%%] object
	 * @param	bool		If the [%%compobject_name%%] has just been created
	 */
	public function on[%%CompObject%%]AfterSave($context, $[%%compobject_code_name%%], $is_new)
	{
		...
		$results = $dispatcher->trigger('onFinderAfterSave', array($context, $[%%compobject_code_name%%], $is_new));

	}
	/**
	 * Finder before save [%%compobject_name%%] method
	 * [%%CompObject_name%%] is passed by reference, but after the save, so no changes will be saved.
	 * Method is called right after the content is saved
	 *
	 * @param	string		The context of the item passed to the plugin (added in 1.6)
	 * @param	object		A JTable[%%CompObject%%] object
	 * @param	bool		If the [%%compobject_name%%] is just about to be created
	 */
	public function on[%%CompObject%%]BeforeSave($context, $[%%compobject_code_name%%], $is_new)
	{
		...
		$results = $dispatcher->trigger('onFinderBeforeSave', array($context, $[%%compobject_code_name%%], $is_new));

	}
	/**
	 * Finder after delete [%%compobject_name%%] method
	 * [%%compobject_name%%] is passed by reference, but after the save, so no changes will be saved.
	 * Method is called right after the [%%compobject_name%%] is saved
	 *
	 * @param	string		The context of the item passed to the plugin (added in 1.6)
	...
	public function on[%%CompObject%%]AfterDelete($context, $[%%compobject_code_name%%])
	{
		...
		$results = $dispatcher->trigger('onFinderAfterDelete', array($context, $[%%compobject_code_name%%]));

	}
			[%%IF INCLUDE_STATUS%%]
	/**
	 * Finder change state [%%compobject_name%%] method
