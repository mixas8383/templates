...
class [%%ArchitectComp%%]Model[%%CompObjectPlural%%] extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		$query->select('CASE WHEN a.modified = ' . $null_date . ' THEN a.created ELSE a.modified END AS modified');
		...
		if (!$can_publish)
		{
			...
			$query->select('CASE WHEN a.publish_up = ' . $null_date . ' THEN a.created ELSE a.publish_up END as publish_up');
			[%%ELSE INCLUDE_CREATED%%]			
