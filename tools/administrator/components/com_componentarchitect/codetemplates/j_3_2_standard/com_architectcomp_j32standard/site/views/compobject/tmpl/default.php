...
<div class="[%%architectcomp%%] [%%compobject%%]-view<?php echo $params->get('pageclass_sfx')?>">
	...
	<?php if ($params->get('show_[%%compobject%%]_hits') != '0' OR 
		...
		[%%IF INCLUDE_HITS%%]
			<?php
				if ($params->get('show_[%%compobject%%]_hits')) : 
					echo '<br />'.JText::sprintf('[%%COM_ARCHITECTCOMP%%]_HITS', $this->item->hits); 
				endif;
			?>	
		[%%ENDIF INCLUDE_HITS%%]
		</div>
		...
		$display_fieldset = (
			...
			<?php if ($params->get('show_[%%compobject%%]_category')) : ?>
				...
				<div class="formelm">				
					...
					<span>
						<?php
							if ($params->get('link_[%%compobject%%]_category') AND $this->item->catslug) :
								echo $url;
							else :
								echo $title;
							endif;
