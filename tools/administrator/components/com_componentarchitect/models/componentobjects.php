...
/**
 * Methods supporting a list of componentobject records.
 *
 */
class ComponentArchitectModelComponentObjects extends JModelList
{
	/**
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$component_id = $this->getUserStateFromRequest($this->context.'.filter.component_id', 'filter_component_id', 0, 'int');
	...
	protected function getListQuery()
	{
		...
		// Select the required objects from the table.
		$query->select(
			$this->getState(
					'list.select',
					'a.*'
		...
		// Filter by and return name for default_fieldset_id level.
		$query->select($db->quoteName('f.name').' AS f_fieldset_name');
		$query->select($db->quoteName('f.ordering').' AS f_fieldset_ordering');

		$query->join('LEFT', '#__componentarchitect_fieldsets AS f ON f.id = a.default_fieldset_id');	
	...
	public function getItems()
	{
		if ($items = parent::getItems())
		{
			...
			foreach ($items as $item)
			{
				...
				$query->where($db->quoteName('a.component_object_id').' = ' . $db->quote($item->id));
	...
	public function getCreators()
	{
		...
		$query->join('INNER', '#__componentarchitect_componentobjects AS a ON a.created_by = u.id');
	...
	public function getC1Components()
	{
		...
		$query->join('INNER', $db->quoteName('#__componentarchitect_componentobjects').' AS a ON '.$db->quoteName('a.component_id').' = '.$db->quoteName('c1.id'));
