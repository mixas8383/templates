...
/**
 * View class for a list of componentobjects.
 *
 */
class ComponentArchitectViewComponentObjects extends JViewLegacy
{
	protected $items;
	...
	protected function addToolbar()
	{
		...
		JToolbarHelper::title(JText::_('COM_COMPONENTARCHITECT_COMPONENTOBJECTS_LIST_HEADER'), 'stack componentobjects');

		JToolbarHelper::addNew('componentobject.add','JTOOLBAR_NEW');
		
		JToolbarHelper::editList('componentobject.edit','JTOOLBAR_EDIT');
		
		JToolbarHelper::custom('componentobjects.copy', 'copy.png', 'copy_f2.png','COM_COMPONENTARCHITECT_COPY', true);		

			JToolbarHelper::divider();
		
		
		JToolbarHelper::custom('componentobjects.checkin', 'checkin.png', 'checkin_f2.png', 'JTOOLBAR_CHECKIN', true);

		JToolbarHelper::deleteList('', 'componentobjects.delete','JTOOLBAR_DELETE');
		JToolbarHelper::divider();
		
		JToolbarHelper::preferences('com_componentarchitect');
		
		$params = $this->state->get('params');
		if ($params->get('local_remote_help') == '1' OR filter_var($params->get('help_url',''), FILTER_VALIDATE_URL) === false)
		{
			JToolbarHelper::help('JHELP_COMPONENTS_COMPONENTARCHITECT_COMPONENTOBJECTS',true);
		}
		else
		{
			$lang = JFactory::getLanguage();
			JToolbarHelper::help('JHELP_COMPONENTS_COMPONENTARCHITECT_COMPONENTOBJECTS',false, $params->get('help_url').$lang->getTag().'/'.JText::_('JHELP_COMPONENTS_COMPONENTARCHITECT_COMPONENTOBJECTS').'.html');
		}
	...
	{	
		...
		JHtmlSidebar::setAction('index.php?option=com_componentarchitect&view=componentobjects');
	...
	protected function getSortFields()
	{
		return array(
			...
			'c1_component_name' => JText::_('COM_COMPONENTARCHITECT_COMPONENTOBJECTS_HEADING_COMPONENT_ID'),
