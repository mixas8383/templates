...
{
	$save_ordering_url = 'index.php?option=com_componentarchitect&task=fieldsets.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'fieldset-list', 'adminForm', strtolower($list_dirn), $save_ordering_url);
}
...
<form action="<?php echo JRoute::_('index.php?option=com_componentarchitect&view=fieldsets'); ?>" method="post" name="adminForm" id="adminForm">
			...
			<?php endif; ?>				
					...
					<?php endif; ?>				
							<select name="filter_component_id" class="input-medium" onchange="this.form.submit()">
								<option value=""><?php echo JText::_('COM_COMPONENTARCHITECT_FIELDSETS_SELECT_C1_COMPONENT');?></option>
					...
					<?php endif; ?>				
							<select name="filter_component_object_id" class="input-medium" onchange="this.form.submit()">
								<option value=""><?php echo JText::_('COM_COMPONENTARCHITECT_FIELDSETS_SELECT_O_COMPONENT_OBJECT');?></option>
					...
					<?php if (version_compare(JVERSION, '3.2', 'ge')) : ?>
						<div class="js-stools-field-filter">	
					<?php endif; ?>				
							<select name="filter_predefined_fieldset" class="input-medium" onchange="this.form.submit()">
								<option value=""><?php echo JText::_('COM_COMPONENTARCHITECT_FIELDSETS_SELECT_PREDEFINED_FIELDSET');?></option>
								<?php echo JHtml::_('select.options', $this->predefined_fieldset_values, 'value', 'text', $this->state->get('filter.predefined_fieldset'));?>
							</select>	
					<?php if (version_compare(JVERSION, '3.2', 'ge')) : ?>
						</div>	
					<?php endif; ?>				
	...
	<?php if (empty($this->items)) : ?>
		<div class="alert alert-no-items">
			<?php echo JText::_('COM_COMPONENTARCHITECT_FIELDSETS_NO_MATCHING_RESULTS'); ?>
		</div>
	<?php else : ?>
		<table class="<?php echo version_compare(JVERSION, '3.0', 'ge') ? 'table table-striped':'adminlist'; ?>" id="fieldset-list">
			<thead>
				<tr>
					<?php if (version_compare(JVERSION, '3.0', 'ge')) : ?>
					...
					<th width="10%" class="nowrap center hidden-phone">
						<?php echo JHtml::_('grid.sort', 'COM_COMPONENTARCHITECT_FIELDSETS_HEADING_COMPONENT_ID', 'c1_component_name', $list_dirn, $list_order); ?>
					</th>	
					<th width="10%" class="nowrap center hidden-phone">
						<?php echo JHtml::_('grid.sort', 'COM_COMPONENTARCHITECT_FIELDSETS_HEADING_COMPONENT_OBJECT_ID', 'o_component_object_name', $list_dirn, $list_order); ?>
					</th>
					<th class="nowrap center hidden-phone">
						<a><?php echo JText::_('COM_COMPONENTARCHITECT_FIELDS') ?></a>
					</th>
					<th width="10%" class="nowrap center hidden-phone">
						<?php echo JHtml::_('grid.sort', 'COM_COMPONENTARCHITECT_FIELDSETS_HEADING_PREDEFINED_FIELDSET', 'a.predefined_fieldset', $list_dirn, $list_order); ?>
					</th>	
					...
					<?php if (version_compare(JVERSION, '3.1', 'lt')) :?>
						<th width="10%" class="center">
							<?php echo JHtml::_('grid.sort',  'JGRID_HEADING_ORDERING', 'a.ordering', $list_dirn, $list_order); ?>
								<?php if ($save_order): ?>					
									<?php echo JHtml::_('grid.order',  $this->items, 'filesave.png', 'fieldsets.saveorder'); ?>
								<?php endif;?>
						</th>
						<?php endif; ?>
					<th width="1%" class="nowrap center">
				...
				<tr class="row<?php echo $i % 2; ?>">
					...
					<td class="nowrap has-context">
						...
						<?php if ($item->checked_out) : ?>
							<?php echo JHtml::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'fieldsets.', $can_checkin); ?>
						<?php endif; ?>

							<a href="<?php echo JRoute::_('index.php?option=com_componentarchitect&task=fieldset.edit&id='.(int) $item->id); ?>">
					...
					<td class="nowrap center">
						<?php 
							$link = JRoute::_('index.php?option=com_componentarchitect&view=fields&'.$fieldset_filter.'='.$item->id.'&'.$componentobject_filter.'='.$item->component_object_id.'&'.$component_filter.'='.$item->component_id.'&'.$predefined_field_filter.'=');
							...
							if (!empty($count)) :
								...
								echo '<span class="badge hasTooltip" '.$style.' title="'.JText::_('COM_COMPONENTARCHITECTFIELDS_COUNT_ZERO_DESC').'">0</span></a>';
							endif;
						?>
						<?php 
							...
							$link = JRoute::_('index.php?option=com_componentarchitect&view=fields&'.$fieldset_filter.'=' . $item->id.'&'.$componentobject_filter.'='.$item->component_object_id.'&'.$component_filter.'='.$item->component_id.'&'.$predefined_field_filter.'=0');
					...
					<td class="nowrap small center hidden-phone">
						<?php 
								switch ($item->predefined_fieldset) :
									case '0':
										echo JText::_('JNO');
										break;
									case '1':
						...
						<td class="order">
								<?php if ($save_order) : ?>
									...
									<?php if ($list_dirn == 'asc') : ?>
										<span><?php echo $this->pagination->orderUpIcon($i,$condition_up,'fieldsets.orderup', 'JLIB_HTML_MOVE_UP', $save_order); ?></span>
										<span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total,$condition_down, 'fieldsets.orderdown', 'JLIB_HTML_MOVE_DOWN', $save_order); ?></span>
									<?php elseif ($list_dirn == 'desc') : ?>
										<span><?php echo $this->pagination->orderUpIcon($i, $condition_up, 'fieldsets.orderdown', 'JLIB_HTML_MOVE_UP', $save_order); ?></span>
										<span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total,$condition_down, 'fieldsets.orderup', 'JLIB_HTML_MOVE_DOWN', $save_order); ?></span>
									<?php endif; ?>
									</div>
								<?php endif; ?>
