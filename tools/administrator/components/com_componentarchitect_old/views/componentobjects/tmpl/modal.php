...
$function	= $app->input->get('function', 'jSelectComponentObject');
...
?>
<h3><?php echo JText::_('COM_COMPONENTARCHITECT_COMPONENTOBJECTS_SELECT_ITEM_LABEL'); ?></h3>
<form action="<?php echo JRoute::_('index.php?option=com_componentarchitect&view=componentobjects&layout=modal&tmpl=component&function='.$function.'&'.JSession::getFormToken().'=1');?>" method="post" name="adminForm" id="adminForm">
	...
	<?php else : ?>	
		<table class="<?php echo version_compare(JVERSION, '3.0', 'ge') ? 'table table-striped table-condensed':'adminlist'; ?>">
			...
			<?php foreach ($this->items as $i => $item) : ?>
				...
				<!--[%%START_CUSTOM_CODE%%]-->
					<td>
						<a class="pointer" href="javascript:void(0)" onclick="if (window.parent) window.parent.<?php echo $this->escape($function);?>('<?php echo $item->id; ?>', '<?php echo $this->escape(addslashes($item->name)); ?>', '<?php echo $item->component_id; ?>', '<?php echo $item->c1_component_name; ?>');">
