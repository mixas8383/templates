...
<script type="text/javascript">
	...
	//[%%START_CUSTOM_CODE%%]
	jQuery('#toolbar-cogs button').addClass('btn-primary');
	//[%%END_CUSTOM_CODE%%]	
</script>
<?php endif; ?>
<script type="text/javascript">
...
<form action="<?php echo JRoute::_('index.php?option=com_componentarchitect&view=components'); ?>" method="post" name="adminForm" id="adminForm">
	...
	<?php else : ?>
		<table class="<?php echo version_compare(JVERSION, '3.0', 'ge') ? 'table table-striped':'adminlist'; ?>" id="component-list">
			<thead>
				<tr>
					<?php if (version_compare(JVERSION, '3.0', 'ge')) : ?>
					...
					<th class="nowrap center hidden-phone">
						<a><?php echo JText::_('COM_COMPONENTARCHITECT_COMPONENTOBJECTS') ?></a>
					</th>
					<!-- //[%%START_CUSTOM_CODE%%] -->
					<th class="nowrap center hidden-phone">
			...
			<?php foreach ($this->items as $i => $item) :
				...
				<tr class="row<?php echo $i % 2; ?>">
					...
					<td class="nowrap center">
						<?php 
							$link = JRoute::_('index.php?option=com_componentarchitect&view=componentobjects&'.$component_filter.'='.$item->id);
							$count = (int) $item->componentobjects_count;
							if (!empty($count)) :
								echo '<a href="'.$link.'"><span class="badge badge-info hasTooltip" '.$style.' title="'.JText::_('COM_COMPONENTARCHITECT_COMPONENTOBJECTS_COUNT_DESC').'">'.$count.'</span></a>';
							else :
								echo '<span class="badge hasTooltip" '.$style.' title="'.JText::_('COM_COMPONENTARCHITECT_COMPONENTOBJECTS_COUNT_ZERO_DESC').'">0</span></a>';
							endif;
						?>
					</td>
					<td class="nowrap center">
						<?php 
							$link = JRoute::_('index.php?option=com_componentarchitect&view=fieldsets&'.$component_filter.'='.$item->id.'&'.$componentobject_filter.'=');
					...
					<td class="nowrap center">
						<?php 
							$link = JRoute::_('index.php?option=com_componentarchitect&view=fields&'.$component_filter.'='.$item->id.'&'.$componentobject_filter.'='.'&'.$fieldset_filter.'='.'&'.$predefined_field_filter.'=');
						...
						<?php 
							...
							$link = JRoute::_('index.php?option=com_componentarchitect&view=fields&'.$component_filter.'='.$item->id.'&'.$componentobject_filter.'='.'&'.$fieldset_filter.'='.'&'.$predefined_field_filter.'=0');
						...
						<td class="order">
								<?php if ($save_order) : ?>
									...
									<?php if ($list_dirn == 'asc') : ?>
										<span><?php echo $this->pagination->orderUpIcon($i,$condition_up,'components.orderup', 'JLIB_HTML_MOVE_UP', $save_order); ?></span>
										<span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total,$condition_down, 'components.orderdown', 'JLIB_HTML_MOVE_DOWN', $save_order); ?></span>
									<?php elseif ($list_dirn == 'desc') : ?>
										<span><?php echo $this->pagination->orderUpIcon($i, $condition_up, 'components.orderdown', 'JLIB_HTML_MOVE_UP', $save_order); ?></span>
										<span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total,$condition_down, 'components.orderup', 'JLIB_HTML_MOVE_DOWN', $save_order); ?></span>
									<?php endif; ?>
									</div>
								<?php endif; ?>
