...
/**
 * View to edit a fieldset.
 *
 */
class ComponentArchitectViewFieldset extends JViewLegacy
{
	protected $form;
	protected $item;
	protected $state;
	...
	protected function addToolbar()
	{
		...
		//[%%START_CUSTOM_CODE%%]
		$is_predefined	= ($this->item->predefined_fieldset == '1');
		//[%%END_CUSTOM_CODE%%]
		
		JToolbarHelper::title($is_new ? JText::_('COM_COMPONENTARCHITECT_FIELDSETS_NEW_HEADER') : JText::_('COM_COMPONENTARCHITECT_FIELDSETS_EDIT_HEADER'), 'fieldsets.png');
		
		//[%%START_CUSTOM_CODE%%]
		// A pre-defined fieldset cannot be changed from the edit form
		if (!$is_predefined)	
		{
		//[%%END_CUSTOM_CODE%%]			
		...
		else
		{
			...
			$app->enqueueMessage(JText::_('COM_COMPONENTARCHITECT_FIELDSETS_ERROR_PREDEFINED_FIELDSET_CANNOT_CHANGE'), 'warning');
		}
		//[%%END_CUSTOM_CODE%%]
		...
		JToolbarHelper::help('JHELP_COMPONENTS_COMPONENTARCHITECT_FIELDSET_EDIT', true, null, 'com_componentarchitect');
	}
	/**
