...
{
	$save_ordering_url = 'index.php?option=com_componentarchitect&task=codetemplates.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'codetemplate-list', 'adminForm', strtolower($list_dirn), $save_ordering_url);
}
...
<form action="<?php echo JRoute::_('index.php?option=com_componentarchitect&view=codetemplates'); ?>" method="post" name="adminForm" id="adminForm">
	...
	<?php endif; ?>	
			...
			<?php endif; ?>				
					...
					<?php endif; ?>				
							<select name="filter_predefined_code_template" class="input-medium" onchange="this.form.submit()">
								<option value=""><?php echo JText::_('COM_COMPONENTARCHITECT_CODETEMPLATES_SELECT_PREDEFINED_CODE_TEMPLATE');?></option>
								<?php echo JHtml::_('select.options', $this->predefined_code_template_values, 'value', 'text', $this->state->get('filter.predefined_code_template'));?>
							</select>	
					<?php if (version_compare(JVERSION, '3.2', 'ge')) : ?>
						</div>	
					<?php endif; ?>				
	...
	<?php if (empty($this->items)) : ?>
		<div class="alert alert-no-items">
			<?php echo JText::_('COM_COMPONENTARCHITECT_CODETEMPLATES_NO_MATCHING_RESULTS'); ?>
		</div>
	<?php else : ?>
		<table class="<?php echo version_compare(JVERSION, '3.0', 'ge') ? 'table table-striped':'adminlist'; ?>" id="codetemplate-list">
			<thead>
				<tr>
					<?php if (version_compare(JVERSION, '3.0', 'ge')) : ?>
					...
					<th width="10%" class="nowrap center hidden-phone">
						<?php echo JHtml::_('grid.sort', 'COM_COMPONENTARCHITECT_CODETEMPLATES_HEADING_PREDEFINED_CODE_TEMPLATE', 'a.predefined_code_template', $list_dirn, $list_order); ?>
					</th>	
				...
				<tr class="row<?php echo $i % 2; ?>" sortable-group-id="<?php echo $item->catid; ?>">
					...
					<td class="nowrap has-context">
						...
						<?php if ($item->checked_out) : ?>
							<?php echo JHtml::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'codetemplates.', $can_checkin); ?>
						<?php endif; ?>

							<a href="<?php echo JRoute::_('index.php?option=com_componentarchitect&task=codetemplate.edit&id='.(int) $item->id); ?>">
							<?php echo $this->escape($item->name); ?></a>
						</div>
					</td>
					<td class="nowrap small center hidden-phone">
						<?php 
								switch ($item->predefined_code_template) :
									case '0':
										echo JText::_('JNO');
										break;
									case '1':
