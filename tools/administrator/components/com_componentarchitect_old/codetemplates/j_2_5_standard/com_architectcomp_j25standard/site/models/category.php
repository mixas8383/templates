 ...
 *
 * @version			$Id: category.php 418 2014-10-22 14:42:36Z BrianWade $
...
require_once dirname(__FILE__) . '/[%%compobjectplural%%].php';
...
class [%%ArchitectComp%%]ModelCategory extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$this->setState('list.[%%compobject%%]_filter', JRequest::getString('[%%compobject%%]-filter-search'));
		...
		$order_col	= JRequest::getCmd('filter_order', 'ordering');
				[%%ELSE INCLUDE_ORDERING%%]
					[%%IF INCLUDE_NAME%%]
		$order_col	= JRequest::getCmd('filter_order', 'name');
					[%%ELSE INCLUDE_NAME%%]
		$order_col	= JRequest::getCmd('filter_order', 'id');
					[%%ENDIF INCLUDE_NAME%%]		
				[%%ENDIF INCLUDE_ORDERING%%]
		...
		{
			$limit = $app->getUserStateFromRequest('[%%com_architectcomp%%].category.list.' . $item_id . '.limit', 'limit', $app->getCfg('list_limit'));
		}
	...
	public function getItems()
	{
		...
		if ($params->get('items_to_display') AND $params->get('items_to_display') !='None')
		{
			...
			{
				$model = JModel::getInstance($object_upper_case, '[%%ArchitectComp%%]Model', array('ignore_request' => true));
