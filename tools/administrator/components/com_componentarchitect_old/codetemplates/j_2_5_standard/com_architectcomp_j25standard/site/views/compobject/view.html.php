...
class [%%ArchitectComp%%]View[%%CompObject%%] extends JView
{
	...
	function display($tpl = null)
	{
		...
		if ($item->params->get('access-view') != true)
		{
			...
			if (!$this->params->get('show_[%%compobject%%]_noauth'))
			{
				...
				if( $user->get('guest'))
				{
					// Redirect to login
					$uri = JFactory::getURI();
					$app->redirect('index.php?option=com_users&view=login&return=' . base64_encode(urlencode($uri)), JText::_('[%%COM_ARCHITECTCOMP%%]_[%%COMPOBJECTPLURAL%%]_ERROR_LOGIN_TO_VIEW_ITEM'));
