 ...
 *
 * @version			$Id: compobjecticon.php 417 2014-10-22 14:42:10Z BrianWade $
...
class JHTML[%%CompObject%%]Icon
{
	...
	public static function edit($[%%compobject_code_name%%], $params, $attribs = array(), $legacy = false)
	{
		...
		{
			[%%IF INCLUDE_STATUS%%]
			$icon = $[%%compobject_code_name%%]->state ? 'edit.png' : 'edit_unpublished.png';
			[%%ELSE INCLUDE_STATUS%%]
			$icon = 'edit.png';
			[%%ENDIF INCLUDE_STATUS%%]
			$text = JHtml::_('image', 'system/' . $icon, JText::_('[%%COM_ARCHITECTCOMP%%]_[%%COMPOBJECTPLURAL%%]_EDIT_ITEM'), null, true);
	...
	public static function delete($[%%compobject_code_name%%], $params, $attribs = array(), $legacy = false)
	{
		...
		$url = 'index.php?option=[%%com_architectcomp%%]&task=[%%compobject%%].delete&cid='.$[%%compobject_code_name%%]->id.'&'.JSession::getFormToken().'=1'.'&return='.base64_encode($uri);
