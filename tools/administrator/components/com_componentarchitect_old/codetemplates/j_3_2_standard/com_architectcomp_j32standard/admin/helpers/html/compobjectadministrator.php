 ...
 *
 * @version			$Id: compobjectadministrator.php 417 2014-10-22 14:42:10Z BrianWade $
...
abstract class JHtml[%%CompObject%%]Administrator
{
	...
	public static function association($[%%compobject%%]_id)
	{
		...
		{		
			...
			[%%ENDIF GENERATE_CATEGORIES%%]
			$query->where('a.id IN ('.implode(',', array_values($associations)).')');
			$query->join('LEFT', '#__languages as l ON a.language=l.lang_code');
			$query->select('l.image');
			$query->select('l.title as language_title');
			$query->select('l.sef as lang_sef');
	...
	public static function featured($value = 0, $i, $can_change = true)
	{
		...
		$states	= array(
			0	=> array('unfeatured', '[%%compobjectplural%%].featured', '[%%COM_ARCHITECTCOMP%%]_[%%COMPOBJECTPLURAL%%]_UNFEATURED', '[%%COM_ARCHITECTCOMP%%]_[%%COMPOBJECTPLURAL%%]_TOGGLE_TO_FEATURE'),
			1	=> array('featured', '[%%compobjectplural%%].unfeatured', 'JFEATURED', '[%%COM_ARCHITECTCOMP%%]_[%%COMPOBJECTPLURAL%%]_TOGGLE_TO_UNFEATURE'),
		);
