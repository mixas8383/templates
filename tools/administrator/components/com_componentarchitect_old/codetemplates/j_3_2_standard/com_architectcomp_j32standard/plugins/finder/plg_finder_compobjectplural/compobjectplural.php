...
class PlgFinder[%%CompObjectPlural%%] extends FinderIndexerAdapter
{
	...
	protected function getListQuery($query = null)
	{
		...
		[%%ENDIF INCLUDE_NAME%%]		
		[%%IF INCLUDE_INTRO%%]
		$query->select('a.intro AS summary');
		[%%ENDIF INCLUDE_INTRO%%]
		[%%IF INCLUDE_DESCRIPTION%%]
		$query->select('a.description AS body');
		[%%ENDIF INCLUDE_DESCRIPTION%%]
		[%%IF INCLUDE_STATUS%%]
		...
		[%%ENDIF GENERATE_CATEGORIES%%]
		[%%IF INCLUDE_CREATED%%]
		$query->select('a.created AS start_date, a.created_by');
		[%%ENDIF INCLUDE_CREATED%%]
		[%%IF INCLUDE_MODIFIED%%]
		$query->select('a.modified, a.modified_by');
		[%%ENDIF INCLUDE_MODIFIED%%]
		[%%IF INCLUDE_METADATA%%]
		$query->select('a.metakey, a.metadesc'); 
		[%%ENDIF INCLUDE_METADATA%%]
		[%%IF INCLUDE_LANGUAGE%%]
