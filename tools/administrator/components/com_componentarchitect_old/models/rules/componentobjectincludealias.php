<?php
/**
 * @version 		$Id: componentobjectincludealias.php 411 2014-10-19 18:39:07Z BrianWade $
...
/**
 * Form Rule class for the Object/Table - Include Alias field.
 */
class JFormRulecomponentobjectincludealias extends JFormRule
{
	/**
	...
	public function test(SimpleXMLElement $element, $value, $group = null, JRegistry $input = null, JForm $form = null)
	{
		...
		else
		{
			...
			if ($input->get('component_id') > 0 AND ($value == '' OR $joomla_features['include_name'] == ''))
			{
				$db = JFactory::getDBO();
				...
				if ($value == '')
				{
					$include_alias = $component_joomla_features['include_alias'];
				}
				else
				{
					$include_alias = $value;
				}
				if ($joomla_features['include_name'] == '')
				{
					$include_name = $component_joomla_features['include_name'];
				}
				else
				{
					$include_name = $joomla_features['include_name'];
				}
				
				if ($include_name == '0' AND $include_alias == '1')
				{
					// Change to RuntimeException when Joomla! 2.5 no longer supported as JException (deprecated) and will be removed from Joomla
					return new JException(JText::_('COM_COMPONENTARCHITECT_COMPONENTOBJECTS_FIELD_INCLUDE_ALIAS_ERROR_NAME_NOT_SET'), 1, E_WARNING);
				}				
			}
			else
			{
				if ($joomla_features['include_name'] == '0' AND $value == '1')
				{
					// Change to RuntimeException when Joomla! 2.5 no longer supported as JException (deprecated) and will be removed from Joomla
					return new JException(JText::_('COM_COMPONENTARCHITECT_COMPONENTOBJECTS_FIELD_INCLUDE_ALIAS_ERROR_NAME_NOT_SET'), 1, E_WARNING);
				}
