...
$results = new modAcepollsHelper();
$results = $results->getResults($poll_id);
...
$voted = new modAcepollsHelper();
$voted = $voted->alreadyVoted($poll_id);

// check if the registrated user has voted and if user has logged in
$user = JFactory::getUser();
$userVoted = new modAcepollsHelper();
$userVoted = $userVoted->userVoted($user->id, $poll_id);
$guest = $user->guest;

// check if this ip has been recorder
$ipVoted = new modAcepollsHelper();
$ipVoted = $ipVoted->ipVoted($poll_id);
...
if ($poll && $poll->id) {
	...
	$tabcnt = 0;
	$helps = new modAcepollsHelper();
	$options = $helps->getPollOptions($poll_id);
	$itemid = $helps->getItemid($poll_id);
	require($layout);
}