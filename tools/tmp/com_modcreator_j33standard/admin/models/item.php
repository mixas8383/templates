 ...
 *
 */
class ModcreatorModelItem extends JModelAdmin
{
	/**
	 * @var		string	The prefix to use with controller messages.
	 */
	protected $text_prefix = 'COM_MODCREATOR_ITEMS';
	/**
	 * @var      string	The type alias for this object (for example, 'com_modcreator.item')
	 */
	public $typeAlias = 'com_modcreator.item';	
	/**
	 * @var		string	The context for the app call.
	 */
	protected $context = 'com_modcreator.items';	
	/**
	 ...
	 */
	public function getTable($type = 'Items', $prefix = 'ModcreatorTable', $config = array())
	{
	...
	protected function canDelete($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.delete', 'com_modcreator.item.'.(int) $record->id);
		}
		else
		{
			return $user->authorise('core.delete', 'com_modcreator');
		}							
	}
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.edit.state', 'com_modcreator.item.'.(int) $record->id);
		}
		else
		{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_modcreator.item');
			}	
		}
		...
		if ($assoc)
		{
			...
			if ($item->id != null)
			{
				$associations = JLanguageAssociations::getAssociations('com_modcreator', '#__modcreator_items', 'com_modcreator.item.item', $item->id, 'id', 'alias', null);
	...
	{
		$form = $this->loadForm('com_modcreator.edit.item', 'item', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = $app->getUserState('com_modcreator.edit.item.data', array());
	...
	{
		// Include the modcreator plugins for the onSave events.
		JPluginHelper::importPlugin('modcreator');	
		...
		if (parent::save($data))
		{
			...
			if ($assoc)
			{
				...
				{
					JError::raiseNotice(403, JText::_('COM_MODCREATOR_ERROR_ALL_LANGUAGE_ASSOCIATED'));
				}
				...
				try
				{
					...
					$query->where('context='.$db->quote('com_modcreator.item.item'));
				...
				if (!$all_language AND count($associations))
				{
					...
					{
						$query->values($id.','.$db->quote('com_modcreator.item.item') . ',' . $db->quote($key));
					}
	...
	{	
		// Include the modcreator plugins for the change of state event.
		JPluginHelper::importPlugin('modcreator');	
	...
	{
		// Include the modcreator plugins for the delete events.
		JPluginHelper::importPlugin('modcreator');	
	...
	public function featured($pks, $value = 0)
	{
		...
		if (empty($pks))
		{
			$this->setError(JText::_('COM_MODCREATOR_ITEMS_NO_ITEM_SELECTED'));
			return false;
		}
		...
		try
		{
			...
			$query = $db->getQuery(true);
			$query->update($db->quoteName('#__modcreator_items'));
	...
	/**
	 * Custom clean the cache of com_modcreator and modcreator modules
	 *
	 */
	protected function cleanCache($group = null, $client_id = 0)
	{
		parent::cleanCache('com_modcreator');
	...
	* @param   array   $data	The data where the original name is stored
	...
	protected function preprocessForm(JForm $form, $data, $group = 'item')
	{
		...
		if ($assoc)
		{
			...
			$fieldset->addAttribute('description', 'COM_MODCREATOR_ITEMS_ITEM_ASSOCIATIONS_FIELDSET_DESC');
			$add = false;
