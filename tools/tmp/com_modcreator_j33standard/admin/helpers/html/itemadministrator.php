...
JLoader::register('ContentHelper', JPATH_ADMINISTRATOR . '/components/com_modcreator/helpers/modcreator.php');
/**
 * Items component helper.
 *
 */
abstract class JHtmlItemAdministrator
{
	/**
	 * Render the list of associated items
	 * 
	 * @param	integer $item_id	The item item id
	 * 
	 * @return  string  The language HTML
     */
	public static function association($item_id)
	{
		...
		// Get the associations
		if ($associations = JLanguageAssociations::getAssociations('com_modcreator', '#__modcreator_items', 'com_modcreator.item.item',  $item_id, 'id', 'alias', null))
		{		
			...
			if ($items)
			{
				foreach ($items as &$item)
				{
					...
					$url = JRoute::_('index.php?option=com_modcreator&task=item.edit&id=' . (int) $item->id);
					$tooltip_parts = array(
	...
	public static function featured($value = 0, $i, $can_change = true)
	{
		...
		$states	= array(
			0	=> array('unfeatured', 'items.featured', 'COM_MODCREATOR_ITEMS_UNFEATURED', 'COM_MODCREATOR_ITEMS_TOGGLE_TO_FEATURE'),
			1	=> array('featured', 'items.unfeatured', 'JFEATURED', 'COM_MODCREATOR_ITEMS_TOGGLE_TO_UNFEATURE'),
		);
