 ...
 * @subpackage		com_nicegallery.search.nicegallery.votes
 * @copyright		
 ...
 *
 */
class PlgSearchVotes extends JPlugin
{
	/**
	...
	public function onContentSearch($text, $phrase='', $ordering='', $areas=null)
	{
		...
		$sVote	= $this->params->get('search_votes',1);
		$sVoteArchived		= $this->params->get('search_archived_votes',0);
		...
		$search_section = JText::_('PLG_SEARCH_VOTES_VOTES');
		// search votes
		if ($sVote AND $limit > 0)
		{
			$query->clear();
			...
			if (isset($list))
			{
				...
				{
					$list[$key]->href = JRoute::_(NicegalleryHelperRoute::getVoteRoute($item->slug, $item->language, $this->layout, $keep_item_id));
		...
		// search archived votes
		if ($sVoteArchived AND $limit > 0)
		{
			$query->clear();
			...
			$item	= $app->getMenu()->getItems('link', 'index.php?option=com_nicegallery&view=votearchive', true);
			...
			if (isset($list3))
			{
				foreach($list3 as $key => $item)
				{
					...
					$list3[$key]->href	= JRoute::_('index.php?option=com_nicegallery&view=votearchive'.
		...
		if (count($rows))
		{
			foreach($rows as $row)
			{
				...
				foreach($row as $vote)
				{
					if (SearchHelper::checkNoHTML($vote, $search_text, array('title',
																						'description',
																						'metadesc', 
																						'metakey'
																						)))
