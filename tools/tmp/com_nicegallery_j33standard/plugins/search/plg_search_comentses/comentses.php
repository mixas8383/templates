 ...
 * @subpackage		com_nicegallery.search.nicegallery.comentses
 * @copyright		
...
/**
 * Comentses Search plugin
 *
 */
class PlgSearchComentses extends JPlugin
{
	/**
	...
	{
		static $areas = array(
			'comentses' => 'PLG_SEARCH_COMENTSES_COMENTSES'
			);
			return $areas;
	}

	/**
	 * Comentses Search method
	...
	public function onContentSearch($text, $phrase='', $ordering='', $areas=null)
	{
		...
		$sComents	= $this->params->get('search_comentses',1);
		$sComentsArchived		= $this->params->get('search_archived_comentses',0);
		...
		$search_section = JText::_('PLG_SEARCH_COMENTSES_COMENTSES');
		// search comentses
		if ($sComents AND $limit > 0)
		{
			$query->clear();
			...
			if (isset($list))
			{
				...
				{
					$list[$key]->href = JRoute::_(NicegalleryHelperRoute::getComentsRoute($item->slug, $item->language, $this->layout, $keep_item_id));
		...
		// search archived comentses
		if ($sComentsArchived AND $limit > 0)
		{
			$query->clear();
			...
			$item	= $app->getMenu()->getItems('link', 'index.php?option=com_nicegallery&view=comentsarchive', true);
			...
			if (isset($list3))
			{
				foreach($list3 as $key => $item)
				{
					...
					$list3[$key]->href	= JRoute::_('index.php?option=com_nicegallery&view=comentsarchive'.
		...
		if (count($rows))
		{
			...
			{
				$new_row = array();
				foreach($row as $coments)
				{
					if (SearchHelper::checkNoHTML($coments, $search_text, array('title',
																						'description',
																						'metadesc', 
																						'metakey'
																						)))
