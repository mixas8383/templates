...
$layout		= $this->params->get('image_layout', 'default');
...
<div class="nicegallery images-list<?php echo $this->params->get('pageclass_sfx');?>">
	...
	<?php
		$show_actions = false;
		if ($this->params->get('show_image_icons',-1) >= 0) :
	...
	<form action="<?php echo JFilterOutput::ampReplace(JUri::getInstance()->toString()); ?>" method="post" name="adminForm" id="adminForm">
		<?php if (($this->params->get('show_image_filter_field') != '' AND $this->params->get('show_image_filter_field') != 'hide')) :?>
			<div class="filter-search">
				<?php if ($this->params->get('show_image_filter_field') != '' AND $this->params->get('show_image_filter_field') != 'hide') :?>
					<input type="text" name="filter_search" id="filter_search" value="<?php echo $this->escape($this->state->get('filter.search')); ?>" onchange="document.adminForm.submit();" title="<?php echo JText::_('COM_NICEGALLERY_FILTER_SEARCH_DESC'); ?>" placeholder="<?php echo JText::_('COM_NICEGALLERY_'.$this->params->get('show_image_filter_field').'_FILTER_LABEL'); ?>" />
				<?php endif; ?>	
		...
		<?php if ($this->params->get('show_image_pagination_limit')) : ?>
		...
		<?php if (empty($this->items)) : ?>
			...
			<?php if ($this->params->get('show_no_images',1)) : ?>
			<p><?php echo JText::_('COM_NICEGALLERY_IMAGES_NO_ITEMS'); ?></p>
			<?php endif; ?>
		...
		<?php else : ?>
			...
			<thead>
				<tr>
					...
					<?php endif; ?>
					<?php if ($this->params->get('list_show_image_hits',0)) : ?>
					...
					<?php endif; ?>
					<?php if ($this->params->get('list_show_image_ordering',0)) : ?>
						<th width="10%">
			...
			<tbody>
				...
				?>			
					...
					<?php if (in_array($item->access, $user->getAuthorisedViewLevels()) OR $params->get('show_image_noauth')) : ?>
					<td class="list-name">
						<a href="<?php echo JRoute::_(NicegalleryHelperRoute::getImageRoute($item->slug, 
																									$item->catid, 
																									$item->language,
																									$layout,									
																									$params->get('keep_image_itemid'))); ?>" 
																									itemprop="url"
																									>
					...
					<?php if ($this->params->get('list_show_image_date')) : ?>
						<td class="list-date">
							<time datetime="<?php echo JHtml::_('date', $item->display_date, 'c'); ?>">
								<?php echo JHtml::_('date',$item->display_date, $this->escape($this->params->get('image_date_format', JText::_('DATE_FORMAT_LC3')))); ?>
							</time>
						</td>
					<?php endif; ?>
					<?php if ($this->params->get('list_show_image_created_by',0)) : ?>
						<td class="createdby" itemprop="creator" itemscope itemtype="http://schema.org/Person">
							<?php 
								...
								if (!empty($item->created_by )) :
									if ($this->params->get('link_image_created_by') == 1) :
									...
									if ($this->params->get('show_image_headings')) :
										echo $created_by;
									else :
					...
					<?php endif; ?>
					<?php if ($this->params->get('list_show_image_category',0)) : ?>
							...
							<?php if ($can_order) : ?>
								<?php if ($save_order) : ?>
									...
									<?php if ($list_dirn == 'asc') : ?>
										<span><?php echo $this->pagination->orderUpIcon($i,$condition_up,'images.orderup', 'JLIB_HTML_MOVE_UP', $save_order); ?></span>
										<span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total,$condition_down, 'images.orderdown', 'JLIB_HTML_MOVE_DOWN', $save_order); ?></span>
									<?php elseif ($list_dirn == 'desc') : ?>
										<span><?php echo $this->pagination->orderUpIcon($i, $condition_up, 'images.orderdown', 'JLIB_HTML_MOVE_UP', $save_order); ?></span>
										<span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total,$condition_down, 'images.orderup', 'JLIB_HTML_MOVE_DOWN', $save_order); ?></span>
									<?php endif; ?>
								<?php endif; ?>
						...
						<td class="list-actions">
							<?php if ($can_edit OR $can_delete ) : ?>
								<ul class="actions">
									<?php if ($can_edit ) : ?>
										<li class="edit-icon">
											<?php echo JHtml::_('imageicon.edit',$item, $params); ?>
										</li>
									<?php endif; ?>					
									<?php if ($can_delete) : ?>
										<li class="delete-icon">
											<?php echo JHtml::_('imageicon.delete',$item, $params); ?>
										</li>
									<?php endif; ?>					
								</ul>
							<?php endif; ?>
			...
			<?php if (($this->params->def('show_image_pagination', 2) == 1  OR ($this->params->get('show_image_pagination') == 2)) AND ($this->pagination->get('pages.total') > 1)) : ?>
			<div class="pagination">

				<?php if ($this->params->def('show_image_pagination_results', 0)) : ?>
				<p class="counter">
		...
		<?php endif; ?>
		<?php // Code to add a link to submit an image. ?>
		<?php if ($this->params->get('show_image_add_link', 1)) : ?>
			<?php if ($can_create) : ?>
				<?php echo JHtml::_('imageicon.create', $this->params); ?>
			<?php  endif; ?>
		<?php endif; ?>		
	</form>
</div>
