...
$layout		= $params->get('image_layout', 'default');
$info		= $this->item->params->get('image_info_block_position', 0);
...
<h2 itemprop="name">
		<?php if ($params->get('link_image_names') ) : ?>
			<a href="<?php echo JRoute::_(NicegalleryHelperRoute::getImageRoute($this->item->slug, 
											$this->item->catid, 
											$this->item->language,
											$layout,									
											$params->get('keep_image_itemid'))); ?>" itemprop="url">
...
<?php if (($params->get('show_image_image', '0') == '1' AND isset($images['image_url'])  AND $images['image_url'] <> "")): ?>	
...
<?php 
	...
	$use_def_list = (
		...
		($params->get('show_image_hits')) OR
		$dummy
	);
?>

<?php if ($params->get('access-view') 
		AND (!$params->get('show_image_readmore')
		OR !$params->get('show_image_intro'))
		):?>
	<?php
...
<?php else : ?>
	...
	<?php if ($params->get('show_image_intro')) : ?>
		<?php if ($params->get('access-view') OR (!$params->get('access-view') AND $params->get('show_image_noauth'))) : ?>
			<span itemprop="description">
	...
	<?php if ($params->get('show_image_readmore')) :
			if ($params->get('access-view')) :
				$link = JRoute::_(NicegalleryHelperRoute::getImageRoute($this->item->slug, 
																							$this->item->catid, 
																							$this->item->language,
																							$layout,									
																							$params->get('keep_image_itemid')));
			else :
				if ($params->get('show_image_noauth') AND $user->get('guest')) :
					$menu = JFactory::getApplication()->getMenu();
					$active = $menu->getActive();
					$item_id = $active->id;
					$link_1 = JRoute::_('index.php?option=com_users&view=login&Itemid=' . $item_id);
					$return_url = JRoute::_(NicegalleryHelperRoute::getImageRoute($this->item->slug, 
																								$this->item->catid, 
																								$this->item->language,									
																								$layout,									
																								$params->get('keep_image_itemid')));
			...
			<?php
				...
				if ($item_params->get('image_alternative_readmore') == null) :
						if ($params->get('access-view')) :
