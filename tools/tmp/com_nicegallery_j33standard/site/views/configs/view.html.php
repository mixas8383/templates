 ...
 *
 */
class NicegalleryViewConfigs extends JViewLegacy
{
	...
	public function display($tpl = null)
	{
		...
		foreach ($items as $i => &$item)
		{
			...
			$dispatcher->trigger('onConfigPrepare', array ('com_nicegallery.config', &$item, &$item->params,$i));

			$item->event = new stdClass;

			$results = $dispatcher->trigger('onConfigAfterName', array('com_nicegallery.config', &$item, &$item->params, $i));
			$item->event->afterDisplayConfigName = JString::trim(implode("\n", $results));

			$results = $dispatcher->trigger('onConfigBeforeDisplay', array('com_nicegallery.config', &$item, &$item->params, $i));
			$item->event->beforeDisplayConfig = JString::trim(implode("\n", $results));

			$results = $dispatcher->trigger('onConfigAfterDisplay', array('com_nicegallery.config', &$item, &$item->params,$i));
	...
	protected function prepareDocument()
	{
		...
		{
			$this->params->def('page_heading', JText::_('COM_NICEGALLERY_CONFIGS'));
		}
