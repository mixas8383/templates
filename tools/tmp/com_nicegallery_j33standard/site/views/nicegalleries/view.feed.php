...
class NicegalleryViewNicegalleries extends JViewLegacy
{
	...
	public function display($tpl = null)
	{
		...
		$doc->link = JRoute::_('index.php?option=com_nicegallery&view=nicegalleries');
		...
		foreach ($rows as $row)
		{
			...
			// url link to nicegallery
			$link = JRoute::_(NicegalleryHelperRoute::getNicegalleryRoute($row->slug, 
																						$row->catid, 
																						$row->language,	
																						'default',								
			...
			if ($item->featured == 1)
			{
				$item->category[]	= JText::_('JFEATURED'); // All featured nicegalleries are categorized as "Featured"
			}
