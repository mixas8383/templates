...
<div class="nicegallery coments-view<?php echo $params->get('pageclass_sfx')?>" itemscope itemtype="http://schema.org/CreativeWork">
	...
	<?php endif; ?>
	<?php if ($params->get('show_coments_icons',-1) >= 0) : ?>
		<?php if ($params->get('show_coments_print_icon') 
			OR $params->get('show_coments_email_icon') 
			OR $can_edit 
			OR $can_delete 
			): ?>
				...
				<ul class="dropdown-menu">
					<?php if (!$this->print) : ?>
						<?php if ($params->get('access-view')) : ?>
							<?php if ($params->get('show_coments_print_icon')) : ?>
								<li class="print-icon">
										<?php echo JHtml::_('comentsicon.print_popup',  $this->item, $params); ?>
								</li>
							<?php endif; ?>

							<?php if ($params->get('show_coments_email_icon')) : ?>
								<li class="email-icon">
										<?php echo JHtml::_('comentsicon.email',  $this->item, $params); ?>
								</li>
							<?php endif; ?>
							<?php if ($can_edit) : ?>
								<li class="edit-icon">
									<?php echo JHtml::_('comentsicon.edit', $this->item, $params); ?>
								</li>
							<?php endif; ?>
							<?php if ($can_delete) : ?>
								<li class="delete-icon">
									<?php echo JHtml::_('comentsicon.delete',$this->item, $params); ?>
								</li>					
							<?php endif; ?>
						<?php endif; ?>
					<?php else : ?>
						<li>
							<?php echo JHtml::_('comentsicon.print_screen',  $this->item, $params); ?>
						</li>
					<?php endif; ?>
				</ul>
			</div>
	...
	<?php if ($params->get('show_coments_name')) : ?>
		<div style="float: left;">
			<h2 itemprop="name">
				<?php if ($params->get('link_coments_names') AND !empty($this->item->readmore_link)) : ?>
	...
	<?php endif; ?>
	<?php  echo $this->item->event->afterDisplayComentsName;	?>
	
	<?php echo $this->item->event->beforeDisplayComents; ?>
	<?php $images = $this->item->images; ?>
	
	<?php if ($params->get('show_coments_hits') != '0' OR 
		($params->get('show_coments_image', '0') == '1' AND isset($images['image_url']) AND $images['image_url'] != '')): ?>	
			<div class="pull-<?php echo htmlspecialchars($params->get('show_coments_image_float','right')); ?>">
			<?php if ($params->get('show_coments_image') == '1' AND isset($images['image_url']) AND $images['image_url'] != '') : ?>
					...
					<?php 
						...
						$display_width = (int) $params->get('show_coments_intro_image_width','100');
						$display_height = (int) $params->get('show_coments_intro_image_height','0');
					...
					/>
			<?php endif; ?>			 
			<?php if ($params->get('show_coments_hits')) : ?>
	...
	<div style="clear:both; padding-top: 10px;">
		...
		<?php if ($params->get('access-view')) :?>
		<?php
			if (isset($this->item->urls) AND $params->get('show_coments_urls') == '1' AND $params->get('show_coments_urls_position')=='0') :
		...
		<?php elseif ($params->get('show_coments_noauth') == true AND  $user->get('guest') ) : ?>
			<?php if ($params->get('show_coments_intro')) : ?>
				<span itemprop="description">
				<?php echo $this->item->intro; ?>
				</span>
			<?php endif; ?>	
			<?php //Optional link to let them register to see the whole coments. ?>
			<?php if ($params->get('show_coments_readmore')) :
				...
				<p class="readmore">
					<a href="<?php echo $link; ?>" itemprop="url">
						<?php
						if ($this->item->coments_alternative_readmore == null) :
							if ($params->get('show_coments_readmore_name') == 0) :					
								echo JText::_('COM_NICEGALLERY_REGISTER_TO_READ_MORE');
							else :
								echo JText::_('COM_NICEGALLERY_REGISTER_TO_READMORE_NAME');
								echo JHtml::_('string.truncate', ($this->item->name), $params->get('coments_readmore_limit'));
							endif;
						else :
							echo $this->item->coments_alternative_readmore;
							if ($params->get('show_coments_readmore_name') == 1) :
								echo JHtml::_('string.truncate', ': '.($this->item->name), $params->get('coments_readmore_limit'));
							endif;
						endif;
						?>
	...
	<div style="padding-top: 10px;">
		<?php if ($params->get('access-view')) : ?>	
			...
			<form action="" name="comentsForm" id="comentsForm">
			<?php
				$dummy = false;
		$display_fieldset = (
							($params->get('show_coments_tags')) OR
							($params->get('show_coments_created_by')) OR
							($params->get('show_coments_created')) OR
							($params->get('show_coments_modified')) OR
							($params->get('show_coments_publish_up')) OR
							($params->get('show_coments_publish_down')) OR
							($params->get('show_coments_admin') AND $this->item->params->get('access-change')) OR							
							$dummy
							);
			?>
			...
			<?php if ($params->get('show_coments_tags')  == '1' AND !empty($this->item->tags) AND !empty($this->item->tags->itemTags)) : ?>
				<div class="formelm">
					<label>
			...
			<?php endif; ?>
			<?php if ($params->get('show_coments_created_by') ) : ?>
				...
				<div class="formelm">				
					...
					<span itemprop="creator" itemscope itemtype="http://schema.org/Person">
						<?php if (!empty($this->item->created_by ) AND  $this->params->get('link_coments_created_by') == 1):?>
							<?php echo JHtml::_(
									'link',
			...
			<?php endif; ?>	
			<?php if ($params->get('show_coments_created_by_alias')) : ?>
			...
			<?php endif; ?>				
			<?php if ($params->get('show_coments_created')) : ?>
				<div class="formelm">
					<label>
			...
			<?php endif; ?>
			<?php if ($params->get('show_coments_modified')) : ?>
				<div class="formelm">
					<label>
			...
			<?php endif; ?>	
			<?php if ($params->get('show_coments_publish_up')) : ?>
				<div class="formelm">
					<label>
			...
			<?php endif; ?>
			<?php if ($params->get('show_coments_publish_down')) : ?>
				<div class="formelm">
					<label>
			...
			<?php if ($params->get('access-change')): ?>
				<?php if ($params->get('show_coments_admin')) : ?>
			...
			<?php
				if (isset($this->item->urls) AND $params->get('show_coments_urls') == '1' AND $params->get('show_coments_urls_position')=='1') :
					echo $this->loadTemplate('urls');
				endif;
			?>	
			<?php if ($display_fieldset) : ?>				
		...
		<?php endif; ?>	
		<?php echo $this->item->event->afterDisplayComents; ?>
	</div>		
</div>