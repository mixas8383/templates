...
<div class="nicegallery vote-view<?php echo $params->get('pageclass_sfx')?>" itemscope itemtype="http://schema.org/CreativeWork">
	...
	<?php endif; ?>
	<?php if ($params->get('show_vote_icons',-1) >= 0) : ?>
		<?php if ($params->get('show_vote_print_icon') 
			OR $params->get('show_vote_email_icon') 
			OR $can_edit 
			OR $can_delete 
			): ?>
				...
				<ul class="dropdown-menu">
					<?php if (!$this->print) : ?>
						<?php if ($params->get('access-view')) : ?>
							<?php if ($params->get('show_vote_print_icon')) : ?>
								<li class="print-icon">
										<?php echo JHtml::_('voteicon.print_popup',  $this->item, $params); ?>
								</li>
							<?php endif; ?>

							<?php if ($params->get('show_vote_email_icon')) : ?>
								<li class="email-icon">
										<?php echo JHtml::_('voteicon.email',  $this->item, $params); ?>
								</li>
							<?php endif; ?>
							<?php if ($can_edit) : ?>
								<li class="edit-icon">
									<?php echo JHtml::_('voteicon.edit', $this->item, $params); ?>
								</li>
							<?php endif; ?>
							<?php if ($can_delete) : ?>
								<li class="delete-icon">
									<?php echo JHtml::_('voteicon.delete',$this->item, $params); ?>
								</li>					
							<?php endif; ?>
						<?php endif; ?>
					<?php else : ?>
						<li>
							<?php echo JHtml::_('voteicon.print_screen',  $this->item, $params); ?>
						</li>
					<?php endif; ?>
				</ul>
			</div>
	...
	<?php if ($params->get('show_vote_name')) : ?>
		<div style="float: left;">
			<h2 itemprop="name">
				<?php if ($params->get('link_vote_names') AND !empty($this->item->readmore_link)) : ?>
	...
	<?php endif; ?>
	<?php  echo $this->item->event->afterDisplayVoteName;	?>
	
	<?php echo $this->item->event->beforeDisplayVote; ?>
	<?php $images = $this->item->images; ?>
	
	<?php if ($params->get('show_vote_hits') != '0' OR 
		($params->get('show_vote_image', '0') == '1' AND isset($images['image_url']) AND $images['image_url'] != '')): ?>	
			<div class="pull-<?php echo htmlspecialchars($params->get('show_vote_image_float','right')); ?>">
			<?php if ($params->get('show_vote_image') == '1' AND isset($images['image_url']) AND $images['image_url'] != '') : ?>
					...
					<?php 
						...
						$display_width = (int) $params->get('show_vote_intro_image_width','100');
						$display_height = (int) $params->get('show_vote_intro_image_height','0');
					...
					/>
			<?php endif; ?>			 
			<?php if ($params->get('show_vote_hits')) : ?>
	...
	<div style="clear:both; padding-top: 10px;">
		...
		<?php if ($params->get('access-view')) :?>
		<?php
			if (isset($this->item->urls) AND $params->get('show_vote_urls') == '1' AND $params->get('show_vote_urls_position')=='0') :
		...
		<?php elseif ($params->get('show_vote_noauth') == true AND  $user->get('guest') ) : ?>
			<?php if ($params->get('show_vote_intro')) : ?>
				<span itemprop="description">
				<?php echo $this->item->intro; ?>
				</span>
			<?php endif; ?>	
			<?php //Optional link to let them register to see the whole vote. ?>
			<?php if ($params->get('show_vote_readmore')) :
				...
				<p class="readmore">
					<a href="<?php echo $link; ?>" itemprop="url">
						<?php
						if ($this->item->vote_alternative_readmore == null) :
							if ($params->get('show_vote_readmore_name') == 0) :					
								echo JText::_('COM_NICEGALLERY_REGISTER_TO_READ_MORE');
							else :
								echo JText::_('COM_NICEGALLERY_REGISTER_TO_READMORE_NAME');
								echo JHtml::_('string.truncate', ($this->item->name), $params->get('vote_readmore_limit'));
							endif;
						else :
							echo $this->item->vote_alternative_readmore;
							if ($params->get('show_vote_readmore_name') == 1) :
								echo JHtml::_('string.truncate', ': '.($this->item->name), $params->get('vote_readmore_limit'));
							endif;
						endif;
						?>
	...
	<div style="padding-top: 10px;">
		<?php if ($params->get('access-view')) : ?>	
			...
			<form action="" name="voteForm" id="voteForm">
			<?php
				$dummy = false;
		$display_fieldset = (
							($params->get('show_vote_tags')) OR
							($params->get('show_vote_created_by')) OR
							($params->get('show_vote_created')) OR
							($params->get('show_vote_modified')) OR
							($params->get('show_vote_publish_up')) OR
							($params->get('show_vote_publish_down')) OR
							($params->get('show_vote_admin') AND $this->item->params->get('access-change')) OR							
							$dummy
							);
			?>
			...
			<?php if ($params->get('show_vote_tags')  == '1' AND !empty($this->item->tags) AND !empty($this->item->tags->itemTags)) : ?>
				<div class="formelm">
					<label>
			...
			<?php endif; ?>
			<?php if ($params->get('show_vote_created_by') ) : ?>
				...
				<div class="formelm">				
					...
					<span itemprop="creator" itemscope itemtype="http://schema.org/Person">
						<?php if (!empty($this->item->created_by ) AND  $this->params->get('link_vote_created_by') == 1):?>
							<?php echo JHtml::_(
									'link',
			...
			<?php endif; ?>	
			<?php if ($params->get('show_vote_created_by_alias')) : ?>
			...
			<?php endif; ?>				
			<?php if ($params->get('show_vote_created')) : ?>
				<div class="formelm">
					<label>
			...
			<?php endif; ?>
			<?php if ($params->get('show_vote_modified')) : ?>
				<div class="formelm">
					<label>
			...
			<?php endif; ?>	
			<?php if ($params->get('show_vote_publish_up')) : ?>
				<div class="formelm">
					<label>
			...
			<?php endif; ?>
			<?php if ($params->get('show_vote_publish_down')) : ?>
				<div class="formelm">
					<label>
			...
			<?php if ($params->get('access-change')): ?>
				<?php if ($params->get('show_vote_admin')) : ?>
			...
			<?php
				if (isset($this->item->urls) AND $params->get('show_vote_urls') == '1' AND $params->get('show_vote_urls_position')=='1') :
					echo $this->loadTemplate('urls');
				endif;
			?>	
			<?php if ($display_fieldset) : ?>				
		...
		<?php endif; ?>	
		<?php echo $this->item->event->afterDisplayVote; ?>
	</div>		
</div>