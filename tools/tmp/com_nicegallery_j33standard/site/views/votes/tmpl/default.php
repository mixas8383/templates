...
$layout		= $this->params->get('vote_layout', 'default');
...
<div class="nicegallery votes-list<?php echo $this->params->get('pageclass_sfx');?>">
	...
	<?php
		$show_actions = false;
		if ($this->params->get('show_vote_icons',-1) >= 0) :
	...
	<form action="<?php echo JFilterOutput::ampReplace(JUri::getInstance()->toString()); ?>" method="post" name="adminForm" id="adminForm">
		<?php if (($this->params->get('show_vote_filter_field') != '' AND $this->params->get('show_vote_filter_field') != 'hide')) :?>
			<div class="filter-search">
				<?php if ($this->params->get('show_vote_filter_field') != '' AND $this->params->get('show_vote_filter_field') != 'hide') :?>
					<input type="text" name="filter_search" id="filter_search" value="<?php echo $this->escape($this->state->get('filter.search')); ?>" onchange="document.adminForm.submit();" title="<?php echo JText::_('COM_NICEGALLERY_FILTER_SEARCH_DESC'); ?>" placeholder="<?php echo JText::_('COM_NICEGALLERY_'.$this->params->get('show_vote_filter_field').'_FILTER_LABEL'); ?>" />
				<?php endif; ?>	
			</div>
		<?php endif; ?>

		<?php if ($this->params->get('show_vote_pagination_limit')) : ?>
		...
		<?php if (empty($this->items)) : ?>
			...
			<?php if ($this->params->get('show_no_votes',1)) : ?>
			<p><?php echo JText::_('COM_NICEGALLERY_VOTES_NO_ITEMS'); ?></p>
			<?php endif; ?>

		<?php else : ?>
			<table class="table table-striped" id="votes">
			<?php if ($this->params->get('show_vote_headings')) :?>
			<thead>
				<tr>
					...
					<?php endif; ?>
					<?php if ($this->params->get('list_show_vote_hits',0)) : ?>
					...
					<?php endif; ?>
					<?php if ($this->params->get('list_show_vote_ordering',0)) : ?>
						<th width="10%">
			...
			<tbody>
				...
				?>			
					...
					<?php if (in_array($item->access, $user->getAuthorisedViewLevels()) OR $params->get('show_vote_noauth')) : ?>
					<td class="list-name">
						<a href="<?php echo JRoute::_(NicegalleryHelperRoute::getVoteRoute($item->slug, 
																									$item->language,									
																									$layout,									
																									$params->get('keep_vote_itemid'))); ?>" 
																									itemprop="url"
																									>
					...
					<?php if ($this->params->get('list_show_vote_date')) : ?>
						<td class="list-date">
							<time datetime="<?php echo JHtml::_('date', $item->display_date, 'c'); ?>">
								<?php echo JHtml::_('date',$item->display_date, $this->escape($this->params->get('vote_date_format', JText::_('DATE_FORMAT_LC3')))); ?>
							</time>
						</td>
					<?php endif; ?>
					<?php if ($this->params->get('list_show_vote_created_by',0)) : ?>
						<td class="createdby" itemprop="creator" itemscope itemtype="http://schema.org/Person">
							<?php 
								...
								if (!empty($item->created_by )) :
									if ($this->params->get('link_vote_created_by') == 1) :
									...
									if ($this->params->get('show_vote_headings')) :
										echo $created_by;
									else :
							...
							<?php if ($can_order) : ?>
								<?php if ($save_order) : ?>
									...
									<?php if ($list_dirn == 'asc') : ?>
										<span><?php echo $this->pagination->orderUpIcon($i,$condition_up,'votes.orderup', 'JLIB_HTML_MOVE_UP', $save_order); ?></span>
										<span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total,$condition_down, 'votes.orderdown', 'JLIB_HTML_MOVE_DOWN', $save_order); ?></span>
									<?php elseif ($list_dirn == 'desc') : ?>
										<span><?php echo $this->pagination->orderUpIcon($i, $condition_up, 'votes.orderdown', 'JLIB_HTML_MOVE_UP', $save_order); ?></span>
										<span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total,$condition_down, 'votes.orderup', 'JLIB_HTML_MOVE_DOWN', $save_order); ?></span>
									<?php endif; ?>
								<?php endif; ?>
						...
						<td class="list-actions">
							<?php if ($can_edit OR $can_delete ) : ?>
								<ul class="actions">
									<?php if ($can_edit ) : ?>
										<li class="edit-icon">
											<?php echo JHtml::_('voteicon.edit',$item, $params); ?>
										</li>
									<?php endif; ?>					
									<?php if ($can_delete) : ?>
										<li class="delete-icon">
											<?php echo JHtml::_('voteicon.delete',$item, $params); ?>
										</li>
									<?php endif; ?>					
								</ul>
							<?php endif; ?>
			...
			<?php if (($this->params->def('show_vote_pagination', 2) == 1  OR ($this->params->get('show_vote_pagination') == 2)) AND ($this->pagination->get('pages.total') > 1)) : ?>
			<div class="pagination">

				<?php if ($this->params->def('show_vote_pagination_results', 0)) : ?>
				<p class="counter">
		...
		<?php endif; ?>
		<?php // Code to add a link to submit an vote. ?>
		<?php if ($this->params->get('show_vote_add_link', 1)) : ?>
			<?php if ($can_create) : ?>
				<?php echo JHtml::_('voteicon.create', $this->params); ?>
			<?php  endif; ?>
		<?php endif; ?>		
	</form>
</div>
