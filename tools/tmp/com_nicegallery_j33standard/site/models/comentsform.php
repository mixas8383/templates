...
require_once JPATH_ADMINISTRATOR.'/components/com_nicegallery/models/coments.php';

class NicegalleryModelComentsForm extends NicegalleryModelComents
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_nicegallery.edit.coments';
	/**
	...
	*/
	 ...
	 *
	 * @param	integer	$item_id	The id of the coments.
	 *
	 * @return	mixed	Coments item data object on success, false on failure.
	 */
	public function getItem($item_id = null)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('coments.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_nicegallery.edit.coments.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('coments.id');
		$is_new		= true;
		...
		// Reorder the comentses so the new coments is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onComentsBeforeSave', array('com_nicegallery.coments', &$table, $is_new));
		...
		$dispatcher->trigger('onComentsAfterSave', array('com_nicegallery.coments', $table, $is_new));

		$this->setState('coments.id', $table->id);
