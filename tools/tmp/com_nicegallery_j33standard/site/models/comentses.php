...
/**
 * This models supports retrieving lists of comentses.
 *
 */
class NicegalleryModelComentses extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_nicegallery.comentses';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single coments has been specified either as a parameter or in the url Request
		$pk = $params->get('coments_id', '') == '' ? $app->input->getInt('id', '') : $params->get('coments_id');
		$this->setState('filter.coments_id', $pk);
		
		// List state information
		if ($app->input->getString('layout', 'default') == 'blog')
		{
			$limit = $params->def('coments_num_leading', 1) + $params->def('coments_num_intro', 4) + $params->def('coments_num_links', 4);
		}
		else
		{		
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('coments_num_per_page'),'integer');
		}
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('coments_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('coments_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('coments_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('coments_initial_direction','ASC');
		}
		...
		// process show_coments_noauth parameter
		if (!$params->get('show_coments_noauth'))
		{
		...
		if ($params->get('filter_coments_featured') <> "")
		{
			$this->setState('filter.featured', $params->get('filter_coments_featured'));
			
		}
		if ($params->get('filter_coments_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_coments_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.coments_id'));
		$id .= ':'.$this->getState('filter.coments_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of comentses subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		// Filter by a single or group of comentses.
		$coments_id = $this->getState('filter.coments_id');
		if ($coments_id != '')
		{
			if (is_numeric($coments_id))
			{
				$type = $this->getState('filter.coments_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $coments_id);
			}
			else
			{
				if (is_array($coments_id))
				{
					JArrayHelper::toInteger($coments_id);
					$coments_id = implode(',', $coments_id);
					$type = $this->getState('filter.coments_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$coments_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_coments_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			...
			switch ($params->get('show_coments_filter_field'))
			{
				case 'hits':
	...
	/**
	 * Method to get a list of comentses.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				$coments_params = new JRegistry;
				$coments_params->loadString($item->params);
				...
				$item->coments_alternative_readmore = $coments_params->get('coments_alternative_readmore');
				$item->layout = $coments_params->get('layout');
							
				if (!is_object($this->getState('params')))
				{
					$item->params = $coments_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Coments params override menu item params only if menu param = 'use_coments'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_coments' and there is no coments param, use global

					// create an array of just the params set to 'use_coments'
					$menu_params_array = $this->getState('params')->toArray();
					$coments_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_coments')
						{
							// if the coments has a value, use it
							if ($coments_params->get($key) != '')
							{
								// get the value from the coments
								$coments_array[$key] = $coments_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$coments_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected coments params
					if (count($coments_array) > 0)
					{
						$coments_params = new JRegistry;
						$coments_params->loadArray($coments_array);
						$item->params->merge($coments_params);
					}


					// get display date
					switch ($item->params->get('list_show_coments_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an coments, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_nicegallery.coments.'.$item->id;
				...
				if ($access) 
				{
					// If the access filter has been set, we already have only the comentses this user can view.
				...
				$item->tags->getItemTags('com_nicegallery.coments', $item->id);
			}
		}
		return $items;
	}
