 ...
 *
 */
class NicegalleryModelImages extends JModelList
{
	/**
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single image has been specified either as a parameter or in the url Request
		$pk = $params->get('image_id', '') == '' ? $app->input->getInt('id', '') : $params->get('image_id');
		$this->setState('filter.image_id', $pk);
		
		// List state information
		if ($app->input->getString('layout', 'default') == 'blog')
		{
			$limit = $params->def('image_num_leading', 1) + $params->def('image_num_intro', 4) + $params->def('image_num_links', 4);
		}
		else
		{		
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('image_num_per_page'),'integer');
		}
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('image_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('image_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('image_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('image_initial_direction','ASC');
		}
		...
		// process show_image_noauth parameter
		if (!$params->get('show_image_noauth'))
		{
		...
		if ($params->get('filter_image_categories') AND implode(',', $params->get('filter_image_categories'))  == true)
		{
			$selected_categories = $params->get('filter_image_categories');
			$this->setState('filter.category_id', $selected_categories);
		}			
		if ($params->get('filter_image_featured') <> "")
		{
			$this->setState('filter.featured', $params->get('filter_image_featured'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.image_id'));
		$id .= ':'.$this->getState('filter.image_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of images subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$image_id = $this->getState('filter.image_id');
		if ($image_id != '')
		{
			if (is_numeric($image_id))
			{
				$type = $this->getState('filter.image_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $image_id);
			}
			else
			{
				if (is_array($image_id))
				{
					...
					$type = $this->getState('filter.image_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$image_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_image_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			...
			switch ($params->get('show_image_filter_field'))
			{
				case 'hits':
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				else
				{
					...
					// get display date
					switch ($item->params->get('list_show_image_date'))
					{
						case 'modified':
				...
				if ($access) 
				{
					// If the access filter has been set, we already have only the images this user can view.
