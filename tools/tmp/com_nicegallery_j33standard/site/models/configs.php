 ...
 *
 */
class NicegalleryModelConfigs extends JModelList
{
	/**
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		{
			$limit = $params->def('config_num_leading', 1) + $params->def('config_num_intro', 4) + $params->def('config_num_links', 4);
		}
		else
		{		
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('config_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('config_initial_sort','a.ordering');
		}
		...
		// process show_config_noauth parameter
		if (!$params->get('show_config_noauth'))
		{
		...
		if ($params->get('filter_config_featured') <> "")
		{
			$this->setState('filter.featured', $params->get('filter_config_featured'));
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				if ($access) 
				{
					// If the access filter has been set, we already have only the configs this user can view.
