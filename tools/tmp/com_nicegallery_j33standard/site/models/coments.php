...
/**
 * Nicegallery Component Coments Model
 *
 */
class NicegalleryModelComents extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_nicegallery.coments';
	...
	{
		if (empty($config['coments_filter_fields']))
		{
			$config['coments_filter_fields'] = array(
				'id', 'a.id',
				'name', 'a.name',
				'alias', 'a.alias',
	...
	protected function canDelete($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.delete', 'com_nicegallery.coments.'.(int) $record->id)
					  OR ($user->authorise('core.delete', 'com_nicegallery.coments.'.(int) $record->id)
	...
	*/
	...
	/**
	 * Method to get Coments data.
	 *
	 * @param	integer	$pk	The id of the coments.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('coments.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				$can_publish = $user->authorise('core.edit.state', 'com_nicegallery.coments.'.$pk);
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_NICEGALLERY_COMENTSES_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onComentsChangeState', array('com_nicegallery.coments', $pks, $value));
	...
	*/
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onComentsBeforeDelete', array('com_nicegallery.coments', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onComentsAfterDelete', array('com_nicegallery.coments', &$table));
				}
				else
				{
					// Prune items that you can't change.
	...
	/**
	 * Increment the hit counter for the coments.
	 *
	 * @pk		int		Optional primary key of the coments to increment.
	 *
