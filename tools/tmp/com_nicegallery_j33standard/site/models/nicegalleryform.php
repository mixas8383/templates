...
require_once JPATH_ADMINISTRATOR.'/components/com_nicegallery/models/nicegallery.php';

class NicegalleryModelNicegalleryForm extends NicegalleryModelNicegallery
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_nicegallery.edit.nicegallery';
	/**
	...
	protected function populateState()
	{
		...
		$this->setState('nicegallery.catid', $app->input->getInt('catid'));
	...
	*/
	...
	public function getForm($data = array(), $load_data = true)
	{
		...
		if ($id = (int) $this->getState('nicegallery.id'))
		{
	 ...
	 *
	 * @param	integer	$item_id	The id of the nicegallery.
	 *
	 * @return	mixed	Nicegallery item data object on success, false on failure.
	 */
	public function getItem($item_id = null)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('nicegallery.id');
		...
		{
			// New item.
			$cat_id = (int) $this->getState('nicegallery.catid');
			if ($cat_id)
			{
				$item->params->set('access-change', $user->authorise('core.edit.state', 'com_nicegallery.category.'.$cat_id));
				$item->catid = $cat_id;
			}
			else 
			{
				$item->params->set('access-change', $user->authorise('core.edit.state', 'com_nicegallery'));
			}
		}
	...
	protected function setAccessFilters(&$form, $data)
	{
		...
		if (!$user->authorise('core.edit.state', 'com_nicegallery'))
		{
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_nicegallery.edit.nicegallery.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('nicegallery.id');
		$is_new		= true;
		...
		else
		{
			...
			$actions = JAccess::getActions('com_nicegallery');
			$action_array = array();
		...
		// Reorder the nicegalleries so the new nicegallery is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onNicegalleryBeforeSave', array('com_nicegallery.nicegallery', &$table, $is_new));
		...
		// Clean the cache.
		$cache = JFactory::getCache('com_nicegallery');
		$cache->clean();

		$dispatcher->trigger('onNicegalleryAfterSave', array('com_nicegallery.nicegallery', $table, $is_new));

		$this->setState('nicegallery.id', $table->id);
