...
/**
 * This models supports retrieving lists of votes.
 *
 */
class NicegalleryModelVotes extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_nicegallery.votes';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single vote has been specified either as a parameter or in the url Request
		$pk = $params->get('vote_id', '') == '' ? $app->input->getInt('id', '') : $params->get('vote_id');
		$this->setState('filter.vote_id', $pk);
		
		// List state information
		if ($app->input->getString('layout', 'default') == 'blog')
		{
			$limit = $params->def('vote_num_leading', 1) + $params->def('vote_num_intro', 4) + $params->def('vote_num_links', 4);
		}
		else
		{		
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('vote_num_per_page'),'integer');
		}
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('vote_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('vote_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('vote_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('vote_initial_direction','ASC');
		}
		...
		// process show_vote_noauth parameter
		if (!$params->get('show_vote_noauth'))
		{
		...
		if ($params->get('filter_vote_featured') <> "")
		{
			$this->setState('filter.featured', $params->get('filter_vote_featured'));
			
		}
		if ($params->get('filter_vote_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_vote_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.vote_id'));
		$id .= ':'.$this->getState('filter.vote_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of votes subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		// Filter by a single or group of votes.
		$vote_id = $this->getState('filter.vote_id');
		if ($vote_id != '')
		{
			if (is_numeric($vote_id))
			{
				$type = $this->getState('filter.vote_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $vote_id);
			}
			else
			{
				if (is_array($vote_id))
				{
					JArrayHelper::toInteger($vote_id);
					$vote_id = implode(',', $vote_id);
					$type = $this->getState('filter.vote_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$vote_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_vote_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			...
			switch ($params->get('show_vote_filter_field'))
			{
				case 'hits':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				$vote_params = new JRegistry;
				$vote_params->loadString($item->params);
				...
				$item->vote_alternative_readmore = $vote_params->get('vote_alternative_readmore');
				$item->layout = $vote_params->get('layout');
							
				if (!is_object($this->getState('params')))
				{
					$item->params = $vote_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Vote params override menu item params only if menu param = 'use_vote'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_vote' and there is no vote param, use global

					// create an array of just the params set to 'use_vote'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_vote')
						{
							// if the vote has a value, use it
							if ($vote_params->get($key) != '')
							{
								// get the value from the vote
								$vote_array[$key] = $vote_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$vote_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected vote params
					if (count($vote_array) > 0)
					{
						$vote_params = new JRegistry;
						$vote_params->loadArray($vote_array);
						$item->params->merge($vote_params);
					}


					// get display date
					switch ($item->params->get('list_show_vote_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an vote, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_nicegallery.vote.'.$item->id;
				...
				if ($access) 
				{
					// If the access filter has been set, we already have only the votes this user can view.
				...
				$item->tags->getItemTags('com_nicegallery.vote', $item->id);
			}
		}
		return $items;
	}
