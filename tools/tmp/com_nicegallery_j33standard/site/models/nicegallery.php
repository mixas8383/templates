...
/**
 * Nicegallery Component Nicegallery Model
 *
 */
class NicegalleryModelNicegallery extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_nicegallery.nicegallery';
	...
	{
		if (empty($config['nicegallery_filter_fields']))
		{
			$config['nicegallery_filter_fields'] = array(
				'id', 'a.id',
				'name', 'a.name',
				'alias', 'a.alias',
	...
	protected function canDelete($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.delete', 'com_nicegallery.nicegallery.'.(int) $record->id)
					  OR ($user->authorise('core.delete', 'com_nicegallery.nicegallery.'.(int) $record->id)
	...
	protected function populateState()
	{
		...
		$this->setState('nicegallery.id', $pk);
	...
	*/
	...
	/**
	 * Method to get Nicegallery data.
	 *
	 * @param	integer	$pk	The id of the nicegallery.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('nicegallery.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				// If all categories are published, badcats.id will be null, and we just use the nicegallery state
				...
				$can_publish = $user->authorise('core.edit.state', 'com_nicegallery.nicegallery.'.$pk);
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_NICEGALLERY_NICEGALLERIES_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onNicegalleryChangeState', array('com_nicegallery.nicegallery', $pks, $value));
	...
	*/
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onNicegalleryBeforeDelete', array('com_nicegallery.nicegallery', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onNicegalleryAfterDelete', array('com_nicegallery.nicegallery', &$table));
				}
				else
				{
					// Prune items that you can't change.
	...
	/**
	 * Increment the hit counter for the nicegallery.
	 *
	 * @pk		int		Optional primary key of the nicegallery to increment.
	 *
