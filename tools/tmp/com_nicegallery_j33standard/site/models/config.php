...
/**
 * Nicegallery Component Config Model
 *
 */
class NicegalleryModelConfig extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_nicegallery.config';
	...
	protected function canDelete($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.delete', 'com_nicegallery.config.'.(int) $record->id)
					  OR ($user->authorise('core.delete', 'com_nicegallery.config.'.(int) $record->id)
	...
	*/
	...
	public function getItem($pk = null)
	{
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				$can_publish = $user->authorise('core.edit.state', 'com_nicegallery.config.'.$pk);
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_NICEGALLERY_CONFIGS_ERROR_ITEM_NOT_FOUND'));
				}
				...
				$item->tags->getItemTags('com_nicegallery.config', $item->id);
				...
				{
					$user_id	= $user->get('id');
					$asset	= 'com_nicegallery.config.'.$item->id;
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onConfigChangeState', array('com_nicegallery.config', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onConfigBeforeDelete', array('com_nicegallery.config', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onConfigAfterDelete', array('com_nicegallery.config', &$table));
				}
				else
				{
					// Prune items that you can't change.
