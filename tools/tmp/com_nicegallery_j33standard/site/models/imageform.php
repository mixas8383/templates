...
require_once JPATH_ADMINISTRATOR.'/components/com_nicegallery/models/image.php';

class NicegalleryModelImageForm extends NicegalleryModelImage
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_nicegallery.edit.image';
	/**
	...
	protected function populateState()
	{
		...
		$this->setState('image.catid', $app->input->getInt('catid'));
	...
	*/
	...
	public function getForm($data = array(), $load_data = true)
	{
		...
		if ($id = (int) $this->getState('image.id'))
		{
	 ...
	 *
	 * @param	integer	$item_id	The id of the image.
	 *
	 * @return	mixed	Image item data object on success, false on failure.
	 */
	public function getItem($item_id = null)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('image.id');
		...
		{
			// New item.
			$cat_id = (int) $this->getState('image.catid');
			if ($cat_id)
			{
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_nicegallery.edit.image.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('image.id');
		$is_new		= true;
		...
		// Reorder the images so the new image is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onImageBeforeSave', array('com_nicegallery.image', &$table, $is_new));
		...
		$dispatcher->trigger('onImageAfterSave', array('com_nicegallery.image', $table, $is_new));

		$this->setState('image.id', $table->id);
