...
/**
 * Nicegallery Component Vote Model
 *
 */
class NicegalleryModelVote extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_nicegallery.vote';
	...
	{
		if (empty($config['vote_filter_fields']))
		{
			$config['vote_filter_fields'] = array(
				'id', 'a.id',
				'name', 'a.name',
				'alias', 'a.alias',
	...
	protected function canDelete($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.delete', 'com_nicegallery.vote.'.(int) $record->id)
					  OR ($user->authorise('core.delete', 'com_nicegallery.vote.'.(int) $record->id)
	...
	*/
	 ...
	 *
	 * @param	integer	$pk	The id of the vote.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('vote.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				$can_publish = $user->authorise('core.edit.state', 'com_nicegallery.vote.'.$pk);
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_NICEGALLERY_VOTES_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onVoteChangeState', array('com_nicegallery.vote', $pks, $value));
	...
	*/
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onVoteBeforeDelete', array('com_nicegallery.vote', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onVoteAfterDelete', array('com_nicegallery.vote', &$table));
				}
				else
				{
					// Prune items that you can't change.
	...
	/**
	 * Increment the hit counter for the vote.
	 *
	 * @pk		int		Optional primary key of the vote to increment.
	 *
