...
/**
 * This models supports retrieving lists of nicegalleries.
 *
 */
class NicegalleryModelNicegalleries extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_nicegallery.nicegalleries';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single nicegallery has been specified either as a parameter or in the url Request
		$pk = $params->get('nicegallery_id', '') == '' ? $app->input->getInt('id', '') : $params->get('nicegallery_id');
		$this->setState('filter.nicegallery_id', $pk);
		
		// List state information
		if ($app->input->getString('layout', 'default') == 'blog')
		{
			$limit = $params->def('nicegallery_num_leading', 1) + $params->def('nicegallery_num_intro', 4) + $params->def('nicegallery_num_links', 4);
		}
		else
		{		
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('nicegallery_num_per_page'),'integer');
		}
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('nicegallery_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('nicegallery_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('nicegallery_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('nicegallery_initial_direction','ASC');
		}
		...
		// process show_nicegallery_noauth parameter
		if (!$params->get('show_nicegallery_noauth'))
		{
		...
		if ($params->get('filter_nicegallery_categories') AND implode(',', $params->get('filter_nicegallery_categories'))  == true)
		{
			$selected_categories = $params->get('filter_nicegallery_categories');
			$this->setState('filter.category_id', $selected_categories);
		}			
		if ($params->get('filter_nicegallery_featured') <> "")
		{
			$this->setState('filter.featured', $params->get('filter_nicegallery_featured'));
			
		}
		if ($params->get('filter_nicegallery_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_nicegallery_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.nicegallery_id'));
		$id .= ':'.$this->getState('filter.nicegallery_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of nicegalleries subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__nicegallery_nicegalleries').' AS a');
		...
		$can_publish = $user->authorise('core.edit.state', 'com_nicegallery');
		...
		// Filter by a single or group of nicegalleries.
		$nicegallery_id = $this->getState('filter.nicegallery_id');
		if ($nicegallery_id != '')
		{
			if (is_numeric($nicegallery_id))
			{
				$type = $this->getState('filter.nicegallery_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $nicegallery_id);
			}
			else
			{
				if (is_array($nicegallery_id))
				{
					JArrayHelper::toInteger($nicegallery_id);
					$nicegallery_id = implode(',', $nicegallery_id);
					$type = $this->getState('filter.nicegallery_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$nicegallery_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_nicegallery_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			...
			switch ($params->get('show_nicegallery_filter_field'))
			{
				case 'hits':
	...
	/**
	 * Method to get a list of nicegalleries.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				$nicegallery_params = new JRegistry;
				$nicegallery_params->loadString($item->params);
				...
				$item->nicegallery_alternative_readmore = $nicegallery_params->get('nicegallery_alternative_readmore');
				$item->layout = $nicegallery_params->get('layout');
							
				if (!is_object($this->getState('params')))
				{
					$item->params = $nicegallery_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Nicegallery params override menu item params only if menu param = 'use_nicegallery'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_nicegallery' and there is no nicegallery param, use global

					// create an array of just the params set to 'use_nicegallery'
					$menu_params_array = $this->getState('params')->toArray();
					$nicegallery_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_nicegallery')
						{
							// if the nicegallery has a value, use it
							if ($nicegallery_params->get($key) != '')
							{
								// get the value from the nicegallery
								$nicegallery_array[$key] = $nicegallery_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$nicegallery_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected nicegallery params
					if (count($nicegallery_array) > 0)
					{
						$nicegallery_params = new JRegistry;
						$nicegallery_params->loadArray($nicegallery_array);
						$item->params->merge($nicegallery_params);
					}


					// get display date
					switch ($item->params->get('list_show_nicegallery_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an nicegallery, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_nicegallery.nicegallery.'.$item->id;
				...
				if ($access) 
				{
					// If the access filter has been set, we already have only the nicegalleries this user can view.
