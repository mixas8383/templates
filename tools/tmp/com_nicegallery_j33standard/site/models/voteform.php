...
require_once JPATH_ADMINISTRATOR.'/components/com_nicegallery/models/vote.php';

class NicegalleryModelVoteForm extends NicegalleryModelVote
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_nicegallery.edit.vote';
	/**
	...
	*/
	 ...
	 *
	 * @param	integer	$item_id	The id of the vote.
	 *
	 * @return	mixed	Vote item data object on success, false on failure.
	 */
	public function getItem($item_id = null)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('vote.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_nicegallery.edit.vote.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('vote.id');
		$is_new		= true;
		...
		// Reorder the votes so the new vote is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onVoteBeforeSave', array('com_nicegallery.vote', &$table, $is_new));
		...
		$dispatcher->trigger('onVoteAfterSave', array('com_nicegallery.vote', $table, $is_new));

		$this->setState('vote.id', $table->id);
