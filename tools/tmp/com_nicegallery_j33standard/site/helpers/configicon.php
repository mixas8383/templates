...
class JHTMLConfigIcon
{
	...
	public static function create($params, $attribs = array(), $legacy = false)
	{
		...
		$url = 'index.php?option=com_nicegallery&task=config.add&layout=edit&return='.base64_encode($uri);
		
		if ($params->get('show_config_icons'))
		{
			if ($legacy)
			{
				$text = JHtml::_('image', 'com_nicegallery/new.png', JText::_('COM_NICEGALLERY_CONFIGS_CREATE_ITEM'), NULL, true);
			}
			else
			{
				$text = '<span class="icon-plus"></span>&#160;' . JText::_('COM_NICEGALLERY_CONFIGS_CREATE_ITEM') . '&#160;';
			}		
		}
		else
		{
		...
		$output = '<span class="hasTooltip tip" title="'.JHtml::tooltipText('COM_NICEGALLERY_CONFIGS_CREATE_ITEM').'">'.$button.'</span>';
		return $output;
	}
	/**
	...
	public static function edit($config, $params, $attribs = array(), $legacy = false)
	{
		...
		$url	= 'index.php?option=com_nicegallery&task=config.edit&layout=edit&id='.$config->id.'&return='.base64_encode($uri);
		...
		if ($params->get('show_config_icons'))
		{
			if ($legacy)
			{
				$icon	= $config->state ? 'com_nicegallery/edit.png' : 'com_nicegallery/edit_unpublished.png';
				...
				$text	= JHtml::_('image','system/' .$icon, JText::_('COM_NICEGALLERY_CONFIGS_EDIT_ITEM'), NULL, true);
			}
			else
			{
				...
				$text = '<span class="hasTooltip icon-' . $icon . ' tip" title="' . JHtml::tooltipText(JText::_('COM_NICEGALLERY_CONFIGS_EDIT_ITEM'), $overlib, 0) . '"></span>&#160;' . JText::_('JGLOBAL_EDIT') . '&#160;';
			}			
		}
		else
		{
			$text = '<span class="hasTooltip tip" title="' . JHtml::tooltipText(JText::_('COM_NICEGALLERY_CONFIGS_EDIT_ITEM'), $overlib, 0) . '"></span>' . JText::_('JGLOBAL_EDIT') . '</span>';
		}
	...
	public static function delete($config, $params, $attribs = array(), $legacy = false)
	{
		...
		$url = 'index.php?option=com_nicegallery&task=config.delete&id='.$config->id.'&'.JSession::getFormToken().'=1'.'&return='.base64_encode($uri);
		...
		$overlib .= '&lt;br /&gt;';
		$overlib .= JText::sprintf('COM_NICEGALLERY_CREATED_BY', htmlspecialchars($created_by, ENT_COMPAT, 'UTF-8'));
		$attribs['onclick'] = "if(confirm('".JText::_('COM_NICEGALLERY_CONFIGS_DELETE_ITEM_CONFIRM')."'))
		...
		{		
			if ($legacy)
			{
				...
				$text = JHtml::_('image', 'system/' . $icon, JText::_('COM_NICEGALLERY_CONFIGS_DELETE_ITEM'), null, true);
			}
			else
			{
				$text = '<span class="hasTooltip icon-delete tip" title="' . JHtml::tooltipText(JText::_('COM_NICEGALLERY_CONFIGS_DELETE_ITEM'), $overlib, 0) . '"></span>&#160;' . JText::_('JACTION_DELETE') . '&#160;';
			}
		}
		else
		{
			$text = '<span class="hasTooltip tip" title="' . JHtml::tooltipText(JText::_('COM_NICEGALLERY_CONFIGS_DELETE_ITEM'), $overlib, 0) . '">' . JText::_('JACTION_DELETE') . '</span>';
		}		
	...
	public static function email($config, $params, $attribs = array(), $legacy = false)
	{
		...
		$link	= $base.JRoute::_(NicegalleryHelperRoute::getConfigRoute($config->slug,
									$config->language,
									$layout,
	...
	public static function print_popup($config, $params, $attribs = array(), $legacy = false)
	{
		...
		$link	= JRoute::_(NicegalleryHelperRoute::getConfigRoute($config->slug,
									$config->language,
									$layout,
									$params->get('keep_config_itemid')) , false);
