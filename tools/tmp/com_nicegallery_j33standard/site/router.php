...
/**
 * Routing class for the Nicegallery component
 *
 */
class NicegalleryRouter extends JComponentRouterBase
{
	/**
	 * Build the route for the Nicegallery component
	 *
	...
	public function build(&$query)
	{
		...
		$menu	= $app->getMenu();
		$params	= JComponentHelper::getParams('com_nicegallery');
		...
		// Are we dealing with a nicegallery that is attached to a menu item?
	...
	public function parse(&$segments)
	{
		...
		{
			$categories = JCategories::getInstance('Nicegallery',$options)->get($id)->getChildren();
		}
		else
		{
			$categories = JCategories::getInstance('Nicegallery',$options);	
		...
		foreach($segments as $segment)
		{
			...
			if ($found == 0)
			{
				if ($item->query['view'] == 'comentses' OR $item->query['view'] == 'coments'
					OR $view == 'comentses' OR $view == 'coments')
				{
					if($advanced)
					{
						$db = JFactory::getDbo();
						...
						$query->from($db->quoteName('#__nicegallery_comentses'));
				...
				if ($item->query['view'] == 'votes' OR $item->query['view'] == 'vote'
					OR $view == 'votes' OR $view == 'vote')
				{
					if($advanced)
					{
						$db = JFactory::getDbo();
						...
						$query = $db->getQuery(true);
						$query->select($db->quoteName('id'));
						$query->from($db->quoteName('#__nicegallery_votes'));
						$query->where($db->quoteName('alias') . ' = ' . $db->quote($segment));
				...
				if ($item->query['view'] == 'configs' OR $item->query['view'] == 'config'
					OR $view == 'configs' OR $view == 'config')
				{
					if($advanced)
					{
						$db = JFactory::getDbo();
						...
						$query = $db->getQuery(true);
						$query->select($db->quoteName('id'));
						$query->from($db->quoteName('#__nicegallery_configs'));
						$query->where($db->quoteName('alias') . ' = ' . $db->quote($segment));
				...
				if ($item->query['view'] == 'images' OR $item->query['view'] == 'image'
					OR $view == 'images' OR $view == 'image')
				{
					if($advanced)
					{
						$db = JFactory::getDbo();
						...
						$query = $db->getQuery(true);
						$query->select($db->quoteName('id'));
						$query->from($db->quoteName('#__nicegallery_images'));
