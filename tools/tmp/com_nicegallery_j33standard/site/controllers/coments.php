...
/**
 * Coments controller class.
 * 
 */
class NicegalleryControllerComents extends JControllerForm
{
	/**
	 * @var    string	$view_item	The URL view item variable.
	 */
	protected $view_item = 'comentsform';
	/**
	 * @var    string	$view_list	The URL view list variable.
	 */
	protected $view_list = 'comentses';
	/**
	 * 
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		$user		= JFactory::getUser();
		$asset		= 'com_nicegallery.coments.'.$record_id;
		...
		if ($user->authorise('core.edit.own', $asset))
		{
			...
			if (empty($owner_id) AND $record_id)
			{
				...
				$record		= $this->getModel('comentsform')->getItem($record_id);
	 ...
	 * 
	 */
	public function getModel($name = 'comentsform', $prefix = '',$config = array('ignore_request' => true))
	{
	 ...
	 *
	 * @return  mixed  True if the Coments can be added, a error object if not.
	 *
	 */
	public function add()
	{
	...
	public function edit($key = 'id', $url_var = null)
	{
		...
		$model = $this->getModel('comentsform');
		// Set the return url
		$this->setReturnPage($context);
	...
	public function save($key = 'id', $url_var = null)
	{
		...
		if ($record_id == 0) 
		{
			$this->setMessage(JText::_('COM_NICEGALLERY_COMENTSES_SUBMIT_SAVE_SUCCESS'));
		} 
		else 
		{
			$this->setMessage(JText::_('COM_NICEGALLERY_COMENTSES_SAVE_SUCCESS'));
		}
		...
		switch ($task)
		{
			case 'apply':
				...
				$record_id = $model->getState('coments.id');
				$this->holdEditId($context, $record_id);
				$app->setUserState($context.'.data',	null);
	...
	public function delete()
	{
		...
		if ($id > 0) 
		{
			...
			{
				$this->setMessage(JText::_('COM_NICEGALLERY_COMENTSES_DELETE_SUCCESS'));
				
			}
			else
			{
				$this->setMessage(JText::_('COM_NICEGALLERY_COMENTSES_DELETE_FAILED'));
			}
		}
