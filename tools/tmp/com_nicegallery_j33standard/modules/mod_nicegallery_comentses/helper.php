...
JModelLegacy::addIncludePath(JPATH_SITE.'/components/com_nicegallery/models', 'NicegalleryModelComentses');

abstract class ModComentsesHelper
{
	/**
	...
	public static function getList(&$params)
	{
		...
		// Get an instance of the generic comentses model
		$model = JModelLegacy::getInstance('Comentses', 'NicegalleryModel', array('ignore_request' => true));
		...
		// Access filter
		$access = !JComponentHelper::getParams('com_nicegallery')->get('show_coments_noauth');
		...
		if ($params->get('coments_layout') <> '')
		{
			$layout = str_replace('_:','',(string) $params->get('coments_layout'));
		...
		foreach ($items as &$item)
		{
			...
			if ($access OR in_array($item->access, $authorised))
			{
				if ($item_id_str == '')
				{
					// We know that user has the privilege to view the coments
					$item->link = JRoute::_(NicegalleryHelperRoute::getComentsRoute($item->slug, $item->language, $layout));
				}
				else
				{
					$item->link = JRoute::_('index.php?option=com_nicegallery&view=coments'.$layout_str.$item_id_str.'&id='.$item->id);
				}					
			}
			else
			{
