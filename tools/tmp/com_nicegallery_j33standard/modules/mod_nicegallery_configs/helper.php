...
JModelLegacy::addIncludePath(JPATH_SITE.'/components/com_nicegallery/models', 'NicegalleryModelConfigs');
...
abstract class ModConfigsHelper
{
	...
	public static function getList(&$params)
	{
		...
		$model = JModelLegacy::getInstance('Configs', 'NicegalleryModel', array('ignore_request' => true));
		...
		// Access filter
		$access = !JComponentHelper::getParams('com_nicegallery')->get('show_config_noauth');
		...
		foreach ($items as &$item)
		{
			...
			if ($access OR in_array($item->access, $authorised))
			{
				if ($item_id_str == '')
				{
					...
					$item->link = JRoute::_(NicegalleryHelperRoute::getConfigRoute($item->slug, $item->language, $layout));
				}
				else
				{
					$item->link = JRoute::_('index.php?option=com_nicegallery&view=config'.$layout_str.$item_id_str.'&id='.$item->id);
				}					
			}
			else
			{
