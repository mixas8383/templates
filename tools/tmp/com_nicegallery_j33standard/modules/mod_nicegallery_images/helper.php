...
JModelLegacy::addIncludePath(JPATH_SITE.'/components/com_nicegallery/models', 'NicegalleryModelImages');
...
abstract class ModImagesHelper
{
	...
	public static function getList(&$params)
	{
		...
		$model = JModelLegacy::getInstance('Images', 'NicegalleryModel', array('ignore_request' => true));
		...
		// Access filter
		$access = !JComponentHelper::getParams('com_nicegallery')->get('show_image_noauth');
		...
		if ($params->get('image_layout') <> '')
		{
			$layout = str_replace('_:','',(string) $params->get('image_layout'));
		...
		foreach ($items as &$item)
		{
			...
			if ($access OR in_array($item->access, $authorised))
			{
				if ($item_id_str == '')
				{
					// We know that user has the privilege to view the image
					$item->link = JRoute::_(NicegalleryHelperRoute::getImageRoute($item->slug, $item->catslug, $item->language, $layout));
				}
				else
				{
					$item->link = JRoute::_('index.php?option=com_nicegallery&view=image'.$layout_str.$item_id_str.'&id='.$item->id);
				}					
			}
			else
			{
