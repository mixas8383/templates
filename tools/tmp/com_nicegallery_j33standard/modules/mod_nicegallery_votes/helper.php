...
JModelLegacy::addIncludePath(JPATH_SITE.'/components/com_nicegallery/models', 'NicegalleryModelVotes');

abstract class ModVotesHelper
{
	/**
	...
	public static function getList(&$params)
	{
		...
		// Get an instance of the generic votes model
		$model = JModelLegacy::getInstance('Votes', 'NicegalleryModel', array('ignore_request' => true));
		...
		// Access filter
		$access = !JComponentHelper::getParams('com_nicegallery')->get('show_vote_noauth');
		...
		if ($params->get('vote_layout') <> '')
		{
			$layout = str_replace('_:','',(string) $params->get('vote_layout'));
		...
		foreach ($items as &$item)
		{
			...
			if ($access OR in_array($item->access, $authorised))
			{
				if ($item_id_str == '')
				{
					// We know that user has the privilege to view the vote
					$item->link = JRoute::_(NicegalleryHelperRoute::getVoteRoute($item->slug, $item->language, $layout));
				}
				else
				{
					$item->link = JRoute::_('index.php?option=com_nicegallery&view=vote'.$layout_str.$item_id_str.'&id='.$item->id);
				}					
			}
			else
			{
