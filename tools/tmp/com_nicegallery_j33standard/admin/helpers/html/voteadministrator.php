...
/**
 * Votes component helper.
 *
 */
abstract class JHtmlVoteAdministrator
{
	/**
	 * Render the list of associated items
	 * 
	 * @param	integer $vote_id	The vote item id
	 * 
	 * @return  string  The language HTML
     */
	public static function association($vote_id)
	{
		...
		// Get the associations
		if ($associations = JLanguageAssociations::getAssociations('com_nicegallery', '#__nicegallery_votes', 'com_nicegallery.vote.item',  $vote_id, 'id', 'alias', null))
		{		
			...
			if ($items)
			{
				foreach ($items as &$item)
				{
					...
					$url = JRoute::_('index.php?option=com_nicegallery&task=vote.edit&id=' . (int) $item->id);
					$tooltip_parts = array(
	...
	public static function featured($value = 0, $i, $can_change = true)
	{
		...
		$states	= array(
			0	=> array('unfeatured', 'votes.featured', 'COM_NICEGALLERY_VOTES_UNFEATURED', 'COM_NICEGALLERY_VOTES_TOGGLE_TO_FEATURE'),
			1	=> array('featured', 'votes.unfeatured', 'JFEATURED', 'COM_NICEGALLERY_VOTES_TOGGLE_TO_UNFEATURE'),
		);
