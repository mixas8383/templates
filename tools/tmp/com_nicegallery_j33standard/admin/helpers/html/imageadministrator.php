...
/**
 * Images component helper.
 *
 */
abstract class JHtmlImageAdministrator
{
	/**
	 * Render the list of associated items
	 * 
	 * @param	integer $image_id	The image item id
	 * 
	 * @return  string  The language HTML
     */
	public static function association($image_id)
	{
		...
		// Get the associations
		if ($associations = JLanguageAssociations::getAssociations('com_nicegallery', '#__nicegallery_images', 'com_nicegallery.image.item',  $image_id, 'id', 'alias', 'catid'))
		{		
			...
			if ($items)
			{
				foreach ($items as &$item)
				{
					...
					$url = JRoute::_('index.php?option=com_nicegallery&task=image.edit&id=' . (int) $item->id);
					$tooltip_parts = array(
	...
	public static function featured($value = 0, $i, $can_change = true)
	{
		...
		$states	= array(
			0	=> array('unfeatured', 'images.featured', 'COM_NICEGALLERY_IMAGES_UNFEATURED', 'COM_NICEGALLERY_IMAGES_TOGGLE_TO_FEATURE'),
			1	=> array('featured', 'images.unfeatured', 'JFEATURED', 'COM_NICEGALLERY_IMAGES_TOGGLE_TO_UNFEATURE'),
		);
