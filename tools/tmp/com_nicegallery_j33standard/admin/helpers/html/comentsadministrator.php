...
/**
 * Comentses component helper.
 *
 */
abstract class JHtmlComentsAdministrator
{
	/**
	 * Render the list of associated items
	 * 
	 * @param	integer $coments_id	The coments item id
	 * 
	 * @return  string  The language HTML
     */
	public static function association($coments_id)
	{
		...
		// Get the associations
		if ($associations = JLanguageAssociations::getAssociations('com_nicegallery', '#__nicegallery_comentses', 'com_nicegallery.coments.item',  $coments_id, 'id', 'alias', null))
		{		
			...
			if ($items)
			{
				foreach ($items as &$item)
				{
					...
					$url = JRoute::_('index.php?option=com_nicegallery&task=coments.edit&id=' . (int) $item->id);
					$tooltip_parts = array(
	...
	public static function featured($value = 0, $i, $can_change = true)
	{
		...
		$states	= array(
			0	=> array('unfeatured', 'comentses.featured', 'COM_NICEGALLERY_COMENTSES_UNFEATURED', 'COM_NICEGALLERY_COMENTSES_TOGGLE_TO_FEATURE'),
			1	=> array('featured', 'comentses.unfeatured', 'JFEATURED', 'COM_NICEGALLERY_COMENTSES_TOGGLE_TO_UNFEATURE'),
		);
