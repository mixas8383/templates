...
abstract class JHtmlConfigAdministrator
{
	...
	public static function association($config_id)
	{
		...
		// Get the associations
		if ($associations = JLanguageAssociations::getAssociations('com_nicegallery', '#__nicegallery_configs', 'com_nicegallery.config.item',  $config_id, 'id', 'alias', null))
		{		
			...
			if ($items)
			{
				foreach ($items as &$item)
				{
					...
					$url = JRoute::_('index.php?option=com_nicegallery&task=config.edit&id=' . (int) $item->id);
					$tooltip_parts = array(
	...
	public static function featured($value = 0, $i, $can_change = true)
	{
		...
		$states	= array(
			0	=> array('unfeatured', 'configs.featured', 'COM_NICEGALLERY_CONFIGS_UNFEATURED', 'COM_NICEGALLERY_CONFIGS_TOGGLE_TO_FEATURE'),
			1	=> array('featured', 'configs.unfeatured', 'JFEATURED', 'COM_NICEGALLERY_CONFIGS_TOGGLE_TO_UNFEATURE'),
		);
