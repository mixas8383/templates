...
JLoader::register('ContentHelper', JPATH_ADMINISTRATOR . '/components/com_nicegallery/helpers/nicegallery.php');
/**
 * Nicegalleries component helper.
 *
 */
abstract class JHtmlNicegalleryAdministrator
{
	/**
	 * Render the list of associated items
	 * 
	 * @param	integer $nicegallery_id	The nicegallery item id
	 * 
	 * @return  string  The language HTML
     */
	public static function association($nicegallery_id)
	{
		...
		// Get the associations
		if ($associations = JLanguageAssociations::getAssociations('com_nicegallery', '#__nicegallery_nicegalleries', 'com_nicegallery.nicegallery.item',  $nicegallery_id, 'id', 'alias', 'catid'))
		{		
			...
			if ($items)
			{
				foreach ($items as &$item)
				{
					...
					$url = JRoute::_('index.php?option=com_nicegallery&task=nicegallery.edit&id=' . (int) $item->id);
					$tooltip_parts = array(
	...
	public static function featured($value = 0, $i, $can_change = true)
	{
		...
		$states	= array(
			0	=> array('unfeatured', 'nicegalleries.featured', 'COM_NICEGALLERY_NICEGALLERIES_UNFEATURED', 'COM_NICEGALLERY_NICEGALLERIES_TOGGLE_TO_FEATURE'),
			1	=> array('featured', 'nicegalleries.unfeatured', 'JFEATURED', 'COM_NICEGALLERY_NICEGALLERIES_TOGGLE_TO_UNFEATURE'),
		);
