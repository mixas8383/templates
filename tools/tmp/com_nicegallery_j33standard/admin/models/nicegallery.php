 ...
 *
 */
class NicegalleryModelNicegallery extends JModelAdmin
{
	/**
	 * @var		string	The prefix to use with controller messages.
	 */
	protected $text_prefix = 'COM_NICEGALLERY_NICEGALLERIES';
	/**
	 * @var      string	The type alias for this object (for example, 'com_nicegallery.nicegallery')
	 */
	public $typeAlias = 'com_nicegallery.nicegallery';	
	/**
	 ...
	 */
	protected $event_before_save = 'onNicegalleryBeforeSave';
	/**
	 * @var		string	The event to trigger after saving the data.
	 */
	protected $event_after_save = 'onNicegalleryAfterSave';

	/**
	 * @var    string	The event to trigger before deleting the data.
	 */
	protected $event_before_delete = 'onNicegalleryBeforeDelete';	
	/**
	 * @var    string	The event to trigger after deleting the data.
	 */
	protected $event_after_delete = 'onNicegalleryAfterDelete';	
	/**
	 * @var    string	The event to trigger after changing the data's state field.
	 */
	protected $event_change_state = 'onNicegalleryChangeState';	
	 ...
	 */
	public function getTable($type = 'Nicegalleries', $prefix = 'NicegalleryTable', $config = array())
	{
	...
	protected function canDelete($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.delete', 'com_nicegallery.nicegallery.'.(int) $record->id);
		}
		else
		{
			return $user->authorise('core.delete', 'com_nicegallery');
		}							
	}
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.edit.state', 'com_nicegallery.nicegallery.'.(int) $record->id);
		}
		else
		{
			// New nicegallery, so check against the category.		
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_nicegallery.nicegallery');
			}	
		}
		...
		if ($assoc)
		{
			...
			if ($item->id != null)
			{
				$associations = JLanguageAssociations::getAssociations('com_nicegallery', '#__nicegallery_nicegalleries', 'com_nicegallery.nicegallery.item', $item->id, 'id', 'alias', 'catid');
	...
	{
		$form = $this->loadForm('com_nicegallery.edit.nicegallery', 'nicegallery', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
		...
		if ($this->getState('nicegallery.id'))
		{
			$id = $this->getState('nicegallery.id');		
			// Existing record. Can only edit in selected categories.
			$form->setFieldAttribute('catid', 'action', 'core.edit');
			// Existing record. Can only edit own nicegalleries in selected categories.
		...
		// Prevent messing with Nicegallery language and category when editing existing Nicegallery with associations
	...
	protected function loadFormData()
	{
		...
		$data = $app->getUserState('com_nicegallery.edit.nicegallery.data', array());

		if (empty($data))
		{
			$data = $this->getItem();
			// Prime some default values.
			if ($this->getState('nicegallery.id') == 0)
			{
				$filters = (array) $app->getUserState('com_nicegallery.nicegalleries.filter');
	...
	protected function prepareTable($table)
	{
		...
		// Increment the nicegallery version number.
		$table->version++;
	...
	{
		// Include the nicegallery plugins for the onSave events.
		...
		if (parent::save($data))
		{
			...
			if ($assoc)
			{
				...
				{
					JError::raiseNotice(403, JText::_('COM_NICEGALLERY_ERROR_ALL_LANGUAGE_ASSOCIATED'));
				}
				...
				try
				{
					...
					$query->where('context='.$db->quote('com_nicegallery.nicegallery.item'));
				...
				if (!$all_language AND count($associations))
				{
					...
					{
						$query->values($id.','.$db->quote('com_nicegallery.nicegallery.item') . ',' . $db->quote($key));
					}
	...
	{
		// Include the nicegallery plugins for the delete events.
		JPluginHelper::importPlugin('nicegallery');	
	...
	/**
	 * Method to toggle the featured setting of nicegalleries.
	 *
	...
	public function featured($pks, $value = 0)
	{
		...
		if (empty($pks))
		{
			$this->setError(JText::_('COM_NICEGALLERY_NICEGALLERIES_NO_ITEM_SELECTED'));
			return false;
		}
		...
		try
		{
			...
			$query = $db->getQuery(true);
			$query->update($db->quoteName('#__nicegallery_nicegalleries'));
	...
	/**
	 * Custom clean the cache of com_nicegallery and nicegallery modules
	 *
	 */
	protected function cleanCache($group = null, $client_id = 0)
	{
		parent::cleanCache('com_nicegallery');
	...
	* @param   array   $data	The data where the original name is stored
	...
	/**
	 * Pre process the form to pick up nicegalleries associated by language
	 *
	 ...
	 */
	protected function preprocessForm(JForm $form, $data, $group = 'nicegallery')
	{
		...
		if ($assoc)
		{
			...
			$fieldset->addAttribute('description', 'COM_NICEGALLERY_NICEGALLERIES_ITEM_ASSOCIATIONS_FIELDSET_DESC');
			$add = false;
			foreach ($languages as $tag => $language)
			{
				if (empty($data->language) OR $tag != $data->language)
				{
					...
					$field->addAttribute('type', 'modal_nicegalleries');
