 ...
 *
 */
class NicegalleryModelComents extends JModelAdmin
{
	/**
	 * @var		string	The prefix to use with controller messages.
	 */
	protected $text_prefix = 'COM_NICEGALLERY_COMENTSES';
	/**
	 * @var      string	The type alias for this object (for example, 'com_nicegallery.coments')
	 */
	public $typeAlias = 'com_nicegallery.coments';	
	/**
	 ...
	 */
	protected $event_before_save = 'onComentsBeforeSave';
	/**
	 * @var		string	The event to trigger after saving the data.
	 */
	protected $event_after_save = 'onComentsAfterSave';

	/**
	 * @var    string	The event to trigger before deleting the data.
	 */
	protected $event_before_delete = 'onComentsBeforeDelete';	
	/**
	 * @var    string	The event to trigger after deleting the data.
	 */
	protected $event_after_delete = 'onComentsAfterDelete';	
	/**
	 * @var    string	The event to trigger after changing the data's state field.
	 */
	protected $event_change_state = 'onComentsChangeState';	
	 ...
	 */
	public function getTable($type = 'Comentses', $prefix = 'NicegalleryTable', $config = array())
	{
	...
	protected function canDelete($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.delete', 'com_nicegallery.coments.'.(int) $record->id);
		}
		else
		{
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.edit.state', 'com_nicegallery.coments.'.(int) $record->id);
		}
		else
		{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_nicegallery.coments');
			}	
		}
		...
		if ($assoc)
		{
			...
			if ($item->id != null)
			{
				$associations = JLanguageAssociations::getAssociations('com_nicegallery', '#__nicegallery_comentses', 'com_nicegallery.coments.item', $item->id, 'id', 'alias', null);
	...
	{
		$form = $this->loadForm('com_nicegallery.edit.coments', 'coments', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
		...
		// Prevent messing with Coments language and category when editing existing Coments with associations
	...
	protected function loadFormData()
	{
		...
		$data = $app->getUserState('com_nicegallery.edit.coments.data', array());
	...
	protected function prepareTable($table)
	{
		...
		// Increment the coments version number.
		$table->version++;
		...
		if (parent::save($data))
		{
			...
			if ($assoc)
			{
				...
				try
				{
					...
					$query->where('context='.$db->quote('com_nicegallery.coments.item'));
				...
				if (!$all_language AND count($associations))
				{
					...
					{
						$query->values($id.','.$db->quote('com_nicegallery.coments.item') . ',' . $db->quote($key));
					}
	...
	/**
	 * Method to toggle the featured setting of comentses.
	 *
	...
	public function featured($pks, $value = 0)
	{
		...
		if (empty($pks))
		{
			$this->setError(JText::_('COM_NICEGALLERY_COMENTSES_NO_ITEM_SELECTED'));
			return false;
		}
		...
		try
		{
			...
			$query = $db->getQuery(true);
			$query->update($db->quoteName('#__nicegallery_comentses'));
	...
	protected function cleanCache($group = null, $client_id = 0)
	{
		...
		parent::cleanCache('mod_nicegallery');
		parent::cleanCache('mod_nicegallery_comentses');
	...
	* @param   array   $data	The data where the original name is stored
	...
	/**
	 * Pre process the form to pick up comentses associated by language
	 *
	 ...
	 */
	protected function preprocessForm(JForm $form, $data, $group = 'coments')
	{
		...
		if ($assoc)
		{
			...
			$fieldset->addAttribute('description', 'COM_NICEGALLERY_COMENTSES_ITEM_ASSOCIATIONS_FIELDSET_DESC');
			$add = false;
			foreach ($languages as $tag => $language)
			{
				if (empty($data->language) OR $tag != $data->language)
				{
					...
					$field->addAttribute('type', 'modal_comentses');
