 ...
 *
 */
class NicegalleryModelConfig extends JModelAdmin
{
	/**
	 * @var		string	The prefix to use with controller messages.
	 */
	protected $text_prefix = 'COM_NICEGALLERY_CONFIGS';
	/**
	 * @var      string	The type alias for this object (for example, 'com_nicegallery.config')
	 */
	public $typeAlias = 'com_nicegallery.config';	
	/**
	 * @var		string	The context for the app call.
	 */
	protected $context = 'com_nicegallery.configs';	
	/**
	 * @var		string	The event to trigger after before the data.
	 */
	protected $event_before_save = 'onConfigBeforeSave';
	/**
	 * @var		string	The event to trigger after saving the data.
	 */
	protected $event_after_save = 'onConfigAfterSave';

	/**
	 * @var    string	The event to trigger before deleting the data.
	 */
	protected $event_before_delete = 'onConfigBeforeDelete';	
	/**
	 * @var    string	The event to trigger after deleting the data.
	 */
	protected $event_after_delete = 'onConfigAfterDelete';	
	/**
	 * @var    string	The event to trigger after changing the data's state field.
	 */
	protected $event_change_state = 'onConfigChangeState';	
	 ...
	 */
	public function getTable($type = 'Configs', $prefix = 'NicegalleryTable', $config = array())
	{
	...
	protected function canDelete($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.delete', 'com_nicegallery.config.'.(int) $record->id);
		}
		else
		{
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.edit.state', 'com_nicegallery.config.'.(int) $record->id);
		}
		else
		{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_nicegallery.config');
			}	
		}
		...
		if ($assoc)
		{
			...
			if ($item->id != null)
			{
				$associations = JLanguageAssociations::getAssociations('com_nicegallery', '#__nicegallery_configs', 'com_nicegallery.config.item', $item->id, 'id', 'alias', null);
	...
	{
		$form = $this->loadForm('com_nicegallery.edit.config', 'config', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
		...
		// Prevent messing with Config language and category when editing existing Config with associations
	...
	protected function loadFormData()
	{
		...
		$data = $app->getUserState('com_nicegallery.edit.config.data', array());
		...
		if (parent::save($data))
		{
			...
			if ($assoc)
			{
				...
				try
				{
					...
					$query->where('context='.$db->quote('com_nicegallery.config.item'));
				...
				if (!$all_language AND count($associations))
				{
					...
					{
						$query->values($id.','.$db->quote('com_nicegallery.config.item') . ',' . $db->quote($key));
					}
	...
	/**
	 * Method to toggle the featured setting of configs.
	 *
	...
	public function featured($pks, $value = 0)
	{
		...
		if (empty($pks))
		{
			$this->setError(JText::_('COM_NICEGALLERY_CONFIGS_NO_ITEM_SELECTED'));
			return false;
		}
		...
		try
		{
			...
			$query = $db->getQuery(true);
			$query->update($db->quoteName('#__nicegallery_configs'));
	...
	protected function cleanCache($group = null, $client_id = 0)
	{
		...
		parent::cleanCache('mod_nicegallery_configs');
	...
	* @param   array   $data	The data where the original name is stored
	...
	/**
	 * Pre process the form to pick up configs associated by language
	 *
	 ...
	 */
	protected function preprocessForm(JForm $form, $data, $group = 'config')
	{
		...
		if ($assoc)
		{
			...
			$fieldset->addAttribute('description', 'COM_NICEGALLERY_CONFIGS_ITEM_ASSOCIATIONS_FIELDSET_DESC');
			$add = false;
			foreach ($languages as $tag => $language)
			{
				if (empty($data->language) OR $tag != $data->language)
				{
					...
					$field->addAttribute('type', 'modal_configs');
