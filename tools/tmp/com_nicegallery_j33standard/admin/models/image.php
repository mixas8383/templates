 ...
 *
 */
class NicegalleryModelImage extends JModelAdmin
{
	/**
	 * @var		string	The prefix to use with controller messages.
	 */
	protected $text_prefix = 'COM_NICEGALLERY_IMAGES';
	/**
	 * @var      string	The type alias for this object (for example, 'com_nicegallery.image')
	 */
	public $typeAlias = 'com_nicegallery.image';	
	/**
	 * @var		string	The context for the app call.
	 */
	protected $context = 'com_nicegallery.images';	
	/**
	 * @var		string	The event to trigger after before the data.
	 */
	protected $event_before_save = 'onImageBeforeSave';
	/**
	 * @var		string	The event to trigger after saving the data.
	 */
	protected $event_after_save = 'onImageAfterSave';

	/**
	 * @var    string	The event to trigger before deleting the data.
	 */
	protected $event_before_delete = 'onImageBeforeDelete';	
	/**
	 * @var    string	The event to trigger after deleting the data.
	 */
	protected $event_after_delete = 'onImageAfterDelete';	
	/**
	 * @var    string	The event to trigger after changing the data's state field.
	 */
	protected $event_change_state = 'onImageChangeState';	
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_nicegallery.image');
			}	
		}
		...
		if ($assoc)
		{
			...
			if ($item->id != null)
			{
				$associations = JLanguageAssociations::getAssociations('com_nicegallery', '#__nicegallery_images', 'com_nicegallery.image.item', $item->id, 'id', 'alias', 'catid');
	...
	{
		$form = $this->loadForm('com_nicegallery.edit.image', 'image', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
		...
		if ($this->getState('image.id'))
		{
			...
			// Existing record. Can only edit own images in selected categories.
		...
		// Prevent messing with Image language and category when editing existing Image with associations
	...
	protected function loadFormData()
	{
		...
		$data = $app->getUserState('com_nicegallery.edit.image.data', array());
		...
		if (empty($data))
		{
			...
			{
				$filters = (array) $app->getUserState('com_nicegallery.images.filter');
	...
	protected function prepareTable($table)
	{
		...
		// Increment the image version number.
		$table->version++;
		...
		if (parent::save($data))
		{
			...
			if ($assoc)
			{
				...
				try
				{
					...
					$query->where('context='.$db->quote('com_nicegallery.image.item'));
				...
				if (!$all_language AND count($associations))
				{
					...
					{
						$query->values($id.','.$db->quote('com_nicegallery.image.item') . ',' . $db->quote($key));
					}
	...
	/**
	 * Method to toggle the featured setting of images.
	 *
	...
	public function featured($pks, $value = 0)
	{
		...
		if (empty($pks))
		{
			$this->setError(JText::_('COM_NICEGALLERY_IMAGES_NO_ITEM_SELECTED'));
			return false;
		}
		...
		try
		{
			...
			$query = $db->getQuery(true);
			$query->update($db->quoteName('#__nicegallery_images'));
	...
	protected function cleanCache($group = null, $client_id = 0)
	{
		...
		parent::cleanCache('mod_nicegallery_images');
	...
	* @param   array   $data	The data where the original name is stored
	...
	/**
	 * Pre process the form to pick up images associated by language
	 *
	 ...
	 */
	protected function preprocessForm(JForm $form, $data, $group = 'image')
	{
		...
		if ($assoc)
		{
			...
			$fieldset->addAttribute('description', 'COM_NICEGALLERY_IMAGES_ITEM_ASSOCIATIONS_FIELDSET_DESC');
			$add = false;
			foreach ($languages as $tag => $language)
			{
				if (empty($data->language) OR $tag != $data->language)
				{
					...
					$field->addAttribute('type', 'modal_images');
