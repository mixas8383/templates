 ...
 *
 */
class NicegalleryModelVote extends JModelAdmin
{
	/**
	...
	/**
	 * @var      string	The type alias for this object (for example, 'com_nicegallery.vote')
	 */
	public $typeAlias = 'com_nicegallery.vote';	
	/**
	 ...
	 */
	protected $event_before_save = 'onVoteBeforeSave';
	/**
	 * @var		string	The event to trigger after saving the data.
	 */
	protected $event_after_save = 'onVoteAfterSave';

	/**
	 * @var    string	The event to trigger before deleting the data.
	 */
	protected $event_before_delete = 'onVoteBeforeDelete';	
	/**
	 * @var    string	The event to trigger after deleting the data.
	 */
	protected $event_after_delete = 'onVoteAfterDelete';	
	/**
	 * @var    string	The event to trigger after changing the data's state field.
	 */
	protected $event_change_state = 'onVoteChangeState';	
	 ...
	 */
	public function getTable($type = 'Votes', $prefix = 'NicegalleryTable', $config = array())
	{
	...
	protected function canDelete($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.delete', 'com_nicegallery.vote.'.(int) $record->id);
		}
		else
		{
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.edit.state', 'com_nicegallery.vote.'.(int) $record->id);
		}
		else
		{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_nicegallery.vote');
			}	
		}
		...
		if ($assoc)
		{
			...
			if ($item->id != null)
			{
				$associations = JLanguageAssociations::getAssociations('com_nicegallery', '#__nicegallery_votes', 'com_nicegallery.vote.item', $item->id, 'id', 'alias', null);
	...
	{
		$form = $this->loadForm('com_nicegallery.edit.vote', 'vote', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
		...
		// Prevent messing with Vote language and category when editing existing Vote with associations
	...
	protected function loadFormData()
	{
		...
		$data = $app->getUserState('com_nicegallery.edit.vote.data', array());
	...
	protected function prepareTable($table)
	{
		...
		// Increment the vote version number.
		$table->version++;
		...
		if (parent::save($data))
		{
			...
			if ($assoc)
			{
				...
				try
				{
					...
					$query->where('context='.$db->quote('com_nicegallery.vote.item'));
				...
				if (!$all_language AND count($associations))
				{
					...
					{
						$query->values($id.','.$db->quote('com_nicegallery.vote.item') . ',' . $db->quote($key));
					}
	...
	/**
	 * Method to toggle the featured setting of votes.
	 *
	...
	public function featured($pks, $value = 0)
	{
		...
		if (empty($pks))
		{
			$this->setError(JText::_('COM_NICEGALLERY_VOTES_NO_ITEM_SELECTED'));
			return false;
		}
		...
		try
		{
			...
			$query = $db->getQuery(true);
			$query->update($db->quoteName('#__nicegallery_votes'));
	...
	protected function cleanCache($group = null, $client_id = 0)
	{
		...
		parent::cleanCache('mod_nicegallery_votes');
	...
	* @param   array   $data	The data where the original name is stored
	...
	/**
	 * Pre process the form to pick up votes associated by language
	 *
	 ...
	 */
	protected function preprocessForm(JForm $form, $data, $group = 'vote')
	{
		...
		if ($assoc)
		{
			...
			$fieldset->addAttribute('description', 'COM_NICEGALLERY_VOTES_ITEM_ASSOCIATIONS_FIELDSET_DESC');
			$add = false;
			foreach ($languages as $tag => $language)
			{
				if (empty($data->language) OR $tag != $data->language)
				{
					...
					$field->addAttribute('type', 'modal_votes');
