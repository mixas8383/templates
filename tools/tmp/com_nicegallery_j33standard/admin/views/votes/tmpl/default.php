...
if ($save_order)
{
	$save_ordering_url = 'index.php?option=com_nicegallery&task=votes.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'vote-list', 'adminForm', strtolower($list_dirn), $save_ordering_url);
}
...
<form action="<?php echo JRoute::_('index.php?option=com_nicegallery&view=votes'); ?>" method="post" name="adminForm" id="adminForm">
	...
	<?php else : ?>
		<table class="table table-striped" id="vote-list">
			<thead>
				<tr>
			...
			<?php foreach ($this->items as $i => $item) :
				...
				$can_edit	= $user->authorise('core.edit',	'com_nicegallery.vote.'.$item->id);
		
				$can_edit_own	= $user->authorise('core.edit.own',		'com_nicegallery.vote.'.$item->id) 
								AND $item->created_by == $user_id
								;
				$can_change	= $user->authorise('core.edit.state',	'com_nicegallery.vote.'.$item->id) 
								AND $can_checkin
								;
				if ($item->language == '*'):
				...
				<tr class="row<?php echo $i % 2; ?>">
					...
					<td class="center">
						<div class="btn-group">
							<?php echo JHtml::_('jgrid.published', $item->state, $i, 'votes.', $can_change, 'cb', $item->publish_up, $item->publish_down); ?>
							<?php echo JHtml::_('voteadministrator.featured', $item->featured, $i, $can_change); ?>
							<?php
								if ($archived) :
									JHtml::_('actionsdropdown.unarchive', 'cb' . $i, 'votes');
								else :
									JHtml::_('actionsdropdown.archive', 'cb' . $i, 'votes');
								endif;
								if ($trashed) :
									JHtml::_('actionsdropdown.untrash', 'cb' . $i, 'votes');
								else :
									JHtml::_('actionsdropdown.trash', 'cb' . $i, 'votes');
								endif;
					...
					<td class="nowrap has-context">
						<div class="pull-left">
							<?php if ($item->checked_out) : ?>
								<?php echo JHtml::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'votes.', $can_checkin); ?>
							<?php endif; ?>	
							<?php if ($can_edit OR $can_edit_own) : ?>
								<a href="<?php echo JRoute::_('index.php?option=com_nicegallery&task=vote.edit&id='.(int) $item->id); ?>">
					...
					<?php if ($assoc) : ?>
						<td class="small hidden-phone">
							<?php if ($item->association) : ?>
								<?php echo JHtml::_('voteadministrator.association', $item->id); ?>
							<?php endif; ?>
						</td>
					<?php endif;?>				
