...
/**
 * View class for a list of votes.
 *
 */
class NicegalleryViewVotes extends JViewLegacy
{
	...
	protected function addToolbar()
	{
		...
		JToolbarHelper::title(JText::_('COM_NICEGALLERY_VOTES_LIST_HEADER'), 'stack votes');
		...
		if ($this->can_do->get('core.edit.state') ) 
		{
			...
			if ($this->state->get('filter.state') != -1 ) 
			{
				...
				else 
				{
					if ($this->state->get('filter.state') == 2) 
					{
						JToolbarHelper::unarchiveList('votes.publish', 'JTOOLBAR_UNARCHIVE');
					}
				}
			}
		}
	...
	{	
		JHtmlSidebar::setAction('index.php?option=com_nicegallery&view=votes');
