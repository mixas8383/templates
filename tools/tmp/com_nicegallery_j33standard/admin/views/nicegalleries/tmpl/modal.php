...
<form action="<?php echo JRoute::_('index.php?option=com_nicegallery&view=nicegalleries&layout=modal&tmpl=component&function='.$function.'&'.JSession::getFormToken().'=1');?>" method="post" name="adminForm" id="adminForm">
		...
		<div class="js-stools-container-filters hidden-phone clearfix shown" style="display: block;">
			...
			<?php if ($this->state->get('filter.forcedLanguage')) : ?>
				<div class="js-stools-field-filter">				
					...
					<?php echo JHtml::_('select.options', JHtml::_('category.options', 'com_nicegallery', array('filter.language' => array('*', $this->state->get('filter.forcedLanguage')))), 'value', 'text', $this->state->get('filter.category_id'));?>
					</select>
	...
	<table class="table table-striped table-condensed">
		...
		<?php foreach ($this->items as $i => $item) : ?>
			...
			<tr class="row<?php echo $i % 2; ?>">
				...
				<td class="center">
					<a class="pointer" href="javascript:void(0)" onclick="if (window.parent) window.parent.<?php echo $this->escape($function);?>('<?php echo $item->id; ?>', '<?php echo $this->escape(addslashes($item->name)); ?>');">
						<?php echo JHtml::_('jgrid.published', $item->state, $i, 'nicegalleries.', false, 'cb'); ?>
					</a>		
				</td>
				<td class="center">
					<a class="pointer" href="javascript:void(0)" onclick="if (window.parent) window.parent.<?php echo $this->escape($function);?>('<?php echo $item->id; ?>', '<?php echo $this->escape(addslashes($item->name)); ?>');">
