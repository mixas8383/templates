...
class NicegalleryViewImages extends JViewLegacy
{
	...
	protected function addToolbar()
	{
		...
		JToolbarHelper::title(JText::_('COM_NICEGALLERY_IMAGES_LIST_HEADER'), 'stack images');
		...
		if ($this->can_do->get('core.edit.state') ) 
		{
			...
			if ($this->state->get('filter.state') != -1 ) 
			{
				if ($this->state->get('filter.state') != 2) 
				{
					JToolbarHelper::archiveList('images.archive','JTOOLBAR_ARCHIVE');
				}
				else 
				{
					if ($this->state->get('filter.state') == 2) 
					{
						JToolbarHelper::unarchiveList('images.publish', 'JTOOLBAR_UNARCHIVE');
					}
				}
			}
		}
		...
		if ($this->can_do->get('core.edit.state')) 
		{
			JToolbarHelper::custom('images.checkin', 'checkin.png', 'checkin_f2.png', 'JTOOLBAR_CHECKIN', true);
		}
	...
	{	
		JHtmlSidebar::setAction('index.php?option=com_nicegallery&view=images');
