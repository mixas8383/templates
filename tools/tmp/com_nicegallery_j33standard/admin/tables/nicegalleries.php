 ...
 *
 */
class NicegalleryTableNicegalleries extends JTable
{
	...
	public function __construct(&$db)
	{
		...
		parent::__construct('#__nicegallery_nicegalleries', 'id', $db);
		...
		JTableObserverTags::createObserver($this, array('typeAlias' => 'com_nicegallery.nicegallery'));

		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_nicegallery.nicegallery'));
	}
	...
	public function check()
	{
		...
		$table = JTable::getInstance('Nicegalleries', 'NicegalleryTable');
		if ($table->load(array('alias'=>$this->alias)) AND ($table->id != $this->id OR $this->id==0))
		{
			$this->setError(JText::_('COM_NICEGALLERY_FIELD_ALIAS_DUPLICATED'));
			return false;
		}		
	...
	public function store($update_nulls = false)
	{
		...
		if (empty($this->id))
		{
			// New Nicegallery. A created and created_by field can be set by the user,
	...
	{
		$k = $this->_tbl_key;
		return 'com_nicegallery.nicegallery.'.(int) $this->$k;
	}
	...
	protected function _getAssetParentId(JTable $table = null, $id = null)
	{
		...
		if (($this->catid) AND !empty($this->catid))
		{
			...
			if ($asset->loadByName('com_nicegallery.category.' . (int) $this->catid))
			{
				return $asset->id;
			}
		}
		
		if ($asset->loadByName('com_nicegallery'))
		{
			return $asset->id;
		}
		else
