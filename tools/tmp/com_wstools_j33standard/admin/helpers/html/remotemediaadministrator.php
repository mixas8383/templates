...
JLoader::register('ContentHelper', JPATH_ADMINISTRATOR . '/components/com_wstools/helpers/wstools.php');
/**
 * Remotemedias component helper.
 *
 */
abstract class JHtmlRemotemediaAdministrator
{
	/**
	 * Render the list of associated items
	 * 
	 * @param	integer $remotemedia_id	The remotemedia item id
	 * 
	 * @return  string  The language HTML
     */
	public static function association($remotemedia_id)
	{
		...
		// Get the associations
		if ($associations = JLanguageAssociations::getAssociations('com_wstools', '#__wstools_remotemedias', 'com_wstools.remotemedia.item',  $remotemedia_id, 'id', 'alias', null))
		{		
			...
			$query->select('a.*');
			$query->from($db->quoteName('#__wstools_remotemedias').' as a');
			...
			if ($items)
			{
				foreach ($items as &$item)
				{
					...
					$url = JRoute::_('index.php?option=com_wstools&task=remotemedia.edit&id=' . (int) $item->id);
					$tooltip_parts = array(
	...
	public static function featured($value = 0, $i, $can_change = true)
	{
		...
		$states	= array(
			0	=> array('unfeatured', 'remotemedias.featured', 'COM_WSTOOLS_REMOTEMEDIAS_UNFEATURED', 'COM_WSTOOLS_REMOTEMEDIAS_TOGGLE_TO_FEATURE'),
			1	=> array('featured', 'remotemedias.unfeatured', 'JFEATURED', 'COM_WSTOOLS_REMOTEMEDIAS_TOGGLE_TO_UNFEATURE'),
		);
