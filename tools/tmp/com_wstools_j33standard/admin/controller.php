...
class WstoolsController extends JControllerLegacy
{
	/**
	 * @var		string	The default view.
	 * 
	 */
	protected $default_view = 'remotemedias';
	...
	public function display($cachable = false, $url_params = false)
	{
		...
		// Load the submenu.
		WstoolsHelper::addSubmenu($view);

		// Check for edit form.
		switch ($view)
		{
			case 'remotemedia': 
				if ($layout == 'edit' AND !$this->checkEditId('com_wstools.edit.remotemedia', $id))
				{

					// Somehow the person just went to the form - we don't allow that.
					$this->setError(JText::sprintf('JLIB_APPLICATION_ERROR_UNHELD_ID', $id));
					$this->setMessage($this->getError(), 'error');
					$this->setRedirect(JRoute::_('index.php?option=com_wstools&view=remotemedias', false));
