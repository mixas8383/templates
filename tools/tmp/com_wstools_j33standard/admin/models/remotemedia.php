 ...
 *
 */
class WstoolsModelRemotemedia extends JModelAdmin
{
	/**
	...
	/**
	 * @var      string	The type alias for this object (for example, 'com_wstools.remotemedia')
	 */
	public $typeAlias = 'com_wstools.remotemedia';	
	/**
	 * @var		string	The context for the app call.
	 */
	protected $context = 'com_wstools.remotemedias';	
	/**
	 * @var		string	The event to trigger after before the data.
	 */
	protected $event_before_save = 'onRemotemediaBeforeSave';
	/**
	 * @var		string	The event to trigger after saving the data.
	 */
	protected $event_after_save = 'onRemotemediaAfterSave';

	/**
	 * @var    string	The event to trigger before deleting the data.
	 */
	protected $event_before_delete = 'onRemotemediaBeforeDelete';	
	/**
	 * @var    string	The event to trigger after deleting the data.
	 */
	protected $event_after_delete = 'onRemotemediaAfterDelete';	
	/**
	 * @var    string	The event to trigger after changing the data's state field.
	 */
	protected $event_change_state = 'onRemotemediaChangeState';	
	 ...
	 */
	public function getTable($type = 'Remotemedias', $prefix = 'WstoolsTable', $config = array())
	{
	...
	protected function canDelete($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.delete', 'com_wstools.remotemedia.'.(int) $record->id);
		}
		else
		{
			return $user->authorise('core.delete', 'com_wstools');
		}							
	}
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.edit.state', 'com_wstools.remotemedia.'.(int) $record->id);
		}
		else
		{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_wstools.remotemedia');
			}	
		}
		...
		if ($assoc)
		{
			...
			if ($item->id != null)
			{
				$associations = JLanguageAssociations::getAssociations('com_wstools', '#__wstools_remotemedias', 'com_wstools.remotemedia.item', $item->id, 'id', 'alias', null);
	...
	{
		$form = $this->loadForm('com_wstools.edit.remotemedia', 'remotemedia', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
		...
		// Prevent messing with Remotemedia language and category when editing existing Remotemedia with associations
	...
	protected function loadFormData()
	{
		...
		$data = $app->getUserState('com_wstools.edit.remotemedia.data', array());
	...
	protected function prepareTable($table)
	{
		...
		// Increment the remotemedia version number.
		$table->version++;
	...
	{
		// Include the wstools plugins for the onSave events.
		JPluginHelper::importPlugin('wstools');	
		...
		if (parent::save($data))
		{
			...
			if ($assoc)
			{
				...
				{
					JError::raiseNotice(403, JText::_('COM_WSTOOLS_ERROR_ALL_LANGUAGE_ASSOCIATED'));
				}
				...
				try
				{
					...
					$query->where('context='.$db->quote('com_wstools.remotemedia.item'));
				...
				if (!$all_language AND count($associations))
				{
					...
					{
						$query->values($id.','.$db->quote('com_wstools.remotemedia.item') . ',' . $db->quote($key));
					}
	...
	{	
		// Include the wstools plugins for the change of state event.
		JPluginHelper::importPlugin('wstools');	
	...
	{
		// Include the wstools plugins for the delete events.
		JPluginHelper::importPlugin('wstools');	
	...
	/**
	 * Method to toggle the featured setting of remotemedias.
	 *
	...
	public function featured($pks, $value = 0)
	{
		...
		if (empty($pks))
		{
			$this->setError(JText::_('COM_WSTOOLS_REMOTEMEDIAS_NO_ITEM_SELECTED'));
			return false;
		}
		...
		try
		{
			...
			$query = $db->getQuery(true);
			$query->update($db->quoteName('#__wstools_remotemedias'));
	...
	/**
	 * Custom clean the cache of com_wstools and wstools modules
	 *
	 */
	protected function cleanCache($group = null, $client_id = 0)
	{
		parent::cleanCache('com_wstools');
	...
	* @param   array   $data	The data where the original name is stored
	...
	/**
	 * Pre process the form to pick up remotemedias associated by language
	 *
	 ...
	 */
	protected function preprocessForm(JForm $form, $data, $group = 'remotemedia')
	{
		...
		if ($assoc)
		{
			...
			$fieldset->addAttribute('description', 'COM_WSTOOLS_REMOTEMEDIAS_ITEM_ASSOCIATIONS_FIELDSET_DESC');
			$add = false;
			foreach ($languages as $tag => $language)
			{
				if (empty($data->language) OR $tag != $data->language)
				{
					...
					$field->addAttribute('type', 'modal_remotemedias');
