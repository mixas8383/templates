 ...
 *
 */
class WstoolsTableRemotemedias extends JTable
{
	...
	public function __construct(&$db)
	{
		...
		parent::__construct('#__wstools_remotemedias', 'id', $db);
		...
		JTableObserverTags::createObserver($this, array('typeAlias' => 'com_wstools.remotemedia'));

		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_wstools.remotemedia'));
	}
	...
	public function check()
	{
		...
		$table = JTable::getInstance('Remotemedias', 'WstoolsTable');
		if ($table->load(array('alias'=>$this->alias)) AND ($table->id != $this->id OR $this->id==0))
		{
			$this->setError(JText::_('COM_WSTOOLS_FIELD_ALIAS_DUPLICATED'));
			return false;
		}		
	...
	public function store($update_nulls = false)
	{
		...
		if (empty($this->id))
		{
			// New Remotemedia. A created and created_by field can be set by the user,
	...
	{
		$k = $this->_tbl_key;
		return 'com_wstools.remotemedia.'.(int) $this->$k;
	}
	...
	protected function _getAssetParentId(JTable $table = null, $id = null)
	{
		...
		if ($asset->loadByName('com_wstools'))
		{
			return $asset->id;
		}
		else
