 ...
 *
 */
class Slider2ControllerItem extends JControllerForm
{
	/**
	...
	protected function allowAdd($data = array())
	{
		...
		$result = $user->authorise('core.create', 'com_slider2');
		if($result === null)
		{
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		if ($user->authorise('core.edit', 'com_slider2.item.'.$record_id))
		{
			return true;
		}
		// Fallback on edit.own.
		// First test if the permission is available.
		if ($user->authorise('core.edit.own', 'com_slider2.item.'.$record_id))
		{
	...
	public function batch($model = null)
	{
		...
		// Set the model
		$model = $this->getModel('Item', 'Slider2Model', array());

		// Preset the redirect
		$this->setRedirect(JRoute::_('index.php?option=com_slider2&view=items' . $this->getRedirectToListAppend(), false));
