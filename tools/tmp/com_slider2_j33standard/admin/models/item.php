 ...
 *
 */
class Slider2ModelItem extends JModelAdmin
{
	/**
	 * @var		string	The prefix to use with controller messages.
	 */
	protected $text_prefix = 'COM_SLIDER2_ITEMS';
	/**
	 * @var      string	The type alias for this object (for example, 'com_slider2.item')
	 */
	public $typeAlias = 'com_slider2.item';	
	/**
	...
	protected function canDelete($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.delete', 'com_slider2.item.'.(int) $record->id);
		}
		else
		{
			return $user->authorise('core.delete', 'com_slider2');
		}							
	}
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.edit.state', 'com_slider2.item.'.(int) $record->id);
		}
		else
		{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_slider2.item');
			}	
		}
		...
		if ($assoc)
		{
			...
			if ($item->id != null)
			{
				$associations = JLanguageAssociations::getAssociations('com_slider2', '#__slider2_items', 'com_slider2.item.item', $item->id, 'id', 'alias', null);
	...
	{
		$form = $this->loadForm('com_slider2.edit.item', 'item', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = $app->getUserState('com_slider2.edit.item.data', array());
	...
	public function save($data)
	{
		...
		JPluginHelper::importPlugin('slider2');	
		...
		if (parent::save($data))
		{
			...
			if ($assoc)
			{
				...
				{
					JError::raiseNotice(403, JText::_('COM_SLIDER2_ERROR_ALL_LANGUAGE_ASSOCIATED'));
				}
				...
				try
				{
					...
					$query->where('context='.$db->quote('com_slider2.item.item'));
				...
				if (!$all_language AND count($associations))
				{
					...
					{
						$query->values($id.','.$db->quote('com_slider2.item.item') . ',' . $db->quote($key));
					}
	...
	{	
		// Include the slider2 plugins for the change of state event.
		JPluginHelper::importPlugin('slider2');	
	...
	{
		// Include the slider2 plugins for the delete events.
		JPluginHelper::importPlugin('slider2');	
	...
	public function featured($pks, $value = 0)
	{
		...
		if (empty($pks))
		{
			$this->setError(JText::_('COM_SLIDER2_ITEMS_NO_ITEM_SELECTED'));
			return false;
		}
		...
		try
		{
			...
			$query = $db->getQuery(true);
			$query->update($db->quoteName('#__slider2_items'));
	...
	/**
	 * Custom clean the cache of com_slider2 and slider2 modules
	 *
	 */
	protected function cleanCache($group = null, $client_id = 0)
	{
		parent::cleanCache('com_slider2');
	...
	* @param   array   $data	The data where the original name is stored
	...
	protected function preprocessForm(JForm $form, $data, $group = 'item')
	{
		...
		if ($assoc)
		{
			...
			$fieldset->addAttribute('description', 'COM_SLIDER2_ITEMS_ITEM_ASSOCIATIONS_FIELDSET_DESC');
			$add = false;
