...
require_once JPATH_ADMINISTRATOR.'/components/com_slider2/models/item.php';

class Slider2ModelItemForm extends Slider2ModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_slider2.edit.item';
	/**
	...
	*/
	...
	public function getItem($item_id = null)
	{
		...
		{
			// New item.
			$item->params->set('access-change', $user->authorise('core.edit.state', 'com_slider2'));
		}
	...
	protected function setAccessFilters(&$form, $data)
	{
		...
		if (!$user->authorise('core.edit.state', 'com_slider2'))
		{
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_slider2.edit.item.data', array());
	...
	public function save($data)
	{
		...
		else
		{
			...
			$actions = JAccess::getActions('com_slider2');
			$action_array = array();
		...
		$result = $dispatcher->trigger('onItemBeforeSave', array('com_slider2.item', &$table, $is_new));
		...
		// Clean the cache.
		$cache = JFactory::getCache('com_slider2');
		$cache->clean();

		$dispatcher->trigger('onItemAfterSave', array('com_slider2.item', $table, $is_new));
