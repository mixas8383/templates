...
/**
 * Slider Component Item Model
 *
 */
class Slider2ModelItem extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_slider2.item';
	...
	protected function canDelete($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.delete', 'com_slider2.item.'.(int) $record->id)
					  OR ($user->authorise('core.delete', 'com_slider2.item.'.(int) $record->id)
	...
	protected function canEditState($record)
	{
		...
		else
		{
			...
			return $user->authorise('core.edit.state', 'com_slider2');
		}
	}
	/**
	...
	protected function populateState()
	{
		...
		if ((!$user->authorise('core.edit.state', 'com_slider2')) AND  (!$user->authorise('core.edit', 'com_slider2')))
		{
	...
	*/
	...
	public function getItem($pk = null)
	{
		...
		$global_params = JComponentHelper::getParams('com_slider2', true);
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				$can_publish = $user->authorise('core.edit.state', 'com_slider2.item.'.$pk);
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_SLIDER2_ITEMS_ERROR_ITEM_NOT_FOUND'));
				}
				...
				$item->tags->getItemTags('com_slider2.item', $item->id);
				...
				{
					$user_id	= $user->get('id');
					$asset	= 'com_slider2.item.'.$item->id;
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onItemChangeState', array('com_slider2.item', $pks, $value));
	...
	*/
	...
	public function delete(&$pks)
	{
		...
		// Include the slider2 plugins for the on delete events.
		JPluginHelper::importPlugin('slider2');
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onItemBeforeDelete', array('com_slider2.item', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onItemAfterDelete', array('com_slider2.item', &$table));
				}
				else
				{
					// Prune items that you can't change.
