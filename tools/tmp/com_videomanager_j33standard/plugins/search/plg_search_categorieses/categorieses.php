 ...
 * @subpackage		com_videomanager.search.videomanager.categorieses
 * @copyright		
...
/**
 * Categorieses Search plugin
 *
 */
class PlgSearchCategorieses extends JPlugin
{
	/**
	...
	{
		static $areas = array(
			'categorieses' => 'PLG_SEARCH_CATEGORIESES_CATEGORIESES'
			);
			return $areas;
	}

	/**
	 * Categorieses Search method
	...
	public function onContentSearch($text, $phrase='', $ordering='', $areas=null)
	{
		...
		$sCategories	= 1;
		$sCategoriesArchived 	= 0;
		$limit = 50;		

		$sCategories	= $this->params->get('search_categorieses',1);
		$sCategoriesArchived		= $this->params->get('search_archived_categorieses',0);
		...
		$search_section = JText::_('PLG_SEARCH_CATEGORIESES_CATEGORIESES');
		// search categorieses
		if ($sCategories AND $limit > 0)
		{
			$query->clear();
			...
			if (isset($list))
			{
				...
				{
					$list[$key]->href = JRoute::_(VideomanagerHelperRoute::getCategoriesRoute($item->slug, $item->catslug, $item->language, $this->layout, $keep_item_id));
		...
		// search archived categorieses
		if ($sCategoriesArchived AND $limit > 0)
		{
			$query->clear();
			...
			$item	= $app->getMenu()->getItems('link', 'index.php?option=com_videomanager&view=categoriesarchive', true);
			...
			if (isset($list3))
			{
				foreach($list3 as $key => $item)
				{
					...
					$list3[$key]->href	= JRoute::_('index.php?option=com_videomanager&view=categoriesarchive'.
		...
		if (count($rows))
		{
			foreach($rows as $row)
			{
				...
				{
					if (SearchHelper::checkNoHTML($categories, $search_text, array('title',
																						'description',
																						'metadesc', 
																						'metakey'
																						)))
