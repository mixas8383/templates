 ...
 * @subpackage		com_videomanager.search.videomanager.configs
 * @copyright		
...
/**
 * Configs Search plugin
 *
 */
class PlgSearchConfigs extends JPlugin
{
	/**
	...
	{
		static $areas = array(
			'configs' => 'PLG_SEARCH_CONFIGS_CONFIGS'
			);
			return $areas;
	}

	/**
	 * Configs Search method
	...
	public function onContentSearch($text, $phrase='', $ordering='', $areas=null)
	{
		...
		$sConfig	= $this->params->get('search_configs',1);
		$sConfigArchived		= $this->params->get('search_archived_configs',0);
		...
		$search_section = JText::_('PLG_SEARCH_CONFIGS_CONFIGS');
		// search configs
		if ($sConfig AND $limit > 0)
		{
			$query->clear();
			...
			if (isset($list))
			{
				...
				{
					$list[$key]->href = JRoute::_(VideomanagerHelperRoute::getConfigRoute($item->slug, $item->language, $this->layout, $keep_item_id));
		...
		// search archived configs
		if ($sConfigArchived AND $limit > 0)
		{
			$query->clear();
			...
			$item	= $app->getMenu()->getItems('link', 'index.php?option=com_videomanager&view=configarchive', true);
			...
			if (isset($list3))
			{
				foreach($list3 as $key => $item)
				{
					...
					$list3[$key]->href	= JRoute::_('index.php?option=com_videomanager&view=configarchive'.
		...
		if (count($rows))
		{
			foreach($rows as $row)
			{
				...
				{
					if (SearchHelper::checkNoHTML($config, $search_text, array('title',
																						'description',
																						'metadesc', 
																						'metakey'
																						)))
