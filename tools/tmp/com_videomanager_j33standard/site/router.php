...
/**
 * Routing class for the Videomanager component
 *
 */
class VideomanagerRouter extends JComponentRouterBase
{
	/**
	 * Build the route for the Videomanager component
	 *
	...
	public function build(&$query)
	{
		...
		$menu	= $app->getMenu();
		$params	= JComponentHelper::getParams('com_videomanager');
		...
		// Are we dealing with a videomanager that is attached to a menu item?
	...
	public function parse(&$segments)
	{
		...
		{
			$categories = JCategories::getInstance('Videomanager',$options)->get($id)->getChildren();
		}
		else
		{
			$categories = JCategories::getInstance('Videomanager',$options);	
			...
			if ($found == 0)
			{
					...
					OR $view == 'configs' OR $view == 'config')
				{
					if($advanced)
					{
						...
						$query->from($db->quoteName('#__videomanager_configs'));
				...
				if ($item->query['view'] == 'videos' OR $item->query['view'] == 'video'
					OR $view == 'videos' OR $view == 'video')
				{
					if($advanced)
					{
						$db = JFactory::getDbo();
						...
						$query = $db->getQuery(true);
						$query->select($db->quoteName('id'));
						$query->from($db->quoteName('#__videomanager_videos'));
