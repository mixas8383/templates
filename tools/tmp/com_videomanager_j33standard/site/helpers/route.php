...
/**
 * Videomanager Component Route Helper
 *
 */
abstract class VideomanagerHelperRoute
{
	...
	public static function getConfigRoute($id, $language = '*', $layout = 'default', $keep_item_id = false)
	{
		...
		{
			//Create the link
			$link = 'index.php?option=com_videomanager&view=config&id='. $id;
		}
		else
		{
			//Create the link with a layout
			$link = 'index.php?option=com_videomanager&view=config&layout='.$layout.'&id='. $id;
		}
	...
	/**
	 * @param	integer	The route of the Video
	 */
	public static function getVideoRoute($id, $cat_id = 0, $language = '*', $layout = 'default', $keep_item_id = false)
	{
		$needles = array(
			'video'  => array((int) $id)
		);
		...
		{
			//Create the link
			$link = 'index.php?option=com_videomanager&view=video&id='. $id;
		}
		else
		{
			//Create the link with a layout
			$link = 'index.php?option=com_videomanager&view=video&layout='.$layout.'&id='. $id;
		}
	...
	public static function getCategoryRoute($cat_id, $keep_item_id = false, $language = 0)
	{
		...
		else
		{
			...
			$options['countItems'] = false;
			$options['table'] = '';			
			$category = JCategories::getInstance('Videomanager', $options)->get($id);
		}
		...
		else
		{
			...
			$link = 'index.php?option=com_videomanager&view=category&id='.$id;
	...
	protected static function findItem($needles = null, $keep_item_id = false, $layout = 'default')
	{
		...
		if ($active AND $active->component == 'com_videomanager' AND ($language == '*' OR in_array($active->language, array('*', $language)) OR !JLanguageMultilang::isEnabled()))
		{
			return $active->id;
		}
