...
/**
 * Videomanager Component Video Model
 *
 */
class VideomanagerModelVideo extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_videomanager.video';
	...
	{
		if (empty($config['video_filter_fields']))
		{
			$config['video_filter_fields'] = array(
				'id', 'a.id',
				'name', 'a.name',
				'alias', 'a.alias',
				'metadata','a.metadata',
	...
	protected function canDelete($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.delete', 'com_videomanager.video.'.(int) $record->id)
					  OR ($user->authorise('core.delete', 'com_videomanager.video.'.(int) $record->id)
	...
	protected function canEditState($record)
	{
		...
		else
		{
			...
			// Default to component settings.			
				return $user->authorise('core.edit.state', 'com_videomanager');
			}
		}
	}
	/**
	...
	*/
	...
	public function getItem($pk = null)
	{
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				$sub_query .= 'WHERE '.$db->quoteName('parent.extension').' = ' . $db->quote('com_videomanager');
				...
				$can_publish = $user->authorise('core.edit.state', 'com_videomanager.video.'.$pk);
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_VIDEOMANAGER_VIDEOS_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onVideoChangeState', array('com_videomanager.video', $pks, $value));
	...
	*/
	...
	public function delete(&$pks)
	{
		...
		// Include the videomanager plugins for the on delete events.
		JPluginHelper::importPlugin('videomanager');
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onVideoBeforeDelete', array('com_videomanager.video', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onVideoAfterDelete', array('com_videomanager.video', &$table));
				}
				else
				{
					// Prune items that you can't change.
	 ...
	 *
	 * @pk		int		Optional primary key of the video to increment.
	 *
