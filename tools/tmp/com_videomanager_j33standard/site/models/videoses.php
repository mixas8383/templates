...
/**
 * This models supports retrieving lists of videoses.
 *
 */
class VideomanagerModelVideoses extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_videomanager.videoses';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single videos has been specified either as a parameter or in the url Request
		$pk = $params->get('videos_id', '') == '' ? $app->input->getInt('id', '') : $params->get('videos_id');
		$this->setState('filter.videos_id', $pk);
		
		// List state information
		if ($app->input->getString('layout', 'default') == 'blog')
		{
			$limit = $params->def('videos_num_leading', 1) + $params->def('videos_num_intro', 4) + $params->def('videos_num_links', 4);
		}
		else
		{		
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('videos_num_per_page'),'integer');
		}
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('videos_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('videos_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('videos_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('videos_initial_direction','ASC');
		}
		...
		// process show_videos_noauth parameter
		if (!$params->get('show_videos_noauth'))
		{
		...
		if ($params->get('filter_videos_featured') <> "")
		{
			$this->setState('filter.featured', $params->get('filter_videos_featured'));
			
		}
		if ($params->get('filter_videos_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_videos_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.videos_id'));
		$id .= ':'.$this->getState('filter.videos_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of videoses subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		// Filter by a single or group of videoses.
		$videos_id = $this->getState('filter.videos_id');
		if ($videos_id != '')
		{
			if (is_numeric($videos_id))
			{
				$type = $this->getState('filter.videos_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $videos_id);
			}
			else
			{
				if (is_array($videos_id))
				{
					JArrayHelper::toInteger($videos_id);
					$videos_id = implode(',', $videos_id);
					$type = $this->getState('filter.videos_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$videos_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_videos_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			...
			switch ($params->get('show_videos_filter_field'))
			{
				case 'hits':
	...
	/**
	 * Method to get a list of videoses.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				$videos_params = new JRegistry;
				$videos_params->loadString($item->params);
				...
				$item->videos_alternative_readmore = $videos_params->get('videos_alternative_readmore');
				$item->layout = $videos_params->get('layout');
							
				if (!is_object($this->getState('params')))
				{
					$item->params = $videos_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Videos params override menu item params only if menu param = 'use_videos'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_videos' and there is no videos param, use global

					// create an array of just the params set to 'use_videos'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_videos')
						{
							// if the videos has a value, use it
							if ($videos_params->get($key) != '')
							{
								// get the value from the videos
								$videos_array[$key] = $videos_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$videos_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected videos params
					if (count($videos_array) > 0)
					{
						$videos_params = new JRegistry;
						$videos_params->loadArray($videos_array);
						$item->params->merge($videos_params);
					}


					// get display date
					switch ($item->params->get('list_show_videos_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an videos, but lets not check that to improve performance a little.
				if (!$guest) 
				{
				...
				if ($access) 
				{
					// If the access filter has been set, we already have only the videoses this user can view.
				...
				$item->tags->getItemTags('com_videomanager.videos', $item->id);
			}
		}
		return $items;
	}
