...
/**
 * Videomanager Component Config Model
 *
 */
class VideomanagerModelConfig extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_videomanager.config';
	...
	protected function canDelete($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.delete', 'com_videomanager.config.'.(int) $record->id)
					  OR ($user->authorise('core.delete', 'com_videomanager.config.'.(int) $record->id)
	...
	protected function populateState()
	{
		...
		if ($params->get('filter_config_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_config_archived'));
	...
	*/
	...
	public function getItem($pk = null)
	{
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				$can_publish = $user->authorise('core.edit.state', 'com_videomanager.config.'.$pk);
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_VIDEOMANAGER_CONFIGS_ERROR_ITEM_NOT_FOUND'));
				}
				...
				$item->tags->getItemTags('com_videomanager.config', $item->id);
				...
				$config_params->loadString($item->params);

				// Unpack readmore and layout params
				$item->config_alternative_readmore = $config_params->get('config_alternative_readmore');
				$item->layout = $config_params->get('layout');
				...
				// Technically guest could edit an config, but lets not check that to improve performance a little.
				if (!$user->get('guest')) 
				{
					$user_id	= $user->get('id');
					$asset	= 'com_videomanager.config.'.$item->id;
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onConfigChangeState', array('com_videomanager.config', $pks, $value));
	...
	*/
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onConfigBeforeDelete', array('com_videomanager.config', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onConfigAfterDelete', array('com_videomanager.config', &$table));
				}
				else
				{
					// Prune items that you can't change.
	...
	/**
	 * Increment the hit counter for the config.
	 *
	 * @pk		int		Optional primary key of the config to increment.
	 *
