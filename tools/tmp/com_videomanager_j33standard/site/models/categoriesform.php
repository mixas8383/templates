...
require_once JPATH_ADMINISTRATOR.'/components/com_videomanager/models/categories.php';

class VideomanagerModelCategoriesForm extends VideomanagerModelCategories
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_videomanager.edit.categories';
	/**
	...
	protected function populateState()
	{
		...
		$this->setState('categories.id', $pk);

		$return = $app->input->get('return', null, 'base64');
		$this->setState('return_page', base64_decode($return));		
		$this->setState('categories.catid', $app->input->getInt('catid'));
	...
	*/
	...
	public function getForm($data = array(), $load_data = true)
	{
		...
		if ($id = (int) $this->getState('categories.id'))
		{
	 ...
	 *
	 * @param	integer	$item_id	The id of the categories.
	 *
	 * @return	mixed	Categories item data object on success, false on failure.
	 */
	public function getItem($item_id = null)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('categories.id');
		...
		{
			// New item.
			$cat_id = (int) $this->getState('categories.catid');
			if ($cat_id)
			{
				$item->params->set('access-change', $user->authorise('core.edit.state', 'com_videomanager.category.'.$cat_id));
				$item->catid = $cat_id;
			}
			else 
			{
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_videomanager.edit.categories.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('categories.id');
		$is_new		= true;
		...
		// Reorder the categorieses so the new categories is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onCategoriesBeforeSave', array('com_videomanager.categories', &$table, $is_new));
		...
		$dispatcher->trigger('onCategoriesAfterSave', array('com_videomanager.categories', $table, $is_new));

		$this->setState('categories.id', $table->id);
	...
	{
		$pk	= (!empty($pk)) ? $pk : (int) $this->getState('categories.id');
