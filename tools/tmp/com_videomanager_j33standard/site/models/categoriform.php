...
require_once JPATH_ADMINISTRATOR.'/components/com_videomanager/models/categori.php';

class VideomanagerModelCategoriForm extends VideomanagerModelCategori
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_videomanager.edit.categori';
	/**
	...
	protected function populateState()
	{
		...
		$this->setState('categori.catid', $app->input->getInt('catid'));
	...
	*/
	...
	public function getForm($data = array(), $load_data = true)
	{
		...
		if ($id = (int) $this->getState('categori.id'))
		{
	 ...
	 *
	 * @param	integer	$item_id	The id of the categori.
	 *
	 * @return	mixed	Categori item data object on success, false on failure.
	 */
	public function getItem($item_id = null)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('categori.id');
		...
		{
			// New item.
			$cat_id = (int) $this->getState('categori.catid');
			if ($cat_id)
			{
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_videomanager.edit.categori.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('categori.id');
		$is_new		= true;
		...
		// Reorder the categoris so the new categori is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onCategoriBeforeSave', array('com_videomanager.categori', &$table, $is_new));
		...
		$dispatcher->trigger('onCategoriAfterSave', array('com_videomanager.categori', $table, $is_new));

		$this->setState('categori.id', $table->id);
