...
/**
 * Videomanager Component Categori Model
 *
 */
class VideomanagerModelCategori extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_videomanager.categori';
	...
	{
		if (empty($config['categori_filter_fields']))
		{
			$config['categori_filter_fields'] = array(
				'id', 'a.id',
				'name', 'a.name',
				'alias', 'a.alias',
	...
	protected function canDelete($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.delete', 'com_videomanager.categori.'.(int) $record->id)
					  OR ($user->authorise('core.delete', 'com_videomanager.categori.'.(int) $record->id)
	...
	*/
	...
	/**
	 * Method to get Categori data.
	 *
	 * @param	integer	$pk	The id of the categori.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('categori.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				// If all categories are published, badcats.id will be null, and we just use the categori state
				...
				$can_publish = $user->authorise('core.edit.state', 'com_videomanager.categori.'.$pk);
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_VIDEOMANAGER_CATEGORIS_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onCategoriChangeState', array('com_videomanager.categori', $pks, $value));
	...
	*/
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onCategoriBeforeDelete', array('com_videomanager.categori', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onCategoriAfterDelete', array('com_videomanager.categori', &$table));
				}
				else
				{
					// Prune items that you can't change.
	...
	/**
	 * Increment the hit counter for the categori.
	 *
	 * @pk		int		Optional primary key of the categori to increment.
	 *
