...
class VideomanagerModelVideos extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single video has been specified either as a parameter or in the url Request
		$pk = $params->get('video_id', '') == '' ? $app->input->getInt('id', '') : $params->get('video_id');
		$this->setState('filter.video_id', $pk);
		
		// List state information
		if ($app->input->getString('layout', 'default') == 'blog')
		{
			$limit = $params->def('video_num_leading', 1) + $params->def('video_num_intro', 4) + $params->def('video_num_links', 4);
		}
		else
		{		
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('video_num_per_page'),'integer');
		}
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('video_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('video_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('video_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('video_initial_direction','ASC');
		}
		...
		// process show_video_noauth parameter
		if (!$params->get('show_video_noauth'))
		{
		...
		if ($params->get('filter_video_categories') AND implode(',', $params->get('filter_video_categories'))  == true)
		{
			$selected_categories = $params->get('filter_video_categories');
			$this->setState('filter.category_id', $selected_categories);
		}			
		if ($params->get('filter_video_featured') <> "")
		{
			$this->setState('filter.featured', $params->get('filter_video_featured'));
			
		}
		if ($params->get('filter_video_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_video_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.video_id'));
	...
	/**
	 * Get the main query for retrieving a list of videos subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__videomanager_videos').' AS a');
		...
		$video_id = $this->getState('filter.video_id');
		if ($video_id != '')
		{
			if (is_numeric($video_id))
			{
				$type = $this->getState('filter.video_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $video_id);
			}
			else
			{
				if (is_array($video_id))
				{
					JArrayHelper::toInteger($video_id);
					$video_id = implode(',', $video_id);
					$type = $this->getState('filter.video_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$video_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_video_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			...
			switch ($params->get('show_video_filter_field'))
			{
				case 'hits':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				$video_params = new JRegistry;
				$video_params->loadString($item->params);
				...
				$item->video_alternative_readmore = $video_params->get('video_alternative_readmore');
				$item->layout = $video_params->get('layout');
							
				if (!is_object($this->getState('params')))
				{
					$item->params = $video_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Video params override menu item params only if menu param = 'use_video'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_video')
						{
							...
							if ($video_params->get($key) != '')
							{
								...
								$video_array[$key] = $video_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$video_array[$key] = $global_params->get($key);
							}
						}
					}
					...
					{
						$video_params = new JRegistry;
						$video_params->loadArray($video_array);
						$item->params->merge($video_params);
					}


					// get display date
					switch ($item->params->get('list_show_video_date'))
					{
						case 'modified':
				...
				if (!$guest) 
				{
					$asset	= 'com_videomanager.video.'.$item->id;
				...
				$item->tags->getItemTags('com_videomanager.video', $item->id);
			}
		}
		return $items;
	}
