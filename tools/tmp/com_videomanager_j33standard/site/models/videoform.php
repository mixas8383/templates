...
require_once JPATH_ADMINISTRATOR.'/components/com_videomanager/models/video.php';

class VideomanagerModelVideoForm extends VideomanagerModelVideo
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_videomanager.edit.video';
	/**
	...
	protected function populateState()
	{
		...
		$this->setState('video.catid', $app->input->getInt('catid'));
	...
	*/
	...
	public function getForm($data = array(), $load_data = true)
	{
		...
		if ($id = (int) $this->getState('video.id'))
		{
	 ...
	 *
	 * @param	integer	$item_id	The id of the video.
	 *
	 * @return	mixed	Video item data object on success, false on failure.
	 */
	public function getItem($item_id = null)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('video.id');
		...
		{
			// New item.
			$cat_id = (int) $this->getState('video.catid');
			if ($cat_id)
			{
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_videomanager.edit.video.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('video.id');
		$is_new		= true;
		...
		// Reorder the videos so the new video is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onVideoBeforeSave', array('com_videomanager.video', &$table, $is_new));
		...
		$dispatcher->trigger('onVideoAfterSave', array('com_videomanager.video', $table, $is_new));
