...
/**
 * This models supports retrieving lists of categorieses.
 *
 */
class VideomanagerModelCategorieses extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_videomanager.categorieses';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single categories has been specified either as a parameter or in the url Request
		$pk = $params->get('categories_id', '') == '' ? $app->input->getInt('id', '') : $params->get('categories_id');
		$this->setState('filter.categories_id', $pk);
		
		// List state information
		if ($app->input->getString('layout', 'default') == 'blog')
		{
			$limit = $params->def('categories_num_leading', 1) + $params->def('categories_num_intro', 4) + $params->def('categories_num_links', 4);
		}
		else
		{		
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('categories_num_per_page'),'integer');
		}
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('categories_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('categories_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('categories_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('categories_initial_direction','ASC');
		}
		...
		// process show_categories_noauth parameter
		if (!$params->get('show_categories_noauth'))
		{
		...
		if ($params->get('filter_categories_categories') AND implode(',', $params->get('filter_categories_categories'))  == true)
		{
			$selected_categories = $params->get('filter_categories_categories');
			$this->setState('filter.category_id', $selected_categories);
		}			
		if ($params->get('filter_categories_featured') <> "")
		{
			$this->setState('filter.featured', $params->get('filter_categories_featured'));
			
		}
		if ($params->get('filter_categories_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_categories_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.categories_id'));
		$id .= ':'.$this->getState('filter.categories_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of categorieses subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__videomanager_categorieses').' AS a');
		...
		// Filter by a single or group of categorieses.
		$categories_id = $this->getState('filter.categories_id');
		if ($categories_id != '')
		{
			if (is_numeric($categories_id))
			{
				$type = $this->getState('filter.categories_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $categories_id);
			}
			else
			{
				if (is_array($categories_id))
				{
					JArrayHelper::toInteger($categories_id);
					$categories_id = implode(',', $categories_id);
					$type = $this->getState('filter.categories_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$categories_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_categories_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			...
			switch ($params->get('show_categories_filter_field'))
			{
				case 'hits':
	...
	/**
	 * Method to get a list of categorieses.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				$categories_params = new JRegistry;
				$categories_params->loadString($item->params);
				...
				$item->categories_alternative_readmore = $categories_params->get('categories_alternative_readmore');
				$item->layout = $categories_params->get('layout');
							
				if (!is_object($this->getState('params')))
				{
					$item->params = $categories_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Categories params override menu item params only if menu param = 'use_categories'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_categories' and there is no categories param, use global

					// create an array of just the params set to 'use_categories'
					$menu_params_array = $this->getState('params')->toArray();
					$categories_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_categories')
						{
							// if the categories has a value, use it
							if ($categories_params->get($key) != '')
							{
								// get the value from the categories
								$categories_array[$key] = $categories_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$categories_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected categories params
					if (count($categories_array) > 0)
					{
						$categories_params = new JRegistry;
						$categories_params->loadArray($categories_array);
						$item->params->merge($categories_params);
					}


					// get display date
					switch ($item->params->get('list_show_categories_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an categories, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_videomanager.categories.'.$item->id;
				...
				if ($access) 
				{
					// If the access filter has been set, we already have only the categorieses this user can view.
				...
				$item->tags->getItemTags('com_videomanager.categories', $item->id);
			}
		}
		return $items;
	}
