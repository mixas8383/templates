 ...
 * 
 */
class VideomanagerControllerVideo extends JControllerForm
{
	/**
	 * @var    string	$view_item	The URL view item variable.
	 */
	protected $view_item = 'videoform';
	/**
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		$user		= JFactory::getUser();
		$asset		= 'com_videomanager.video.'.$record_id;
		...
		if ($user->authorise('core.edit.own', $asset))
		{
			...
			if (empty($owner_id) AND $record_id)
			{
				...
				$record		= $this->getModel('videoform')->getItem($record_id);
	 ...
	 * 
	 */
	public function getModel($name = 'videoform', $prefix = '',$config = array('ignore_request' => true))
	{
	 ...
	 *
	 * @return  mixed  True if the Video can be added, a error object if not.
	 *
	 */
	public function add()
	{
	...
	public function save($key = 'id', $url_var = null)
	{
		...
		if ($record_id == 0) 
		{
			$this->setMessage(JText::_('COM_VIDEOMANAGER_VIDEOS_SUBMIT_SAVE_SUCCESS'));
		} 
		else 
		{
			$this->setMessage(JText::_('COM_VIDEOMANAGER_VIDEOS_SAVE_SUCCESS'));
		}
		...
		switch ($task)
		{
			case 'apply':
				...
				$record_id = $model->getState('video.id');
				$this->holdEditId($context, $record_id);
				$app->setUserState($context.'.data',	null);
	...
	public function delete()
	{
		...
		if ($id > 0) 
		{
			...
			{
				$this->setMessage(JText::_('COM_VIDEOMANAGER_VIDEOS_DELETE_SUCCESS'));
				
			}
			else
			{
				$this->setMessage(JText::_('COM_VIDEOMANAGER_VIDEOS_DELETE_FAILED'));
			}
		}
