...
/**
 * Categori controller class.
 * 
 */
class VideomanagerControllerCategori extends JControllerForm
{
	/**
	 * @var    string	$view_item	The URL view item variable.
	 */
	protected $view_item = 'categoriform';
	/**
	 * @var    string	$view_list	The URL view list variable.
	 */
	protected $view_list = 'categoris';
	/**
	 * 
	...
	protected function allowAdd($data = array())
	{
		...
		if ($category_id)
		{
			...
			$allow	= $user->authorise('core.create', 'com_videomanager.category.'.$category_id);
		}
		if ($allow === null)
		{
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		$user		= JFactory::getUser();
		$asset		= 'com_videomanager.categori.'.$record_id;
		...
		if ($user->authorise('core.edit.own', $asset))
		{
			...
			if (empty($owner_id) AND $record_id)
			{
				...
				$record		= $this->getModel('categoriform')->getItem($record_id);
	 ...
	 * 
	 */
	public function getModel($name = 'categoriform', $prefix = '',$config = array('ignore_request' => true))
	{
	 ...
	 *
	 * @return  mixed  True if the Categori can be added, a error object if not.
	 *
	 */
	public function add()
	{
	...
	public function edit($key = 'id', $url_var = null)
	{
		...
		$model = $this->getModel('categoriform');
		// Set the return url
		$this->setReturnPage($context);
	...
	public function save($key = 'id', $url_var = null)
	{
		...
		if ($record_id == 0) 
		{
			$this->setMessage(JText::_('COM_VIDEOMANAGER_CATEGORIS_SUBMIT_SAVE_SUCCESS'));
		} 
		else 
		{
			$this->setMessage(JText::_('COM_VIDEOMANAGER_CATEGORIS_SAVE_SUCCESS'));
		}
		...
		switch ($task)
		{
			case 'apply':
				...
				$record_id = $model->getState('categori.id');
				$this->holdEditId($context, $record_id);
				$app->setUserState($context.'.data',	null);
	...
	public function delete()
	{
		...
		if ($id > 0) 
		{
			...
			{
				$this->setMessage(JText::_('COM_VIDEOMANAGER_CATEGORIS_DELETE_SUCCESS'));
				
			}
			else
			{
				$this->setMessage(JText::_('COM_VIDEOMANAGER_CATEGORIS_DELETE_FAILED'));
			}
		}
