...
/**
 * HTML Video View class for the Videomanager component
 *
 */
class VideomanagerViewVideo extends JViewLegacy
{
	...
	public function display($tpl = null)
	{
		...
		if ($active)
		{
			...
			if (JString::strpos($current_link, 'view=video') AND (JString::strpos($current_link, '&id='.(string) $item->id)))
			{
		...
		$item->readmore_link = JRoute::_(VideomanagerHelperRoute::getVideoRoute($item->slug, 
										$item->catid, 
										$item->language,
										$this->getLayout(),									
		...
		if ($this->params->get('show_video_tags')  == '1')
		{
		...
		if ($item->params->get('access-view') != true)
		{
			...
			if (!$item->params->get('show_video_noauth'))
			{
				// If a guest user, they may be able to log in to view the full video
				
				if( $user->get('guest'))
				{
					// Redirect to login
					$uri = JUri::getInstance();
					$app->redirect('index.php?option=com_users&view=login&return=' . base64_encode($uri), JText::_('COM_VIDEOMANAGER_VIDEOS_ERROR_LOGIN_TO_VIEW_ITEM'));
		...
		$results = $dispatcher->trigger('onVideoPrepare', array ('com_videomanager.video', &$item, &$this->params, $offset));

		$item->event = new stdClass;
		$results = $dispatcher->trigger('onVideoAfterName', array('com_videomanager.video', &$item, &$this->params, $offset));
		$item->event->afterDisplayVideoName = JString::trim(implode("\n", $results));
		
		$results = $dispatcher->trigger('onVideoBeforeDisplay', array('com_videomanager.video', &$item, &$this->params, $offset));
		$item->event->beforeDisplayVideo = JString::trim(implode("\n", $results));

		$results = $dispatcher->trigger('onVideoAfterDisplay', array('com_videomanager.video', &$item, &$this->params, $offset));
		$item->event->afterDisplayVideo = JString::trim(implode("\n", $results));
	...
	protected function prepareDocument()
	{
		...
		if ($menu AND ($menu->query['option'] != 'com_videomanager' OR $menu->query['view'] != 'video' OR $id != $this->item->id))
		{
			// If this is not a single video menu item, set the page title to the video name
			if ($this->item->name)
			{
				$title = $this->item->name;
			}
			$path = array(array('title' => $this->item->name, 'link' => ''));
			if ( $this->params->get('show_video_category_breadcrumb', '0'))
			{			
				$options['countItems'] = false;
				$options['table'] = '#__videomanager_videos';					
				$category = JCategories::getInstance('Videomanager', $options)->get($this->item->catid);
				while ($category AND ($menu->query['option'] != 'com_videomanager' OR $menu->query['view'] == 'video' OR $id != $category->id) AND $category->id > 1)
				{
					$path[] = array('title' => $category->title, 'link' => VideomanagerHelperRoute::getCategoryRoute($category->id, $this->params->get('keep_video_itemid')));
