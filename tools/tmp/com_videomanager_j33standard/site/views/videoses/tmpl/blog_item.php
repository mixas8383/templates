...
$layout		= $params->get('videos_layout', 'default');
...
<h2 itemprop="name">
		<?php if ($params->get('link_videos_names') ) : ?>
			<a href="<?php echo JRoute::_(VideomanagerHelperRoute::getVideosRoute($this->item->slug, 
											$this->item->language,									
											$layout,									
											$params->get('keep_videos_itemid'))); ?>" itemprop="url">
...
<?php if ($params->get('access-view') 
		AND (!$params->get('show_videos_readmore')
		OR !$params->get('show_videos_intro'))
		):?>
	<?php
...
<?php else : ?>
	...
	<?php if ($params->get('show_videos_intro')) : ?>
		<?php if ($params->get('access-view') OR (!$params->get('access-view') AND $params->get('show_videos_noauth'))) : ?>
			<span itemprop="description">
	...
	<?php if ($params->get('show_videos_readmore')) :
			if ($params->get('access-view')) :
				$link = JRoute::_(VideomanagerHelperRoute::getVideosRoute($this->item->slug, 
																							$this->item->language,									
																							$layout,									
																							$params->get('keep_videos_itemid')));
			else :
				if ($params->get('show_videos_noauth') AND $user->get('guest')) :
					$menu = JFactory::getApplication()->getMenu();
					$active = $menu->getActive();
					$item_id = $active->id;
					$link_1 = JRoute::_('index.php?option=com_users&view=login&Itemid=' . $item_id);
					$return_url = JRoute::_(VideomanagerHelperRoute::getVideosRoute($this->item->slug, 
																								$this->item->language,									
																								$layout,									
																								$params->get('keep_videos_itemid')));
			...
			<?php
				...
				if ($item_params->get('videos_alternative_readmore') == null) :
						if ($params->get('access-view')) :
