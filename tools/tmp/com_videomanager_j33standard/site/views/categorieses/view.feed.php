...
class VideomanagerViewCategorieses extends JViewLegacy
{
	...
	public function display($tpl = null)
	{
		...
		$doc->link = JRoute::_('index.php?option=com_videomanager&view=categorieses');

		// Get some data from the model
		$app->input->set('limit', $app->get('feed_limit'));
		$options['countItems'] = false;
		$options['table'] = '#__videomanager_categorieses';				
		...
		foreach ($rows as $row)
		{
			...
			// url link to categories
			$link = JRoute::_(VideomanagerHelperRoute::getCategoriesRoute($row->slug, 
																						$row->catid, 
																						$row->language,	
																						'default',								
			...
			if ($item->featured == 1)
			{
				$item->category[]	= JText::_('JFEATURED'); // All featured categorieses are categorized as "Featured"
			}
