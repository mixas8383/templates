...
class VideomanagerViewCategoris extends JViewLegacy
{
	...
	public function display($tpl = null)
	{
		...
		$doc->link = JRoute::_('index.php?option=com_videomanager&view=categoris');
		...
		foreach ($rows as $row)
		{
			...
			// url link to categori
			$link = JRoute::_(VideomanagerHelperRoute::getCategoriRoute($row->slug, 
																						$row->catid, 
																						$row->language,	
																						'default',								
			...
			if ($item->featured == 1)
			{
				$item->category[]	= JText::_('JFEATURED'); // All featured categoris are categorized as "Featured"
			}
