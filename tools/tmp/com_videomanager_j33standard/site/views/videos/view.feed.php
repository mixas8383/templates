...
class VideomanagerViewVideos extends JViewLegacy
{
	...
	public function display($tpl = null)
	{
		...
		$doc->link = JRoute::_('index.php?option=com_videomanager&view=videos');
		...
		foreach ($rows as $row)
		{
			...
			// url link to video
			$link = JRoute::_(VideomanagerHelperRoute::getVideoRoute($row->slug, 
																						$row->catid, 
																						$row->language,	
																						'default',								
			...
			if ($item->featured == 1)
			{
				$item->category[]	= JText::_('JFEATURED'); // All featured videos are categorized as "Featured"
			}
