...
$layout		= $params->get('video_layout', 'default');
...
<h2 itemprop="name">
		<?php if ($params->get('link_video_names') ) : ?>
			<a href="<?php echo JRoute::_(VideomanagerHelperRoute::getVideoRoute($this->item->slug, 
											$this->item->catid, 
											$this->item->language,
											$layout,									
											$params->get('keep_video_itemid'))); ?>" itemprop="url">
...
<?php if ($params->get('access-view') 
		AND (!$params->get('show_video_readmore')
		OR !$params->get('show_video_intro'))
		):?>
	<?php
...
<?php else : ?>
	...
	<?php if ($params->get('show_video_intro')) : ?>
		<?php if ($params->get('access-view') OR (!$params->get('access-view') AND $params->get('show_video_noauth'))) : ?>
			<span itemprop="description">
	...
	<?php if ($params->get('show_video_readmore')) :
			if ($params->get('access-view')) :
				$link = JRoute::_(VideomanagerHelperRoute::getVideoRoute($this->item->slug, 
																							$this->item->catid, 
																							$this->item->language,
																							$layout,									
																							$params->get('keep_video_itemid')));
			else :
				if ($params->get('show_video_noauth') AND $user->get('guest')) :
					$menu = JFactory::getApplication()->getMenu();
					$active = $menu->getActive();
					$item_id = $active->id;
					$link_1 = JRoute::_('index.php?option=com_users&view=login&Itemid=' . $item_id);
					$return_url = JRoute::_(VideomanagerHelperRoute::getVideoRoute($this->item->slug, 
																								$this->item->catid, 
																								$this->item->language,									
																								$layout,									
																								$params->get('keep_video_itemid')));
			...
			<?php
				...
				if ($item_params->get('video_alternative_readmore') == null) :
						if ($params->get('access-view')) :
