 ...
 *
 */
class VideomanagerModelVideo extends JModelAdmin
{
	/**
	 * @var		string	The prefix to use with controller messages.
	 */
	protected $text_prefix = 'COM_VIDEOMANAGER_VIDEOS';
	/**
	 * @var      string	The type alias for this object (for example, 'com_videomanager.video')
	 */
	public $typeAlias = 'com_videomanager.video';	
	/**
	 ...
	 */
	protected $event_before_save = 'onVideoBeforeSave';
	/**
	 * @var		string	The event to trigger after saving the data.
	 */
	protected $event_after_save = 'onVideoAfterSave';

	/**
	 * @var    string	The event to trigger before deleting the data.
	 */
	protected $event_before_delete = 'onVideoBeforeDelete';	
	/**
	 * @var    string	The event to trigger after deleting the data.
	 */
	protected $event_after_delete = 'onVideoAfterDelete';	
	/**
	 * @var    string	The event to trigger after changing the data's state field.
	 */
	protected $event_change_state = 'onVideoChangeState';	
	 ...
	 */
	public function getTable($type = 'Videos', $prefix = 'VideomanagerTable', $config = array())
	{
	...
	protected function canDelete($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.delete', 'com_videomanager.video.'.(int) $record->id);
		}
		else
		{
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.edit.state', 'com_videomanager.video.'.(int) $record->id);
		}
		else
		{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_videomanager.video');
			}	
		}
		...
		if ($assoc)
		{
			...
			if ($item->id != null)
			{
				$associations = JLanguageAssociations::getAssociations('com_videomanager', '#__videomanager_videos', 'com_videomanager.video.item', $item->id, 'id', 'alias', 'catid');
	...
	{
		$form = $this->loadForm('com_videomanager.edit.video', 'video', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = $app->getUserState('com_videomanager.edit.video.data', array());
		...
		if (empty($data))
		{
			...
			{
				$filters = (array) $app->getUserState('com_videomanager.videos.filter');
	...
	protected function prepareTable($table)
	{
		...
		// Increment the video version number.
		$table->version++;
		...
		if (parent::save($data))
		{
			...
			if ($assoc)
			{
				...
				try
				{
					...
					$query->where('context='.$db->quote('com_videomanager.video.item'));
				...
				if (!$all_language AND count($associations))
				{
					...
					{
						$query->values($id.','.$db->quote('com_videomanager.video.item') . ',' . $db->quote($key));
					}
	...
	public function featured($pks, $value = 0)
	{
		...
		if (empty($pks))
		{
			$this->setError(JText::_('COM_VIDEOMANAGER_VIDEOS_NO_ITEM_SELECTED'));
			return false;
		}
		...
		try
		{
			...
			$query = $db->getQuery(true);
			$query->update($db->quoteName('#__videomanager_videos'));
	...
	* @param   array   $data	The data where the original name is stored
	...
	/**
	 * Pre process the form to pick up videos associated by language
	 *
	 ...
	 */
	protected function preprocessForm(JForm $form, $data, $group = 'video')
	{
		...
		if ($assoc)
		{
			...
			$fieldset->addAttribute('description', 'COM_VIDEOMANAGER_VIDEOS_ITEM_ASSOCIATIONS_FIELDSET_DESC');
			$add = false;
			foreach ($languages as $tag => $language)
			{
				if (empty($data->language) OR $tag != $data->language)
				{
					...
					$field->addAttribute('type', 'modal_videos');
