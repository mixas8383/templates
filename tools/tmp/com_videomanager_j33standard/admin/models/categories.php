 ...
 *
 */
class VideomanagerModelCategories extends JModelAdmin
{
	/**
	 * @var		string	The prefix to use with controller messages.
	 */
	protected $text_prefix = 'COM_VIDEOMANAGER_CATEGORIESES';
	/**
	 * @var      string	The type alias for this object (for example, 'com_videomanager.categories')
	 */
	public $typeAlias = 'com_videomanager.categories';	
	/**
	 ...
	 */
	protected $event_before_save = 'onCategoriesBeforeSave';
	/**
	 * @var		string	The event to trigger after saving the data.
	 */
	protected $event_after_save = 'onCategoriesAfterSave';

	/**
	 * @var    string	The event to trigger before deleting the data.
	 */
	protected $event_before_delete = 'onCategoriesBeforeDelete';	
	/**
	 * @var    string	The event to trigger after deleting the data.
	 */
	protected $event_after_delete = 'onCategoriesAfterDelete';	
	/**
	 * @var    string	The event to trigger after changing the data's state field.
	 */
	protected $event_change_state = 'onCategoriesChangeState';	
	 ...
	 */
	public function getTable($type = 'Categorieses', $prefix = 'VideomanagerTable', $config = array())
	{
	...
	protected function canDelete($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.delete', 'com_videomanager.categories.'.(int) $record->id);
		}
		else
		{
			return $user->authorise('core.delete', 'com_videomanager');
		}							
	}
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.edit.state', 'com_videomanager.categories.'.(int) $record->id);
		}
		else
		{
			// New categories, so check against the category.		
			if (!empty($record->catid))
			{
				return $user->authorise('core.edit.state', 'com_videomanager.category.'.(int) $record->catid);
			}
			else 
			{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_videomanager.categories');
			}	
		}
		...
		if ($assoc)
		{
			...
			if ($item->id != null)
			{
				$associations = JLanguageAssociations::getAssociations('com_videomanager', '#__videomanager_categorieses', 'com_videomanager.categories.item', $item->id, 'id', 'alias', 'catid');
	...
	{
		$form = $this->loadForm('com_videomanager.edit.categories', 'categories', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
		...
		{
			$id = $this->getState('categories.id');		
			// Existing record. Can only edit in selected categories.
			$form->setFieldAttribute('catid', 'action', 'core.edit');
			// Existing record. Can only edit own categorieses in selected categories.
		...
		// Prevent messing with Categories language and category when editing existing Categories with associations
	...
	protected function loadFormData()
	{
		...
		$data = $app->getUserState('com_videomanager.edit.categories.data', array());

		if (empty($data))
		{
			$data = $this->getItem();
			// Prime some default values.
			if ($this->getState('categories.id') == 0)
			{
				$filters = (array) $app->getUserState('com_videomanager.categorieses.filter');
	...
	protected function prepareTable($table)
	{
		...
		// Increment the categories version number.
		$table->version++;
	...
	public function save($data)
	{
		...
		if (parent::save($data))
		{
			...
			if ($assoc)
			{
				...
				{
					JError::raiseNotice(403, JText::_('COM_VIDEOMANAGER_ERROR_ALL_LANGUAGE_ASSOCIATED'));
				}
				...
				try
				{
					...
					$query->where('context='.$db->quote('com_videomanager.categories.item'));
				...
				if (!$all_language AND count($associations))
				{
					...
					{
						$query->values($id.','.$db->quote('com_videomanager.categories.item') . ',' . $db->quote($key));
					}
	...
	{	
		// Include the videomanager plugins for the change of state event.
		JPluginHelper::importPlugin('videomanager');	
	...
	{
		// Include the videomanager plugins for the delete events.
		JPluginHelper::importPlugin('videomanager');	
	...
	/**
	 * Method to toggle the featured setting of categorieses.
	 *
	...
	public function featured($pks, $value = 0)
	{
		...
		if (empty($pks))
		{
			$this->setError(JText::_('COM_VIDEOMANAGER_CATEGORIESES_NO_ITEM_SELECTED'));
			return false;
		}
		...
		try
		{
			...
			$query = $db->getQuery(true);
			$query->update($db->quoteName('#__videomanager_categorieses'));
	...
	/**
	 * Custom clean the cache of com_videomanager and videomanager modules
	 *
	 */
	protected function cleanCache($group = null, $client_id = 0)
	{
		parent::cleanCache('com_videomanager');
	...
	* @param   array   $data	The data where the original name is stored
	...
	/**
	 * Pre process the form to pick up categorieses associated by language
	 *
	 ...
	 */
	protected function preprocessForm(JForm $form, $data, $group = 'categories')
	{
		...
		if ($assoc)
		{
			...
			$fieldset->addAttribute('description', 'COM_VIDEOMANAGER_CATEGORIESES_ITEM_ASSOCIATIONS_FIELDSET_DESC');
			$add = false;
			foreach ($languages as $tag => $language)
			{
				if (empty($data->language) OR $tag != $data->language)
				{
					...
					$field->addAttribute('type', 'modal_categorieses');
