 ...
 *
 */
class VideomanagerModelCategori extends JModelAdmin
{
	/**
	 * @var		string	The prefix to use with controller messages.
	 */
	protected $text_prefix = 'COM_VIDEOMANAGER_CATEGORIS';
	/**
	 * @var      string	The type alias for this object (for example, 'com_videomanager.categori')
	 */
	public $typeAlias = 'com_videomanager.categori';	
	/**
	 ...
	 */
	protected $event_before_save = 'onCategoriBeforeSave';
	/**
	 * @var		string	The event to trigger after saving the data.
	 */
	protected $event_after_save = 'onCategoriAfterSave';

	/**
	 * @var    string	The event to trigger before deleting the data.
	 */
	protected $event_before_delete = 'onCategoriBeforeDelete';	
	/**
	 * @var    string	The event to trigger after deleting the data.
	 */
	protected $event_after_delete = 'onCategoriAfterDelete';	
	/**
	 * @var    string	The event to trigger after changing the data's state field.
	 */
	protected $event_change_state = 'onCategoriChangeState';	
	 ...
	 */
	public function getTable($type = 'Categoris', $prefix = 'VideomanagerTable', $config = array())
	{
	...
	protected function canDelete($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.delete', 'com_videomanager.categori.'.(int) $record->id);
		}
		else
		{
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.edit.state', 'com_videomanager.categori.'.(int) $record->id);
		}
		else
		{
			// New categori, so check against the category.		
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_videomanager.categori');
			}	
		}
		...
		if ($assoc)
		{
			...
			if ($item->id != null)
			{
				$associations = JLanguageAssociations::getAssociations('com_videomanager', '#__videomanager_categoris', 'com_videomanager.categori.item', $item->id, 'id', 'alias', 'catid');
	...
	{
		$form = $this->loadForm('com_videomanager.edit.categori', 'categori', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
		...
		if ($this->getState('categori.id'))
		{
			$id = $this->getState('categori.id');		
			// Existing record. Can only edit in selected categories.
			$form->setFieldAttribute('catid', 'action', 'core.edit');
			// Existing record. Can only edit own categoris in selected categories.
		...
		// Prevent messing with Categori language and category when editing existing Categori with associations
	...
	protected function loadFormData()
	{
		...
		$data = $app->getUserState('com_videomanager.edit.categori.data', array());

		if (empty($data))
		{
			$data = $this->getItem();
			// Prime some default values.
			if ($this->getState('categori.id') == 0)
			{
				$filters = (array) $app->getUserState('com_videomanager.categoris.filter');
	...
	protected function prepareTable($table)
	{
		...
		// Increment the categori version number.
		$table->version++;
		...
		if (parent::save($data))
		{
			...
			if ($assoc)
			{
				...
				try
				{
					...
					$query->where('context='.$db->quote('com_videomanager.categori.item'));
				...
				if (!$all_language AND count($associations))
				{
					...
					{
						$query->values($id.','.$db->quote('com_videomanager.categori.item') . ',' . $db->quote($key));
					}
	...
	/**
	 * Method to toggle the featured setting of categoris.
	 *
	...
	public function featured($pks, $value = 0)
	{
		...
		if (empty($pks))
		{
			$this->setError(JText::_('COM_VIDEOMANAGER_CATEGORIS_NO_ITEM_SELECTED'));
			return false;
		}
		...
		try
		{
			...
			$query = $db->getQuery(true);
			$query->update($db->quoteName('#__videomanager_categoris'));
	...
	* @param   array   $data	The data where the original name is stored
	...
	/**
	 * Pre process the form to pick up categoris associated by language
	 *
	 ...
	 */
	protected function preprocessForm(JForm $form, $data, $group = 'categori')
	{
		...
		if ($assoc)
		{
			...
			$fieldset->addAttribute('description', 'COM_VIDEOMANAGER_CATEGORIS_ITEM_ASSOCIATIONS_FIELDSET_DESC');
			$add = false;
			foreach ($languages as $tag => $language)
			{
				if (empty($data->language) OR $tag != $data->language)
				{
					...
					$field->addAttribute('type', 'modal_categoris');
