...
class VideomanagerModelCategorieses extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		// Select the required categorieses from the table.
		$query->select(
			$this->getState(
					'list.select',
					'a.*'
		...
		if ($assoc)
		{
			...
			$query->join('LEFT', $db->quoteName('#__associations').' AS as1 ON '.$db->quoteName('as1.id').' = '.$db->quoteName('a.id').' AND '.$db->quoteName('as1.context').' = '.$db->quote('com_videomanager.categories.item'));
		...
		if (is_numeric($tag_id))
		{
			...
			$query->join(
					...
					. ' AND ' . $db->quoteName('tagmap.type_alias') . ' = ' . $db->quote('com_videomanager.categories')
				);
		}
