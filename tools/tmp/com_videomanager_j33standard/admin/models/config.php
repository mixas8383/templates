 ...
 *
 */
class VideomanagerModelConfig extends JModelAdmin
{
	/**
	 * @var		string	The prefix to use with controller messages.
	 */
	protected $text_prefix = 'COM_VIDEOMANAGER_CONFIGS';
	/**
	 * @var      string	The type alias for this object (for example, 'com_videomanager.config')
	 */
	public $typeAlias = 'com_videomanager.config';	
	/**
	 * @var		string	The context for the app call.
	 */
	protected $context = 'com_videomanager.configs';	
	/**
	 ...
	 */
	public function getTable($type = 'Configs', $prefix = 'VideomanagerTable', $config = array())
	{
	...
	protected function canDelete($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.delete', 'com_videomanager.config.'.(int) $record->id);
		}
		else
		{
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.edit.state', 'com_videomanager.config.'.(int) $record->id);
		}
		else
		{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_videomanager.config');
			}	
		}
		...
		if ($assoc)
		{
			...
			if ($item->id != null)
			{
				$associations = JLanguageAssociations::getAssociations('com_videomanager', '#__videomanager_configs', 'com_videomanager.config.item', $item->id, 'id', 'alias', null);
	...
	{
		$form = $this->loadForm('com_videomanager.edit.config', 'config', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = $app->getUserState('com_videomanager.edit.config.data', array());
		...
		if (parent::save($data))
		{
			...
			if ($assoc)
			{
				...
				try
				{
					...
					$query->where('context='.$db->quote('com_videomanager.config.item'));
				...
				if (!$all_language AND count($associations))
				{
					...
					{
						$query->values($id.','.$db->quote('com_videomanager.config.item') . ',' . $db->quote($key));
					}
	...
	public function featured($pks, $value = 0)
	{
		...
		if (empty($pks))
		{
			$this->setError(JText::_('COM_VIDEOMANAGER_CONFIGS_NO_ITEM_SELECTED'));
			return false;
		}
		...
		try
		{
			...
			$query = $db->getQuery(true);
			$query->update($db->quoteName('#__videomanager_configs'));
	...
	* @param   array   $data	The data where the original name is stored
	...
	protected function preprocessForm(JForm $form, $data, $group = 'config')
	{
		...
		if ($assoc)
		{
			...
			$fieldset->addAttribute('description', 'COM_VIDEOMANAGER_CONFIGS_ITEM_ASSOCIATIONS_FIELDSET_DESC');
			$add = false;
