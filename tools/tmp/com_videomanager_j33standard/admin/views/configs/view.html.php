...
class VideomanagerViewConfigs extends JViewLegacy
{
	...
	protected function addToolbar()
	{
		...
		JToolbarHelper::title(JText::_('COM_VIDEOMANAGER_CONFIGS_LIST_HEADER'), 'stack configs');
		...
		if ($this->can_do->get('core.edit.state') ) 
		{
			...
			if ($this->state->get('filter.state') != -1 ) 
			{
				...
				else 
				{
					if ($this->state->get('filter.state') == 2) 
					{
						JToolbarHelper::unarchiveList('configs.publish', 'JTOOLBAR_UNARCHIVE');
					}
				}
			}
		}
	...
	{	
		JHtmlSidebar::setAction('index.php?option=com_videomanager&view=configs');
