...
if ($save_order)
{
	$save_ordering_url = 'index.php?option=com_videomanager&task=categorieses.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'categories-list', 'adminForm', strtolower($list_dirn), $save_ordering_url);
}
...
<form action="<?php echo JRoute::_('index.php?option=com_videomanager&view=categorieses'); ?>" method="post" name="adminForm" id="adminForm">
	...
	<?php else : ?>
		<table class="table table-striped" id="categories-list">
			...
			<?php foreach ($this->items as $i => $item) :
				...
				$can_edit	= $user->authorise('core.edit',	'com_videomanager.categories.'.$item->id);
		
				$can_edit_own	= $user->authorise('core.edit.own',		'com_videomanager.categories.'.$item->id) 
								AND $item->created_by == $user_id
								;
				$can_change	= $user->authorise('core.edit.state',	'com_videomanager.categories.'.$item->id) 
								AND $can_checkin
								;
				if ($item->language == '*'):
				...
				<tr class="row<?php echo $i % 2; ?>" sortable-group-id="<?php echo $item->catid; ?>">
					...
					<td class="center">
						<div class="btn-group">
							<?php echo JHtml::_('jgrid.published', $item->state, $i, 'categorieses.', $can_change, 'cb', $item->publish_up, $item->publish_down); ?>
							<?php echo JHtml::_('categoriesadministrator.featured', $item->featured, $i, $can_change); ?>
							<?php
								if ($archived) :
									JHtml::_('actionsdropdown.unarchive', 'cb' . $i, 'categorieses');
								else :
									JHtml::_('actionsdropdown.archive', 'cb' . $i, 'categorieses');
								endif;
								if ($trashed) :
									JHtml::_('actionsdropdown.untrash', 'cb' . $i, 'categorieses');
								else :
									JHtml::_('actionsdropdown.trash', 'cb' . $i, 'categorieses');
								endif;
					...
					<td class="nowrap has-context">
						<div class="pull-left">
							<?php if ($item->checked_out) : ?>
								<?php echo JHtml::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'categorieses.', $can_checkin); ?>
							<?php endif; ?>	
							<?php if ($can_edit OR $can_edit_own) : ?>
								<a href="<?php echo JRoute::_('index.php?option=com_videomanager&task=categories.edit&id='.(int) $item->id); ?>">
