 ...
 *
 */
class VideomanagerControllerConfig extends JControllerForm
{
	/**
	...
	protected function allowAdd($data = array())
	{
		...
		$result = $user->authorise('core.create', 'com_videomanager');
		if($result === null)
		{
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		if ($user->authorise('core.edit', 'com_videomanager.config.'.$record_id))
		{
			return true;
		}
		// Fallback on edit.own.
		// First test if the permission is available.
		if ($user->authorise('core.edit.own', 'com_videomanager.config.'.$record_id))
		{
	...
	public function batch($model = null)
	{
		...
		// Set the model
		$model = $this->getModel('Config', 'VideomanagerModel', array());

		// Preset the redirect
		$this->setRedirect(JRoute::_('index.php?option=com_videomanager&view=configs' . $this->getRedirectToListAppend(), false));
