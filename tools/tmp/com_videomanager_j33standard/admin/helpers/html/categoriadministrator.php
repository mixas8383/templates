...
/**
 * Categoris component helper.
 *
 */
abstract class JHtmlCategoriAdministrator
{
	/**
	 * Render the list of associated items
	 * 
	 * @param	integer $categori_id	The categori item id
	 * 
	 * @return  string  The language HTML
     */
	public static function association($categori_id)
	{
		...
		// Get the associations
		if ($associations = JLanguageAssociations::getAssociations('com_videomanager', '#__videomanager_categoris', 'com_videomanager.categori.item',  $categori_id, 'id', 'alias', 'catid'))
		{		
			...
			if ($items)
			{
				foreach ($items as &$item)
				{
					...
					$url = JRoute::_('index.php?option=com_videomanager&task=categori.edit&id=' . (int) $item->id);
					$tooltip_parts = array(
	...
	public static function featured($value = 0, $i, $can_change = true)
	{
		...
		$states	= array(
			0	=> array('unfeatured', 'categoris.featured', 'COM_VIDEOMANAGER_CATEGORIS_UNFEATURED', 'COM_VIDEOMANAGER_CATEGORIS_TOGGLE_TO_FEATURE'),
			1	=> array('featured', 'categoris.unfeatured', 'JFEATURED', 'COM_VIDEOMANAGER_CATEGORIS_TOGGLE_TO_UNFEATURE'),
		);
