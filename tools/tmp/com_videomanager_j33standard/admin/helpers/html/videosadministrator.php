...
/**
 * Videoses component helper.
 *
 */
abstract class JHtmlVideosAdministrator
{
	/**
	 * Render the list of associated items
	 * 
	 * @param	integer $videos_id	The videos item id
	 * 
	 * @return  string  The language HTML
     */
	public static function association($videos_id)
	{
		...
		// Get the associations
		if ($associations = JLanguageAssociations::getAssociations('com_videomanager', '#__videomanager_videoses', 'com_videomanager.videos.item',  $videos_id, 'id', 'alias', null))
		{		
			...
			if ($items)
			{
				foreach ($items as &$item)
				{
					...
					$url = JRoute::_('index.php?option=com_videomanager&task=videos.edit&id=' . (int) $item->id);
					$tooltip_parts = array(
	...
	public static function featured($value = 0, $i, $can_change = true)
	{
		...
		$states	= array(
			0	=> array('unfeatured', 'videoses.featured', 'COM_VIDEOMANAGER_VIDEOSES_UNFEATURED', 'COM_VIDEOMANAGER_VIDEOSES_TOGGLE_TO_FEATURE'),
			1	=> array('featured', 'videoses.unfeatured', 'JFEATURED', 'COM_VIDEOMANAGER_VIDEOSES_TOGGLE_TO_UNFEATURE'),
		);
