...
/**
 * Videos component helper.
 *
 */
abstract class JHtmlVideoAdministrator
{
	/**
	 * Render the list of associated items
	 * 
	 * @param	integer $video_id	The video item id
	 * 
	 * @return  string  The language HTML
     */
	public static function association($video_id)
	{
		...
		// Get the associations
		if ($associations = JLanguageAssociations::getAssociations('com_videomanager', '#__videomanager_videos', 'com_videomanager.video.item',  $video_id, 'id', 'alias', 'catid'))
		{		
			...
			if ($items)
			{
				foreach ($items as &$item)
				{
					...
					$url = JRoute::_('index.php?option=com_videomanager&task=video.edit&id=' . (int) $item->id);
					$tooltip_parts = array(
	...
	public static function featured($value = 0, $i, $can_change = true)
	{
		...
		$states	= array(
			0	=> array('unfeatured', 'videos.featured', 'COM_VIDEOMANAGER_VIDEOS_UNFEATURED', 'COM_VIDEOMANAGER_VIDEOS_TOGGLE_TO_FEATURE'),
			1	=> array('featured', 'videos.unfeatured', 'JFEATURED', 'COM_VIDEOMANAGER_VIDEOS_TOGGLE_TO_UNFEATURE'),
		);
