...
JLoader::register('ContentHelper', JPATH_ADMINISTRATOR . '/components/com_videomanager/helpers/videomanager.php');
/**
 * Categorieses component helper.
 *
 */
abstract class JHtmlCategoriesAdministrator
{
	 ...
	 * 
	 * @param	integer $categories_id	The categories item id
	 * 
	 * @return  string  The language HTML
     */
	public static function association($categories_id)
	{
		...
		// Get the associations
		if ($associations = JLanguageAssociations::getAssociations('com_videomanager', '#__videomanager_categorieses', 'com_videomanager.categories.item',  $categories_id, 'id', 'alias', 'catid'))
		{		
			...
			if ($items)
			{
				foreach ($items as &$item)
				{
					...
					$url = JRoute::_('index.php?option=com_videomanager&task=categories.edit&id=' . (int) $item->id);
					$tooltip_parts = array(
	...
	public static function featured($value = 0, $i, $can_change = true)
	{
		...
		$states	= array(
			0	=> array('unfeatured', 'categorieses.featured', 'COM_VIDEOMANAGER_CATEGORIESES_UNFEATURED', 'COM_VIDEOMANAGER_CATEGORIESES_TOGGLE_TO_FEATURE'),
			1	=> array('featured', 'categorieses.unfeatured', 'JFEATURED', 'COM_VIDEOMANAGER_CATEGORIESES_TOGGLE_TO_UNFEATURE'),
		);
