...
/**
 * Configs component helper.
 *
 */
abstract class JHtmlConfigAdministrator
{
	/**
	 * Render the list of associated items
	 * 
	 * @param	integer $config_id	The config item id
	 * 
	 * @return  string  The language HTML
     */
	public static function association($config_id)
	{
		...
		// Get the associations
		if ($associations = JLanguageAssociations::getAssociations('com_videomanager', '#__videomanager_configs', 'com_videomanager.config.item',  $config_id, 'id', 'alias', null))
		{		
			...
			if ($items)
			{
				foreach ($items as &$item)
				{
					...
					$url = JRoute::_('index.php?option=com_videomanager&task=config.edit&id=' . (int) $item->id);
					$tooltip_parts = array(
	...
	public static function featured($value = 0, $i, $can_change = true)
	{
		...
		$states	= array(
			0	=> array('unfeatured', 'configs.featured', 'COM_VIDEOMANAGER_CONFIGS_UNFEATURED', 'COM_VIDEOMANAGER_CONFIGS_TOGGLE_TO_FEATURE'),
			1	=> array('featured', 'configs.unfeatured', 'JFEATURED', 'COM_VIDEOMANAGER_CONFIGS_TOGGLE_TO_UNFEATURE'),
		);
