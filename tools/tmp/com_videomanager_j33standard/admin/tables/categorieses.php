 ...
 *
 */
class VideomanagerTableCategorieses extends JTable
{
	...
	public function __construct(&$db)
	{
		...
		parent::__construct('#__videomanager_categorieses', 'id', $db);
		...
		JTableObserverTags::createObserver($this, array('typeAlias' => 'com_videomanager.categories'));

		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_videomanager.categories'));
	}
	...
	public function check()
	{
		...
		$table = JTable::getInstance('Categorieses', 'VideomanagerTable');
		if ($table->load(array('alias'=>$this->alias)) AND ($table->id != $this->id OR $this->id==0))
		{
			$this->setError(JText::_('COM_VIDEOMANAGER_FIELD_ALIAS_DUPLICATED'));
			return false;
		}		
	...
	public function store($update_nulls = false)
	{
		...
		if (empty($this->id))
		{
			// New Categories. A created and created_by field can be set by the user,
	...
	{
		$k = $this->_tbl_key;
		return 'com_videomanager.categories.'.(int) $this->$k;
	}
	...
	protected function _getAssetParentId(JTable $table = null, $id = null)
	{
		...
		if (($this->catid) AND !empty($this->catid))
		{
			...
			if ($asset->loadByName('com_videomanager.category.' . (int) $this->catid))
			{
				return $asset->id;
			}
		}
		
		if ($asset->loadByName('com_videomanager'))
		{
			return $asset->id;
		}
		else
