...
JModelLegacy::addIncludePath(JPATH_SITE.'/components/com_videomanager/models', 'VideomanagerModelVideoses');

abstract class ModVideosesHelper
{
	/**
	...
	public static function getList(&$params)
	{
		...
		// Get an instance of the generic videoses model
		$model = JModelLegacy::getInstance('Videoses', 'VideomanagerModel', array('ignore_request' => true));
		...
		// Access filter
		$access = !JComponentHelper::getParams('com_videomanager')->get('show_videos_noauth');
		...
		if ($params->get('videos_layout') <> '')
		{
			$layout = str_replace('_:','',(string) $params->get('videos_layout'));
		...
		foreach ($items as &$item)
		{
			...
			if ($access OR in_array($item->access, $authorised))
			{
				if ($item_id_str == '')
				{
					// We know that user has the privilege to view the videos
					$item->link = JRoute::_(VideomanagerHelperRoute::getVideosRoute($item->slug, $item->language, $layout));
				}
				else
				{
					$item->link = JRoute::_('index.php?option=com_videomanager&view=videos'.$layout_str.$item_id_str.'&id='.$item->id);
				}					
			}
			else
			{
