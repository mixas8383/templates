 ...
 * @subpackage		com_videomanager.mod_videomanager
 * @copyright		
...
abstract class ModVideomanagerHelper
{
	/**
	 * Helper for mod_videomanager
	 *
	...
	public static function getList(&$params)
	{
		...
		JModelLegacy::addIncludePath(JPATH_SITE.'/components/com_videomanager/models', 'VideomanagerModel'.$component_object_names[1]);

		// Get an instance of the model
		$model = JModelLegacy::getInstance($component_object_names[1], 'VideomanagerModel', array('ignore_request' => true));
		...
		// Access filter
		$access = !JComponentHelper::getParams('com_videomanager')->get('show_'.JString::strtolower($component_object_names[0]).'_noauth');
		...
		if (JString::strtolower($component_object_names[1]) == JString::substr($params->get('ordering'),0,JString::strlen($params->get('ordering'))-6))
		{
			$order_map = array(
				...
				'videoses m_dsc' => 'a.modified DESC, a.created',
				'videoses c_dsc' => 'a.created',
				'videoses p_dsc' => 'a.publish_up',
				'videoses h_dsc' => 'a.hits',
				'videoses n_asc' => 'a.name',
				'videoses n_dsc' => 'a.name',
				'videoses o_asc' => 'a.ordering',
				'categorieses m_dsc' => 'a.modified DESC, a.created',
				'categorieses c_dsc' => 'a.created',
				'categorieses p_dsc' => 'a.publish_up',
				'categorieses h_dsc' => 'a.hits',
				'categorieses n_asc' => 'a.name',
				'categorieses n_dsc' => 'a.name',
				'categorieses o_asc' => 'a.ordering',
			);
		...
		foreach ($items as &$item)
		{
			...
			if (!isset($item->access) OR $access OR in_array($item->access, $authorised))
			{
				...
				{			
					...
					{
						$item->link = JRoute::_(VideomanagerHelperRoute::$route_function($item->slug, $item->catslug, $item->language));
					}
					else
					{
						if (isset($item->catslug))
						{
							$item->link = JRoute::_(VideomanagerHelperRoute::$route_function($item->slug, $item->catslug));
						}
						else
						{
							if (isset($item->language))
							{
								$item->link = JRoute::_(VideomanagerHelperRoute::$route_function($item->slug, $item->language));
							}
							else
							{
								$item->link = JRoute::_(VideomanagerHelperRoute::$route_function($item->slug));
							}
						}
					}
				}
				...
				{
					$item->link = JRoute::_('index.php?option=com_videomanager&view='.JString::strtolower($component_object_names[0]).$item_id_str.'&id='.$item->id);
				}
			}
			else
			{
