...
JModelLegacy::addIncludePath(JPATH_SITE.'/components/com_videomanager/models', 'VideomanagerModelConfigs');

abstract class ModConfigsHelper
{
	/**
	...
	public static function getList(&$params)
	{
		...
		// Get an instance of the generic configs model
		$model = JModelLegacy::getInstance('Configs', 'VideomanagerModel', array('ignore_request' => true));
		...
		// Access filter
		$access = !JComponentHelper::getParams('com_videomanager')->get('show_config_noauth');
		...
		if ($params->get('config_layout') <> '')
		{
			$layout = str_replace('_:','',(string) $params->get('config_layout'));
		...
		foreach ($items as &$item)
		{
			...
			if ($access OR in_array($item->access, $authorised))
			{
				if ($item_id_str == '')
				{
					// We know that user has the privilege to view the config
					$item->link = JRoute::_(VideomanagerHelperRoute::getConfigRoute($item->slug, $item->language, $layout));
				}
				else
				{
					$item->link = JRoute::_('index.php?option=com_videomanager&view=config'.$layout_str.$item_id_str.'&id='.$item->id);
				}					
			}
			else
			{
