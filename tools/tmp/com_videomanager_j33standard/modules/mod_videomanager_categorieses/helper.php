...
JModelLegacy::addIncludePath(JPATH_SITE.'/components/com_videomanager/models', 'VideomanagerModelCategorieses');

abstract class ModCategoriesesHelper
{
	/**
	...
	public static function getList(&$params)
	{
		...
		// Get an instance of the generic categorieses model
		$model = JModelLegacy::getInstance('Categorieses', 'VideomanagerModel', array('ignore_request' => true));
		...
		// Access filter
		$access = !JComponentHelper::getParams('com_videomanager')->get('show_categories_noauth');
		...
		if ($params->get('categories_layout') <> '')
		{
			$layout = str_replace('_:','',(string) $params->get('categories_layout'));
		...
		foreach ($items as &$item)
		{
			...
			if ($access OR in_array($item->access, $authorised))
			{
				if ($item_id_str == '')
				{
					// We know that user has the privilege to view the categories
					$item->link = JRoute::_(VideomanagerHelperRoute::getCategoriesRoute($item->slug, $item->catslug, $item->language, $layout));
				}
				else
				{
					$item->link = JRoute::_('index.php?option=com_videomanager&view=categories'.$layout_str.$item_id_str.'&id='.$item->id);
				}					
			}
			else
			{
