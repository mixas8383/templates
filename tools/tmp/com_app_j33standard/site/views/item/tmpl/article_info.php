		...
		<?php if ($params->get('show_item_parent_category') AND $this->item->parent_slug != '1:root') : ?>
			<dd class="parent-category-name">
				...
				<?php if ($params->get('link_item_parent_category') AND $this->item->parent_slug) : ?>
					<?php $url = '<a href="'.JRoute::_(AppHelperRoute::getCategoryRoute($this->item->parent_slug, $params->get('keep_item_itemid'))).'" itemprop="url">'.$title.'</a>'; ?>
		...
		<?php if ($params->get('show_item_category')) : ?>
			<dd class="category-name">
				...
				<?php if ($params->get('link_item_category') AND $this->item->catslug) : ?>
					<?php $url = '<a href="'.JRoute::_(AppHelperRoute::getCategoryRoute($this->item->catslug, $params->get('keep_item_itemid'))).'" itemprop="url">'.$title.'</a>';?>
		...
		<?php if ($params->get('show_item_created')) : ?>
			<dd class="create">
				...
				<time datetime="<?php echo JHtml::_('date', $this->item->created, 'c'); ?>" itemprop="dateCreated">
					<?php echo JText::sprintf('COM_APP_CREATED_DATE_ON', JHtml::_('date',$this->item->created, JText::_('DATE_FORMAT_LC3'))); ?>
				</time>
			</dd>
		<?php endif; ?>
		<?php if ($params->get('show_item_publish_up') AND $this->item->publish_up > 0) : ?>
			<dd class="published">
				...
				<time datetime="<?php echo JHtml::_('date', $this->item->publish_up, 'c'); ?>" itemprop="datePublished">
					<?php echo JText::sprintf('COM_APP_PUBLISH_UP_ON', JHtml::_('date',$this->item->publish_up, JText::_('DATE_FORMAT_LC3'))); ?>
				</time>
			</dd>
		<?php endif; ?>
		<?php if ($params->get('show_item_publish_down') AND $this->item->publish_down > 0) : ?>
			<dd class="published">
				...
				<time datetime="<?php echo JHtml::_('date', $this->item->publish_down, 'c'); ?>">
					<?php echo JText::sprintf('COM_APP_PUBLISH_DOWN_ON', JHtml::_('date',$this->item->publish_down, JText::_('DATE_FORMAT_LC3'))); ?>
				</time
			</dd>
		<?php endif; ?>
		<?php if ($params->get('show_item_modified')) : ?>
			<dd class="modified">
				...
				<time datetime="<?php echo JHtml::_('date', $this->item->modified, 'c'); ?>" itemprop="dateModified">
					<?php echo JText::sprintf('COM_APP_LAST_UPDATED', JHtml::_('date',$this->item->modified, JText::_('DATE_FORMAT_LC3'))); ?>
				</time>
			</dd>
		<?php endif; ?>
