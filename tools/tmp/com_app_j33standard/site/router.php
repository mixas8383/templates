...
/**
 * Routing class for the App component
 *
 */
class AppRouter extends JComponentRouterBase
{
	/**
	 * Build the route for the App component
	 *
	...
	public function build(&$query)
	{
		...
		$menu	= $app->getMenu();
		$params	= JComponentHelper::getParams('com_app');
		...
		// Are we dealing with a app that is attached to a menu item?
	...
	public function parse(&$segments)
	{
		...
		{
			$categories = JCategories::getInstance('App',$options)->get($id)->getChildren();
		}
		else
		{
			$categories = JCategories::getInstance('App',$options);	
			...
			if ($found == 0)
			{
					...
					OR $view == 'items' OR $view == 'item')
				{
					if($advanced)
					{
						...
						$query->from($db->quoteName('#__app_items'));
