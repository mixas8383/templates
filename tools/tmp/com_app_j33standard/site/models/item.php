 ...
 *
 */
class AppModelItem extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_app.item';
	...
	protected function canDelete($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.delete', 'com_app.item.'.(int) $record->id)
					  OR ($user->authorise('core.delete', 'com_app.item.'.(int) $record->id)
	...
	protected function canEditState($record)
	{
		...
		else
		{
			...
			// Default to component settings.			
				return $user->authorise('core.edit.state', 'com_app');
			}
		}
	}
	/**
	...
	*/
	...
	public function getItem($pk = null)
	{
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				$sub_query .= 'WHERE '.$db->quoteName('parent.extension').' = ' . $db->quote('com_app');
				...
				$can_publish = $user->authorise('core.edit.state', 'com_app.item.'.$pk);
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_APP_ITEMS_ERROR_ITEM_NOT_FOUND'));
				}
				...
				$item->tags->getItemTags('com_app.item', $item->id);
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onItemChangeState', array('com_app.item', $pks, $value));
	...
	*/
	...
	public function delete(&$pks)
	{
		...
		// Include the app plugins for the on delete events.
		JPluginHelper::importPlugin('app');
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onItemBeforeDelete', array('com_app.item', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onItemAfterDelete', array('com_app.item', &$table));
				}
				else
				{
					// Prune items that you can't change.
    ...
    public function storeVote($pk = 0, $rate = 0)
    {
        if ( $rate >= 1 AND $rate <= 5 AND $pk > 0 )
        {
            ...
            $db->setQuery(
                    'SELECT *' .
                    ' FROM '.$db->quoteName('#__app_rating') .
            ...
            if (!$rating)
            {
                ...
                $db->setQuery(
                        'INSERT INTO '.$db->quoteName('#__app_rating').' ( '.$db->quoteName('content_type').', '.$db->quoteName('content_id').', '.$db->quoteName('lastip').', '.$db->quoteName('rating_sum').', '.$db->quoteName('rating_count').' )' .
            ...
            else
            {
                ...
                {
                    $db->setQuery(
                            'UPDATE '.$db->quoteName('#__app_rating') .
        ...
        JError::raiseWarning( 'SOME_ERROR_CODE', JText::sprintf('COM_APP_ITEMS_INVALID_RATING', $rate), "AppModelItem::storeVote($rate)");
        return false;
    }
}
