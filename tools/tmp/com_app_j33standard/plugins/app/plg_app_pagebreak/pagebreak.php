 ...
 *
 */
class PlgAppPagebreak extends JPlugin
{
	/**
	...
	{
		$can_proceed = $context == 'com_app.item';
			...
			else
			{
				...
				foreach ($text as $key => $subtext)
				{
					if ($key >= 1)
					{
						...
						{
							$title	= JText::sprintf('PLG_APP_PAGEBREAK_PAGE_NUM', $key + 1);
						}
	...
	{
		$heading = isset($row->name) ? $row->name : JText::_('PLG_APP_PAGEBREAK_NO_TITLE');
		$input = JFactory::getApplication()->input;
		...
		{
			$headingtext = JText::_('PLG_APP_PAGEBREAK_INDEX_LABEL');
		...
		<li class="' . $class . '">
			...
			<a href="' . JRoute::_(AppHelperRoute::getItemRoute($row->slug, $row->catid) . '&showall=&limitstart=') . '" class="' . $class . '">'
			. $heading .
			'</a>
		...
		{
			$link = JRoute::_(AppHelperRoute::getItemRoute($row->slug, $row->catid) . '&showall=&limitstart=' . ($i - 1));
			...
			if (@$bot[0])
			{
				...
				{
					$title	= JText::sprintf('PLG_APP_PAGEBREAK_PAGE_NUM', $i);
				}
			}
			else
			{
		...
		{
			$link = JRoute::_(AppHelperRoute::getItemRoute($row->slug, $row->catid) . '&showall=1&limitstart=');
			$class = ($show_all == 1) ? 'toclink active' : 'toclink';
			$row->toc .= '
			<li>

					<a href="' . $link . '" class="' . $class . '">'
					. JText::_('PLG_APP_PAGEBREAK_ALL_PAGES') .
					'</a>
	...
	protected function _createItemNavigation(&$row, $page, $n)
	{
		...
		if ($page < $n - 1)
		{
			...
			$link_next = JRoute::_(AppHelperRoute::getItemRoute($row->slug, $row->catid) . '&showall=&limitstart=' . ($page_next));
			// Next >>
		...
		if ($page > 0)
		{
			...
			$link_prev = JRoute::_(AppHelperRoute::getItemRoute($row->slug, $row->catid) . '&showall=&limitstart=' . ($page_prev));
