...
JLoader::register('ContentHelper', JPATH_ADMINISTRATOR . '/components/com_app/helpers/app.php');
/**
...
abstract class JHtmlItemAdministrator
{
	...
	public static function association($item_id)
	{
		...
		// Get the associations
		if ($associations = JLanguageAssociations::getAssociations('com_app', '#__app_items', 'com_app.item.item',  $item_id, 'id', 'alias', 'catid'))
		{		
			...
			if ($items)
			{
				foreach ($items as &$item)
				{
					...
					$url = JRoute::_('index.php?option=com_app&task=item.edit&id=' . (int) $item->id);
					$tooltip_parts = array(
	...
	public static function featured($value = 0, $i, $can_change = true)
	{
		...
		$states	= array(
			0	=> array('unfeatured', 'items.featured', 'COM_APP_ITEMS_UNFEATURED', 'COM_APP_ITEMS_TOGGLE_TO_FEATURE'),
			1	=> array('featured', 'items.unfeatured', 'JFEATURED', 'COM_APP_ITEMS_TOGGLE_TO_UNFEATURE'),
		);
