...
class AppModelItem extends JModelAdmin
{
	...
	/**
	 * @var      string	The type alias for this object (for example, 'com_app.item')
	 */
	public $typeAlias = 'com_app.item';	
	/**
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_app.item');
			}	
		}
		...
		if ($assoc)
		{
			...
			if ($item->id != null)
			{
				$associations = JLanguageAssociations::getAssociations('com_app', '#__app_items', 'com_app.item.item', $item->id, 'id', 'alias', 'catid');
	...
	protected function loadFormData()
	{
		...
		$data = $app->getUserState('com_app.edit.item.data', array());
		...
		if (empty($data))
		{
			...
			{
				$filters = (array) $app->getUserState('com_app.items.filter');
	...
	public function save($data)
	{
		...
		if (parent::save($data))
		{
			...
			if ($assoc)
			{
				...
				{
					JError::raiseNotice(403, JText::_('COM_APP_ERROR_ALL_LANGUAGE_ASSOCIATED'));
				}
				...
				try
				{
					...
					$query->where('context='.$db->quote('com_app.item.item'));
				...
				if (!$all_language AND count($associations))
				{
					...
					{
						$query->values($id.','.$db->quote('com_app.item.item') . ',' . $db->quote($key));
					}
	...
	public function featured($pks, $value = 0)
	{
		...
		try
		{
			...
			$query = $db->getQuery(true);
			$query->update($db->quoteName('#__app_items'));
	...
	* @param   array   $data	The data where the original name is stored
	...
	protected function preprocessForm(JForm $form, $data, $group = 'item')
	{
		...
		if ($assoc)
		{
			...
			$fieldset->addAttribute('description', 'COM_APP_ITEMS_ITEM_ASSOCIATIONS_FIELDSET_DESC');
			$add = false;
