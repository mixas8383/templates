 ...
 *
 */
class AppTableItems extends JTable
{
	...
	public function __construct(&$db)
	{
		...
		parent::__construct('#__app_items', 'id', $db);
		...
		JTableObserverTags::createObserver($this, array('typeAlias' => 'com_app.item'));

		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_app.item'));
	}
	...
	public function check()
	{
		...
		$table = JTable::getInstance('Items', 'AppTable');
		if ($table->load(array('alias'=>$this->alias)) AND ($table->id != $this->id OR $this->id==0))
		{
			$this->setError(JText::_('COM_APP_FIELD_ALIAS_DUPLICATED'));
			return false;
		}		
	...
	{
		$k = $this->_tbl_key;
		return 'com_app.item.'.(int) $this->$k;
	}
	...
	protected function _getAssetParentId(JTable $table = null, $id = null)
	{
		...
		if (($this->catid) AND !empty($this->catid))
		{
			...
			if ($asset->loadByName('com_app.category.' . (int) $this->catid))
			{
				return $asset->id;
			}
		}
		
		if ($asset->loadByName('com_app'))
		{
			return $asset->id;
		}
		else
