...
class JHTMLItemIcon
{
	...
	public static function create($params, $attribs = array(), $legacy = false)
	{
		...
		$url = 'index.php?option=com_example&task=item.add&layout=edit&return='.base64_encode($uri);
		
		if ($params->get('show_item_icons'))
		{
			if ($legacy)
			{
				$text = JHtml::_('image', 'com_example/new.png', JText::_('COM_EXAMPLE_ITEMS_CREATE_ITEM'), NULL, true);
			}
			else
			{
				$text = '<span class="icon-plus"></span>&#160;' . JText::_('COM_EXAMPLE_ITEMS_CREATE_ITEM') . '&#160;';
			}		
		}
		else
		{
		...
		$output = '<span class="hasTooltip tip" title="'.JHtml::tooltipText('COM_EXAMPLE_ITEMS_CREATE_ITEM').'">'.$button.'</span>';
		return $output;
	}
	/**
	...
	public static function edit($item, $params, $attribs = array(), $legacy = false)
	{
			...
			$item->checked_out != $user->get('id'))
		{
			...
			$button = JHtml::_('image','com_example/checked_out.png', NULL, NULL, true);
			$date = JHtml::_('date',$item->checked_out_time);
			$tooltip = JText::_('JLIB_HTML_CHECKED_OUT').' :: '.JText::sprintf('COM_EXAMPLE_CHECKED_OUT_BY', $check_out_user->name).' <br /> '.$date;
			return '<span class="hasTooltip" title="' . JHtml::tooltipText($tooltip. '', 0) . '">' . $button . '</span>';
		}

		$url	= 'index.php?option=com_example&task=item.edit&layout=edit&id='.$item->id.'&return='.base64_encode($uri);
		...
		$overlib .= $date;
		$overlib .= '&lt;br /&gt;';
		$overlib .= JText::sprintf('COM_EXAMPLE_CREATED_BY', htmlspecialchars($created_by, ENT_COMPAT, 'UTF-8'));

		if ($params->get('show_item_icons'))
		{
			if ($legacy)
			{
				$icon	= $item->state ? 'com_example/edit.png' : 'com_example/edit_unpublished.png';
				...
				{
					$icon = 'com_example/edit_unpublished.png';
				}
				$text	= JHtml::_('image','system/' .$icon, JText::_('COM_EXAMPLE_ITEMS_EDIT_ITEM'), NULL, true);
			}
			else
			{
				...
				$text = '<span class="hasTooltip icon-' . $icon . ' tip" title="' . JHtml::tooltipText(JText::_('COM_EXAMPLE_ITEMS_EDIT_ITEM'), $overlib, 0) . '"></span>&#160;' . JText::_('JGLOBAL_EDIT') . '&#160;';
			}			
		}
		else
		{
			$text = '<span class="hasTooltip tip" title="' . JHtml::tooltipText(JText::_('COM_EXAMPLE_ITEMS_EDIT_ITEM'), $overlib, 0) . '"></span>' . JText::_('JGLOBAL_EDIT') . '</span>';
		}
	...
	public static function delete($item, $params, $attribs = array(), $legacy = false)
	{
		...
		$url = 'index.php?option=com_example&task=item.delete&id='.$item->id.'&'.JSession::getFormToken().'=1'.'&return='.base64_encode($uri);
		...
		$overlib .= '&lt;br /&gt;';
		$overlib .= JText::sprintf('COM_EXAMPLE_CREATED_BY', htmlspecialchars($created_by, ENT_COMPAT, 'UTF-8'));
		$attribs['onclick'] = "if(confirm('".JText::_('COM_EXAMPLE_ITEMS_DELETE_ITEM_CONFIRM')."'))
		...
		{		
			if ($legacy)
			{
				$icon	= 'com_example/delete.png';
				$text = JHtml::_('image', 'system/' . $icon, JText::_('COM_EXAMPLE_ITEMS_DELETE_ITEM'), null, true);
			}
			else
			{
				$text = '<span class="hasTooltip icon-delete tip" title="' . JHtml::tooltipText(JText::_('COM_EXAMPLE_ITEMS_DELETE_ITEM'), $overlib, 0) . '"></span>&#160;' . JText::_('JACTION_DELETE') . '&#160;';
			}
		}
		else
		{
			$text = '<span class="hasTooltip tip" title="' . JHtml::tooltipText(JText::_('COM_EXAMPLE_ITEMS_DELETE_ITEM'), $overlib, 0) . '">' . JText::_('JACTION_DELETE') . '</span>';
		}		
	...
	public static function email($item, $params, $attribs = array(), $legacy = false)
	{
		...
		$link	= $base.JRoute::_(ExampleHelperRoute::getItemRoute($item->slug,
									$item->catid,
									$item->language,
									$layout,
		...
		if ($params->get('show_item_icons'))
		{
			if ($legacy)
			{
				$text = JHtml::_('image', 'com_example/emailButton.png', JText::_('JGLOBAL_EMAIL'), null, true);
			}
			else
			{
	...
	public static function print_popup($item, $params, $attribs = array(), $legacy = false)
	{
		...
		$link	= JRoute::_(ExampleHelperRoute::getItemRoute($item->slug,
									$item->catid,
									$item->language,
									$layout,
									$params->get('keep_item_itemid')) , false);
		...
		if ($params->get('show_item_icons'))
		{
			if ($legacy)
			{
				$text = JHtml::_('image', 'com_example/printButton.png', JText::_('JGLOBAL_PRINT'), null, true);
			}
			else
			{
