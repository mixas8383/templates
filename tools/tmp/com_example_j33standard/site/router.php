...
/**
 * Routing class for the Example component
 *
 */
class ExampleRouter extends JComponentRouterBase
{
	/**
	 * Build the route for the Example component
	 *
	...
	public function build(&$query)
	{
		...
		// Are we dealing with a example that is attached to a menu item?
	...
	public function parse(&$segments)
	{
		...
		{
			$categories = JCategories::getInstance('Example',$options)->get($id)->getChildren();
		}
		else
		{
			$categories = JCategories::getInstance('Example',$options);	
			...
			if ($found == 0)
			{
					...
					OR $view == 'items' OR $view == 'item')
				{
					if($advanced)
					{
						...
						$query->from($db->quoteName('#__example_items'));
