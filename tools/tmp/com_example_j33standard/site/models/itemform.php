...
require_once JPATH_ADMINISTRATOR.'/components/com_example/models/item.php';

class ExampleModelItemForm extends ExampleModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_example.edit.item';
	/**
	...
	*/
	...
	public function getItem($item_id = null)
	{
		...
		else
		{
			...
			if ($cat_id)
			{
				$item->params->set('access-change', $user->authorise('core.edit.state', 'com_example.category.'.$cat_id));
				$item->catid = $cat_id;
			}
			else 
			{
				$item->params->set('access-change', $user->authorise('core.edit.state', 'com_example'));
			}
		}
	...
	public function save($data)
	{
		...
		else
		{
			...
			$actions = JAccess::getActions('com_example');
			$action_array = array();
		...
		$result = $dispatcher->trigger('onItemBeforeSave', array('com_example.item', &$table, $is_new));
		...
		$dispatcher->trigger('onItemAfterSave', array('com_example.item', $table, $is_new));
