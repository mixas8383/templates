...
class ExampleControllerItem extends JControllerForm
{
	...
	protected function allowAdd($data = array())
	{
		...
		if ($category_id)
		{
			...
			$allow	= $user->authorise('core.create', 'com_example.category.'.$category_id);
		}
		if ($allow === null)
		{
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		$user		= JFactory::getUser();
		$asset		= 'com_example.item.'.$record_id;
	...
	public function save($key = 'id', $url_var = null)
	{
		...
		if ($record_id == 0) 
		{
			$this->setMessage(JText::_('COM_EXAMPLE_ITEMS_SUBMIT_SAVE_SUCCESS'));
		} 
		else 
		{
			$this->setMessage(JText::_('COM_EXAMPLE_ITEMS_SAVE_SUCCESS'));
		}
	...
	public function delete()
	{
		...
		if ($id > 0) 
		{
			...
			{
				$this->setMessage(JText::_('COM_EXAMPLE_ITEMS_DELETE_SUCCESS'));
				
			}
			else
			{
				$this->setMessage(JText::_('COM_EXAMPLE_ITEMS_DELETE_FAILED'));
			}
		}
	...
	public function vote()
	{
		...
		if ( $user_rating > -1 )
		{
			...
			{
				$this->setRedirect($url, JText::_('COM_EXAMPLE_ITEMS_VOTE_SUCCESS'));
			}
			else
			{
				$this->setRedirect($url, JText::_('COM_EXAMPLE_ITEMS_VOTE_FAILURE'));
			}
		}
	}
	/**
