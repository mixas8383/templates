...
$this->document->addStyleSheet(JUri::root().'media/com_example/css/site_items.css');

if ($lang->isRTL())
{
	$this->document->addStyleSheet(JUri::root().'media/com_example/css/site_example-rtl.css');
	$this->document->addStyleSheet(JUri::root().'media/com_example/css/site_items-rtl.css');
}
	...
	<form action="<?php echo JFilterOutput::ampReplace(JUri::getInstance()->toString()); ?>" method="post" name="adminForm" id="adminForm">
		<?php if (($this->params->get('show_item_filter_field') != '' AND $this->params->get('show_item_filter_field') != 'hide')) :?>
			<div class="filter-search">
				<?php if ($this->params->get('show_item_filter_field') != '' AND $this->params->get('show_item_filter_field') != 'hide') :?>
					<input type="text" name="filter_search" id="filter_search" value="<?php echo $this->escape($this->state->get('filter.search')); ?>" onchange="document.adminForm.submit();" title="<?php echo JText::_('COM_EXAMPLE_FILTER_SEARCH_DESC'); ?>" placeholder="<?php echo JText::_('COM_EXAMPLE_'.$this->params->get('show_item_filter_field').'_FILTER_LABEL'); ?>" />
				<?php endif; ?>	
				<select name="filter_category_id" onchange="this.form.submit()">
					<option value=""><?php echo JText::_('JOPTION_SELECT_CATEGORY');?></option>
					<?php echo JHtml::_('select.options', JHtml::_('category.options', 'com_example'), 'value', 'text', $this->state->get('filter.category_id'));?>
				</select>
			</div>
		<?php endif; ?>
				...
				<tr>
					...
					<?php if ($show_actions) : ?>
						<th width="12%" class="list-actions">
							<?php echo JText::_('COM_EXAMPLE_HEADING_ACTIONS'); ?>						
						</th> 					
					<?php endif; ?>
				</tr>
			</thead>
							...
							<?php 
								...
								if (!empty($item->created_by )) :
									...
									if ($this->params->get('show_item_headings')) :
										...
										echo JText::sprintf('COM_EXAMPLE_CREATED_BY', $created_by);
									endif;
								else:
									echo $empty;
								endif;								
