...
/**
 * HTML Item View class for the Example component
 *
 */
class ExampleViewItem extends JViewLegacy
{
	...
	public function display($tpl = null)
	{
		...
		$item->readmore_link = JRoute::_(ExampleHelperRoute::getItemRoute($item->slug, 
										$item->catid, 
										$item->language,
										$this->getLayout(),									
		...
		if ($item->params->get('access-view') != true)
		{
			...
			if (!$item->params->get('show_item_noauth'))
			{
				...
				if( $user->get('guest'))
				{
					...
					$app->redirect('index.php?option=com_users&view=login&return=' . base64_encode($uri), JText::_('COM_EXAMPLE_ITEMS_ERROR_LOGIN_TO_VIEW_ITEM'));
		...
		$results = $dispatcher->trigger('onItemPrepare', array ('com_example.item', &$item, &$this->params, $offset));

		$item->event = new stdClass;
		$results = $dispatcher->trigger('onItemAfterName', array('com_example.item', &$item, &$this->params, $offset));
		$item->event->afterDisplayItemName = JString::trim(implode("\n", $results));
		
		$results = $dispatcher->trigger('onItemBeforeDisplay', array('com_example.item', &$item, &$this->params, $offset));
		$item->event->beforeDisplayItem = JString::trim(implode("\n", $results));

		$results = $dispatcher->trigger('onItemAfterDisplay', array('com_example.item', &$item, &$this->params, $offset));
	...
	protected function prepareDocument()
	{
		...
		if ($menu AND ($menu->query['option'] != 'com_example' OR $menu->query['view'] != 'item' OR $id != $this->item->id))
		{
			...
			{			
				...
				$category = JCategories::getInstance('Example', $options)->get($this->item->catid);
				while ($category AND ($menu->query['option'] != 'com_example' OR $menu->query['view'] == 'item' OR $id != $category->id) AND $category->id > 1)
				{
					$path[] = array('title' => $category->title, 'link' => ExampleHelperRoute::getCategoryRoute($category->id, $this->params->get('keep_item_itemid')));
		...
		if (!empty($this->item->page_title))
		{
			...
			$this->document->setTitle($this->item->page_title . ' - ' . JText::sprintf('COM_EXAMPLE_PAGEBREAK_PAGE_NUM', $this->state->get('list.offset') + 1));
		}
