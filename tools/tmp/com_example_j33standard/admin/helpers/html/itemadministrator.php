...
JLoader::register('ContentHelper', JPATH_ADMINISTRATOR . '/components/com_example/helpers/example.php');
/**
...
abstract class JHtmlItemAdministrator
{
	...
	public static function association($item_id)
	{
		...
		// Get the associations
		if ($associations = JLanguageAssociations::getAssociations('com_example', '#__example_items', 'com_example.item.item',  $item_id, 'id', 'alias', 'catid'))
		{		
			...
			if ($items)
			{
				foreach ($items as &$item)
				{
					...
					$url = JRoute::_('index.php?option=com_example&task=item.edit&id=' . (int) $item->id);
					$tooltip_parts = array(
	...
	public static function featured($value = 0, $i, $can_change = true)
	{
		...
		$states	= array(
			0	=> array('unfeatured', 'items.featured', 'COM_EXAMPLE_ITEMS_UNFEATURED', 'COM_EXAMPLE_ITEMS_TOGGLE_TO_FEATURE'),
			1	=> array('featured', 'items.unfeatured', 'JFEATURED', 'COM_EXAMPLE_ITEMS_TOGGLE_TO_UNFEATURE'),
		);
