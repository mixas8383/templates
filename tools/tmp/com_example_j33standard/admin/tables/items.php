 ...
 *
 */
class ExampleTableItems extends JTable
{
	...
	public function __construct(&$db)
	{
		...
		JTableObserverTags::createObserver($this, array('typeAlias' => 'com_example.item'));

		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_example.item'));
	}
	...
	public function check()
	{
		...
		$table = JTable::getInstance('Items', 'ExampleTable');
		if ($table->load(array('alias'=>$this->alias)) AND ($table->id != $this->id OR $this->id==0))
		{
			$this->setError(JText::_('COM_EXAMPLE_FIELD_ALIAS_DUPLICATED'));
			return false;
		}		
	...
	{
		$k = $this->_tbl_key;
		return 'com_example.item.'.(int) $this->$k;
	}
	...
	protected function _getAssetParentId(JTable $table = null, $id = null)
	{
		...
		if (($this->catid) AND !empty($this->catid))
		{
			...
			if ($asset->loadByName('com_example.category.' . (int) $this->catid))
			{
				return $asset->id;
			}
		}
		
		if ($asset->loadByName('com_example'))
		{
			return $asset->id;
		}
		else
