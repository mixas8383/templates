...
class JFormFieldModal_Items extends JFormField
{
	...
	protected function getInput()
	{
		...
		if ($allow_clear AND !$script_clear)
		{
			...
			$script[] = '		document.getElementById(id + "_name").value = "'.htmlspecialchars(JText::_('COM_EXAMPLE_ITEMS_SELECT_ITEM_LABEL', true), ENT_COMPAT, 'UTF-8').'";';
		...
		$html[] = '<a class="modal btn hasTooltip" title="'.JHtml::tooltipText('COM_EXAMPLE_ITEMS_SELECT_BUTTON_DESC').'"  href="'.$link.'&amp;'.JSession::getFormToken().'=1" rel="{handler: \'iframe\', size: {x: 800, y: 450}}"><i class="icon-file"></i> '.JText::_('JSELECT').'</a>';

		// Edit item button
		if ($allow_edit)
		{
			$html[] = '<a class="btn hasTooltip'.($value ? '' : ' hidden').'" href="index.php?option=com_example&layout=modal&tmpl=component&task=item.edit&id=' . $value. '" target="_blank" title="'.JHtml::tooltipText('COM_EXAMPLE_ITEMS_EDIT_DESC').'" ><span class="icon-edit"></span> ' . JText::_('JACTION_EDIT') . '</a>';
		}
