...
class ExampleModelItem extends JModelAdmin
{
	...
	/**
	 * @var      string	The type alias for this object (for example, 'com_example.item')
	 */
	...
	protected function canDelete($record)
	{
		...
		{
			return $user->authorise('core.delete', 'com_example');
		}							
	}
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_example.item');
			}	
		}
		...
		if ($assoc)
		{
			...
			if ($item->id != null)
			{
				$associations = JLanguageAssociations::getAssociations('com_example', '#__example_items', 'com_example.item.item', $item->id, 'id', 'alias', 'catid');
	...
	{
		$form = $this->loadForm('com_example.edit.item', 'item', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = $app->getUserState('com_example.edit.item.data', array());
		...
		if (empty($data))
		{
			...
			{
				$filters = (array) $app->getUserState('com_example.items.filter');
	...
	{
		// Include the example plugins for the onSave events.
		...
		if (parent::save($data))
		{
			...
			if ($assoc)
			{
				...
				{
					JError::raiseNotice(403, JText::_('COM_EXAMPLE_ERROR_ALL_LANGUAGE_ASSOCIATED'));
				}
				...
				try
				{
					...
					$query->where('context='.$db->quote('com_example.item.item'));
				...
				if (!$all_language AND count($associations))
				{
					...
					{
						$query->values($id.','.$db->quote('com_example.item.item') . ',' . $db->quote($key));
					}
	...
	{
		// Include the example plugins for the delete events.
		JPluginHelper::importPlugin('example');	
	...
	public function featured($pks, $value = 0)
	{
		...
		if (empty($pks))
		{
			$this->setError(JText::_('COM_EXAMPLE_ITEMS_NO_ITEM_SELECTED'));
			return false;
		}
		...
		try
		{
			...
			$query = $db->getQuery(true);
			$query->update($db->quoteName('#__example_items'));
	...
	/**
	 * Custom clean the cache of com_example and example modules
	 *
	 */
	protected function cleanCache($group = null, $client_id = 0)
	{
		parent::cleanCache('com_example');
		parent::cleanCache('mod_example');
		parent::cleanCache('mod_example_items');
	...
	* @param   array   $data	The data where the original name is stored
	...
	protected function preprocessForm(JForm $form, $data, $group = 'item')
	{
		...
		if ($assoc)
		{
			...
			$fieldset->addAttribute('description', 'COM_EXAMPLE_ITEMS_ITEM_ASSOCIATIONS_FIELDSET_DESC');
			$add = false;
