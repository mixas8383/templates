...
$can_order	= $user->authorise('core.edit.state', 'com_example');

$save_order	= ($list_order=='ordering' OR $list_order=='a.ordering');

if ($save_order)
{
	$save_ordering_url = 'index.php?option=com_example&task=items.saveOrderAjax&tmpl=component';
		...
		<table class="table table-striped" id="item-list">
			<thead>
				<tr>
					...
					<th width="10%" class="nowrap center hidden-phone">
						<?php echo JHtml::_('grid.sort',  'COM_EXAMPLE_HEADING_CREATED_BY', 'a.created_by', $list_dirn, $list_order); ?>
					</th>
					<th width="10%" class="nowrap center hidden-phone">
						<?php echo JHtml::_('grid.sort', 'COM_EXAMPLE_HEADING_CREATED', 'a.created', $list_dirn, $list_order); ?>
					</th>		
					<?php if ($assoc) : ?>
						<th width="5%" class="nowrap center hidden-phone">
							<?php echo JHtml::_('grid.sort', 'COM_EXAMPLE_HEADING_ASSOCIATION', 'association', $list_dirn, $list_order); ?>
						</th>
					<?php endif;?>				
			...
			<?php foreach ($this->items as $i => $item) :
				...
				$can_edit	= $user->authorise('core.edit',	'com_example.item.'.$item->id);
		
				$can_edit_own	= $user->authorise('core.edit.own',		'com_example.item.'.$item->id) 
								AND $item->created_by == $user_id
								;
				$can_change	= $user->authorise('core.edit.state',	'com_example.item.'.$item->id) 
								AND $can_checkin
								;
				if ($item->language == '*'):
					...
					<td class="nowrap has-context">
						<div class="pull-left">
							...
							<?php if ($can_edit OR $can_edit_own) : ?>
								<a href="<?php echo JRoute::_('index.php?option=com_example&task=item.edit&id='.(int) $item->id); ?>">
