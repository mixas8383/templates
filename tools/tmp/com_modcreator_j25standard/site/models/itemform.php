...
class ModcreatorModelItemForm extends JModelForm
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_modcreator.edit.item';
	...
	*/
	...
	public function getItem($item_id = null)
	{
		...
		{
			// New item.
			$item->params->set('access-change', $user->authorise('core.edit.state', 'com_modcreator'));
		}
		return $item;
	}
	...
	protected function _setAccessFilters(&$form, $data)
	{
		...
		if (!$user->authorise('core.edit.state', 'com_modcreator'))
		{
	...
	public function save($data)
	{
		...
		else
		{
			...
			$actions = JAccess::getActions('com_modcreator');
			$action_array = array();
		...
		$result = $dispatcher->trigger('onItemBeforeSave', array('com_modcreator.item', &$table, $is_new));
		...
		// Clean the cache.
		$cache = JFactory::getCache('com_modcreator');
		$cache->clean();

		$dispatcher->trigger('onItemAfterSave', array('com_modcreator.item', &$table, $is_new));
