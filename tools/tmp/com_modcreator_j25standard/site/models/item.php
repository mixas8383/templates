...
/**
 * ModCreator Component Item Model
 *
 */
class ModcreatorModelItem extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_modcreator.item';
	...
	protected function canDelete($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.delete', 'com_modcreator.item.'.(int) $record->id)
					  OR ($user->authorise('core.delete', 'com_modcreator.item.'.(int) $record->id)
	...
	protected function canEditState($record)
	{
		...
		else
		{
			...
			return $user->authorise('core.edit.state', 'com_modcreator');
		}
	}
	/**
	...
	*/
	...
	public function &getItem($pk = null)
	{
		...
		$global_params = JComponentHelper::getParams('com_modcreator', true);
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				$query->from($db->quoteName('#__modcreator_items').' AS a');
				// Join on user table.
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_MODCREATOR_ITEMS_ERROR_ITEM_NOT_FOUND'));
				}
				...
				{
					$user_id	= $user->get('id');
					$asset		= 'com_modcreator.item.'.$item->id;
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onItemChangeState', array('com_modcreator.item', $pks, $value));
	...
	*/
	...
	public function delete(&$pks)
	{
		...
		// Include the modcreator plugins for the on delete events.
		JPluginHelper::importPlugin('modcreator');
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				...
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onItemBeforeDelete', array('com_modcreator.item', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onItemAfterDelete', array('com_modcreator.item', &$table));
	...
	public function hit($pk = 0)
	{
		...
		if ($hit_count)
		{
			...
			$db->setQuery(
				'UPDATE '.$db->quoteName('#__modcreator_items') .
