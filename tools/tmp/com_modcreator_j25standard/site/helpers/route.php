...
/**
 * ModCreator Component Route Helper
 *
 */
abstract class ModcreatorHelperRoute
{
	...
	public static function getItemRoute($id, $language = '*', $layout = 'default', $keep_item_id = false)
	{
		...
		{
			//Create the link
			$link = 'index.php?option=com_modcreator&view=item&id='. $id;
		}
		else
		{
			//Create the link with a layout
			$link = 'index.php?option=com_modcreator&view=item&layout='.$layout.'&id='. $id;
		}
	...
	protected static function _findItem($needles = null, $keep_item_id = false, $layout = 'default')
	{
		...
		if ($active AND $active->component == 'com_modcreator' AND ($language == '*' OR in_array($active->language, array('*', $language)) OR !JLanguageMultilang::isEnabled()))
		{
			return $active->id;
		}
