 ...
 *
 */
class ModcreatorViewItem extends JView
{
	protected $item;
	protected $params;
	protected $print;
	...
	function display($tpl = null)
	{
		...
		$item->readmore_link = JRoute::_(ModcreatorHelperRoute::getItemRoute($item->slug, 
										$item->catid, 
										$item->language,
										$this->getLayout(),									
		...
		if ($item->params->get('access-view') != true)
		{
			...
			if (!$this->params->get('show_item_noauth'))
			{
				...
				if( $user->get('guest'))
				{
					// Redirect to login
					$uri = JFactory::getURI();
					$app->redirect('index.php?option=com_users&view=login&return=' . base64_encode(urlencode($uri)), JText::_('COM_MODCREATOR_ITEMS_ERROR_LOGIN_TO_VIEW_ITEM'));
		...
		//
		// Process the modcreator plugins.
		//
		JPluginHelper::importPlugin('modcreator');
		$results = $dispatcher->trigger('onItemPrepare', array ('com_modcreator.item', &$item, &$this->params, $offset));

		$item->event = new stdClass();
		$results = $dispatcher->trigger('onItemAfterName', array('com_modcreator.item', &$item, &$this->params, $offset));
		$item->event->afterDisplayItemName = JString::trim(implode("\n", $results));
		
		$results = $dispatcher->trigger('onItemBeforeDisplay', array('com_modcreator.item', &$item, &$this->params, $offset));
		$item->event->beforeDisplayItem = JString::trim(implode("\n", $results));

		$results = $dispatcher->trigger('onItemAfterDisplay', array('com_modcreator.item', &$item, &$this->params, $offset));
	...
	protected function _prepareDocument()
	{
		...
		if ($menu AND ($menu->query['option'] != 'com_modcreator' OR $menu->query['view'] != 'item' OR $id != $this->item->id))
		{
			...
			{			
				...
				$options['table'] = '#__modcreator_items';					
				$category = JCategories::getInstance('Modcreator', $options)->get($this->item->catid);
				while ($category AND ($menu->query['option'] != 'com_modcreator' OR $menu->query['view'] == 'item' OR $id != $category->id) AND $category->id > 1)
				{
					$path[] = array('title' => $category->title, 'link' => ModcreatorHelperRoute::getCategoryRoute($category->id, $this->params->get('keep_item_itemid')));
		...
		if (!empty($this->item->page_title))
		{
			...
			$this->document->setTitle($this->item->page_title . ' - ' . JText::sprintf('COM_MODCREATOR_PAGEBREAK_PAGE_NUM', $this->state->get('list.offset') + 1));
		}
