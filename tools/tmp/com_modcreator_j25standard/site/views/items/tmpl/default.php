...
$can_create	= $user->authorise('core.create', 'com_modcreator');
...
<div class="modcreator items-list<?php echo $this->params->get('pageclass_sfx');?>">
	...
	<?php if (empty($this->items)) : ?>
		...
		<p><?php echo JText::_('COM_MODCREATOR_ITEMS_NO_ITEMS'); ?></p>
		<?php endif; ?>
				...
				<fieldset class="filters">
					...
					<div class="ca-filter-search">
						<?php if ($this->params->get('item_filter_field') != '' AND $this->params->get('item_filter_field') != 'hide') :?>
							<label class="filter-search-lbl" for="filter_search"><?php echo JText::_('COM_MODCREATOR_'.$this->params->get('item_filter_field').'_FILTER_LABEL').'&#160;'; ?></label>
							<input type="text" name="filter_search" id="filter_search" value="<?php echo $this->escape($this->state->get('filter.search')); ?>" class="inputbox" onchange="document.adminForm.submit();" title="<?php echo JText::_('COM_MODCREATOR_FILTER_SEARCH_DESC'); ?>" />
							<button type="submit">
			...
			<thead>
				<tr>
					...
					<?php if ($date = $this->params->get('list_show_item_date')) : ?>
						...
						<?php echo JHTML::_('grid.sort', 'COM_MODCREATOR_FIELD_'.JString::strtoupper($date).'_LABEL', 'a.'.$date, $list_dirn, $list_order); ?>
						</th>
					<?php endif; ?>

					<?php if ($this->params->get('list_show_item_creator',0)) : ?>
						<th class="list-creator" id="tableOrderingcreated_by">
						<?php echo JHTML::_('grid.sort', 'COM_MODCREATOR_HEADING_CREATED_BY', 'created_by_name', $list_dirn, $list_order); ?>
						</th>
					<?php endif; ?>
					<?php if ($this->params->get('list_show_item_hits',0)) : ?>
						<th class="list-hits" id="tableOrderinghits">
						<?php echo JHTML::_('grid.sort', 'COM_MODCREATOR_HEADING_HITS', 'a.hits', $list_dirn, $list_order); ?>
						</th>
					<?php endif; ?>
					<?php if ($this->params->get('list_show_item_ordering',0)) : ?>
						<th width="10%">
							<?php echo JHtml::_('grid.sort',  'COM_MODCREATOR_HEADING_ORDERING', 'a.ordering', $list_dirn, $list_order); ?>
						</th>
					<?php endif; ?>	
					<?php if ($show_actions) : ?>
			...
			<tbody>
				...
				?>			
					...
					<?php if (in_array($item->access, $user->getAuthorisedViewLevels()) OR $params->get('show_item_noauth')) : ?>
						<td class="list-name">
							<a href="<?php echo JRoute::_(ModcreatorHelperRoute::getItemRoute($item->slug, 
																									$item->catid, 
																									$item->language,
																									$layout,									
