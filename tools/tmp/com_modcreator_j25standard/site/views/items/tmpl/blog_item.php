...
<h2>
		<?php if ($params->get('link_item_names') ) : ?>
			<a href="<?php echo JRoute::_(ModcreatorHelperRoute::getItemRoute($this->item->slug, 
											$this->item->catid, 
											$this->item->language,
											$layout,									
...
<?php 
	...
	<dl class="item-info">
 <dt class="item-info-term"><?php  echo JText::_('COM_MODCREATOR_ITEMS_INFO'); ?></dt>
<?php endif; ?>
...
<?php if ($params->get('show_item_creator')) : ?>
	<dd class="createdby"> 
		...
		<?php if (!empty($this->item->created_by ) AND  $this->params->get('link_item_creator') == 1):?>
			<?php echo JText::sprintf('COM_MODCREATOR_CREATED_BY',JHTML::_('link',
...
<?php else : ?>
	...
	<?php if ($params->get('show_item_readmore')) :
			if ($params->get('access-view')) :
				$link = JRoute::_(ModcreatorHelperRoute::getItemRoute($this->item->slug, 
																							$this->item->catid, 
																							$this->item->language,
																							$layout,									
																							$params->get('keep_item_itemid')));
				...
				if ($params->get('show_item_noauth') AND $user->get('guest')) :
					...
					$return_url = JRoute::_(ModcreatorHelperRoute::getItemRoute($this->item->slug, 
																								$this->item->catid, 
																								$this->item->language,									
																								$layout,									
																								$params->get('keep_item_itemid')));
				...
				if ($item_params->get('item_alternative_readmore') == null) :
						if ($params->get('access-view')) :
							if ($params->get('show_item_readmore_name') == 0) :
								echo JText::sprintf('COM_MODCREATOR_READ_MORE');
							else :
								echo JText::_('COM_MODCREATOR_READMORE_NAME');
