...
class ModcreatorViewItems extends JView
{
	...
	function display($tpl = null)
	{
		...
		foreach ($items as $i => &$item)
		{
			...
			$results = $dispatcher->trigger('onItemPrepare', array ('com_modcreator.item', &$item, &$item->params,$i));

			$item->event = new stdClass();

			$results = $dispatcher->trigger('onItemAfterName', array('com_modcreator.item', &$item, &$item->params, $i));
			$item->event->afterDisplayItemName = JString::trim(implode("\n", $results));

			$results = $dispatcher->trigger('onItemBeforeDisplay', array('com_modcreator.item', &$item, &$item->params, $i));
			$item->event->beforeDisplayItem = JString::trim(implode("\n", $results));

			$results = $dispatcher->trigger('onItemAfterDisplay', array('com_modcreator.item', &$item, &$item->params,$i));
		...
		{			
			...
			if ($order == 0 AND $this->columns > 1)
			{
				...
				$this->intro_items = ModcreatorHelperQuery::orderDownColumns($this->intro_items, $this->columns);
			}
	...
	protected function _prepareDocument()
	{
		...
		{
			$this->params->def('page_heading', JText::_('COM_MODCREATOR_ITEMS'));
		}
