 ...
 * @subpackage		com_modcreator.itemnavigation
 * @copyright		
...
/**
 * Modcreator navigation plugin class.
 *
 */
class plgModcreatorItemnavigation extends JPlugin
{
	/**
	 * Constructor
	 *
	...
	public function onItemBeforeDisplay ($context, &$row, &$params, $page=0)
	{
		...
		if ($params->get('show_item_navigation') AND 
			($context == 'com_modcreator.item') AND 
			($view == 'item')) 
		{
			...
			if ($row->prev) 
			{
				$row->prev = JRoute::_(ModcreatorHelperRoute::getItemRoute($row->prev->slug,$row->prev->catslug, $row->prev->language, $layout, $keep_item_id));
			} 
			else 
			{
				$row->prev = '';
			...
			if ($row->next) 
			{
				$row->next = JRoute::_(ModcreatorHelperRoute::getItemRoute($row->next->slug,$row->next->catslug, $row->next->language, $layout, $keep_item_id));
