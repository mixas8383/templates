 ...
 * @subpackage		com_modcreator.mod_modcreator
 * @copyright		
...
abstract class modModcreatorHelper
{
	public static function getList(&$params)
	{
		...
		JModel::addIncludePath(JPATH_SITE.'/components/com_modcreator/models', 'ModcreatorModel'.$component_object_names[1]);

		// Get an instance of the model
		$model = JModel::getInstance($component_object_names[1], 'ModcreatorModel', array('ignore_request' => true));
		...
		// Access filter
		$access = !JComponentHelper::getParams('com_modcreator')->get('show_'.JString::strtolower($component_object_names[0]).'_noauth');
		...
		foreach ($items as &$item)
		{
			...
			if (!isset($item->access) OR $access OR in_array($item->access, $authorised))
			{
				...
				{			
					...
					{
						$item->link = JRoute::_(ModcreatorHelperRoute::$route_function($item->slug, $item->language));
					}
					else
					{
						$item->link = JRoute::_(ModcreatorHelperRoute::$route_function($item->slug));
					}
				}
				else
				{
					$item->link = JRoute::_('index.php?option=com_modcreator&view='.JString::strtolower($component_object_names[0]).$item_id_str.'&id='.$item->id);
				}
			}
			else
			{
