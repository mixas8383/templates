...
JModel::addIncludePath(JPATH_SITE.'/components/com_modcreator/models', 'ModcreatorModelItems');
...
abstract class modItemsHelper
{
	public static function getList(&$params)
	{
		...
		$model = JModel::getInstance('Items', 'ModcreatorModel', array('ignore_request' => true));
		...
		// Access filter
		$access = !JComponentHelper::getParams('com_modcreator')->get('show_item_noauth');
		...
		foreach ($items as &$item)
		{
			...
			if ($access OR in_array($item->access, $authorised))
			{
				if ($item_id_str == '')
				{
					...
					$item->link = JRoute::_(ModcreatorHelperRoute::getItemRoute($item->slug, $item->language, $layout));
				}
				else
				{
					$item->link = JRoute::_('index.php?option=com_modcreator&view=item'.$layout_str.$item_id_str.'&id='.$item->id);
				}					
			}
			else
			{
