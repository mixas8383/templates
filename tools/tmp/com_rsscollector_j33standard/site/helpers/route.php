...
/**
 * Rss_collector Component Route Helper
 *
 */
abstract class RssCollectorHelperRoute
{
	...
	/**
	 * @param	integer	The route of the Item
	 */
	public static function getItemRoute($id, $language = '*', $layout = 'default', $keep_item_id = false)
	{
		$needles = array(
			'item'  => array((int) $id)
		);
		...
		{
			//Create the link
			$link = 'index.php?option=com_rsscollector&view=item&id='. $id;
		}
		else
		{
			//Create the link with a layout
			$link = 'index.php?option=com_rsscollector&view=item&layout='.$layout.'&id='. $id;
		}
	...
	protected static function findItem($needles = null, $keep_item_id = false, $layout = 'default')
	{
		...
		if ($active AND $active->component == 'com_rsscollector' AND ($language == '*' OR in_array($active->language, array('*', $language)) OR !JLanguageMultilang::isEnabled()))
		{
			return $active->id;
		}
