...
require_once JPATH_ADMINISTRATOR.'/components/com_rsscollector/models/rssitems.php';

class RssCollectorModelRssItemsForm extends RssCollectorModelRssItems
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_rsscollector.edit.rssitems';
	/**
	...
	*/
	 ...
	 *
	 * @param	integer	$item_id	The id of the rss_items.
	 *
	 * @return	mixed	Rss_items item data object on success, false on failure.
	 */
	public function getItem($item_id = null)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('rssitems.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_rsscollector.edit.rssitems.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('rssitems.id');
		$is_new		= true;
		...
		// Reorder the rss_itemses so the new rss_items is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onRssItemsBeforeSave', array('com_rsscollector.rssitems', &$table, $is_new));
		...
		$dispatcher->trigger('onRssItemsAfterSave', array('com_rsscollector.rssitems', $table, $is_new));

		$this->setState('rssitems.id', $table->id);
