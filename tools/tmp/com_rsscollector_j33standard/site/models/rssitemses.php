...
/**
 * This models supports retrieving lists of rss_itemses.
 *
 */
class RssCollectorModelRssItemses extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_rsscollector.rssitemses';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single rss_items has been specified either as a parameter or in the url Request
		$pk = $params->get('rss_items_id', '') == '' ? $app->input->getInt('id', '') : $params->get('rss_items_id');
		$this->setState('filter.rss_items_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('rssitems_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('rssitems_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('rssitems_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('rssitems_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('rssitems_initial_direction','ASC');
		}
		...
		// process show_rssitems_noauth parameter
		if (!$params->get('show_rssitems_noauth'))
		{
		...
		if ($params->get('filter_rssitems_featured') <> "")
		{
			$this->setState('filter.featured', $params->get('filter_rssitems_featured'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.rss_items_id'));
		$id .= ':'.$this->getState('filter.rss_items_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of rssitemses subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		// Filter by a single or group of rssitemses.
		$rss_items_id = $this->getState('filter.rss_items_id');
		if ($rss_items_id != '')
		{
			if (is_numeric($rss_items_id))
			{
				$type = $this->getState('filter.rss_items_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $rss_items_id);
			}
			else
			{
				if (is_array($rss_items_id))
				{
					JArrayHelper::toInteger($rss_items_id);
					$rss_items_id = implode(',', $rss_items_id);
					$type = $this->getState('filter.rss_items_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$rss_items_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_rssitems_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			...
			switch ($params->get('show_rssitems_filter_field'))
			{
				case 'hits':
	...
	/**
	 * Method to get a list of rss_itemses.
	 *
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				else
				{
					...
					// get display date
					switch ($item->params->get('list_show_rssitems_date'))
					{
						case 'modified':
				...
				if ($access) 
				{
					// If the access filter has been set, we already have only the rssitemses this user can view.
