...
/**
 * Rss_collector Component Rss_collector Model
 *
 */
class RssCollectorModelRssCollector extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_rsscollector.rsscollector';
	...
	{
		if (empty($config['rsscollector_filter_fields']))
		{
			$config['rsscollector_filter_fields'] = array(
				'id', 'a.id',
				'name', 'a.name',
				'alias', 'a.alias',
				'metadata','a.metadata',
	...
	protected function canDelete($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.delete', 'com_rsscollector.rsscollector.'.(int) $record->id)
					  OR ($user->authorise('core.delete', 'com_rsscollector.rsscollector.'.(int) $record->id)
	...
	*/
	 ...
	 *
	 * @param	integer	$pk	The id of the rss_collector.
	 *
	...
	public function getItem($pk = null)
	{
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				$can_publish = $user->authorise('core.edit.state', 'com_rsscollector.rsscollector.'.$pk);
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_RSSCOLLECTOR_RSSCOLLECTORS_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onRssCollectorChangeState', array('com_rsscollector.rsscollector', $pks, $value));
	...
	*/
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onRssCollectorBeforeDelete', array('com_rsscollector.rsscollector', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onRssCollectorAfterDelete', array('com_rsscollector.rsscollector', &$table));
				}
				else
				{
					// Prune items that you can't change.
	...
	/**
	 * Increment the hit counter for the rss_collector.
	 *
	 * @pk		int		Optional primary key of the rss_collector to increment.
	 *
