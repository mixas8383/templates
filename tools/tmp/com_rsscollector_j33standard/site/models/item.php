...
/**
 * Rss_collector Component Item Model
 *
 */
class RssCollectorModelItem extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_rsscollector.item';
	...
	protected function canDelete($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.delete', 'com_rsscollector.item.'.(int) $record->id)
					  OR ($user->authorise('core.delete', 'com_rsscollector.item.'.(int) $record->id)
	...
	*/
	...
	public function getItem($pk = null)
	{
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				$query->from($db->quoteName('#__rsscollector_items').' AS a');
				// Join on user table.
				...
				$can_publish = $user->authorise('core.edit.state', 'com_rsscollector.item.'.$pk);
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_RSSCOLLECTOR_ITEMS_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onItemChangeState', array('com_rsscollector.item', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onItemBeforeDelete', array('com_rsscollector.item', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onItemAfterDelete', array('com_rsscollector.item', &$table));
				}
				else
				{
					// Prune items that you can't change.
