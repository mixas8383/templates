...
require_once JPATH_ADMINISTRATOR.'/components/com_rsscollector/models/rsscollector.php';

class RssCollectorModelRssCollectorForm extends RssCollectorModelRssCollector
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_rsscollector.edit.rsscollector';
	/**
	...
	protected function populateState()
	{
		...
		$this->setState('rsscollector.id', $pk);
	...
	*/
	...
	/**
	 * Method to get rss_collector data.
	 *
	 * @param	integer	$item_id	The id of the rss_collector.
	 *
	 * @return	mixed	Rss_collector item data object on success, false on failure.
	 */
	public function getItem($item_id = null)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('rsscollector.id');
		...
		{
			// New item.
			$item->params->set('access-change', $user->authorise('core.edit.state', 'com_rsscollector'));
		}
	...
	protected function setAccessFilters(&$form, $data)
	{
		...
		if (!$user->authorise('core.edit.state', 'com_rsscollector'))
		{
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_rsscollector.edit.rsscollector.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('rsscollector.id');
		$is_new		= true;
		...
		else
		{
			...
			$actions = JAccess::getActions('com_rsscollector');
			$action_array = array();
		...
		// Reorder the rss_collectors so the new rss_collector is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onRssCollectorBeforeSave', array('com_rsscollector.rsscollector', &$table, $is_new));
		...
		// Clean the cache.
		$cache = JFactory::getCache('com_rsscollector');
		$cache->clean();

		$dispatcher->trigger('onRssCollectorAfterSave', array('com_rsscollector.rsscollector', $table, $is_new));

		$this->setState('rsscollector.id', $table->id);
	...
	{
		$pk	= (!empty($pk)) ? $pk : (int) $this->getState('rsscollector.id');
