...
class RssCollectorControllerRssCollector extends JControllerForm
{
	 ...
	 */
	protected $view_item = 'rsscollectorform';
	/**
	 * @var    string	$view_list	The URL view list variable.
	 */
	protected $view_list = 'rsscollectors';
	/**
	 * 
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		$user		= JFactory::getUser();
		$asset		= 'com_rsscollector.rsscollector.'.$record_id;
		...
		if ($user->authorise('core.edit.own', $asset))
		{
			...
			if (empty($owner_id) AND $record_id)
			{
				...
				$record		= $this->getModel('rsscollectorform')->getItem($record_id);
	 ...
	 * 
	 */
	public function getModel($name = 'rsscollectorform', $prefix = '',$config = array('ignore_request' => true))
	{
	 ...
	 *
	 * @return  mixed  True if the Rss_collector can be added, a error object if not.
	 *
	 */
	public function add()
	{
	...
	public function edit($key = 'id', $url_var = null)
	{
		...
		$model = $this->getModel('rsscollectorform');
		// Set the return url
		$this->setReturnPage($context);
	...
	public function save($key = 'id', $url_var = null)
	{
		...
		if ($record_id == 0) 
		{
			$this->setMessage(JText::_('COM_RSSCOLLECTOR_RSSCOLLECTORS_SUBMIT_SAVE_SUCCESS'));
		} 
		else 
		{
			$this->setMessage(JText::_('COM_RSSCOLLECTOR_RSSCOLLECTORS_SAVE_SUCCESS'));
		}
		...
		switch ($task)
		{
			case 'apply':
				...
				$record_id = $model->getState('rsscollector.id');
				$this->holdEditId($context, $record_id);
				$app->setUserState($context.'.data',	null);
	...
	public function delete()
	{
		...
		// Get the menu item model.
		$model = $this->getModel('rsscollector');
		...
		if ($id > 0) 
		{
			...
			{
				$this->setMessage(JText::_('COM_RSSCOLLECTOR_RSSCOLLECTORS_DELETE_SUCCESS'));
				
			}
			else
			{
				$this->setMessage(JText::_('COM_RSSCOLLECTOR_RSSCOLLECTORS_DELETE_FAILED'));
			}
		}
