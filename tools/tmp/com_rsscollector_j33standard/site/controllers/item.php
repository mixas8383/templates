...
class RssCollectorControllerItem extends JControllerForm
{
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		$user		= JFactory::getUser();
		$asset		= 'com_rsscollector.item.'.$record_id;
	...
	public function save($key = 'id', $url_var = null)
	{
		...
		if ($record_id == 0) 
		{
			$this->setMessage(JText::_('COM_RSSCOLLECTOR_ITEMS_SUBMIT_SAVE_SUCCESS'));
		} 
		else 
		{
			$this->setMessage(JText::_('COM_RSSCOLLECTOR_ITEMS_SAVE_SUCCESS'));
		}
	...
	public function delete()
	{
		...
		if ($id > 0) 
		{
			...
			{
				$this->setMessage(JText::_('COM_RSSCOLLECTOR_ITEMS_DELETE_SUCCESS'));
				
			}
			else
			{
				$this->setMessage(JText::_('COM_RSSCOLLECTOR_ITEMS_DELETE_FAILED'));
			}
		}
