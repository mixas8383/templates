...
/**
 * HTML Item View class for the Rss_collector component
 *
 */
class RssCollectorViewItem extends JViewLegacy
{
	protected $item;
	protected $params;
	protected $print;
	...
	public function display($tpl = null)
	{
		...
		$item->readmore_link = JRoute::_(RssCollectorHelperRoute::getItemRoute($item->slug, 
										$item->language,
										$this->getLayout(),									
		...
		if ($this->params->get('show_item_tags')  == '1')
		{
			...
			$item->tags->getItemTags('com_rsscollector.item', $item->id);	
		...
		if ($item->params->get('access-view') != true)
		{
			...
			if (!$item->params->get('show_item_noauth'))
			{
				...
				if( $user->get('guest'))
				{
					...
					$app->redirect('index.php?option=com_users&view=login&return=' . base64_encode($uri), JText::_('COM_RSSCOLLECTOR_ITEMS_ERROR_LOGIN_TO_VIEW_ITEM'));
		...
		$results = $dispatcher->trigger('onItemPrepare', array ('com_rsscollector.item', &$item, &$this->params, $offset));

		$item->event = new stdClass;
		$results = $dispatcher->trigger('onItemAfterName', array('com_rsscollector.item', &$item, &$this->params, $offset));
		$item->event->afterDisplayItemName = JString::trim(implode("\n", $results));
		
		$results = $dispatcher->trigger('onItemBeforeDisplay', array('com_rsscollector.item', &$item, &$this->params, $offset));
		$item->event->beforeDisplayItem = JString::trim(implode("\n", $results));

		$results = $dispatcher->trigger('onItemAfterDisplay', array('com_rsscollector.item', &$item, &$this->params, $offset));
	...
	protected function prepareDocument()
	{
		...
		if ($menu AND ($menu->query['option'] != 'com_rsscollector' OR $menu->query['view'] != 'item' OR $id != $this->item->id))
		{
		...
		if (!empty($this->item->page_title))
		{
			...
			$this->document->setTitle($this->item->page_title . ' - ' . JText::sprintf('COM_RSSCOLLECTOR_PAGEBREAK_PAGE_NUM', $this->state->get('list.offset') + 1));
		}
