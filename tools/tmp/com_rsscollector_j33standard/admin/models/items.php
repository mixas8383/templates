 ...
 *
 */
class RssCollectorModelItems extends JModelList
{
	/**
	...
	protected function getListQuery()
	{
		...
		if ($assoc)
		{
			...
			$query->join('LEFT', $db->quoteName('#__associations').' AS as1 ON '.$db->quoteName('as1.id').' = '.$db->quoteName('a.id').' AND '.$db->quoteName('as1.context').' = '.$db->quote('com_rsscollector.item.item'));
		...
		if (is_numeric($tag_id))
		{
			...
			$query->join(
					...
					. ' AND ' . $db->quoteName('tagmap.type_alias') . ' = ' . $db->quote('com_rsscollector.item')
				);
		}
