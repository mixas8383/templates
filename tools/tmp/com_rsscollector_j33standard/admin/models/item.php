 ...
 *
 */
class RssCollectorModelItem extends JModelAdmin
{
	/**
	 * @var		string	The prefix to use with controller messages.
	 */
	protected $text_prefix = 'COM_RSSCOLLECTOR_ITEMS';
	/**
	 * @var      string	The type alias for this object (for example, 'com_rsscollector.item')
	 */
	public $typeAlias = 'com_rsscollector.item';	
	/**
	 * @var		string	The context for the app call.
	 */
	protected $context = 'com_rsscollector.items';	
	/**
	 ...
	 */
	public function getTable($type = 'Items', $prefix = 'RssCollectorTable', $config = array())
	{
	...
	protected function canDelete($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.delete', 'com_rsscollector.item.'.(int) $record->id);
		}
		else
		{
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.edit.state', 'com_rsscollector.item.'.(int) $record->id);
		}
		else
		{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_rsscollector.item');
			}	
		}
		...
		if ($assoc)
		{
			...
			if ($item->id != null)
			{
				$associations = JLanguageAssociations::getAssociations('com_rsscollector', '#__rsscollector_items', 'com_rsscollector.item.item', $item->id, 'id', 'alias', null);
	...
	{
		$form = $this->loadForm('com_rsscollector.edit.item', 'item', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = $app->getUserState('com_rsscollector.edit.item.data', array());
		...
		if (parent::save($data))
		{
			...
			if ($assoc)
			{
				...
				try
				{
					...
					$query->where('context='.$db->quote('com_rsscollector.item.item'));
				...
				if (!$all_language AND count($associations))
				{
					...
					{
						$query->values($id.','.$db->quote('com_rsscollector.item.item') . ',' . $db->quote($key));
					}
	...
	public function featured($pks, $value = 0)
	{
		...
		if (empty($pks))
		{
			$this->setError(JText::_('COM_RSSCOLLECTOR_ITEMS_NO_ITEM_SELECTED'));
			return false;
		}
		...
		try
		{
			...
			$query = $db->getQuery(true);
			$query->update($db->quoteName('#__rsscollector_items'));
	...
	* @param   array   $data	The data where the original name is stored
	...
	protected function preprocessForm(JForm $form, $data, $group = 'item')
	{
		...
		if ($assoc)
		{
			...
			$fieldset->addAttribute('description', 'COM_RSSCOLLECTOR_ITEMS_ITEM_ASSOCIATIONS_FIELDSET_DESC');
			$add = false;
