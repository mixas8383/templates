 ...
 *
 */
class RssCollectorModelRssCollector extends JModelAdmin
{
	/**
	 * @var		string	The prefix to use with controller messages.
	 */
	protected $text_prefix = 'COM_RSSCOLLECTOR_RSSCOLLECTORS';
	/**
	 * @var      string	The type alias for this object (for example, 'com_rsscollector.rsscollector')
	 */
	public $typeAlias = 'com_rsscollector.rsscollector';	
	/**
	 ...
	 */
	protected $event_before_save = 'onRssCollectorBeforeSave';
	/**
	 * @var		string	The event to trigger after saving the data.
	 */
	protected $event_after_save = 'onRssCollectorAfterSave';

	/**
	 * @var    string	The event to trigger before deleting the data.
	 */
	protected $event_before_delete = 'onRssCollectorBeforeDelete';	
	/**
	 * @var    string	The event to trigger after deleting the data.
	 */
	protected $event_after_delete = 'onRssCollectorAfterDelete';	
	/**
	 * @var    string	The event to trigger after changing the data's state field.
	 */
	protected $event_change_state = 'onRssCollectorChangeState';	
	 ...
	 */
	public function getTable($type = 'RssCollectors', $prefix = 'RssCollectorTable', $config = array())
	{
	...
	protected function canDelete($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.delete', 'com_rsscollector.rsscollector.'.(int) $record->id);
		}
		else
		{
			return $user->authorise('core.delete', 'com_rsscollector');
		}							
	}
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.edit.state', 'com_rsscollector.rsscollector.'.(int) $record->id);
		}
		else
		{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_rsscollector.rsscollector');
			}	
		}
		...
		if ($assoc)
		{
			...
			if ($item->id != null)
			{
				$associations = JLanguageAssociations::getAssociations('com_rsscollector', '#__rsscollector_rsscollectors', 'com_rsscollector.rsscollector.item', $item->id, 'id', 'alias', null);
	...
	{
		$form = $this->loadForm('com_rsscollector.edit.rsscollector', 'rsscollector', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
		...
		// Prevent messing with Rss_collector language and category when editing existing Rss_collector with associations
	...
	protected function loadFormData()
	{
		...
		$data = $app->getUserState('com_rsscollector.edit.rsscollector.data', array());
	...
	protected function prepareTable($table)
	{
		...
		// Increment the rss_collector version number.
		$table->version++;
	...
	{
		// Include the rss_collector plugins for the onSave events.
		...
		if (parent::save($data))
		{
			...
			if ($assoc)
			{
				...
				{
					JError::raiseNotice(403, JText::_('COM_RSSCOLLECTOR_ERROR_ALL_LANGUAGE_ASSOCIATED'));
				}
				...
				try
				{
					...
					$query->where('context='.$db->quote('com_rsscollector.rsscollector.item'));
				...
				if (!$all_language AND count($associations))
				{
					...
					{
						$query->values($id.','.$db->quote('com_rsscollector.rsscollector.item') . ',' . $db->quote($key));
					}
	...
	{
		// Include the rsscollector plugins for the delete events.
		JPluginHelper::importPlugin('rsscollector');	
	...
	/**
	 * Method to toggle the featured setting of rss_collectors.
	 *
	...
	public function featured($pks, $value = 0)
	{
		...
		if (empty($pks))
		{
			$this->setError(JText::_('COM_RSSCOLLECTOR_RSSCOLLECTORS_NO_ITEM_SELECTED'));
			return false;
		}
		...
		try
		{
			...
			$query = $db->getQuery(true);
			$query->update($db->quoteName('#__rsscollector_rsscollectors'));
	...
	/**
	 * Custom clean the cache of com_rsscollector and rsscollector modules
	 *
	 */
	protected function cleanCache($group = null, $client_id = 0)
	{
		parent::cleanCache('com_rsscollector');
	...
	* @param   array   $data	The data where the original name is stored
	...
	/**
	 * Pre process the form to pick up rss_collectors associated by language
	 *
	 ...
	 */
	protected function preprocessForm(JForm $form, $data, $group = 'rsscollector')
	{
		...
		if ($assoc)
		{
			...
			$fieldset->addAttribute('description', 'COM_RSSCOLLECTOR_RSSCOLLECTORS_ITEM_ASSOCIATIONS_FIELDSET_DESC');
			$add = false;
			foreach ($languages as $tag => $language)
			{
				if (empty($data->language) OR $tag != $data->language)
				{
					...
					$field->addAttribute('type', 'modal_rsscollectors');
