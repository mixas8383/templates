 ...
 *
 */
class RssCollectorModelRssItems extends JModelAdmin
{
	/**
	 * @var		string	The prefix to use with controller messages.
	 */
	protected $text_prefix = 'COM_RSSCOLLECTOR_RSSITEMSES';
	/**
	 * @var      string	The type alias for this object (for example, 'com_rsscollector.rssitems')
	 */
	public $typeAlias = 'com_rsscollector.rssitems';	
	/**
	 ...
	 */
	protected $event_before_save = 'onRssItemsBeforeSave';
	/**
	 * @var		string	The event to trigger after saving the data.
	 */
	protected $event_after_save = 'onRssItemsAfterSave';

	/**
	 * @var    string	The event to trigger before deleting the data.
	 */
	protected $event_before_delete = 'onRssItemsBeforeDelete';	
	/**
	 * @var    string	The event to trigger after deleting the data.
	 */
	protected $event_after_delete = 'onRssItemsAfterDelete';	
	/**
	 * @var    string	The event to trigger after changing the data's state field.
	 */
	protected $event_change_state = 'onRssItemsChangeState';	
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_rsscollector.rssitems');
			}	
		}
		...
		if ($assoc)
		{
			...
			if ($item->id != null)
			{
				$associations = JLanguageAssociations::getAssociations('com_rsscollector', '#__rsscollector_rssitemses', 'com_rsscollector.rssitems.item', $item->id, 'id', 'alias', null);
	...
	{
		$form = $this->loadForm('com_rsscollector.edit.rssitems', 'rssitems', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
		...
		// Prevent messing with Rss_items language and category when editing existing Rss_items with associations
	...
	protected function loadFormData()
	{
		...
		$data = $app->getUserState('com_rsscollector.edit.rssitems.data', array());
	...
	protected function prepareTable($table)
	{
		...
		// Increment the rss_items version number.
		$table->version++;
		...
		if (parent::save($data))
		{
			...
			if ($assoc)
			{
				...
				try
				{
					...
					$query->where('context='.$db->quote('com_rsscollector.rssitems.item'));
				...
				if (!$all_language AND count($associations))
				{
					...
					{
						$query->values($id.','.$db->quote('com_rsscollector.rssitems.item') . ',' . $db->quote($key));
					}
	...
	/**
	 * Method to toggle the featured setting of rss_itemses.
	 *
	...
	public function featured($pks, $value = 0)
	{
		...
		if (empty($pks))
		{
			$this->setError(JText::_('COM_RSSCOLLECTOR_RSSITEMSES_NO_ITEM_SELECTED'));
			return false;
		}
		...
		try
		{
			...
			$query = $db->getQuery(true);
			$query->update($db->quoteName('#__rsscollector_rssitemses'));
	...
	* @param   array   $data	The data where the original name is stored
	...
	/**
	 * Pre process the form to pick up rss_itemses associated by language
	 *
	 ...
	 */
	protected function preprocessForm(JForm $form, $data, $group = 'rssitems')
	{
		...
		if ($assoc)
		{
			...
			$fieldset->addAttribute('description', 'COM_RSSCOLLECTOR_RSSITEMSES_ITEM_ASSOCIATIONS_FIELDSET_DESC');
			$add = false;
			foreach ($languages as $tag => $language)
			{
				if (empty($data->language) OR $tag != $data->language)
				{
					...
					$field->addAttribute('type', 'modal_rssitemses');
