...
if ($save_order)
{
	$save_ordering_url = 'index.php?option=com_rsscollector&task=items.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'item-list', 'adminForm', strtolower($list_dirn), $save_ordering_url);
}
...
<form action="<?php echo JRoute::_('index.php?option=com_rsscollector&view=items'); ?>" method="post" name="adminForm" id="adminForm">
	...
	<?php else : ?>
		<table class="table table-striped" id="item-list">
			...
			<?php foreach ($this->items as $i => $item) :
				...
				$can_edit	= $user->authorise('core.edit',	'com_rsscollector.item.'.$item->id);
		
				$can_edit_own	= $user->authorise('core.edit.own',		'com_rsscollector.item.'.$item->id) 
								AND $item->created_by == $user_id
								;
				$can_change	= $user->authorise('core.edit.state',	'com_rsscollector.item.'.$item->id) 
								AND $can_checkin
								;
				if ($item->language == '*'):
				...
				<tr class="row<?php echo $i % 2; ?>">
					...
					<td class="center">
						<div class="btn-group">
							<?php echo JHtml::_('jgrid.published', $item->state, $i, 'items.', $can_change, 'cb', $item->publish_up, $item->publish_down); ?>
							<?php echo JHtml::_('itemadministrator.featured', $item->featured, $i, $can_change); ?>
							<?php
								if ($archived) :
					...
					<td class="nowrap has-context">
						<div class="pull-left">
							...
							<?php if ($can_edit OR $can_edit_own) : ?>
								<a href="<?php echo JRoute::_('index.php?option=com_rsscollector&task=item.edit&id='.(int) $item->id); ?>">
					...
					<?php if ($assoc) : ?>
						<td class="small hidden-phone">
							<?php if ($item->association) : ?>
								<?php echo JHtml::_('itemadministrator.association', $item->id); ?>
							<?php endif; ?>
						</td>
					<?php endif;?>				
