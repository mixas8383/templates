 ...
 *
 */
class RssCollectorTableRssCollectors extends JTable
{
	...
	public function __construct(&$db)
	{
		...
		parent::__construct('#__rsscollector_rsscollectors', 'id', $db);
		...
		JTableObserverTags::createObserver($this, array('typeAlias' => 'com_rsscollector.rsscollector'));

		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_rsscollector.rsscollector'));
	}
	...
	public function check()
	{
		...
		$table = JTable::getInstance('RssCollectors', 'RssCollectorTable');
		if ($table->load(array('alias'=>$this->alias)) AND ($table->id != $this->id OR $this->id==0))
		{
			$this->setError(JText::_('COM_RSSCOLLECTOR_FIELD_ALIAS_DUPLICATED'));
			return false;
		}		
	...
	public function store($update_nulls = false)
	{
		...
		if (empty($this->id))
		{
			// New Rss_collector. A created and created_by field can be set by the user,
	...
	{
		$k = $this->_tbl_key;
		return 'com_rsscollector.rsscollector.'.(int) $this->$k;
	}
	...
	protected function _getAssetParentId(JTable $table = null, $id = null)
	{
		...
		if ($asset->loadByName('com_rsscollector'))
		{
			return $asset->id;
		}
		else
