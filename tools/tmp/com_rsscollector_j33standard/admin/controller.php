...
class RssCollectorController extends JControllerLegacy
{
	/**
	...
	public function display($cachable = false, $url_params = false)
	{
		...
		// Load the submenu.
		RssCollectorHelper::addSubmenu($view);

		// Check for edit form.
		switch ($view)
		{
			case 'item': 
				if ($layout == 'edit' AND !$this->checkEditId('com_rsscollector.edit.item', $id))
				{

					// Somehow the person just went to the form - we don't allow that.
					$this->setError(JText::sprintf('JLIB_APPLICATION_ERROR_UNHELD_ID', $id));
					$this->setMessage($this->getError(), 'error');
					$this->setRedirect(JRoute::_('index.php?option=com_rsscollector&view=items', false));
