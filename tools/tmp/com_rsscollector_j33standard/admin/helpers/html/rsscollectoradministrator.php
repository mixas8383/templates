...
/**
 * Rss_collectors component helper.
 *
 */
abstract class JHtmlRssCollectorAdministrator
{
	/**
	 * Render the list of associated items
	 * 
	 * @param	integer $rsscollector_id	The rss_collector item id
	 * 
	 * @return  string  The language HTML
     */
	public static function association($rsscollector_id)
	{
		...
		// Get the associations
		if ($associations = JLanguageAssociations::getAssociations('com_rsscollector', '#__rsscollector_rsscollectors', 'com_rsscollector.rsscollector.item',  $rsscollector_id, 'id', 'alias', null))
		{		
			...
			if ($items)
			{
				foreach ($items as &$item)
				{
					...
					$url = JRoute::_('index.php?option=com_rsscollector&task=rsscollector.edit&id=' . (int) $item->id);
					$tooltip_parts = array(
	...
	public static function featured($value = 0, $i, $can_change = true)
	{
		...
		$states	= array(
			0	=> array('unfeatured', 'rsscollectors.featured', 'COM_RSSCOLLECTOR_RSSCOLLECTORS_UNFEATURED', 'COM_RSSCOLLECTOR_RSSCOLLECTORS_TOGGLE_TO_FEATURE'),
			1	=> array('featured', 'rsscollectors.unfeatured', 'JFEATURED', 'COM_RSSCOLLECTOR_RSSCOLLECTORS_TOGGLE_TO_UNFEATURE'),
		);
