...
class MmanagerViewEditions extends JViewLegacy
{
	...
	public function display($tpl = null)
	{
		...
		$doc->link = JRoute::_('index.php?option=com_mmanager&view=editions');
		...
		foreach ($rows as $row)
		{
			...
			// url link to edition
			$link = JRoute::_(MmanagerHelperRoute::getEditionRoute($row->slug, 
																						$row->catid, 
																						$row->language,	
																						'default',								
			...
			if ($item->featured == 1)
			{
				$item->category[]	= JText::_('JFEATURED'); // All featured editions are categorized as "Featured"
			}
