...
class MmanagerViewEditions extends JViewLegacy
{
	...
	public function display($tpl = null)
	{
		...
		// PREPARE THE DATA
		// Compute the edition slugs and set the trigger events.
		foreach ($items as $i => &$item)
		{
			...
			$dispatcher->trigger('onEditionPrepare', array ('com_mmanager.edition', &$item, &$item->params,$i));

			$item->event = new stdClass;

			$results = $dispatcher->trigger('onEditionAfterName', array('com_mmanager.edition', &$item, &$item->params, $i));
			$item->event->afterDisplayEditionName = JString::trim(implode("\n", $results));

			$results = $dispatcher->trigger('onEditionBeforeDisplay', array('com_mmanager.edition', &$item, &$item->params, $i));
			$item->event->beforeDisplayEdition = JString::trim(implode("\n", $results));

			$results = $dispatcher->trigger('onEditionAfterDisplay', array('com_mmanager.edition', &$item, &$item->params,$i));
	...
	protected function prepareDocument()
	{
		...
		{
			$this->params->def('page_heading', JText::_('COM_MMANAGER_EDITIONS'));
		}
