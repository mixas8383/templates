...
$layout		= $this->params->get('edition_layout', 'default');
...
<div class="mmanager editions-list<?php echo $this->params->get('pageclass_sfx');?>">
	...
	<?php
		$show_actions = false;
		if ($this->params->get('show_edition_icons',-1) >= 0) :
	...
	<form action="<?php echo JFilterOutput::ampReplace(JUri::getInstance()->toString()); ?>" method="post" name="adminForm" id="adminForm">
		<?php if (($this->params->get('show_edition_filter_field') != '' AND $this->params->get('show_edition_filter_field') != 'hide')) :?>
			<div class="filter-search">
				<?php if ($this->params->get('show_edition_filter_field') != '' AND $this->params->get('show_edition_filter_field') != 'hide') :?>
					<input type="text" name="filter_search" id="filter_search" value="<?php echo $this->escape($this->state->get('filter.search')); ?>" onchange="document.adminForm.submit();" title="<?php echo JText::_('COM_MMANAGER_FILTER_SEARCH_DESC'); ?>" placeholder="<?php echo JText::_('COM_MMANAGER_'.$this->params->get('show_edition_filter_field').'_FILTER_LABEL'); ?>" />
				<?php endif; ?>	
				<select name="filter_category_id" onchange="this.form.submit()">
					<option value=""><?php echo JText::_('JOPTION_SELECT_CATEGORY');?></option>
					<?php echo JHtml::_('select.options', JHtml::_('category.options', 'com_mmanager'), 'value', 'text', $this->state->get('filter.category_id'));?>
				</select>
			</div>
		<?php endif; ?>

		<?php if ($this->params->get('show_edition_pagination_limit')) : ?>
		...
		<?php if (empty($this->items)) : ?>
			...
			<?php if ($this->params->get('show_no_editions',1)) : ?>
			<p><?php echo JText::_('COM_MMANAGER_EDITIONS_NO_ITEMS'); ?></p>
			<?php endif; ?>

		<?php else : ?>
			<table class="table table-striped" id="editions">
			<?php if ($this->params->get('show_edition_headings')) :?>
			<thead>
				<tr>
			...
			<tbody>
				...
				?>			
					...
					<?php if (in_array($item->access, $user->getAuthorisedViewLevels()) OR $params->get('show_edition_noauth')) : ?>
					<td class="list-name">
						<a href="<?php echo JRoute::_(MmanagerHelperRoute::getEditionRoute($item->slug, 
																									$item->catid, 
																									$item->language,
																									$layout,									
																									$params->get('keep_edition_itemid'))); ?>" 
																									itemprop="url"
																									>
					...
					<?php if ($this->params->get('list_show_edition_date')) : ?>
						<td class="list-date">
							<time datetime="<?php echo JHtml::_('date', $item->display_date, 'c'); ?>">
								<?php echo JHtml::_('date',$item->display_date, $this->escape($this->params->get('edition_date_format', JText::_('DATE_FORMAT_LC3')))); ?>
							</time>
						</td>
					<?php endif; ?>
					<?php if ($this->params->get('list_show_edition_created_by',0)) : ?>
						<td class="createdby" itemprop="creator" itemscope itemtype="http://schema.org/Person">
							<?php 
								...
								if (!empty($item->created_by )) :
									if ($this->params->get('link_edition_created_by') == 1) :
									...
									if ($this->params->get('show_edition_headings')) :
										echo $created_by;
									else :
					...
					<?php endif; ?>
					<?php if ($this->params->get('list_show_edition_category',0)) : ?>
					...
					<?php endif; ?>
					<?php if ($this->params->get('list_show_edition_modified_time',0)) : ?>
					...
					<?php endif; ?>
					<?php if ($this->params->get('list_show_edition_modified_user_id',0)) : ?>
					...
					<?php endif; ?>
					<?php if ($this->params->get('list_show_edition_created_time',0)) : ?>
					...
					<?php endif; ?>
					<?php if ($this->params->get('list_show_edition_created_user_id',0)) : ?>
					...
					<?php endif; ?>
					<?php if ($this->params->get('list_show_edition_metadata',0)) : ?>
					...
					<?php endif; ?>
					<?php if ($this->params->get('list_show_edition_published',0)) : ?>
					...
					<?php endif; ?>
					<?php if ($this->params->get('list_show_edition_note',0)) : ?>
						<td class="list-note">
							<?php 
					...
					<?php endif; ?>
					<?php if ($this->params->get('list_show_edition_title',0)) : ?>
						<td class="list-title">
							<?php 
					...
					<?php endif; ?>
					<?php if ($this->params->get('list_show_edition_extension',0)) : ?>
					...
					<?php endif; ?>
					<?php if ($this->params->get('list_show_edition_path',0)) : ?>
						<td class="list-path">
							<?php 
					...
					<?php endif; ?>
					<?php if ($this->params->get('list_show_edition_level',0)) : ?>
						<td class="list-level">
							<?php 
					...
					<?php endif; ?>
					<?php if ($this->params->get('list_show_edition_rgt',0)) : ?>
						<td class="list-rgt">
							<?php 
					...
					<?php endif; ?>
					<?php if ($this->params->get('list_show_edition_lft',0)) : ?>
						<td class="list-lft">
							<?php 
					...
					<?php endif; ?>
					<?php if ($this->params->get('list_show_edition_parent_id',0)) : ?>
							...
							<?php if ($can_order) : ?>
								<?php if ($save_order) : ?>
									...
									<?php if ($list_dirn == 'asc') : ?>
										<span><?php echo $this->pagination->orderUpIcon($i,$condition_up,'editions.orderup', 'JLIB_HTML_MOVE_UP', $save_order); ?></span>
										<span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total,$condition_down, 'editions.orderdown', 'JLIB_HTML_MOVE_DOWN', $save_order); ?></span>
									<?php elseif ($list_dirn == 'desc') : ?>
										<span><?php echo $this->pagination->orderUpIcon($i, $condition_up, 'editions.orderdown', 'JLIB_HTML_MOVE_UP', $save_order); ?></span>
										<span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total,$condition_down, 'editions.orderup', 'JLIB_HTML_MOVE_DOWN', $save_order); ?></span>
									<?php endif; ?>
								<?php endif; ?>
						...
						<td class="list-actions">
							<?php if ($can_edit OR $can_delete ) : ?>
								<ul class="actions">
									<?php if ($can_edit ) : ?>
										<li class="edit-icon">
											<?php echo JHtml::_('editionicon.edit',$item, $params); ?>
										</li>
									<?php endif; ?>					
									<?php if ($can_delete) : ?>
										<li class="delete-icon">
											<?php echo JHtml::_('editionicon.delete',$item, $params); ?>
										</li>
									<?php endif; ?>					
								</ul>
							<?php endif; ?>
			...
			<?php if (($this->params->def('show_edition_pagination', 2) == 1  OR ($this->params->get('show_edition_pagination') == 2)) AND ($this->pagination->get('pages.total') > 1)) : ?>
			<div class="pagination">

				<?php if ($this->params->def('show_edition_pagination_results', 0)) : ?>
				<p class="counter">
		...
		<?php endif; ?>
		<?php // Code to add a link to submit an edition. ?>
		<?php if ($this->params->get('show_edition_add_link', 1)) : ?>
			<?php if ($can_create) : ?>
				<?php echo JHtml::_('editionicon.create', $this->params); ?>
			<?php  endif; ?>
		<?php endif; ?>		
	</form>
</div>
