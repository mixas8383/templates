...
$this->document->addStyleSheet(JUri::root().'media/com_mmanager/css/site_'.JString::strtolower(str_replace(' ','',$this->params->get('items_to_display'))).'.css');
$this->document->addStyleSheet(JUri::root().'media/com_mmanager/css/site_categories.css');

if ($lang->isRTL())
{
	$this->document->addStyleSheet(JUri::root().'media/com_mmanager/css/site_mmanager-rtl.css');
	$this->document->addStyleSheet(JUri::root().'media/com_mmanager/css/site_'.JString::strtolower(str_replace(' ','',$this->params->get('items_to_display'))).'-rtl.css');
	$this->document->addStyleSheet(JUri::root().'media/com_mmanager/css/site_categories-rtl.css');
}
...
<div class="mmanager category-list <?php echo $this->pageclass_sfx;?>" itemscope itemtype="http://schema.org/Blog">
	...
	<div class="category-desc clearfix">
		...
		<?php if ($this->params->get('show_cat_description') AND $this->category->description) : ?>
			<?php echo JHtml::_('content.prepare', $this->category->description, '', 'com_mmanager.categories'); ?>
		<?php endif; ?>
	</div>
	<?php endif; ?>
