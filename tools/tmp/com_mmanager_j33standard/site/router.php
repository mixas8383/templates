...
/**
 * Routing class for the Mmanager component
 *
 */
class MmanagerRouter extends JComponentRouterBase
{
	/**
	 * Build the route for the Mmanager component
	 *
	...
	public function build(&$query)
	{
		...
		// Are we dealing with a mmanager that is attached to a menu item?
	...
	public function parse(&$segments)
	{
		...
		{
			$categories = JCategories::getInstance('Mmanager',$options)->get($id)->getChildren();
		}
		else
		{
			$categories = JCategories::getInstance('Mmanager',$options);	
		...
		foreach($segments as $segment)
		{
			...
			if ($found == 0)
			{
				if ($item->query['view'] == 'editions' OR $item->query['view'] == 'edition'
					OR $view == 'editions' OR $view == 'edition')
				{
					if($advanced)
					{
						$db = JFactory::getDbo();
						...
						$query->from($db->quoteName('#__mmanager_editions'));
