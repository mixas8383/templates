...
/**
 * Mmanager Component Edition Model
 *
 */
class MmanagerModelEdition extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_mmanager.edition';
	...
	{
		if (empty($config['edition_filter_fields']))
		{
			$config['edition_filter_fields'] = array(
				'id', 'a.id',
				'name', 'a.name',
				'alias', 'a.alias',
	...
	protected function canDelete($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.delete', 'com_mmanager.edition.'.(int) $record->id)
					  OR ($user->authorise('core.delete', 'com_mmanager.edition.'.(int) $record->id)
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.edit.state', 'com_mmanager.edition.'.(int) $record->id);
		}
		else
		{
			// New edition, so check against the category.		
			if (!empty($record->catid))
			{
				return $user->authorise('core.edit.state', 'com_mmanager.category.'.(int) $record->catid);
			}
			else 
			{
			// Default to component settings.			
				return $user->authorise('core.edit.state', 'com_mmanager');
			}
		}
	}
	/**
	...
	*/
	public function getTable($type = 'Editions', $prefix = 'MmanagerTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get Edition data.
	 *
	 * @param	integer	$pk	The id of the edition.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('edition.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				// If all categories are published, badcats.id will be null, and we just use the edition state
				...
				$sub_query .= 'WHERE '.$db->quoteName('parent.extension').' = ' . $db->quote('com_mmanager');
				...
				$can_publish = $user->authorise('core.edit.state', 'com_mmanager.edition.'.$pk);
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_MMANAGER_EDITIONS_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		// Include the mmanager plugins for the change of state event.
		...
		$result = $dispatcher->trigger('onEditionChangeState', array('com_mmanager.edition', $pks, $value));
	...
	*/
	...
	public function delete(&$pks)
	{
		...
		// Include the mmanager plugins for the on delete events.
		JPluginHelper::importPlugin('mmanager');
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onEditionBeforeDelete', array('com_mmanager.edition', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onEditionAfterDelete', array('com_mmanager.edition', &$table));
				}
				else
				{
					// Prune items that you can't change.
	...
	/**
	 * Increment the hit counter for the edition.
	 *
	 * @pk		int		Optional primary key of the edition to increment.
	 *
