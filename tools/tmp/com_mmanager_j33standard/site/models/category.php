...
class MmanagerModelCategory extends JModelList
{
	/**
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		{
			$limit = $app->getUserStateFromRequest('com_mmanager.category.list.' . $item_id . '.limit', 'limit', $app->get('list_limit'));
		}
	...
	public function getItems()
	{
		...
		if ($params->get('items_to_display') AND $params->get('items_to_display') !='')
		{
			...
			{
				$model = JModelLegacy::getInstance($object_upper_case, 'MmanagerModel', array('ignore_request' => true));
	...
	protected function buildOrderBy()
	{
		...
		$item_id	= $app->input->getInt('id', 0) . ':' . $app->input->getInt('Itemid', 0);
		$order_col	= $app->getUserStateFromRequest('com_mmanager.category.list.' . $item_id . '.filter_order', 'filter_order', '', 'string');
		$order_dirn	= $app->getUserStateFromRequest('com_mmanager.category.list.' . $item_id . '.filter_order_Dir', 'filter_order_Dir', '', 'cmd');
		$order_by	= ' ';
		...
		{
			case 'Editions' :
				$secondary_order_by	= $params->get('edition_orderby_sec', 'none');
				$order_date			= $params->get('edition_order_date');
				$category_order_by	= $params->def('edition_orderby_pri', '');
				
				$primary			= MmanagerHelperQuery::orderbyPrimary($category_order_by);	
				$secondary			= MmanagerHelperQuery::orderbySecondary($secondary_order_by, $order_date, 'ordering') . ', ';
	...
	public function getCategory()
	{
		if(!is_object($this->_item))
		{
			...
			if ($params->get('items_to_display') AND $params->get('items_to_display') !='')
			{
				$options['table'] = '#__mmanager_'.JString::strtolower(str_replace(' ','',$params->get('items_to_display')));
			}
			else
			{
				$options['table'] = '';
			...
			$categories = JCategories::getInstance('Mmanager', $options);
			$this->_item = $categories->get($this->getState('category.id', 'root'));
			if(is_object($this->_item))
			{
				$user	= JFactory::getUser();
				$asset	= 'com_mmanager.category.'.$this->_item->id;
		...
		$this->tags->getItemTags('com_mmanager.category', $this->_item->get('id'));
		return $this->_item;
	}
