...
/**
 * This models supports retrieving lists of editions.
 *
 */
class MmanagerModelEditions extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_mmanager.editions';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single edition has been specified either as a parameter or in the url Request
		$pk = $params->get('edition_id', '') == '' ? $app->input->getInt('id', '') : $params->get('edition_id');
		$this->setState('filter.edition_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('edition_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('edition_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('edition_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('edition_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('edition_initial_direction','ASC');
		}
		$this->setState('list.direction', $list_order);
		
				
		if ((!$user->authorise('core.edit.state', 'com_mmanager')) AND  (!$user->authorise('core.edit', 'com_mmanager')))
		{
		...
		// process show_edition_noauth parameter
		if (!$params->get('show_edition_noauth'))
		{
		...
		if ($params->get('filter_edition_categories') AND implode(',', $params->get('filter_edition_categories'))  == true)
		{
			$selected_categories = $params->get('filter_edition_categories');
			$this->setState('filter.category_id', $selected_categories);
		}			
		if ($params->get('filter_edition_featured') <> "")
		{
			$this->setState('filter.featured', $params->get('filter_edition_featured'));
			
		}
		if ($params->get('filter_edition_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_edition_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.edition_id'));
		$id .= ':'.$this->getState('filter.edition_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of editions subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__mmanager_editions').' AS a');
		...
		$can_publish = $user->authorise('core.edit.state', 'com_mmanager');
		...
		// Filter by a single or group of editions.
		$edition_id = $this->getState('filter.edition_id');
		if ($edition_id != '')
		{
			if (is_numeric($edition_id))
			{
				$type = $this->getState('filter.edition_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $edition_id);
			}
			else
			{
				if (is_array($edition_id))
				{
					JArrayHelper::toInteger($edition_id);
					$edition_id = implode(',', $edition_id);
					$type = $this->getState('filter.edition_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$edition_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_edition_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			...
			switch ($params->get('show_edition_filter_field'))
			{
				case 'hits':
	...
	/**
	 * Method to get a list of editions.
	 *
	...
	public function getItems()
	{
		...
		$global_params = JComponentHelper::getParams('com_mmanager', true);
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				$edition_params = new JRegistry;
				$edition_params->loadString($item->params);
				...
				$item->edition_alternative_readmore = $edition_params->get('edition_alternative_readmore');
				$item->layout = $edition_params->get('layout');
							
				if (!is_object($this->getState('params')))
				{
					$item->params = $edition_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Edition params override menu item params only if menu param = 'use_edition'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_edition' and there is no edition param, use global

					// create an array of just the params set to 'use_edition'
					$menu_params_array = $this->getState('params')->toArray();
					$edition_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_edition')
						{
							// if the edition has a value, use it
							if ($edition_params->get($key) != '')
							{
								// get the value from the edition
								$edition_array[$key] = $edition_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$edition_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected edition params
					if (count($edition_array) > 0)
					{
						$edition_params = new JRegistry;
						$edition_params->loadArray($edition_array);
						$item->params->merge($edition_params);
					}


					// get display date
					switch ($item->params->get('list_show_edition_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an edition, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_mmanager.edition.'.$item->id;
				...
				if ($access) 
				{
					// If the access filter has been set, we already have only the editions this user can view.
