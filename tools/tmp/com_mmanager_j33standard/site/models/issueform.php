...
require_once JPATH_ADMINISTRATOR.'/components/com_mmanager/models/issue.php';

class MmanagerModelIssueForm extends MmanagerModelIssue
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_mmanager.edit.issue';
	/**
	...
	*/
	 ...
	 *
	 * @param	integer	$item_id	The id of the issue.
	 *
	 * @return	mixed	Issue item data object on success, false on failure.
	 */
	public function getItem($item_id = null)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('issue.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_mmanager.edit.issue.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('issue.id');
		$is_new		= true;
		...
		// Reorder the issues so the new issue is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onIssueBeforeSave', array('com_mmanager.issue', &$table, $is_new));
		...
		$dispatcher->trigger('onIssueAfterSave', array('com_mmanager.issue', $table, $is_new));
