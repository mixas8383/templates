...
/**
 * This models supports retrieving lists of issues.
 *
 */
class MmanagerModelIssues extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_mmanager.issues';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single issue has been specified either as a parameter or in the url Request
		$pk = $params->get('issue_id', '') == '' ? $app->input->getInt('id', '') : $params->get('issue_id');
		$this->setState('filter.issue_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('issue_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('issue_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('issue_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('issue_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('issue_initial_direction','ASC');
		}
		...
		// process show_issue_noauth parameter
		if (!$params->get('show_issue_noauth'))
		{
		...
		if ($params->get('filter_issue_featured') <> "")
		{
			$this->setState('filter.featured', $params->get('filter_issue_featured'));
			
		}
		if ($params->get('filter_issue_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_issue_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.issue_id'));
	...
	/**
	 * Get the main query for retrieving a list of issues subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__mmanager_issues').' AS a');
		...
		// Filter by a single or group of issues.
		$issue_id = $this->getState('filter.issue_id');
		if ($issue_id != '')
		{
			if (is_numeric($issue_id))
			{
				$type = $this->getState('filter.issue_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $issue_id);
			}
			else
			{
				if (is_array($issue_id))
				{
					JArrayHelper::toInteger($issue_id);
					$issue_id = implode(',', $issue_id);
					$type = $this->getState('filter.issue_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$issue_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_issue_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			...
			switch ($params->get('show_issue_filter_field'))
			{
				case 'hits':
	...
	/**
	 * Method to get a list of issues.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				$issue_params = new JRegistry;
				$issue_params->loadString($item->params);
				...
				$item->issue_alternative_readmore = $issue_params->get('issue_alternative_readmore');
				$item->layout = $issue_params->get('layout');
							
				if (!is_object($this->getState('params')))
				{
					$item->params = $issue_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Issue params override menu item params only if menu param = 'use_issue'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_issue' and there is no issue param, use global

					// create an array of just the params set to 'use_issue'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_issue')
						{
							// if the issue has a value, use it
							if ($issue_params->get($key) != '')
							{
								// get the value from the issue
								$issue_array[$key] = $issue_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$issue_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected issue params
					if (count($issue_array) > 0)
					{
						$issue_params = new JRegistry;
						$issue_params->loadArray($issue_array);
						$item->params->merge($issue_params);
					}


					// get display date
					switch ($item->params->get('list_show_issue_date'))
					{
						case 'modified':
				...
				if (!$guest) 
				{
					$asset	= 'com_mmanager.issue.'.$item->id;
				...
				if ($access) 
				{
					// If the access filter has been set, we already have only the issues this user can view.
				...
				$item->tags->getItemTags('com_mmanager.issue', $item->id);
			}
		}
		return $items;
	}
