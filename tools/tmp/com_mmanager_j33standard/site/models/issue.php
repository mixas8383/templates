...
/**
 * Mmanager Component Issue Model
 *
 */
class MmanagerModelIssue extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_mmanager.issue';
	...
	{
		if (empty($config['issue_filter_fields']))
		{
			$config['issue_filter_fields'] = array(
				'id', 'a.id',
				'name', 'a.name',
				'alias', 'a.alias',
	...
	protected function canDelete($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.delete', 'com_mmanager.issue.'.(int) $record->id)
					  OR ($user->authorise('core.delete', 'com_mmanager.issue.'.(int) $record->id)
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.edit.state', 'com_mmanager.issue.'.(int) $record->id);
		}
		else
		{
	...
	*/
	public function getTable($type = 'Issues', $prefix = 'MmanagerTable', $config = array())
	{
	 ...
	 *
	 * @param	integer	$pk	The id of the issue.
	 *
	...
	public function getItem($pk = null)
	{
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				$can_publish = $user->authorise('core.edit.state', 'com_mmanager.issue.'.$pk);
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_MMANAGER_ISSUES_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onIssueChangeState', array('com_mmanager.issue', $pks, $value));
	...
	*/
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onIssueBeforeDelete', array('com_mmanager.issue', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onIssueAfterDelete', array('com_mmanager.issue', &$table));
				}
				else
				{
					// Prune items that you can't change.
	...
	/**
	 * Increment the hit counter for the issue.
	 *
	 * @pk		int		Optional primary key of the issue to increment.
	 *
