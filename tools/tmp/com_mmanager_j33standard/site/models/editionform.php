...
require_once JPATH_ADMINISTRATOR.'/components/com_mmanager/models/edition.php';

class MmanagerModelEditionForm extends MmanagerModelEdition
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_mmanager.edit.edition';
	/**
	...
	protected function populateState()
	{
		...
		$this->setState('edition.catid', $app->input->getInt('catid'));
	...
	*/
	...
	public function getForm($data = array(), $load_data = true)
	{
		...
		if ($id = (int) $this->getState('edition.id'))
		{
	 ...
	 *
	 * @param	integer	$item_id	The id of the edition.
	 *
	 * @return	mixed	Edition item data object on success, false on failure.
	 */
	public function getItem($item_id = null)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('edition.id');
		...
		{
			// New item.
			$cat_id = (int) $this->getState('edition.catid');
			if ($cat_id)
			{
				$item->params->set('access-change', $user->authorise('core.edit.state', 'com_mmanager.category.'.$cat_id));
				$item->catid = $cat_id;
			}
			else 
			{
				$item->params->set('access-change', $user->authorise('core.edit.state', 'com_mmanager'));
			}
		}
	...
	protected function setAccessFilters(&$form, $data)
	{
		...
		if (!$user->authorise('core.edit.state', 'com_mmanager'))
		{
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_mmanager.edit.edition.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('edition.id');
		$is_new		= true;
		...
		else
		{
			...
			$actions = JAccess::getActions('com_mmanager');
			$action_array = array();
		...
		// Reorder the editions so the new edition is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onEditionBeforeSave', array('com_mmanager.edition', &$table, $is_new));
		...
		// Clean the cache.
		$cache = JFactory::getCache('com_mmanager');
		$cache->clean();

		$dispatcher->trigger('onEditionAfterSave', array('com_mmanager.edition', $table, $is_new));

		$this->setState('edition.id', $table->id);
