...
/**
 * Mmanager Component Route Helper
 *
 */
abstract class MmanagerHelperRoute
{
	...
	/**
	 * @param	integer	The route of the Edition
	 */
	public static function getEditionRoute($id, $cat_id = 0, $language = '*', $layout = 'default', $keep_item_id = false)
	{
		$needles = array(
			'edition'  => array((int) $id)
		);
		...
		{
			//Create the link
			$link = 'index.php?option=com_mmanager&view=edition&id='. $id;
		}
		else
		{
			//Create the link with a layout
			$link = 'index.php?option=com_mmanager&view=edition&layout='.$layout.'&id='. $id;
		}
	...
	public static function getCategoryRoute($cat_id, $keep_item_id = false, $language = 0)
	{
		...
		else
		{
			...
			$options['countItems'] = false;
			$options['table'] = '';			
			$category = JCategories::getInstance('Mmanager', $options)->get($id);
		}
		...
		else
		{
			...
			$link = 'index.php?option=com_mmanager&view=category&id='.$id;
	...
	protected static function findItem($needles = null, $keep_item_id = false, $layout = 'default')
	{
		...
		if ($active AND $active->component == 'com_mmanager' AND ($language == '*' OR in_array($active->language, array('*', $language)) OR !JLanguageMultilang::isEnabled()))
		{
			return $active->id;
		}
