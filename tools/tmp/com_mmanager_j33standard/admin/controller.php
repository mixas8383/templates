...
class MmanagerController extends JControllerLegacy
{
	/**
	 * @var		string	The default view.
	 * 
	 */
	protected $default_view = 'editions';
	...
	public function display($cachable = false, $url_params = false)
	{
		...
		// Load the submenu.
		MmanagerHelper::addSubmenu($view);

		// Check for edit form.
		switch ($view)
		{
			case 'edition': 
				if ($layout == 'edit' AND !$this->checkEditId('com_mmanager.edit.edition', $id))
				{

					// Somehow the person just went to the form - we don't allow that.
					$this->setError(JText::sprintf('JLIB_APPLICATION_ERROR_UNHELD_ID', $id));
					$this->setMessage($this->getError(), 'error');
					$this->setRedirect(JRoute::_('index.php?option=com_mmanager&view=editions', false));
