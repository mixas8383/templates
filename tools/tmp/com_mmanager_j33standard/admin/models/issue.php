 ...
 *
 */
class MmanagerModelIssue extends JModelAdmin
{
	/**
	 * @var		string	The prefix to use with controller messages.
	 */
	protected $text_prefix = 'COM_MMANAGER_ISSUES';
	/**
	 * @var      string	The type alias for this object (for example, 'com_mmanager.issue')
	 */
	public $typeAlias = 'com_mmanager.issue';	
	/**
	 ...
	 */
	protected $event_before_save = 'onIssueBeforeSave';
	/**
	 * @var		string	The event to trigger after saving the data.
	 */
	protected $event_after_save = 'onIssueAfterSave';

	/**
	 * @var    string	The event to trigger before deleting the data.
	 */
	protected $event_before_delete = 'onIssueBeforeDelete';	
	/**
	 * @var    string	The event to trigger after deleting the data.
	 */
	protected $event_after_delete = 'onIssueAfterDelete';	
	/**
	 * @var    string	The event to trigger after changing the data's state field.
	 */
	protected $event_change_state = 'onIssueChangeState';	
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_mmanager.issue');
			}	
		}
		...
		if ($assoc)
		{
			...
			if ($item->id != null)
			{
				$associations = JLanguageAssociations::getAssociations('com_mmanager', '#__mmanager_issues', 'com_mmanager.issue.item', $item->id, 'id', 'alias', null);
	...
	{
		$form = $this->loadForm('com_mmanager.edit.issue', 'issue', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
		...
		// Prevent messing with Issue language and category when editing existing Issue with associations
	...
	protected function loadFormData()
	{
		...
		$data = $app->getUserState('com_mmanager.edit.issue.data', array());
	...
	protected function prepareTable($table)
	{
		...
		// Increment the issue version number.
		$table->version++;
		...
		if (parent::save($data))
		{
			...
			if ($assoc)
			{
				...
				try
				{
					...
					$query->where('context='.$db->quote('com_mmanager.issue.item'));
				...
				if (!$all_language AND count($associations))
				{
					...
					{
						$query->values($id.','.$db->quote('com_mmanager.issue.item') . ',' . $db->quote($key));
					}
	...
	/**
	 * Method to toggle the featured setting of issues.
	 *
	...
	public function featured($pks, $value = 0)
	{
		...
		if (empty($pks))
		{
			$this->setError(JText::_('COM_MMANAGER_ISSUES_NO_ITEM_SELECTED'));
			return false;
		}
		...
		try
		{
			...
			$query = $db->getQuery(true);
			$query->update($db->quoteName('#__mmanager_issues'));
	...
	* @param   array   $data	The data where the original name is stored
	...
	/**
	 * Pre process the form to pick up issues associated by language
	 *
	 ...
	 */
	protected function preprocessForm(JForm $form, $data, $group = 'issue')
	{
		...
		if ($assoc)
		{
			...
			$fieldset->addAttribute('description', 'COM_MMANAGER_ISSUES_ITEM_ASSOCIATIONS_FIELDSET_DESC');
			$add = false;
			foreach ($languages as $tag => $language)
			{
				if (empty($data->language) OR $tag != $data->language)
				{
					...
					$field->addAttribute('type', 'modal_issues');
