 ...
 *
 */
class MmanagerModelEdition extends JModelAdmin
{
	/**
	 * @var		string	The prefix to use with controller messages.
	 */
	protected $text_prefix = 'COM_MMANAGER_EDITIONS';
	/**
	 * @var      string	The type alias for this object (for example, 'com_mmanager.edition')
	 */
	public $typeAlias = 'com_mmanager.edition';	
	/**
	 ...
	 */
	protected $event_before_save = 'onEditionBeforeSave';
	/**
	 * @var		string	The event to trigger after saving the data.
	 */
	protected $event_after_save = 'onEditionAfterSave';

	/**
	 * @var    string	The event to trigger before deleting the data.
	 */
	protected $event_before_delete = 'onEditionBeforeDelete';	
	/**
	 * @var    string	The event to trigger after deleting the data.
	 */
	protected $event_after_delete = 'onEditionAfterDelete';	
	/**
	 * @var    string	The event to trigger after changing the data's state field.
	 */
	protected $event_change_state = 'onEditionChangeState';	
	...
	protected function canDelete($record)
	{
		...
		{
			return $user->authorise('core.delete', 'com_mmanager');
		}							
	}
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_mmanager.edition');
			}	
		}
		...
		if ($assoc)
		{
			...
			if ($item->id != null)
			{
				$associations = JLanguageAssociations::getAssociations('com_mmanager', '#__mmanager_editions', 'com_mmanager.edition.item', $item->id, 'id', 'alias', 'catid');
	...
	{
		$form = $this->loadForm('com_mmanager.edit.edition', 'edition', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
		...
		if ($this->getState('edition.id'))
		{
			$id = $this->getState('edition.id');		
			// Existing record. Can only edit in selected categories.
			$form->setFieldAttribute('catid', 'action', 'core.edit');
			// Existing record. Can only edit own editions in selected categories.
		...
		// Prevent messing with Edition language and category when editing existing Edition with associations
	...
	protected function loadFormData()
	{
		...
		$data = $app->getUserState('com_mmanager.edit.edition.data', array());

		if (empty($data))
		{
			$data = $this->getItem();
			// Prime some default values.
			if ($this->getState('edition.id') == 0)
			{
				$filters = (array) $app->getUserState('com_mmanager.editions.filter');
	...
	protected function prepareTable($table)
	{
		...
		// Increment the edition version number.
		$table->version++;
	...
	{
		// Include the mmanager plugins for the onSave events.
		...
		if (parent::save($data))
		{
			...
			if ($assoc)
			{
				...
				{
					JError::raiseNotice(403, JText::_('COM_MMANAGER_ERROR_ALL_LANGUAGE_ASSOCIATED'));
				}
				...
				try
				{
					...
					$query->where('context='.$db->quote('com_mmanager.edition.item'));
				...
				if (!$all_language AND count($associations))
				{
					...
					{
						$query->values($id.','.$db->quote('com_mmanager.edition.item') . ',' . $db->quote($key));
					}
	...
	{
		// Include the mmanager plugins for the delete events.
		JPluginHelper::importPlugin('mmanager');	
	...
	/**
	 * Method to toggle the featured setting of editions.
	 *
	...
	public function featured($pks, $value = 0)
	{
		...
		if (empty($pks))
		{
			$this->setError(JText::_('COM_MMANAGER_EDITIONS_NO_ITEM_SELECTED'));
			return false;
		}
		...
		try
		{
			...
			$query = $db->getQuery(true);
			$query->update($db->quoteName('#__mmanager_editions'));
	...
	/**
	 * Custom clean the cache of com_mmanager and mmanager modules
	 *
	 */
	protected function cleanCache($group = null, $client_id = 0)
	{
		parent::cleanCache('com_mmanager');
	...
	* @param   array   $data	The data where the original name is stored
	...
	/**
	 * Pre process the form to pick up editions associated by language
	 *
	 ...
	 */
	protected function preprocessForm(JForm $form, $data, $group = 'edition')
	{
		...
		if ($assoc)
		{
			...
			$fieldset->addAttribute('description', 'COM_MMANAGER_EDITIONS_ITEM_ASSOCIATIONS_FIELDSET_DESC');
			$add = false;
			foreach ($languages as $tag => $language)
			{
				if (empty($data->language) OR $tag != $data->language)
				{
					...
					$field->addAttribute('type', 'modal_editions');
