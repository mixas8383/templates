 ...
 *
 */
class MmanagerHelper extends JHelperContent
{
	...
	{
		$config = JComponentHelper::getParams(JText::_('COM_MMANAGER_FIELD_CATEGORY_COMPONENT_DEFAULT'));
		$category_component = $config->get('category_component', JText::_('COM_MMANAGER_FIELD_CATEGORY_COMPONENT_DEFAULT'));	

		$active = $view_name == 'editions'? true : false;
		JHtmlSidebar::addEntry(
			JText::_('COM_MMANAGER_EDITIONS_SUBMENU'),
			'index.php?option=com_mmanager&view=editions',
			$view_name == 'editions',
			$active
		);
	
		if ($category_component != JText::_('COM_MMANAGER_FIELD_CATEGORY_COMPONENT_NO_CATEGORIES'))
		{	
			$active = $view_name == 'categories'? true : false;
			
			JHtmlSidebar::addEntry(
				JText::_('COM_MMANAGER_CATEGORIES_SUBMENU'),
