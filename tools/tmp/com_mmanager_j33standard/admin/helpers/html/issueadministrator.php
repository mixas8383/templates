...
/**
 * Issues component helper.
 *
 */
abstract class JHtmlIssueAdministrator
{
	/**
	 * Render the list of associated items
	 * 
	 * @param	integer $issue_id	The issue item id
	 * 
	 * @return  string  The language HTML
     */
	public static function association($issue_id)
	{
		...
		// Get the associations
		if ($associations = JLanguageAssociations::getAssociations('com_mmanager', '#__mmanager_issues', 'com_mmanager.issue.item',  $issue_id, 'id', 'alias', null))
		{		
			...
			if ($items)
			{
				foreach ($items as &$item)
				{
					...
					$url = JRoute::_('index.php?option=com_mmanager&task=issue.edit&id=' . (int) $item->id);
					$tooltip_parts = array(
	...
	public static function featured($value = 0, $i, $can_change = true)
	{
		...
		$states	= array(
			0	=> array('unfeatured', 'issues.featured', 'COM_MMANAGER_ISSUES_UNFEATURED', 'COM_MMANAGER_ISSUES_TOGGLE_TO_FEATURE'),
			1	=> array('featured', 'issues.unfeatured', 'JFEATURED', 'COM_MMANAGER_ISSUES_TOGGLE_TO_UNFEATURE'),
		);
