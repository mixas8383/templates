...
JLoader::register('ContentHelper', JPATH_ADMINISTRATOR . '/components/com_mmanager/helpers/mmanager.php');
/**
 * Editions component helper.
 *
 */
abstract class JHtmlEditionAdministrator
{
	/**
	 * Render the list of associated items
	 * 
	 * @param	integer $edition_id	The edition item id
	 * 
	 * @return  string  The language HTML
     */
	public static function association($edition_id)
	{
		...
		// Get the associations
		if ($associations = JLanguageAssociations::getAssociations('com_mmanager', '#__mmanager_editions', 'com_mmanager.edition.item',  $edition_id, 'id', 'alias', 'catid'))
		{		
			...
			if ($items)
			{
				foreach ($items as &$item)
				{
					...
					$url = JRoute::_('index.php?option=com_mmanager&task=edition.edit&id=' . (int) $item->id);
					$tooltip_parts = array(
	...
	public static function featured($value = 0, $i, $can_change = true)
	{
		...
		$states	= array(
			0	=> array('unfeatured', 'editions.featured', 'COM_MMANAGER_EDITIONS_UNFEATURED', 'COM_MMANAGER_EDITIONS_TOGGLE_TO_FEATURE'),
			1	=> array('featured', 'editions.unfeatured', 'JFEATURED', 'COM_MMANAGER_EDITIONS_TOGGLE_TO_UNFEATURE'),
		);
