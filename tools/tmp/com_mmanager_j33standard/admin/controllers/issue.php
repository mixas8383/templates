...
class MmanagerControllerIssue extends JControllerForm
{
	...
	protected function allowAdd($data = array())
	{
		...
		$result = $user->authorise('core.create', 'com_mmanager');
		if($result === null)
		{
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		if ($user->authorise('core.edit', 'com_mmanager.issue.'.$record_id))
		{
			return true;
		}
		// Fallback on edit.own.
		// First test if the permission is available.
		if ($user->authorise('core.edit.own', 'com_mmanager.issue.'.$record_id))
		{
	...
	public function batch($model = null)
	{
		...
		// Set the model
		$model = $this->getModel('Issue', 'MmanagerModel', array());

		// Preset the redirect
		$this->setRedirect(JRoute::_('index.php?option=com_mmanager&view=issues' . $this->getRedirectToListAppend(), false));
