...
class MmanagerControllerEdition extends JControllerForm
{
	...
	protected function allowAdd($data = array())
	{
		...
		if ($category_id) 
		{
			...
			$result = $user->authorise('core.create', 'com_mmanager.category.'.$category_id) AND
					  $user->authorise('core.create', 'com_mmanager');
			if($result === null)		
			{
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		if ($category_id) 
		{
			...
			if(!$user->authorise('core.edit', 'com_mmanager.category.'.$category_id))
			{
				return false;
			}
		}
		// Check general edit permission first.
		if ($user->authorise('core.edit', 'com_mmanager.edition.'.$record_id))
		{
			return true;
		}
		// Fallback on edit.own.
		// First test if the permission is available.
		if ($user->authorise('core.edit.own', 'com_mmanager.edition.'.$record_id))
		{
	...
	public function batch($model = null)
	{
		...
		// Set the model
		$model = $this->getModel('Edition', 'MmanagerModel', array());

		// Preset the redirect
		$this->setRedirect(JRoute::_('index.php?option=com_mmanager&view=editions' . $this->getRedirectToListAppend(), false));
