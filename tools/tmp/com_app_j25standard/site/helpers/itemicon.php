...
class JHTMLItemIcon
{
	...
	static function create($params)
	{
		...
		$url = 'index.php?option=com_app&task=item.add&layout=edit&return='.base64_encode(urlencode($uri));
		...
		{
			$text = JText::_('COM_APP_ITEMS_CREATE_ITEM').'&#160;';
		}

		$button =  JHtml::_('link', JRoute::_($url), $text);

		$output = '<span class="hasTip" title="'.JText::_('COM_APP_ITEMS_CREATE_ITEM').'">'.$button.'</span>';
		return $output;
	}
	/**
	...
	static function edit($item, $params, $attribs = array())
	{
		...
		$url	= 'index.php?option=com_app&view=itemform&task=item.edit&layout=edit&id='.$item->id.'&return='.base64_encode(urlencode($uri));
		if ($params->get('show_item_icons'))
		{
					$icon	= $item->state ? 'com_app/edit.png' : 'com_app/edit_unpublished.png';
			$text	= JHtml::_('image',$icon, JText::_('COM_APP_ITEMS_EDIT_ITEM'), NULL, true);
		}
		else
		{
		...
		$overlib .= $date;
		$overlib .= '&lt;br /&gt;';
		$overlib .= JText::sprintf('COM_APP_CREATED_BY', htmlspecialchars($creator, ENT_COMPAT, 'UTF-8'));
		$button = JHtml::_('link',JRoute::_($url), $text);

		$output = '<span class="hasTip" title="'.JText::_('COM_APP_ITEMS_EDIT_ITEM').' :: '.$overlib.'">'.$button.'</span>';
	...
	static function delete($item, $params, $attribs = array())
	{
		...
		$url = 'index.php?option=com_app&task=item.delete&cid='.$item->id.'&'.JSession::getFormToken().'=1'.'&return='.base64_encode(urlencode($uri));

		if ($params->get('show_item_icons'))
		{
			$icon	= 'com_app/delete.png';
			$text	= JHtml::_('image',$icon, JText::_('COM_APP_ITEMS_DELETE_ITEM'), NULL, true);
		}
		else
		{
		...
		$output = '<span class="hasTip" title="'.JText::_('COM_APP_ITEMS_DELETE_ITEM').' :: '.$overlib.'">'.$button.'</span>';
