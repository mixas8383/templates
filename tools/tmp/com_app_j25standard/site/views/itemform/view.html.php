 ...
 *
 */
class AppViewItemForm extends JView
{
	protected $state;
	protected $item;
	...
	public function display($tpl = null)
	{
		...
		if (empty($item->id))
		{
			$authorised = $user->authorise('core.create', 'com_app') 
			OR (count($user->getAuthorisedCategories('com_app', 'core.create')))
			;
		}
		else
		{
	...
	protected function _prepareDocument()
	{
		...
		{
			$this->params->def('page_heading', JText::_('COM_APP_ITEMS_EDIT_ITEM'));
		}

		if (!is_null($this->item->id))
		{
			$title = $this->params->def('page_title', JText::_('COM_APP_ITEMS_EDIT_ITEM'));
			
			$pathway->addItem(JText::sprintf('COM_APP_EDIT_ITEM', $this->escape($this->item->name)),''); 
		}
		else
		{
			$title = $this->params->def('page_title', JText::_('COM_APP_ITEMS_CREATE_ITEM'));
			
			$pathway->addItem(JText::_('COM_APP_ITEMS_CREATE_ITEM'),'');
		}
		...
		if (!empty($this->item->page_title))
		{
			...
			$this->document->setTitle($item->page_title.' - '.JText::sprintf('COM_APP_PAGEBREAK_PAGE_NUM', $this->state->get('page.offset') + 1));
		}
		...
		$this->document->addScript(JUri::root() ."components/com_app/assets/js/appvalidate.js");
		
		$this->document->addScript(JUri::root() ."components/com_app/assets/js/formsubmitbutton.js");

		JText::script('COM_APP_ERROR_ON_FORM');		// Include Helpers
