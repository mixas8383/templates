...
/**
 *
 * Team vote plugin class.
 */
class PlgTeamVote extends JPlugin
{
	/**
	...
	protected function OutputRating($context, &$row, &$params, $page=0, $object)
	{
		...
		if ($object <> '')
		{
			...
			$star_image_on = JHtml::_('image', 'system/rating_star.png', JText::_('PLG_TEAM_VOTE_STAR_ACTIVE'), null, true);
			$star_image_off = JHtml::_('image', 'system/rating_star_blank.png', JText::_('PLG_TEAM_VOTE_STAR_INACTIVE'), null, true);
			...
			$html .= '<p class="unseen element-invisible">'.JText::sprintf( 'PLG_TEAM_VOTE_USER_RATING', '<span itemprop="ratingValue">'.$rating.'</span>', '<span itemprop="bestRating">5</span>' )
			...
			if ( ($view == $object) AND isset($row->state) AND $row->state == 1)
			{
				...
				for($i = 1; $i < 6; $i++)
				{
					$options[] = JHtml::_('select.option', $i, JText::sprintf('PLG_TEAM_VOTE_VOTE', $i));
				}
				...
				$html .= '<label class="unseen element-invisible" for="'.$object.'_vote_' . $row->id . '">'.JText::_('PLG_TEAM_VOTE_LABEL').'</label>';
				$html .= JHtml::_('select.genericlist', $options, 'user_rating', null, 'value', 'text', '5', $object.'_vote_'.$row->id);
				$html .= '&#160;<input class="btn btn-mini" type="submit" name="submit_vote" value="' . JText::_('PLG_TEAM_VOTE_RATE') . '" />';
