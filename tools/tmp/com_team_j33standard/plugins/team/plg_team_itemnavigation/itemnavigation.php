 ...
 * @author			 ()
 * @package			com_team
 * @subpackage		com_team.itemnavigation
 * @copyright		
...
/**
 * Team navigation plugin class.
 *
 */
class PlgTeamItemnavigation extends JPlugin
{
	/**
	...
	public function onItemBeforeDisplay ($context, &$row, &$params, $page=0)
	{
			...
			($view == 'item')) 
		{
			...
			if ($row->prev) 
			{
				$row->prev = JRoute::_(TeamHelperRoute::getItemRoute($row->prev->slug,$row->prev->catslug, $row->prev->language, $layout, $keep_item_id));
			} 
			else 
			{
				$row->prev = '';
			...
			if ($row->next) 
			{
				$row->next = JRoute::_(TeamHelperRoute::getItemRoute($row->next->slug,$row->next->catslug, $row->next->language, $layout, $keep_item_id));
