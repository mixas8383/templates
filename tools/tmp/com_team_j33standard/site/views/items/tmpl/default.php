...
$this->document->addStyleSheet(JUri::root().'media/com_team/css/site_items.css');

if ($lang->isRTL())
{
	$this->document->addStyleSheet(JUri::root().'media/com_team/css/site_team-rtl.css');
	$this->document->addStyleSheet(JUri::root().'media/com_team/css/site_items-rtl.css');
}
...
$can_create	= $user->authorise('core.create', 'com_team');
...
<div class="team items-list<?php echo $this->params->get('pageclass_sfx');?>">
	...
	<form action="<?php echo JFilterOutput::ampReplace(JUri::getInstance()->toString()); ?>" method="post" name="adminForm" id="adminForm">
		<?php if (($this->params->get('show_item_filter_field') != '' AND $this->params->get('show_item_filter_field') != 'hide')) :?>
			<div class="filter-search">
				<?php if ($this->params->get('show_item_filter_field') != '' AND $this->params->get('show_item_filter_field') != 'hide') :?>
					<input type="text" name="filter_search" id="filter_search" value="<?php echo $this->escape($this->state->get('filter.search')); ?>" onchange="document.adminForm.submit();" title="<?php echo JText::_('COM_TEAM_FILTER_SEARCH_DESC'); ?>" placeholder="<?php echo JText::_('COM_TEAM_'.$this->params->get('show_item_filter_field').'_FILTER_LABEL'); ?>" />
				<?php endif; ?>	
		...
		<?php if (empty($this->items)) : ?>
			...
			<p><?php echo JText::_('COM_TEAM_ITEMS_NO_ITEMS'); ?></p>
			<?php endif; ?>
		...
		<?php else : ?>
			...
			<thead>
				<tr>
					...
					<?php  echo JHtml::_('grid.sort', 'COM_TEAM_HEADING_NAME', 'a.name', $list_dirn, $list_order) ; ?>
					</th>
					<?php if ($date = $this->params->get('list_show_item_date')) : ?>
						<th class="list-date" id="tableOrderingdate">
							<?php echo JHtml::_('grid.sort', 'COM_TEAM_FIELD_'.JString::strtoupper($date).'_LABEL', 'a.'.$date, $list_dirn, $list_order); ?>
						</th>
					<?php endif; ?>

					<?php if ($this->params->get('list_show_item_created_by',0)) : ?>
						<th class="list-created_by" id="tableOrderingcreated_by">
							<?php echo JHtml::_('grid.sort', 'COM_TEAM_HEADING_CREATED_BY', 'created_by_name', $list_dirn, $list_order); ?>
						</th>
					<?php endif; ?>
					<?php if ($this->params->get('list_show_item_hits',0)) : ?>
						<th class="list-hits" id="tableOrderinghits">
						<?php echo JHtml::_('grid.sort', 'COM_TEAM_HEADING_HITS', 'a.hits', $list_dirn, $list_order); ?>
						</th>
					<?php endif; ?>
					<?php if ($this->params->get('list_show_item_ordering',0)) : ?>
						<th width="10%">
							<?php echo JHtml::_('grid.sort',  'COM_TEAM_HEADING_ORDERING', 'a.ordering', $list_dirn, $list_order); ?>
						</th>
					<?php endif; ?>	
					<?php if ($show_actions) : ?>
						<th width="12%" class="list-actions">
							<?php echo JText::_('COM_TEAM_HEADING_ACTIONS'); ?>						
						</th> 					
					<?php endif; ?>
				</tr>
			</thead>
			...
			<tbody>
				...
				?>			
					...
					<td class="list-name">
						<a href="<?php echo JRoute::_(TeamHelperRoute::getItemRoute($item->slug, 
																									$item->catid, 
																									$item->language,
																									$layout,									
							...
							<?php 
								...
								if (!empty($item->created_by )) :
									...
									if ($this->params->get('show_item_headings')) :
										...
										echo JText::sprintf('COM_TEAM_CREATED_BY', $created_by);
									endif;
								else:
									echo $empty;
								endif;								
