...
<h2 itemprop="name">
		<?php if ($params->get('link_item_names') ) : ?>
			<a href="<?php echo JRoute::_(TeamHelperRoute::getItemRoute($this->item->slug, 
											$this->item->catid, 
											$this->item->language,
											$layout,									
...
<?php else : ?>
	...
	<?php if ($params->get('show_item_readmore')) :
			if ($params->get('access-view')) :
				$link = JRoute::_(TeamHelperRoute::getItemRoute($this->item->slug, 
																							$this->item->catid, 
																							$this->item->language,
																							$layout,									
				...
				if ($params->get('show_item_noauth') AND $user->get('guest')) :
					...
					$return_url = JRoute::_(TeamHelperRoute::getItemRoute($this->item->slug, 
																								$this->item->catid, 
																								$this->item->language,									
																								$layout,									
																								$params->get('keep_item_itemid')));
				...
				if ($item_params->get('item_alternative_readmore') == null) :
						if ($params->get('access-view')) :
							if ($params->get('show_item_readmore_name') == 0) :
								echo JText::sprintf('COM_TEAM_READ_MORE');
							else :
								echo JText::_('COM_TEAM_READMORE_NAME');
