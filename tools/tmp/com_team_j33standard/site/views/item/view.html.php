...
/**
 * HTML Item View class for the Team component
 *
 */
class TeamViewItem extends JViewLegacy
{
	protected $item;
	protected $params;
	protected $print;
	...
	public function display($tpl = null)
	{
		...
		$item->readmore_link = JRoute::_(TeamHelperRoute::getItemRoute($item->slug, 
										$item->catid, 
										$item->language,
										$this->getLayout(),									
		...
		if ($item->params->get('access-view') != true)
		{
			...
			if (!$item->params->get('show_item_noauth'))
			{
				...
				if( $user->get('guest'))
				{
					...
					$app->redirect('index.php?option=com_users&view=login&return=' . base64_encode($uri), JText::_('COM_TEAM_ITEMS_ERROR_LOGIN_TO_VIEW_ITEM'));
		...
		//
		// Process the team plugins.
		//
		JPluginHelper::importPlugin('team');
		$results = $dispatcher->trigger('onItemPrepare', array ('com_team.item', &$item, &$this->params, $offset));

		$item->event = new stdClass;
		$results = $dispatcher->trigger('onItemAfterName', array('com_team.item', &$item, &$this->params, $offset));
		$item->event->afterDisplayItemName = JString::trim(implode("\n", $results));
		
		$results = $dispatcher->trigger('onItemBeforeDisplay', array('com_team.item', &$item, &$this->params, $offset));
		$item->event->beforeDisplayItem = JString::trim(implode("\n", $results));

		$results = $dispatcher->trigger('onItemAfterDisplay', array('com_team.item', &$item, &$this->params, $offset));
	...
	protected function prepareDocument()
	{
		...
		if ($menu AND ($menu->query['option'] != 'com_team' OR $menu->query['view'] != 'item' OR $id != $this->item->id))
		{
			...
			{			
				...
				$options['table'] = '#__team_items';					
				$category = JCategories::getInstance('Team', $options)->get($this->item->catid);
				while ($category AND ($menu->query['option'] != 'com_team' OR $menu->query['view'] == 'item' OR $id != $category->id) AND $category->id > 1)
				{
					$path[] = array('title' => $category->title, 'link' => TeamHelperRoute::getCategoryRoute($category->id, $this->params->get('keep_item_itemid')));
		...
		if (!empty($this->item->page_title))
		{
			...
			$this->document->setTitle($this->item->page_title . ' - ' . JText::sprintf('COM_TEAM_PAGEBREAK_PAGE_NUM', $this->state->get('list.offset') + 1));
		}
