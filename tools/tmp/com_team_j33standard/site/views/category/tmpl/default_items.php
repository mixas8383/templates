...
<?php if (empty($this->items)) : ?>
	...
	<p><?php echo JText::_('COM_TEAM_ITEMS_CATEGORY_NO_ITEMS'); ?></p>
	<?php endif; ?>
		...
		<?php if (($this->params->get('show_item_filter_field') != '' AND $this->params->get('show_item_filter_field') != 'hide') OR $this->params->get('show_item_pagination_limit')) :?>
			<div class="filter-search">
				<?php if ($this->params->get('show_item_filter_field') != '' AND $this->params->get('show_item_filter_field') != 'hide') :?>
					<input type="text" name="filter-search" id="filter-search" value="<?php echo $this->escape($this->state->get('filter.search')); ?>" onchange="document.adminForm.submit();" title="<?php echo JText::_('COM_TEAM_FILTER_SEARCH_DESC'); ?>" placeholder="<?php echo JText::_('COM_TEAM_'.$this->params->get('show_item_filter_field').'_FILTER_LABEL'); ?>" />
				<?php endif; ?>						
				...
				<?php endif; ?>
					<?php if (in_array($item->access, $this->user->getAuthorisedViewLevels()) OR $item->params->get('show_item_noauth')) : ?>
						...
						<td class="createdby">
							...
							<?php if ($this->params->get('show_item_headings')) :?>
								<?php echo $creator; ?>
							<?php else : ?>									
								<?php echo JText::sprintf('COM_TEAM_CREATED_BY', $creator); ?>
							<?php endif; ?>
						</td>
						<?php endif; ?>
						<?php if ($this->params->get('list_show_item_hits',0)) : ?>
					...
					<?php else : // Show unauth links. ?>
						<td>
							...
							$return_url = JRoute::_(TeamHelperRoute::getItemRoute($item->slug, 
																									$item->catid, 
																									$item->language,
																									$layout,									
																									$this->params->get('keep_item_itemid')));
							...
							<a href="<?php echo $full_url; ?>" class="register">
								<?php echo JText::_( 'COM_TEAM_REGISTER_TO_READ_MORE' ); ?></a>
						</td>
					<?php endif; ?>
					<?php if ($show_actions) : ?>
