...
<ul>
	<?php foreach($this->children[$this->parent->id] as $id => $child) : ?>
		...
		if (($this->params->get('show_empty_categories') OR $child->getNumItems(true)  OR count($child->getChildren())) AND $this->max_level_categories > 1) :
			...
			<li <?php echo $class; ?>>
			<?php $class = ''; ?>
				<span class="item-title"><a href="<?php echo JRoute::_(TeamHelperRoute::getCategoryRoute($child->id, $this->params->get('keep_item_itemid'), $child->language));?>">
					<?php echo $this->escape($child->title); ?></a>
				</span>
				<?php if ($this->params->get('show_subcategories_desc') == 1) :?>
					<?php if ($child->description) : ?>
						<div class="category-desc">
							<?php echo JHtml::_('content.prepare', $child->description, '', 'com_team.categories'); ?>
						</div>
					<?php endif; ?>
				<?php endif; ?>
				<?php if ($this->params->get('items_to_display') AND 
					...
					<span class="badge badge-info">
						<?php echo JText::_('COM_TEAM_'.JString::strtoupper(str_replace(' ','',$this->params->get('items_to_display'))).'_NUM_ITEMS') ; ?>
