...
/**
 * Routing class for the Team component
 *
 */
class TeamRouter extends JComponentRouterBase
{
	/**
	 * Build the route for the Team component
	 *
	...
	public function parse(&$segments)
	{
		...
		{
			$categories = JCategories::getInstance('Team',$options)->get($id)->getChildren();
		}
		else
		{
			$categories = JCategories::getInstance('Team',$options);	
			...
			if ($found == 0)
			{
					...
					OR $view == 'items' OR $view == 'item')
				{
					if($advanced)
					{
						...
						$query->from($db->quoteName('#__team_items'));
