...
require_once JPATH_ADMINISTRATOR.'/components/com_team/models/item.php';

class TeamModelItemForm extends TeamModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_team.edit.item';
	/**
	...
	*/
	...
	public function getItem($item_id = null)
	{
		...
		else
		{
			...
			if ($cat_id)
			{
				$item->params->set('access-change', $user->authorise('core.edit.state', 'com_team.category.'.$cat_id));
				$item->catid = $cat_id;
			}
			else 
			{
				$item->params->set('access-change', $user->authorise('core.edit.state', 'com_team'));
			}
		}
	...
	protected function setAccessFilters(&$form, $data)
	{
		...
		if (!$user->authorise('core.edit.state', 'com_team'))
		{
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_team.edit.item.data', array());
	...
	public function save($data)
	{
		...
		else
		{
			...
			$actions = JAccess::getActions('com_team');
			$action_array = array();
		...
		$result = $dispatcher->trigger('onItemBeforeSave', array('com_team.item', &$table, $is_new));
		...
		// Clean the cache.
		$cache = JFactory::getCache('com_team');
		$cache->clean();

		$dispatcher->trigger('onItemAfterSave', array('com_team.item', $table, $is_new));
