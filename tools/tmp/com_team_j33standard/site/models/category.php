...
class TeamModelCategory extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		{
			$limit = $app->getUserStateFromRequest('com_team.category.list.' . $item_id . '.limit', 'limit', $app->get('list_limit'));
		}
	...
	public function getItems()
	{
		...
		if ($params->get('items_to_display') AND $params->get('items_to_display') !='')
		{
			...
			{
				$model = JModelLegacy::getInstance($object_upper_case, 'TeamModel', array('ignore_request' => true));
	...
	protected function buildOrderBy()
	{
		...
		$item_id	= $app->input->getInt('id', 0) . ':' . $app->input->getInt('Itemid', 0);
		$order_col	= $app->getUserStateFromRequest('com_team.category.list.' . $item_id . '.filter_order', 'filter_order', '', 'string');
		$order_dirn	= $app->getUserStateFromRequest('com_team.category.list.' . $item_id . '.filter_order_Dir', 'filter_order_Dir', '', 'cmd');
		$order_by	= ' ';
		...
		switch ($params->get('items_to_display',''))
		{
			case 'Items' :
				...
				$primary			= TeamHelperQuery::orderbyPrimary($category_order_by);	
				$secondary			= TeamHelperQuery::orderbySecondary($secondary_order_by, $order_date, 'ordering') . ', ';
	...
	public function getCategory()
	{
		if(!is_object($this->_item))
		{
			...
			if ($params->get('items_to_display') AND $params->get('items_to_display') !='')
			{
				$options['table'] = '#__team_'.JString::strtolower(str_replace(' ','',$params->get('items_to_display')));
			}
			else
			{
				$options['table'] = '';
		...
		$this->tags->getItemTags('com_team.category', $this->_item->get('id'));
		return $this->_item;
	}
