...
/**
 * Team Component Item Model
 *
 */
class TeamModelItem extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_team.item';
	...
	protected function canDelete($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.delete', 'com_team.item.'.(int) $record->id)
					  OR ($user->authorise('core.delete', 'com_team.item.'.(int) $record->id)
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.edit.state', 'com_team.item.'.(int) $record->id);
		}
		else
		{
	...
	protected function populateState()
	{
		...
		if ((!$user->authorise('core.edit.state', 'com_team')) AND  (!$user->authorise('core.edit', 'com_team')))
		{
	...
	*/
	public function getTable($type = 'Items', $prefix = 'TeamTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		...
		$global_params = JComponentHelper::getParams('com_team', true);
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				$sub_query .= 'WHERE '.$db->quoteName('parent.extension').' = ' . $db->quote('com_team');
				...
				$query->join('LEFT', $db->quoteName('#__team_rating').' AS v ON '.$db->quoteName('a.id').' = '.$db->quoteName('v.content_id').' AND '.$db->quoteName('v.content_type').' = '.$db->quote('items'));

				$can_publish = $user->authorise('core.edit.state', 'com_team.item.'.$pk);
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_TEAM_ITEMS_ERROR_ITEM_NOT_FOUND'));
				}
				...
				$item->tags->getItemTags('com_team.item', $item->id);
	...
	public function publish(&$pks, $value = 1)
	{
		...
		// Include the team plugins for the change of state event.
		...
		$result = $dispatcher->trigger('onItemChangeState', array('com_team.item', $pks, $value));
	...
	*/
	...
	public function delete(&$pks)
	{
		...
		// Include the team plugins for the on delete events.
		JPluginHelper::importPlugin('team');
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onItemBeforeDelete', array('com_team.item', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onItemAfterDelete', array('com_team.item', &$table));
				}
				else
				{
					// Prune items that you can't change.
    ...
    public function storeVote($pk = 0, $rate = 0)
    {
        if ( $rate >= 1 AND $rate <= 5 AND $pk > 0 )
        {
            ...
            $db->setQuery(
                    'SELECT *' .
                    ' FROM '.$db->quoteName('#__team_rating') .
            ...
            if (!$rating)
            {
                ...
                $db->setQuery(
                        'INSERT INTO '.$db->quoteName('#__team_rating').' ( '.$db->quoteName('content_type').', '.$db->quoteName('content_id').', '.$db->quoteName('lastip').', '.$db->quoteName('rating_sum').', '.$db->quoteName('rating_count').' )' .
            ...
            else
            {
                ...
                {
                    $db->setQuery(
                            'UPDATE '.$db->quoteName('#__team_rating') .
        ...
        JError::raiseWarning( 'SOME_ERROR_CODE', JText::sprintf('COM_TEAM_ITEMS_INVALID_RATING', $rate), "TeamModelItem::storeVote($rate)");
        return false;
    }
}
