...
abstract class TeamHelperRoute
{
	...
	public static function getItemRoute($id, $cat_id = 0, $language = '*', $layout = 'default', $keep_item_id = false)
	{
		...
		{
			//Create the link
			$link = 'index.php?option=com_team&view=item&id='. $id;
		}
		else
		{
			//Create the link with a layout
			$link = 'index.php?option=com_team&view=item&layout='.$layout.'&id='. $id;
		}
	...
	public static function getCategoryRoute($cat_id, $keep_item_id = false, $language = 0)
	{
		...
		else
		{
			...
			$options['countItems'] = false;
			$options['table'] = '';			
			$category = JCategories::getInstance('Team', $options)->get($id);
		}
	...
	protected static function findItem($needles = null, $keep_item_id = false, $layout = 'default')
	{
		...
		if ($active AND $active->component == 'com_team' AND ($language == '*' OR in_array($active->language, array('*', $language)) OR !JLanguageMultilang::isEnabled()))
		{
			return $active->id;
		}
