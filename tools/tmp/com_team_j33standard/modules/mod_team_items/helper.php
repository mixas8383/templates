...
JModelLegacy::addIncludePath(JPATH_SITE.'/components/com_team/models', 'TeamModelItems');
...
abstract class ModItemsHelper
{
	...
	public static function getList(&$params)
	{
		...
		$model = JModelLegacy::getInstance('Items', 'TeamModel', array('ignore_request' => true));
		...
		// Access filter
		$access = !JComponentHelper::getParams('com_team')->get('show_item_noauth');
		...
		foreach ($items as &$item)
		{
			...
			if ($access OR in_array($item->access, $authorised))
			{
				if ($item_id_str == '')
				{
					...
					$item->link = JRoute::_(TeamHelperRoute::getItemRoute($item->slug, $item->catslug, $item->language, $layout));
				}
				else
				{
					$item->link = JRoute::_('index.php?option=com_team&view=item'.$layout_str.$item_id_str.'&id='.$item->id);
				}					
			}
			else
			{
