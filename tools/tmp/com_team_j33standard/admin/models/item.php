 ...
 *
 */
class TeamModelItem extends JModelAdmin
{
	/**
	...
	/**
	 * @var      string	The type alias for this object (for example, 'com_team.item')
	 */
	public $typeAlias = 'com_team.item';	
	/**
	 * @var		string	The context for the app call.
	 */
	protected $context = 'com_team.items';	
	/**
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_team.item');
			}	
		}
		...
		if ($assoc)
		{
			...
			if ($item->id != null)
			{
				$associations = JLanguageAssociations::getAssociations('com_team', '#__team_items', 'com_team.item.item', $item->id, 'id', 'alias', 'catid');
	...
	{
		$form = $this->loadForm('com_team.edit.item', 'item', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = $app->getUserState('com_team.edit.item.data', array());
		...
		if (empty($data))
		{
			...
			{
				$filters = (array) $app->getUserState('com_team.items.filter');
	...
	{
		// Include the team plugins for the onSave events.
		...
		if (parent::save($data))
		{
			...
			if ($assoc)
			{
				...
				try
				{
					...
					$query->where('context='.$db->quote('com_team.item.item'));
				...
				if (!$all_language AND count($associations))
				{
					...
					{
						$query->values($id.','.$db->quote('com_team.item.item') . ',' . $db->quote($key));
					}
	...
	{
		// Include the team plugins for the delete events.
		JPluginHelper::importPlugin('team');	
	...
	public function featured($pks, $value = 0)
	{
		...
		if (empty($pks))
		{
			$this->setError(JText::_('COM_TEAM_ITEMS_NO_ITEM_SELECTED'));
			return false;
		}
		...
		try
		{
			...
			$query = $db->getQuery(true);
			$query->update($db->quoteName('#__team_items'));
	...
	/**
	 * Custom clean the cache of com_team and team modules
	 *
	 */
	protected function cleanCache($group = null, $client_id = 0)
	{
		parent::cleanCache('com_team');
		parent::cleanCache('mod_team');
		parent::cleanCache('mod_team_items');
	...
	* @param   array   $data	The data where the original name is stored
	...
	protected function preprocessForm(JForm $form, $data, $group = 'item')
	{
		...
		if ($assoc)
		{
			...
			$fieldset->addAttribute('description', 'COM_TEAM_ITEMS_ITEM_ASSOCIATIONS_FIELDSET_DESC');
			$add = false;
