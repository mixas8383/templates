...
class SelectfileViewSizes extends JViewLegacy
{
	...
	public function display($tpl = null)
	{
		...
		// PREPARE THE DATA
		// Compute the size slugs and set the trigger events.
		foreach ($items as $i => &$item)
		{
			...
			$dispatcher->trigger('onSizePrepare', array ('com_selectfile.size', &$item, &$item->params,$i));

			$item->event = new stdClass;

			$results = $dispatcher->trigger('onSizeAfterName', array('com_selectfile.size', &$item, &$item->params, $i));
			$item->event->afterDisplaySizeName = JString::trim(implode("\n", $results));

			$results = $dispatcher->trigger('onSizeBeforeDisplay', array('com_selectfile.size', &$item, &$item->params, $i));
			$item->event->beforeDisplaySize = JString::trim(implode("\n", $results));

			$results = $dispatcher->trigger('onSizeAfterDisplay', array('com_selectfile.size', &$item, &$item->params,$i));
	...
	protected function prepareDocument()
	{
		...
		{
			$this->params->def('page_heading', JText::_('COM_SELECTFILE_SIZES'));
		}
