...
require_once JPATH_ADMINISTRATOR.'/components/com_selectfile/models/size.php';

class SelectfileModelSizeForm extends SelectfileModelSize
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_selectfile.edit.size';
	/**
	...
	*/
	 ...
	 *
	 * @param	integer	$item_id	The id of the size.
	 *
	 * @return	mixed	Size item data object on success, false on failure.
	 */
	public function getItem($item_id = null)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('size.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_selectfile.edit.size.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('size.id');
		$is_new		= true;
		...
		// Reorder the sizes so the new size is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onSizeBeforeSave', array('com_selectfile.size', &$table, $is_new));
		...
		// Clean the cache.
		$cache = JFactory::getCache('com_selectfile');
		$cache->clean();

		$dispatcher->trigger('onSizeAfterSave', array('com_selectfile.size', $table, $is_new));

		$this->setState('size.id', $table->id);
