...
if ($save_order)
{
	$save_ordering_url = 'index.php?option=com_selectfile&task=sizes.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'size-list', 'adminForm', strtolower($list_dirn), $save_ordering_url);
}
...
<form action="<?php echo JRoute::_('index.php?option=com_selectfile&view=sizes'); ?>" method="post" name="adminForm" id="adminForm">
	...
	<?php else : ?>
		<table class="table table-striped" id="size-list">
			<thead>
				<tr>
					...
					<th width="10%" class="nowrap center hidden-phone">
						<?php echo JHtml::_('grid.sort',  'COM_SELECTFILE_HEADING_CREATED_BY', 'a.created_by', $list_dirn, $list_order); ?>
					</th>
					<th width="10%" class="nowrap center hidden-phone">
						<?php echo JHtml::_('grid.sort', 'COM_SELECTFILE_HEADING_CREATED', 'a.created', $list_dirn, $list_order); ?>
					</th>		
			...
			<?php foreach ($this->items as $i => $item) :
				...
				<tr class="row<?php echo $i % 2; ?>">
					...
					<td class="center">
						<div class="btn-group">
							<?php echo JHtml::_('jgrid.published', $item->state, $i, 'sizes.', $can_change); ?>
							<?php
								if ($archived) :
									JHtml::_('actionsdropdown.unarchive', 'cb' . $i, 'sizes');
								else :
									JHtml::_('actionsdropdown.archive', 'cb' . $i, 'sizes');
								endif;
								if ($trashed) :
									JHtml::_('actionsdropdown.untrash', 'cb' . $i, 'sizes');
								else :
									JHtml::_('actionsdropdown.trash', 'cb' . $i, 'sizes');
								endif;
					...
					<td class="nowrap has-context">
						<div class="pull-left">
							<?php if ($item->checked_out) : ?>
								<?php echo JHtml::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'sizes.', $can_checkin); ?>
							<?php endif; ?>	
								<a href="<?php echo JRoute::_('index.php?option=com_selectfile&task=size.edit&id='.(int) $item->id); ?>">
