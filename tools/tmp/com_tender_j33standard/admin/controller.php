 ...
 *
 * @version			$Id: controller.php 408 2014-10-19 18:31:00Z BrianWade $
...
class [%%ArchitectComp%%]Controller extends JControllerLegacy
{
	...
	public function display($cachable = false, $url_params = false)
	{
		...
		{
			case 'items': 
				if ($layout == 'edit' AND !$this->checkEditId('[%%com_architectcomp%%].edit.items', $id))
				{

					// Somehow the person just went to the form - we don't allow that.
					$this->setError(JText::sprintf('JLIB_APPLICATION_ERROR_UNHELD_ID', $id));
					$this->setMessage($this->getError(), 'error');
					$this->setRedirect(JRoute::_('index.php?option=[%%com_architectcomp%%]&view=itemses', false));
