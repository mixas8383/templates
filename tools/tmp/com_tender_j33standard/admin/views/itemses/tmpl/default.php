...
if ($save_order)
{
	$save_ordering_url = 'index.php?option=[%%com_architectcomp%%]&task=itemses.saveOrderAjax&tmpl=component';
...
<form action="<?php echo JRoute::_('index.php?option=[%%com_architectcomp%%]&view=itemses'); ?>" method="post" name="adminForm" id="adminForm">
	...
	<?php else : ?>
		<table class="table table-striped" id="items-list">
			...
			<?php foreach ($this->items as $i => $item) :
				...
				$can_edit	= $user->authorise('core.edit',	'[%%com_architectcomp%%].items.'.$item->id);
		
				$can_edit_own	= $user->authorise('core.edit.own',		'[%%com_architectcomp%%].items.'.$item->id) 
								AND $item->created_by == $user_id
								;
				$can_change	= $user->authorise('core.edit.state',	'[%%com_architectcomp%%].items.'.$item->id) 
								AND $can_checkin
								;
				if ($item->language == '*'):
					...
					<td class="nowrap has-context">
						<div class="pull-left">
							...
							<?php if ($can_edit OR $can_edit_own) : ?>
								<a href="<?php echo JRoute::_('index.php?option=[%%com_architectcomp%%]&task=items.edit&id='.(int) $item->id); ?>">
