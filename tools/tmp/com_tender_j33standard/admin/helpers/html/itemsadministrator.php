 ...
 *
 * @version			$Id: compobjectadministrator.php 408 2014-10-19 18:31:00Z BrianWade $
...
/**
 * Itemses component helper.
 *
 */
abstract class JHtmlItemsAdministrator
{
	/**
	 * Render the list of associated items
	 * 
	 * @param	integer $items_id	The items item id
	 * 
	 * @return  string  The language HTML
     */
	public static function association($items_id)
	{
		...
		// Get the associations
		if ($associations = JLanguageAssociations::getAssociations('[%%com_architectcomp%%]', '#__[%%architectcomp%%]_itemses', '[%%com_architectcomp%%].items.item',  $items_id, 'id', 'alias', 'catid'))
		{		
			...
			if ($items)
			{
				foreach ($items as &$item)
				{
					...
					$url = JRoute::_('index.php?option=[%%com_architectcomp%%]&task=items.edit&id=' . (int) $item->id);
					$tooltip_parts = array(
	...
	public static function featured($value = 0, $i, $can_change = true)
	{
		...
		$states	= array(
			0	=> array('unfeatured', 'itemses.featured', '[%%COM_ARCHITECTCOMP%%]_ITEMSES_UNFEATURED', '[%%COM_ARCHITECTCOMP%%]_ITEMSES_TOGGLE_TO_FEATURE'),
			1	=> array('featured', 'itemses.unfeatured', 'JFEATURED', '[%%COM_ARCHITECTCOMP%%]_ITEMSES_TOGGLE_TO_UNFEATURE'),
		);
