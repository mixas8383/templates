...
class [%%com_architectcomp%%]InstallerScript
{
	...
	<?php
		...
		$db->setQuery(
						...
						.' WHERE '.$db->quoteName('type_alias').' = '.$db->quote('[%%com_architectcomp%%].items')
					 );
    ...
    private function populateUCM()
    {
		...
		$content_type = array();
		$content_type['type_title'] = 'Items';
		$content_type['type_alias'] = '[%%com_architectcomp%%].items';
		$content_type['table'] = '{"special":{"dbtable":"[%%architectcomp%%]_itemses","key":"id","type":"Itemses","prefix":"[%%ArchitectComp%%]Table","config":"array()"},';
		...
		$content_type['router'] = '[%%ArchitectComp%%]HelperRoute::getItemsRoute';

		$content_type['content_history_options'] = '{"formFile":"administrator\/components\/[%%architectcomp%%]\/models\/forms\/items.xml",';
