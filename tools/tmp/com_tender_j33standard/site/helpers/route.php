 ...
 *
 * @version			$Id: route.php 408 2014-10-19 18:31:00Z BrianWade $
...
abstract class [%%ArchitectComp%%]HelperRoute
{
	...
	/**
	 * @param	integer	The route of the Items
	 */
	public static function getItemsRoute($id, $cat_id = 0, $language = '*', $layout = 'default', $keep_item_id = false)
	{
		$needles = array(
			'items'  => array((int) $id)
		);
		...
		{
			//Create the link
			$link = 'index.php?option=[%%com_architectcomp%%]&view=items&id='. $id;
		}
		else
		{
			//Create the link with a layout
			$link = 'index.php?option=[%%com_architectcomp%%]&view=items&layout='.$layout.'&id='. $id;
		}
