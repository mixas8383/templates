 ...
 *
 * @version			$Id: compobjectplural.php 423 2014-10-23 14:08:16Z BrianWade $
...
/**
 * This models supports retrieving lists of itemses.
 *
 */
class [%%ArchitectComp%%]ModelItemses extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = '[%%com_architectcomp%%].itemses';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single items has been specified either as a parameter or in the url Request
		$pk = $params->get('items_id', '') == '' ? $app->input->getInt('id', '') : $params->get('items_id');
		$this->setState('filter.items_id', $pk);
		
		// List state information
		if ($app->input->getString('layout', 'default') == 'blog')
		{
			$limit = $params->def('items_num_leading', 1) + $params->def('items_num_intro', 4) + $params->def('items_num_links', 4);
		}
		else
		{		
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('items_num_per_page'),'integer');
		}
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('items_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('items_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('items_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('items_initial_direction','ASC');
		}
		...
		// process show_items_noauth parameter
		if (!$params->get('show_items_noauth'))
		{
		...
		if ($params->get('filter_items_categories') AND implode(',', $params->get('filter_items_categories'))  == true)
		{
			$selected_categories = $params->get('filter_items_categories');
			$this->setState('filter.category_id', $selected_categories);
		}			
		if ($params->get('filter_items_featured') <> "")
		{
			$this->setState('filter.featured', $params->get('filter_items_featured'));
			
		}
		if ($params->get('filter_items_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_items_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.items_id'));
		$id .= ':'.$this->getState('filter.items_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of itemses subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__[%%architectcomp%%]_itemses').' AS a');
		...
		$query->join('LEFT', $db->quoteName('#__[%%architectcomp%%]_rating').' AS v ON '.$db->quoteName('a.id').' = '.$db->quoteName('v.content_id').' AND '.$db->quoteName('v.content_type').' = '.$db->quote('itemses'));
		...
		// Filter by a single or group of itemses.
		$items_id = $this->getState('filter.items_id');
		if ($items_id != '')
		{
			if (is_numeric($items_id))
			{
				$type = $this->getState('filter.items_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $items_id);
			}
			else
			{
				if (is_array($items_id))
				{
					JArrayHelper::toInteger($items_id);
					$items_id = implode(',', $items_id);
					$type = $this->getState('filter.items_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$items_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_items_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			...
			switch ($params->get('show_items_filter_field'))
			{
				case 'hits':
	...
	/**
	 * Method to get a list of itemses.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				$items_params = new JRegistry;
				$items_params->loadString($item->params);
				...
				$item->items_alternative_readmore = $items_params->get('items_alternative_readmore');
				$item->layout = $items_params->get('layout');
							
				if (!is_object($this->getState('params')))
				{
					$item->params = $items_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Items params override menu item params only if menu param = 'use_items'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_items' and there is no items param, use global

					// create an array of just the params set to 'use_items'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_items')
						{
							// if the items has a value, use it
							if ($items_params->get($key) != '')
							{
								// get the value from the items
								$items_array[$key] = $items_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$items_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected items params
					if (count($items_array) > 0)
					{
						$items_params = new JRegistry;
						$items_params->loadArray($items_array);
						$item->params->merge($items_params);
					}


					// get display date
					switch ($item->params->get('list_show_items_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an items, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= '[%%com_architectcomp%%].items.'.$item->id;
				...
				if ($access) 
				{
					// If the access filter has been set, we already have only the itemses this user can view.
