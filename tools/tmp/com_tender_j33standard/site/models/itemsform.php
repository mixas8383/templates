 ...
 *
 * @version			$Id: compobjectform.php 408 2014-10-19 18:31:00Z BrianWade $
...
require_once JPATH_ADMINISTRATOR.'/components/[%%com_architectcomp%%]/models/items.php';

class [%%ArchitectComp%%]ModelItemsForm extends [%%ArchitectComp%%]ModelItems
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = '[%%com_architectcomp%%].edit.items';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = '[%%com_architectcomp%%].items';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('[%%com_architectcomp%%].edit.items', 'items', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null)
	{
		...
		if ($item_id)
		{
			...
			$item->tags->getTagIds($item->id, '[%%com_architectcomp%%].items');
		}
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('[%%com_architectcomp%%].edit.items.data', array());
	...
	public function save($data)
	{
		...
		$result = $dispatcher->trigger('onItemsBeforeSave', array('[%%com_architectcomp%%].items', &$table, $is_new));
		...
		$dispatcher->trigger('onItemsAfterSave', array('[%%com_architectcomp%%].items', $table, $is_new));
