 ...
 *
 * @version			$Id: compobject.php 423 2014-10-23 14:08:16Z BrianWade $
...
/**
 * [%%ArchitectComp_name%%] Component Items Model
 *
 */
class [%%ArchitectComp%%]ModelItems extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = '[%%com_architectcomp%%].items';
	...
	{
		if (empty($config['items_filter_fields']))
		{
			$config['items_filter_fields'] = array(
				'id', 'a.id',
				'name', 'a.name',
				'alias', 'a.alias',
	...
	protected function canDelete($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.delete', '[%%com_architectcomp%%].items.'.(int) $record->id)
					  OR ($user->authorise('core.delete', '[%%com_architectcomp%%].items.'.(int) $record->id)
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.edit.state', '[%%com_architectcomp%%].items.'.(int) $record->id);
		}
		else
		{
			// New items, so check against the category.		
	...
	*/
	public function getTable($type = 'Itemses', $prefix = '[%%ArchitectComp%%]Table', $config = array())
	{
	 ...
	 *
	 * @param	integer	$pk	The id of the items.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('items.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				// If all categories are published, badcats.id will be null, and we just use the items state
				...
				$can_publish = $user->authorise('core.edit.state', '[%%com_architectcomp%%].items.'.$pk);
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('[%%COM_ARCHITECTCOMP%%]_ITEMSES_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onItemsChangeState', array('[%%com_architectcomp%%].items', $pks, $value));
	...
	*/
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onItemsBeforeDelete', array('[%%com_architectcomp%%].items', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onItemsAfterDelete', array('[%%com_architectcomp%%].items', &$table));
				}
				else
				{
					// Prune items that you can't change.
	...
	/**
	 * Increment the hit counter for the items.
	 *
	 * @pk		int		Optional primary key of the items to increment.
	 *
	...
	/**
	 * Update the vote rating for the items.
	 *
	 * @pk		int		Optional primary key of the items to rate.
	 * @rate	int		Optional rating for the items.
	 *
	 * @return	boolean	True if successful; false otherwise and internal error set.
	 */	
    public function storeVote($pk = 0, $rate = 0)
    {
        if ( $rate >= 1 AND $rate <= 5 AND $pk > 0 )
        {
            ...
            $db->setQuery(
                    ...
                    ' AND '.$db->quoteName('content_type').' = '.$db->quote('itemses')
            );
            ...
            if (!$rating)
            {
                ...
                $db->setQuery(
                        ...
                        ' VALUES ( '.$db->quote('itemses').', '.(int) $pk.', '.$db->quote($user_ip).', '.(int) $rate.', 1 )'
                );
            ...
            else
            {
                if ($user_ip != ($rating->lastip))
                {
                    $db->setQuery(
                            ...
                            ' WHERE '.$db->quoteName('content_id').' = '.(int) $pk.
							' AND '.$db->quoteName('content_type').' = itemses'
                    );
        ...
        JError::raiseWarning( 'SOME_ERROR_CODE', JText::sprintf('[%%COM_ARCHITECTCOMP%%]_ITEMSES_INVALID_RATING', $rate), "[%%ArchitectComp%%]ModelItems::storeVote($rate)");
        return false;
    }
}
