 ...
 *
 * @version			$Id: compobject.php 408 2014-10-19 18:31:00Z BrianWade $
 ...
 * 
 */
class [%%ArchitectComp%%]ControllerItems extends JControllerForm
{
	/**
	 * @var    string	$view_item	The URL view item variable.
	 */
	protected $view_item = 'itemsform';
	/**
	 * @var    string	$view_list	The URL view list variable.
	 */
	protected $view_list = 'itemses';
	/**
	 * 
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		$user		= JFactory::getUser();
		$asset		= '[%%com_architectcomp%%].items.'.$record_id;
		...
		if ($user->authorise('core.edit.own', $asset))
		{
			...
			if (empty($owner_id) AND $record_id)
			{
				...
				$record		= $this->getModel('itemsform')->getItem($record_id);
	 ...
	 * 
	 */
	public function getModel($name = 'itemsform', $prefix = '',$config = array('ignore_request' => true))
	{
	 ...
	 *
	 * @return  mixed  True if the Items can be added, a error object if not.
	 *
	 */
	public function add()
	{
	...
	public function save($key = 'id', $url_var = null)
	{
		...
		if ($record_id == 0) 
		{
			$this->setMessage(JText::_('[%%COM_ARCHITECTCOMP%%]_ITEMSES_SUBMIT_SAVE_SUCCESS'));
		} 
		else 
		{
			$this->setMessage(JText::_('[%%COM_ARCHITECTCOMP%%]_ITEMSES_SAVE_SUCCESS'));
		}
		...
		switch ($task)
		{
			case 'apply':
				...
				$record_id = $model->getState('items.id');
				$this->holdEditId($context, $record_id);
				$app->setUserState($context.'.data',	null);
	...
	public function delete()
	{
		...
		if ($id > 0) 
		{
			...
			{
				$this->setMessage(JText::_('[%%COM_ARCHITECTCOMP%%]_ITEMSES_DELETE_SUCCESS'));
				
			}
			else
			{
				$this->setMessage(JText::_('[%%COM_ARCHITECTCOMP%%]_ITEMSES_DELETE_FAILED'));
			}
		}
	...
	public function vote()
	{
		...
		if ( $user_rating > -1 )
		{
			...
			{
				$this->setRedirect($url, JText::_('[%%COM_ARCHITECTCOMP%%]_ITEMSES_VOTE_SUCCESS'));
			}
			else
			{
				$this->setRedirect($url, JText::_('[%%COM_ARCHITECTCOMP%%]_ITEMSES_VOTE_FAILURE'));
			}
		}
	}
	/**
