 ...
 *
 * @version			$Id: router.php 408 2014-10-19 18:31:00Z BrianWade $
...
class [%%ArchitectComp%%]Router extends JComponentRouterBase
{
	...
	public function parse(&$segments)
	{
		...
		foreach($segments as $segment)
		{
			...
			if ($found == 0)
			{
				if ($item->query['view'] == 'itemses' OR $item->query['view'] == 'items'
					OR $view == 'itemses' OR $view == 'items')
				{
					if($advanced)
					{
						$db = JFactory::getDbo();
						...
						$query->from($db->quoteName('#__[%%architectcomp%%]_itemses'));
