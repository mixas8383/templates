 ...
 *
 */
class [%%ArchitectComp%%]ViewItemsForm extends JViewLegacy
{
	protected $state;
	protected $item;
	...
	protected function prepareDocument()
	{
		...
		{
			$this->params->def('page_heading', JText::_('[%%COM_ARCHITECTCOMP%%]_ITEMSES_EDIT_ITEM'));
		}

		if (!is_null($this->item->id))
		{
			$title = $this->params->def('page_title', JText::_('[%%COM_ARCHITECTCOMP%%]_ITEMSES_EDIT_ITEM'));
			
			$pathway->addItem(JText::sprintf('[%%COM_ARCHITECTCOMP%%]_EDIT_ITEM', $this->escape($this->item->name)),''); 
		}
		else
		{
			$title = $this->params->def('page_title', JText::_('[%%COM_ARCHITECTCOMP%%]_ITEMSES_CREATE_ITEM'));
			
			$pathway->addItem(JText::_('[%%COM_ARCHITECTCOMP%%]_ITEMSES_CREATE_ITEM'),'');
		}
		...
		if (!empty($this->item->page_title))
		{
			...
			$this->document->setTitle($items->page_title.' - '.JText::sprintf('[%%COM_ARCHITECTCOMP%%]_PAGEBREAK_PAGE_NUM', $this->state->get('page.offset') + 1));
		}
