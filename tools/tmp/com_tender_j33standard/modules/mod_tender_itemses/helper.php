...
JModelLegacy::addIncludePath(JPATH_SITE.'/components/[%%com_architectcomp%%]/models', '[%%ArchitectComp%%]ModelItemses');

abstract class ModItemsesHelper
{
	/**
	...
	public static function getList(&$params)
	{
		...
		// Get an instance of the generic itemses model
		$model = JModelLegacy::getInstance('Itemses', '[%%ArchitectComp%%]Model', array('ignore_request' => true));
		...
		// Access filter
		$access = !JComponentHelper::getParams('[%%com_architectcomp%%]')->get('show_items_noauth');
		...
		if ($params->get('items_layout') <> '')
		{
			$layout = str_replace('_:','',(string) $params->get('items_layout'));
		...
		foreach ($items as &$item)
		{
			...
			if ($access OR in_array($item->access, $authorised))
			{
				if ($item_id_str == '')
				{
					// We know that user has the privilege to view the items
					$item->link = JRoute::_([%%ArchitectComp%%]HelperRoute::getItemsRoute($item->slug, $item->catslug, $item->language, $layout));
				}
				else
				{
					$item->link = JRoute::_('index.php?option=[%%com_architectcomp%%]&view=items'.$layout_str.$item_id_str.'&id='.$item->id);
				}					
			}
			else
			{
