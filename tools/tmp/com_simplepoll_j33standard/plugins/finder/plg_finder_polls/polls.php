 ...
 * @subpackage		com_simplepoll.finder.simplepoll.polls
 * @copyright		
...
/**
 * Finder adapter for com_simplepoll - polls
 *
 */
class PlgFinderPolls extends FinderIndexerAdapter
{
	 ...
	 */
	protected $type_title = 'Polls';

	/**
	 * @var    $table	string	The table name.
	 */
	protected $table = '#__simplepoll_polls';
	...
	{
		// Make sure we're handling com_simplepoll categories
		if ($extension == 'com_simplepoll')
		{
	...
	{
		if ($context == 'com_simplepoll.poll')
		{
			$id = $table->id;
		}
	...
	public function onFinderAfterSave($context, $row, $is_new)
	{
		...
		if ($context == 'com_simplepoll.poll' OR $context == 'com_simplepoll.pollform')
		{
	...
	protected function index(FinderIndexerResult $item, $format = 'html')
	{
		...
		$item->route = SimplepollHelperRoute::getPollRoute($item->slug, $item->catslug, $item->language, $this->sub_layout);
