 ...
 * @subpackage		com_simplepoll.search.simplepoll.polls
 * @copyright		
...
class PlgSearchPolls extends JPlugin
{
	...
	public function onContentSearch($text, $phrase='', $ordering='', $areas=null)
	{
		...
		$sPoll	= $this->params->get('search_polls',1);
		$sPollArchived		= $this->params->get('search_archived_polls',0);
		...
		$search_section = JText::_('PLG_SEARCH_POLLS_POLLS');
		// search polls
		if ($sPoll AND $limit > 0)
		{
			$query->clear();
			...
			if (isset($list))
			{
				...
				{
					$list[$key]->href = JRoute::_(SimplepollHelperRoute::getPollRoute($item->slug, $item->catslug, $item->language, $this->layout, $keep_item_id));
		...
		if ($sPollArchived AND $limit > 0)
		{
			...
			$item	= $app->getMenu()->getItems('link', 'index.php?option=com_simplepoll&view=pollarchive', true);
			...
			if (isset($list3))
			{
				foreach($list3 as $key => $item)
				{
					...
					$list3[$key]->href	= JRoute::_('index.php?option=com_simplepoll&view=pollarchive'.
		...
		if (count($rows))
		{
			foreach($rows as $row)
			{
				...
				foreach($row as $poll)
				{
					if (SearchHelper::checkNoHTML($poll, $search_text, array('title',
																						'description',
																						'metadesc', 
																						'metakey'
																						)))
