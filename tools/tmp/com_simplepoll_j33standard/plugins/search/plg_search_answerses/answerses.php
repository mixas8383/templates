 ...
 * @subpackage		com_simplepoll.search.simplepoll.answerses
 * @copyright		
...
require_once JPATH_SITE.'/components/com_simplepoll/router.php';

/**
 * Answerses Search plugin
 *
 */
class PlgSearchAnswerses extends JPlugin
{
	/**
	...
	{
		static $areas = array(
			'answerses' => 'PLG_SEARCH_ANSWERSES_ANSWERSES'
			);
			return $areas;
	}

	/**
	 * Answerses Search method
	...
	public function onContentSearch($text, $phrase='', $ordering='', $areas=null)
	{
		...
		$sAnswers	= $this->params->get('search_answerses',1);
		$sAnswersArchived		= $this->params->get('search_archived_answerses',0);
		...
		$search_section = JText::_('PLG_SEARCH_ANSWERSES_ANSWERSES');
		// search answerses
		if ($sAnswers AND $limit > 0)
		{
			$query->clear();
			...
			if (isset($list))
			{
				...
				{
					$list[$key]->href = JRoute::_(SimplepollHelperRoute::getAnswersRoute($item->slug, $item->language, $this->layout, $keep_item_id));
		...
		// search archived answerses
		if ($sAnswersArchived AND $limit > 0)
		{
			$query->clear();
			...
			$item	= $app->getMenu()->getItems('link', 'index.php?option=com_simplepoll&view=answersarchive', true);
			...
			if (isset($list3))
			{
				foreach($list3 as $key => $item)
				{
					...
					$list3[$key]->href	= JRoute::_('index.php?option=com_simplepoll&view=answersarchive'.
		...
		if (count($rows))
		{
			...
			{
				$new_row = array();
				foreach($row as $answers)
				{
					if (SearchHelper::checkNoHTML($answers, $search_text, array('title',
																						'description',
																						'metadesc', 
																						'metakey'
																						)))
