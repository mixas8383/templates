 ...
 * @subpackage		com_simplepoll.itemnavigation
 * @copyright		
...
/**
 * Simplepoll navigation plugin class.
 *
 */
class PlgSimplepollItemnavigation extends JPlugin
{
	/**
	 ...
	 * @return  void
	 */			
	public function onAnswersBeforeDisplay ($context, &$row, &$params, $page=0)
	{
		...
		if ($params->get('show_answers_navigation') AND 
			($context == 'com_simplepoll.answers') AND 
			($view == 'answers')) 
		{
			...
			$uid	= $row->id;
			$option	= 'com_simplepoll';
			$can_publish = $user->authorise('core.edit.state', $option.'.answers.'.$row->id);
			...
			if ($params->get('show_answers_noauth') <> 1 AND $params->get('show_answers_noauth') <> 'use_answers')
			{
				$query->where($db->quoteName('a.access').' = ' .(int)$row->access);
			}
			// Add the list ordering clause.
			$initial_sort = $params->get('answers_initial_sort');
			...
			// Location of current answers item in array list.
			...
			{
				$row->prev = $location -1 ; 
				// The previous answers item cannot be in the array position -1.
			...
			{
				$row->next = $location +1;
				// The next answers item cannot be in an array position greater than the number of array postions.
			...
			$keep_item_id = (int) $params->get('keep_answers_itemid', 0);		
					
			if ($row->prev) 
			{
				$row->prev = JRoute::_(SimplepollHelperRoute::getAnswersRoute($row->prev->slug, $row->prev->language, $layout, $keep_item_id));
			} 
			else 
			{
				$row->prev = '';
			...
			if ($row->next) 
			{
				$row->next = JRoute::_(SimplepollHelperRoute::getAnswersRoute($row->next->slug, $row->next->language, $layout, $keep_item_id));
			...
			if ($row->prev OR $row->next) 
			{
				...
				$row->pagination = $html;
				$row->paginationposition = $this->params->get('answers_position', 1);
				// This will default to the 1.5 and 1.6-1.7 behavior.
				$row->paginationrelative = $this->params->get('answers_relative',0);				
			}
		}
	 ...
	 * @return  void
	 */			
	public function onPollBeforeDisplay ($context, &$row, &$params, $page=0)
	{
		$app = JFactory::getApplication();
		...
		if ($params->get('show_poll_navigation') AND 
			($context == 'com_simplepoll.poll') AND 
			($view == 'poll')) 
		{
			...
			$uid	= $row->id;
			$option	= 'com_simplepoll';
			$can_publish = $user->authorise('core.edit.state', $option.'.poll.'.$row->id);
			$query	= $db->getQuery(true);
			...
			if ($params->get('show_poll_noauth') <> 1 AND $params->get('show_poll_noauth') <> 'use_poll')
			{
				$query->where($db->quoteName('a.access').' = ' .(int)$row->access);
			}
			// Add the list ordering clause.
			$initial_sort = $params->get('poll_initial_sort');
			// Falll back to old style if the parameter hasn't been set yet.
			if (empty($initial_sort))
			{
				$query->order($db->quoteName($db->escape($params->get('list.ordering', 'a.ordering'))).' '.$db->escape($params->get('list.direction', 'ASC')));
			...
			// Location of current poll item in array list.
			$location = array_search($uid, array_keys($list));
			...
			{
				$row->prev = $location -1 ; 
				// The previous poll item cannot be in the array position -1.
				for ($i = $location-1; $i >= 0; $i--)
				{
			...
			{
				$row->next = $location +1;
				// The next poll item cannot be in an array position greater than the number of array postions.
				for ($i = $location+1; $i <= count($rows)-1; $i++)
				{
					$row->next = $rows[$i];
					break;
			...
			$keep_item_id = (int) $params->get('keep_poll_itemid', 0);		
					
			if ($row->prev) 
			{
				$row->prev = JRoute::_(SimplepollHelperRoute::getPollRoute($row->prev->slug,$row->prev->catslug, $row->prev->language, $layout, $keep_item_id));
			} 
			else 
			{
				$row->prev = '';
			...
			if ($row->next) 
			{
				$row->next = JRoute::_(SimplepollHelperRoute::getPollRoute($row->next->slug,$row->next->catslug, $row->next->language, $layout, $keep_item_id));
			...
			if ($row->prev OR $row->next) 
			{
				...
				$row->pagination = $html;
				$row->paginationposition = $this->params->get('poll_position', 1);
				// This will default to the 1.5 and 1.6-1.7 behavior.
				$row->paginationrelative = $this->params->get('poll_relative',0);				
			}
		}
