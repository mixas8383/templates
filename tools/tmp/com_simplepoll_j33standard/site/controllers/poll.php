 ...
 * 
 */
class SimplepollControllerPoll extends JControllerForm
{
	/**
	 * @var    string	$view_item	The URL view item variable.
	 */
	protected $view_item = 'pollform';
	/**
	...
	protected function allowAdd($data = array())
	{
		...
		if ($category_id)
		{
			...
			$allow	= $user->authorise('core.create', 'com_simplepoll.category.'.$category_id);
		}
		if ($allow === null)
		{
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		$user		= JFactory::getUser();
		$asset		= 'com_simplepoll.poll.'.$record_id;
		...
		if ($user->authorise('core.edit.own', $asset))
		{
			...
			if (empty($owner_id) AND $record_id)
			{
				...
				$record		= $this->getModel('pollform')->getItem($record_id);
	 ...
	 * 
	 */
	public function getModel($name = 'pollform', $prefix = '',$config = array('ignore_request' => true))
	{
	 ...
	 *
	 * @return  mixed  True if the Poll can be added, a error object if not.
	 *
	 */
	public function add()
	{
	...
	public function save($key = 'id', $url_var = null)
	{
		...
		if ($record_id == 0) 
		{
			$this->setMessage(JText::_('COM_SIMPLEPOLL_POLLS_SUBMIT_SAVE_SUCCESS'));
		} 
		else 
		{
			$this->setMessage(JText::_('COM_SIMPLEPOLL_POLLS_SAVE_SUCCESS'));
		}
		...
		switch ($task)
		{
			case 'apply':
				...
				$record_id = $model->getState('poll.id');
				$this->holdEditId($context, $record_id);
				$app->setUserState($context.'.data',	null);
	...
	public function delete()
	{
		...
		if ($id > 0) 
		{
			...
			{
				$this->setMessage(JText::_('COM_SIMPLEPOLL_POLLS_DELETE_SUCCESS'));
				
			}
			else
			{
				$this->setMessage(JText::_('COM_SIMPLEPOLL_POLLS_DELETE_FAILED'));
			}
		}
	...
	public function vote()
	{
		...
		if ( $user_rating > -1 )
		{
			...
			{
				$this->setRedirect($url, JText::_('COM_SIMPLEPOLL_POLLS_VOTE_SUCCESS'));
			}
			else
			{
				$this->setRedirect($url, JText::_('COM_SIMPLEPOLL_POLLS_VOTE_FAILURE'));
			}
		}
	}
	/**
