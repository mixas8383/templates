 ...
 * 
 */
class SimplepollControllerAnswers extends JControllerForm
{
	/**
	 * @var    string	$view_item	The URL view item variable.
	 */
	protected $view_item = 'answersform';
	/**
	 * @var    string	$view_list	The URL view list variable.
	 */
	protected $view_list = 'answerses';
	/**
	 * 
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		$user		= JFactory::getUser();
		$asset		= 'com_simplepoll.answers.'.$record_id;
		...
		if ($user->authorise('core.edit.own', $asset))
		{
			...
			if (empty($owner_id) AND $record_id)
			{
				...
				$record		= $this->getModel('answersform')->getItem($record_id);
	 ...
	 * 
	 */
	public function getModel($name = 'answersform', $prefix = '',$config = array('ignore_request' => true))
	{
	 ...
	 *
	 * @return  mixed  True if the Answers can be added, a error object if not.
	 *
	 */
	public function add()
	{
	...
	public function edit($key = 'id', $url_var = null)
	{
		...
		$model = $this->getModel('answersform');
		// Set the return url
		$this->setReturnPage($context);
	...
	public function save($key = 'id', $url_var = null)
	{
		...
		if ($record_id == 0) 
		{
			$this->setMessage(JText::_('COM_SIMPLEPOLL_ANSWERSES_SUBMIT_SAVE_SUCCESS'));
		} 
		else 
		{
			$this->setMessage(JText::_('COM_SIMPLEPOLL_ANSWERSES_SAVE_SUCCESS'));
		}
		...
		switch ($task)
		{
			case 'apply':
				...
				$record_id = $model->getState('answers.id');
				$this->holdEditId($context, $record_id);
				$app->setUserState($context.'.data',	null);
	...
	public function delete()
	{
		...
		if ($id > 0) 
		{
			...
			{
				$this->setMessage(JText::_('COM_SIMPLEPOLL_ANSWERSES_DELETE_SUCCESS'));
				
			}
			else
			{
				$this->setMessage(JText::_('COM_SIMPLEPOLL_ANSWERSES_DELETE_FAILED'));
			}
		}
	...
	public function vote()
	{
		...
		if ( $user_rating > -1 )
		{
			...
			{
				$this->setRedirect($url, JText::_('COM_SIMPLEPOLL_ANSWERSES_VOTE_SUCCESS'));
			}
			else
			{
				$this->setRedirect($url, JText::_('COM_SIMPLEPOLL_ANSWERSES_VOTE_FAILURE'));
			}
		}
	}
	/**
