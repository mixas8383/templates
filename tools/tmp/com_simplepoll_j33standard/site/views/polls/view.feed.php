...
class SimplepollViewPolls extends JViewLegacy
{
	...
	public function display($tpl = null)
	{
		...
		$doc->link = JRoute::_('index.php?option=com_simplepoll&view=polls');
		...
		foreach ($rows as $row)
		{
			...
			// url link to poll
			$link = JRoute::_(SimplepollHelperRoute::getPollRoute($row->slug, 
																						$row->catid, 
																						$row->language,	
																						'default',								
			...
			if ($item->featured == 1)
			{
				$item->category[]	= JText::_('JFEATURED'); // All featured polls are categorized as "Featured"
			}
