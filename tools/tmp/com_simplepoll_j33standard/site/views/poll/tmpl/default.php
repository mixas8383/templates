...
<div class="simplepoll poll-view<?php echo $params->get('pageclass_sfx')?>" itemscope itemtype="http://schema.org/CreativeWork">
	...
	<?php if ($params->get('show_poll_hits') != '0' OR 
		($params->get('show_poll_image', '0') == '1' AND isset($images['image_url']) AND $images['image_url'] != '')): ?>	
			<div class="pull-<?php echo htmlspecialchars($params->get('show_poll_image_float','right')); ?>">
			<?php if ($params->get('show_poll_image') == '1' AND isset($images['image_url']) AND $images['image_url'] != '') : ?>
	...
	<div style="clear:both; padding-top: 10px;">
		...
		<?php elseif ($params->get('show_poll_noauth') == true AND  $user->get('guest') ) : ?>
			<?php if ($params->get('show_poll_intro')) : ?>
				<span itemprop="description">
				<?php echo $this->item->intro; ?>
				</span>
			<?php endif; ?>	
			<?php //Optional link to let them register to see the whole poll. ?>
			<?php if ($params->get('show_poll_readmore')) :
	...
	<div style="padding-top: 10px;">
		<?php if ($params->get('access-view')) : ?>	
			...
			<form action="" name="pollForm" id="pollForm">
			<?php
				$dummy = false;
		$display_fieldset = (
							...
							($params->get('show_poll_publish_up')) OR
							($params->get('show_poll_publish_down')) OR
							($params->get('show_poll_admin') AND $this->item->params->get('access-change')) OR							
							$dummy
							);
			?>
			...
			<?php if ($params->get('show_poll_parent_category') AND $this->item->parent_slug != '1:root') : ?>
				...
				<div class="formelm">
					<label>
						<?php echo JText::_('COM_SIMPLEPOLL_FIELD_PARENT_CATEGORY_LABEL'); ?>
					</label>
					<span>
			...
			<?php if ($params->get('show_poll_category')) : ?>
				...
				<div class="formelm">				
					<label>
						<?php echo JText::_('COM_SIMPLEPOLL_FIELD_CATEGORY_LABEL'); ?>
					</label>
					<span>
			...
			<?php endif; ?>	
			<?php if ($params->get('show_poll_publish_up')) : ?>
				<div class="formelm">
					<label>
			...
			<?php endif; ?>
			<?php if ($params->get('show_poll_publish_down')) : ?>
				<div class="formelm">
					<label>
			...
			<?php if ($params->get('access-change')): ?>
				<?php if ($params->get('show_poll_admin')) : ?>
