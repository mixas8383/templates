...
/**
 * HTML Poll View class for the Simplepoll component
 *
 */
class SimplepollViewPoll extends JViewLegacy
{
	protected $item;
	protected $params;
	protected $print;
	...
	public function display($tpl = null)
	{
		...
		if ($active)
		{
			...
			if (JString::strpos($current_link, 'view=poll') AND (JString::strpos($current_link, '&id='.(string) $item->id)))
			{
		...
		$item->readmore_link = JRoute::_(SimplepollHelperRoute::getPollRoute($item->slug, 
										$item->catid, 
										$item->language,
										$this->getLayout(),									
		...
		if ($this->params->get('show_poll_tags')  == '1')
		{
		...
		if ($item->params->get('access-view') != true)
		{
			...
			if (!$item->params->get('show_poll_noauth'))
			{
				// If a guest user, they may be able to log in to view the full poll
				
				if( $user->get('guest'))
				{
					// Redirect to login
					$uri = JUri::getInstance();
					$app->redirect('index.php?option=com_users&view=login&return=' . base64_encode($uri), JText::_('COM_SIMPLEPOLL_POLLS_ERROR_LOGIN_TO_VIEW_ITEM'));
		...
		$results = $dispatcher->trigger('onPollPrepare', array ('com_simplepoll.poll', &$item, &$this->params, $offset));

		$item->event = new stdClass;
		$results = $dispatcher->trigger('onPollAfterName', array('com_simplepoll.poll', &$item, &$this->params, $offset));
		$item->event->afterDisplayPollName = JString::trim(implode("\n", $results));
		
		$results = $dispatcher->trigger('onPollBeforeDisplay', array('com_simplepoll.poll', &$item, &$this->params, $offset));
		$item->event->beforeDisplayPoll = JString::trim(implode("\n", $results));

		$results = $dispatcher->trigger('onPollAfterDisplay', array('com_simplepoll.poll', &$item, &$this->params, $offset));
		$item->event->afterDisplayPoll = JString::trim(implode("\n", $results));
		
		// Increment the hit counter of the poll.
		$model = $this->getModel();
		$model->hit();
	...
	protected function prepareDocument()
	{
		...
		if ($menu AND ($menu->query['option'] != 'com_simplepoll' OR $menu->query['view'] != 'poll' OR $id != $this->item->id))
		{
			// If this is not a single poll menu item, set the page title to the poll name
			if ($this->item->name)
			{
				$title = $this->item->name;
			}
			$path = array(array('title' => $this->item->name, 'link' => ''));
			if ( $this->params->get('show_poll_category_breadcrumb', '0'))
			{			
				$options['countItems'] = false;
				$options['table'] = '#__simplepoll_polls';					
				$category = JCategories::getInstance('Simplepoll', $options)->get($this->item->catid);
				while ($category AND ($menu->query['option'] != 'com_simplepoll' OR $menu->query['view'] == 'poll' OR $id != $category->id) AND $category->id > 1)
				{
					$path[] = array('title' => $category->title, 'link' => SimplepollHelperRoute::getCategoryRoute($category->id, $this->params->get('keep_poll_itemid')));
