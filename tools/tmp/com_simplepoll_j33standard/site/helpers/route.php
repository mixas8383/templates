...
/**
 * Simplepoll Component Route Helper
 *
 */
abstract class SimplepollHelperRoute
{
	...
	/**
	 * @param	integer	The route of the Answers
	 */
	public static function getAnswersRoute($id, $language = '*', $layout = 'default', $keep_item_id = false)
	{
		$needles = array(
			'answers'  => array((int) $id)
		);
		...
		{
			//Create the link
			$link = 'index.php?option=com_simplepoll&view=answers&id='. $id;
		}
		else
		{
			//Create the link with a layout
			$link = 'index.php?option=com_simplepoll&view=answers&layout='.$layout.'&id='. $id;
		}
	...
	/**
	 * @param	integer	The route of the Poll
	 */
	public static function getPollRoute($id, $cat_id = 0, $language = '*', $layout = 'default', $keep_item_id = false)
	{
		$needles = array(
			'poll'  => array((int) $id)
		);
		...
		{
			//Create the link
			$link = 'index.php?option=com_simplepoll&view=poll&id='. $id;
		}
		else
		{
			//Create the link with a layout
			$link = 'index.php?option=com_simplepoll&view=poll&layout='.$layout.'&id='. $id;
		}
	...
	public static function getCategoryRoute($cat_id, $keep_item_id = false, $language = 0)
	{
		...
		else
		{
			...
			$options['countItems'] = false;
			$options['table'] = '';			
			$category = JCategories::getInstance('Simplepoll', $options)->get($id);
		}
		...
		else
		{
			...
			$link = 'index.php?option=com_simplepoll&view=category&id='.$id;
	...
	protected static function findItem($needles = null, $keep_item_id = false, $layout = 'default')
	{
		...
		if ($active AND $active->component == 'com_simplepoll' AND ($language == '*' OR in_array($active->language, array('*', $language)) OR !JLanguageMultilang::isEnabled()))
		{
			return $active->id;
		}
