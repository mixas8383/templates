...
/**
 * Routing class for the Simplepoll component
 *
 */
class SimplepollRouter extends JComponentRouterBase
{
	/**
	 * Build the route for the Simplepoll component
	 *
	...
	public function build(&$query)
	{
		...
		$menu	= $app->getMenu();
		$params	= JComponentHelper::getParams('com_simplepoll');
		...
		// Are we dealing with a simplepoll that is attached to a menu item?
	...
	public function parse(&$segments)
	{
		...
		{
			$categories = JCategories::getInstance('Simplepoll',$options)->get($id)->getChildren();
		}
		else
		{
			$categories = JCategories::getInstance('Simplepoll',$options);	
		...
		foreach($segments as $segment)
		{
			...
			if ($found == 0)
			{
				if ($item->query['view'] == 'answerses' OR $item->query['view'] == 'answers'
					OR $view == 'answerses' OR $view == 'answers')
				{
					if($advanced)
					{
						$db = JFactory::getDbo();
						...
						$query->from($db->quoteName('#__simplepoll_answerses'));
				...
				if ($item->query['view'] == 'polls' OR $item->query['view'] == 'poll'
					OR $view == 'polls' OR $view == 'poll')
				{
					if($advanced)
					{
						$db = JFactory::getDbo();
						...
						$query = $db->getQuery(true);
						$query->select($db->quoteName('id'));
						$query->from($db->quoteName('#__simplepoll_polls'));
