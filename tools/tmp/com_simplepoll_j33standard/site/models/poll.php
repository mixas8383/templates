...
/**
 * Simplepoll Component Poll Model
 *
 */
class SimplepollModelPoll extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_simplepoll.poll';
	...
	{
		if (empty($config['poll_filter_fields']))
		{
			$config['poll_filter_fields'] = array(
				'id', 'a.id',
				'name', 'a.name',
				'alias', 'a.alias',
	...
	protected function canDelete($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.delete', 'com_simplepoll.poll.'.(int) $record->id)
					  OR ($user->authorise('core.delete', 'com_simplepoll.poll.'.(int) $record->id)
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.edit.state', 'com_simplepoll.poll.'.(int) $record->id);
		}
		else
		{
			...
			{
				return $user->authorise('core.edit.state', 'com_simplepoll.category.'.(int) $record->catid);
			}
			else 
			{
	...
	protected function populateState()
	{
		...
		if ($params->get('filter_poll_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_poll_archived'));
	...
	*/
	public function getTable($type = 'Polls', $prefix = 'SimplepollTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				$sub_query .= 'WHERE '.$db->quoteName('parent.extension').' = ' . $db->quote('com_simplepoll');
				...
				$query->join('LEFT', $db->quoteName('#__simplepoll_rating').' AS v ON '.$db->quoteName('a.id').' = '.$db->quoteName('v.content_id').' AND '.$db->quoteName('v.content_type').' = '.$db->quote('polls'));

				$can_publish = $user->authorise('core.edit.state', 'com_simplepoll.poll.'.$pk);
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_SIMPLEPOLL_POLLS_ERROR_ITEM_NOT_FOUND'));
				}
				...
				$item->tags->getItemTags('com_simplepoll.poll', $item->id);
				...
				$poll_params = new JRegistry;
				$poll_params->loadString($item->params);

				// Unpack readmore and layout params
				$item->poll_alternative_readmore = $poll_params->get('poll_alternative_readmore');
				$item->layout = $poll_params->get('layout');
				
				$item->params = clone $this->getState('params');				
								
				// Poll params override menu item params only if menu param = 'use_poll'
				// Otherwise, menu item params control the layout
				// If menu item is 'use_poll' and there is no poll param, use global

				// create an array of just the params set to 'use_poll'
				...
				foreach ($menu_params_array as $key => $value)
				{
					if ($value === 'use_poll')
					{
						// if the poll has a value, use it
						if ($poll_params->get($key) != '')
						{
							// get the value from the poll
							$poll_array[$key] = $poll_params->get($key);
						}
						else
						{
							// otherwise, use the global value
							$poll_array[$key] = $global_params->get($key);
						}
					}
				}

				// merge the selected poll params
				if (count($poll_array) > 0)
				{
					$poll_params = new JRegistry;
					$poll_params->loadArray($poll_array);
					$item->params->merge($poll_params);
				}
				...
				{
					$user_id	= $user->get('id');
					$asset	= 'com_simplepoll.poll.'.$item->id;
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onPollChangeState', array('com_simplepoll.poll', $pks, $value));
	...
	*/
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onPollBeforeDelete', array('com_simplepoll.poll', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onPollAfterDelete', array('com_simplepoll.poll', &$table));
				}
				else
				{
					// Prune items that you can't change.
	...
	/**
	 * Increment the hit counter for the poll.
	 *
	 * @pk		int		Optional primary key of the poll to increment.
	 *
	...
	/**
	 * Update the vote rating for the poll.
	 *
	 * @pk		int		Optional primary key of the poll to rate.
	 * @rate	int		Optional rating for the poll.
	 *
	 * @return	boolean	True if successful; false otherwise and internal error set.
	 */	
    public function storeVote($pk = 0, $rate = 0)
    {
        if ( $rate >= 1 AND $rate <= 5 AND $pk > 0 )
        {
            ...
            $db->setQuery(
                    ...
                    ' AND '.$db->quoteName('content_type').' = '.$db->quote('polls')
            );
            ...
            if (!$rating)
            {
                ...
                $db->setQuery(
                        ...
                        ' VALUES ( '.$db->quote('polls').', '.(int) $pk.', '.$db->quote($user_ip).', '.(int) $rate.', 1 )'
                );
            ...
            else
            {
                if ($user_ip != ($rating->lastip))
                {
                    $db->setQuery(
                            ...
                            ' WHERE '.$db->quoteName('content_id').' = '.(int) $pk.
							' AND '.$db->quoteName('content_type').' = polls'
                    );
        ...
        JError::raiseWarning( 'SOME_ERROR_CODE', JText::sprintf('COM_SIMPLEPOLL_POLLS_INVALID_RATING', $rate), "SimplepollModelPoll::storeVote($rate)");
        return false;
    }
}
