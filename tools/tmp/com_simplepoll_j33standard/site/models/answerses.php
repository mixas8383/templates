...
/**
 * This models supports retrieving lists of answerses.
 *
 */
class SimplepollModelAnswerses extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_simplepoll.answerses';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single answers has been specified either as a parameter or in the url Request
		$pk = $params->get('answers_id', '') == '' ? $app->input->getInt('id', '') : $params->get('answers_id');
		$this->setState('filter.answers_id', $pk);
		
		// List state information
		if ($app->input->getString('layout', 'default') == 'blog')
		{
			$limit = $params->def('answers_num_leading', 1) + $params->def('answers_num_intro', 4) + $params->def('answers_num_links', 4);
		}
		else
		{		
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('answers_num_per_page'),'integer');
		}
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('answers_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('answers_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('answers_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('answers_initial_direction','ASC');
		}
		$this->setState('list.direction', $list_order);
		
				
		if ((!$user->authorise('core.edit.state', 'com_simplepoll')) AND  (!$user->authorise('core.edit', 'com_simplepoll')))
		{
		...
		// process show_answers_noauth parameter
		if (!$params->get('show_answers_noauth'))
		{
		...
		if ($params->get('filter_answers_featured') <> "")
		{
			$this->setState('filter.featured', $params->get('filter_answers_featured'));
			
		}
		if ($params->get('filter_answers_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_answers_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.answers_id'));
		$id .= ':'.$this->getState('filter.answers_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of answerses subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__simplepoll_answerses').' AS a');
		...
		$query->join('LEFT', $db->quoteName('#__simplepoll_rating').' AS v ON '.$db->quoteName('a.id').' = '.$db->quoteName('v.content_id').' AND '.$db->quoteName('v.content_type').' = '.$db->quote('answerses'));
		//  Ignore this check publish date if the user can edit state
		$can_publish = $user->authorise('core.edit.state', 'com_simplepoll');
		...
		// Filter by a single or group of answerses.
		$answers_id = $this->getState('filter.answers_id');
		if ($answers_id != '')
		{
			if (is_numeric($answers_id))
			{
				$type = $this->getState('filter.answers_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $answers_id);
			}
			else
			{
				if (is_array($answers_id))
				{
					JArrayHelper::toInteger($answers_id);
					$answers_id = implode(',', $answers_id);
					$type = $this->getState('filter.answers_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$answers_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_answers_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			...
			switch ($params->get('show_answers_filter_field'))
			{
				case 'hits':
	...
	/**
	 * Method to get a list of answerses.
	 *
	...
	public function getItems()
	{
		...
		$global_params = JComponentHelper::getParams('com_simplepoll', true);
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				$answers_params = new JRegistry;
				$answers_params->loadString($item->params);
				...
				$item->answers_alternative_readmore = $answers_params->get('answers_alternative_readmore');
				$item->layout = $answers_params->get('layout');
							
				if (!is_object($this->getState('params')))
				{
					$item->params = $answers_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Answers params override menu item params only if menu param = 'use_answers'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_answers' and there is no answers param, use global

					// create an array of just the params set to 'use_answers'
					$menu_params_array = $this->getState('params')->toArray();
					$answers_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_answers')
						{
							// if the answers has a value, use it
							if ($answers_params->get($key) != '')
							{
								// get the value from the answers
								$answers_array[$key] = $answers_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$answers_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected answers params
					if (count($answers_array) > 0)
					{
						$answers_params = new JRegistry;
						$answers_params->loadArray($answers_array);
						$item->params->merge($answers_params);
					}


					// get display date
					switch ($item->params->get('list_show_answers_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an answers, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_simplepoll.answers.'.$item->id;
				...
				if ($access) 
				{
					// If the access filter has been set, we already have only the answerses this user can view.
