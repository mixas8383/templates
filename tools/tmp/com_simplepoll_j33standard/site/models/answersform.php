...
require_once JPATH_ADMINISTRATOR.'/components/com_simplepoll/models/answers.php';

class SimplepollModelAnswersForm extends SimplepollModelAnswers
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_simplepoll.edit.answers';
	/**
	...
	*/
	 ...
	 *
	 * @param	integer	$item_id	The id of the answers.
	 *
	 * @return	mixed	Answers item data object on success, false on failure.
	 */
	public function getItem($item_id = null)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('answers.id');
		...
		{
			// New item.
			$item->params->set('access-change', $user->authorise('core.edit.state', 'com_simplepoll'));
		}
	...
	protected function setAccessFilters(&$form, $data)
	{
		...
		if (!$user->authorise('core.edit.state', 'com_simplepoll'))
		{
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_simplepoll.edit.answers.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('answers.id');
		$is_new		= true;
		...
		else
		{
			...
			$actions = JAccess::getActions('com_simplepoll');
			$action_array = array();
		...
		// Reorder the answerses so the new answers is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onAnswersBeforeSave', array('com_simplepoll.answers', &$table, $is_new));
		...
		// Clean the cache.
		$cache = JFactory::getCache('com_simplepoll');
		$cache->clean();

		$dispatcher->trigger('onAnswersAfterSave', array('com_simplepoll.answers', $table, $is_new));

		$this->setState('answers.id', $table->id);
