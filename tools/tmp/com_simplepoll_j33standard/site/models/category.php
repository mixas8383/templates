...
class SimplepollModelCategory extends JModelList
{
	/**
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		{
			$limit = $app->getUserStateFromRequest('com_simplepoll.category.list.' . $item_id . '.limit', 'limit', $app->get('list_limit'));
		}
	...
	public function getItems()
	{
		...
		if ($params->get('items_to_display') AND $params->get('items_to_display') !='')
		{
			...
			{
				$model = JModelLegacy::getInstance($object_upper_case, 'SimplepollModel', array('ignore_request' => true));
	...
	protected function buildOrderBy()
	{
		...
		$item_id	= $app->input->getInt('id', 0) . ':' . $app->input->getInt('Itemid', 0);
		$order_col	= $app->getUserStateFromRequest('com_simplepoll.category.list.' . $item_id . '.filter_order', 'filter_order', '', 'string');
		$order_dirn	= $app->getUserStateFromRequest('com_simplepoll.category.list.' . $item_id . '.filter_order_Dir', 'filter_order_Dir', '', 'cmd');
		$order_by	= ' ';
		...
		{
			case 'Polls' :
				$secondary_order_by	= $params->get('poll_orderby_sec', 'none');
				$order_date			= $params->get('poll_order_date');
				$category_order_by	= $params->def('poll_orderby_pri', '');
				
				$primary			= SimplepollHelperQuery::orderbyPrimary($category_order_by);	
				$secondary			= SimplepollHelperQuery::orderbySecondary($secondary_order_by, $order_date, 'ordering') . ', ';
	...
	public function getCategory()
	{
		if(!is_object($this->_item))
		{
			...
			{
				$user	= JFactory::getUser();
				$asset	= 'com_simplepoll.category.'.$this->_item->id;
		...
		$this->tags->getItemTags('com_simplepoll.category', $this->_item->get('id'));
		return $this->_item;
	}
