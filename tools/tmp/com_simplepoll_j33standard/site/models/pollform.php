...
require_once JPATH_ADMINISTRATOR.'/components/com_simplepoll/models/poll.php';

class SimplepollModelPollForm extends SimplepollModelPoll
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_simplepoll.edit.poll';
	/**
	...
	*/
	...
	public function getForm($data = array(), $load_data = true)
	{
		...
		if ($id = (int) $this->getState('poll.id'))
		{
	 ...
	 *
	 * @param	integer	$item_id	The id of the poll.
	 *
	 * @return	mixed	Poll item data object on success, false on failure.
	 */
	public function getItem($item_id = null)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('poll.id');
		...
		{
			// New item.
			$cat_id = (int) $this->getState('poll.catid');
			if ($cat_id)
			{
				$item->params->set('access-change', $user->authorise('core.edit.state', 'com_simplepoll.category.'.$cat_id));
				$item->catid = $cat_id;
			}
			else 
			{
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_simplepoll.edit.poll.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('poll.id');
		$is_new		= true;
		...
		// Reorder the polls so the new poll is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onPollBeforeSave', array('com_simplepoll.poll', &$table, $is_new));
		...
		$dispatcher->trigger('onPollAfterSave', array('com_simplepoll.poll', $table, $is_new));
