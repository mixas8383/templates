...
/**
 * This models supports retrieving lists of polls.
 *
 */
class SimplepollModelPolls extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_simplepoll.polls';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single poll has been specified either as a parameter or in the url Request
		$pk = $params->get('poll_id', '') == '' ? $app->input->getInt('id', '') : $params->get('poll_id');
		$this->setState('filter.poll_id', $pk);
		
		// List state information
		if ($app->input->getString('layout', 'default') == 'blog')
		{
			$limit = $params->def('poll_num_leading', 1) + $params->def('poll_num_intro', 4) + $params->def('poll_num_links', 4);
		}
		else
		{		
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('poll_num_per_page'),'integer');
		}
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('poll_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('poll_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('poll_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('poll_initial_direction','ASC');
		}
		...
		// process show_poll_noauth parameter
		if (!$params->get('show_poll_noauth'))
		{
		...
		if ($params->get('filter_poll_categories') AND implode(',', $params->get('filter_poll_categories'))  == true)
		{
			$selected_categories = $params->get('filter_poll_categories');
			$this->setState('filter.category_id', $selected_categories);
		}			
		if ($params->get('filter_poll_featured') <> "")
		{
			$this->setState('filter.featured', $params->get('filter_poll_featured'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.poll_id'));
	...
	/**
	 * Get the main query for retrieving a list of polls subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$poll_id = $this->getState('filter.poll_id');
		if ($poll_id != '')
		{
			if (is_numeric($poll_id))
			{
				$type = $this->getState('filter.poll_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $poll_id);
			}
			else
			{
				if (is_array($poll_id))
				{
					JArrayHelper::toInteger($poll_id);
					$poll_id = implode(',', $poll_id);
					$type = $this->getState('filter.poll_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$poll_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_poll_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			...
			switch ($params->get('show_poll_filter_field'))
			{
				case 'hits':
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				else
				{
					...
					// get display date
					switch ($item->params->get('list_show_poll_date'))
					{
						case 'modified':
