...
JModelLegacy::addIncludePath(JPATH_SITE.'/components/com_simplepoll/models', 'SimplepollModelPolls');
...
abstract class ModPollsHelper
{
	...
	public static function getList(&$params)
	{
		...
		// Get an instance of the generic polls model
		$model = JModelLegacy::getInstance('Polls', 'SimplepollModel', array('ignore_request' => true));
		...
		// Access filter
		$access = !JComponentHelper::getParams('com_simplepoll')->get('show_poll_noauth');
		...
		if ($params->get('poll_layout') <> '')
		{
			$layout = str_replace('_:','',(string) $params->get('poll_layout'));
		...
		foreach ($items as &$item)
		{
			...
			if ($access OR in_array($item->access, $authorised))
			{
				if ($item_id_str == '')
				{
					// We know that user has the privilege to view the poll
					$item->link = JRoute::_(SimplepollHelperRoute::getPollRoute($item->slug, $item->catslug, $item->language, $layout));
				}
				else
				{
					$item->link = JRoute::_('index.php?option=com_simplepoll&view=poll'.$layout_str.$item_id_str.'&id='.$item->id);
				}					
			}
			else
			{
