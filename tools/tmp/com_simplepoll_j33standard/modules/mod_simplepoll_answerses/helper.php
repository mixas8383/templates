...
JModelLegacy::addIncludePath(JPATH_SITE.'/components/com_simplepoll/models', 'SimplepollModelAnswerses');

abstract class ModAnswersesHelper
{
	/**
	...
	public static function getList(&$params)
	{
		...
		// Get an instance of the generic answerses model
		$model = JModelLegacy::getInstance('Answerses', 'SimplepollModel', array('ignore_request' => true));
		...
		// Access filter
		$access = !JComponentHelper::getParams('com_simplepoll')->get('show_answers_noauth');
		...
		if ($params->get('answers_layout') <> '')
		{
			$layout = str_replace('_:','',(string) $params->get('answers_layout'));
		...
		foreach ($items as &$item)
		{
			...
			if ($access OR in_array($item->access, $authorised))
			{
				if ($item_id_str == '')
				{
					// We know that user has the privilege to view the answers
					$item->link = JRoute::_(SimplepollHelperRoute::getAnswersRoute($item->slug, $item->language, $layout));
				}
				else
				{
					$item->link = JRoute::_('index.php?option=com_simplepoll&view=answers'.$layout_str.$item_id_str.'&id='.$item->id);
				}					
			}
			else
			{
