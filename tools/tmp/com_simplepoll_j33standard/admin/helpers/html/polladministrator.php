...
/**
 * Polls component helper.
 *
 */
abstract class JHtmlPollAdministrator
{
	/**
	 * Render the list of associated items
	 * 
	 * @param	integer $poll_id	The poll item id
	 * 
	 * @return  string  The language HTML
     */
	public static function association($poll_id)
	{
		...
		// Get the associations
		if ($associations = JLanguageAssociations::getAssociations('com_simplepoll', '#__simplepoll_polls', 'com_simplepoll.poll.item',  $poll_id, 'id', 'alias', 'catid'))
		{		
			...
			if ($items)
			{
				foreach ($items as &$item)
				{
					...
					$url = JRoute::_('index.php?option=com_simplepoll&task=poll.edit&id=' . (int) $item->id);
					$tooltip_parts = array(
	...
	public static function featured($value = 0, $i, $can_change = true)
	{
		...
		$states	= array(
			0	=> array('unfeatured', 'polls.featured', 'COM_SIMPLEPOLL_POLLS_UNFEATURED', 'COM_SIMPLEPOLL_POLLS_TOGGLE_TO_FEATURE'),
			1	=> array('featured', 'polls.unfeatured', 'JFEATURED', 'COM_SIMPLEPOLL_POLLS_TOGGLE_TO_UNFEATURE'),
		);
