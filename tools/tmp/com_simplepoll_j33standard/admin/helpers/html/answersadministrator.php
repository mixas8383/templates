...
JLoader::register('ContentHelper', JPATH_ADMINISTRATOR . '/components/com_simplepoll/helpers/simplepoll.php');
/**
 * Answerses component helper.
 *
 */
abstract class JHtmlAnswersAdministrator
{
	/**
	 * Render the list of associated items
	 * 
	 * @param	integer $answers_id	The answers item id
	 * 
	 * @return  string  The language HTML
     */
	public static function association($answers_id)
	{
		...
		// Get the associations
		if ($associations = JLanguageAssociations::getAssociations('com_simplepoll', '#__simplepoll_answerses', 'com_simplepoll.answers.item',  $answers_id, 'id', 'alias', null))
		{		
			...
			if ($items)
			{
				foreach ($items as &$item)
				{
					...
					$url = JRoute::_('index.php?option=com_simplepoll&task=answers.edit&id=' . (int) $item->id);
					$tooltip_parts = array(
	...
	public static function featured($value = 0, $i, $can_change = true)
	{
		...
		$states	= array(
			0	=> array('unfeatured', 'answerses.featured', 'COM_SIMPLEPOLL_ANSWERSES_UNFEATURED', 'COM_SIMPLEPOLL_ANSWERSES_TOGGLE_TO_FEATURE'),
			1	=> array('featured', 'answerses.unfeatured', 'JFEATURED', 'COM_SIMPLEPOLL_ANSWERSES_TOGGLE_TO_UNFEATURE'),
		);
