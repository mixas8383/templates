 ...
 *
 */
class SimplepollTablePolls extends JTable
{
	...
	public function __construct(&$db)
	{
		...
		parent::__construct('#__simplepoll_polls', 'id', $db);
		...
		JTableObserverTags::createObserver($this, array('typeAlias' => 'com_simplepoll.poll'));

		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_simplepoll.poll'));
	}
	...
	public function check()
	{
		...
		$table = JTable::getInstance('Polls', 'SimplepollTable');
		if ($table->load(array('alias'=>$this->alias)) AND ($table->id != $this->id OR $this->id==0))
		{
			$this->setError(JText::_('COM_SIMPLEPOLL_FIELD_ALIAS_DUPLICATED'));
			return false;
		}		
	...
	public function store($update_nulls = false)
	{
		...
		if (empty($this->id))
		{
			// New Poll. A created and created_by field can be set by the user,
	...
	{
		$k = $this->_tbl_key;
		return 'com_simplepoll.poll.'.(int) $this->$k;
	}
	...
	protected function _getAssetParentId(JTable $table = null, $id = null)
	{
		...
		if (($this->catid) AND !empty($this->catid))
		{
			...
			if ($asset->loadByName('com_simplepoll.category.' . (int) $this->catid))
			{
				return $asset->id;
			}
		}
		
		if ($asset->loadByName('com_simplepoll'))
		{
			return $asset->id;
		}
		else
