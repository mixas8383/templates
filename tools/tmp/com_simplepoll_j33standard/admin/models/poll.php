 ...
 *
 */
class SimplepollModelPoll extends JModelAdmin
{
	/**
	 * @var		string	The prefix to use with controller messages.
	 */
	protected $text_prefix = 'COM_SIMPLEPOLL_POLLS';
	/**
	 * @var      string	The type alias for this object (for example, 'com_simplepoll.poll')
	 */
	public $typeAlias = 'com_simplepoll.poll';	
	/**
	 ...
	 */
	protected $event_before_save = 'onPollBeforeSave';
	/**
	 * @var		string	The event to trigger after saving the data.
	 */
	protected $event_after_save = 'onPollAfterSave';

	/**
	 * @var    string	The event to trigger before deleting the data.
	 */
	protected $event_before_delete = 'onPollBeforeDelete';	
	/**
	 * @var    string	The event to trigger after deleting the data.
	 */
	protected $event_after_delete = 'onPollAfterDelete';	
	/**
	 * @var    string	The event to trigger after changing the data's state field.
	 */
	protected $event_change_state = 'onPollChangeState';	
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_simplepoll.poll');
			}	
		}
		...
		if ($assoc)
		{
			...
			if ($item->id != null)
			{
				$associations = JLanguageAssociations::getAssociations('com_simplepoll', '#__simplepoll_polls', 'com_simplepoll.poll.item', $item->id, 'id', 'alias', 'catid');
	...
	{
		$form = $this->loadForm('com_simplepoll.edit.poll', 'poll', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = $app->getUserState('com_simplepoll.edit.poll.data', array());
		...
		if (empty($data))
		{
			...
			{
				$filters = (array) $app->getUserState('com_simplepoll.polls.filter');
	...
	protected function prepareTable($table)
	{
		...
		// Increment the poll version number.
		$table->version++;
		...
		if (parent::save($data))
		{
			...
			if ($assoc)
			{
				...
				try
				{
					...
					$query->where('context='.$db->quote('com_simplepoll.poll.item'));
				...
				if (!$all_language AND count($associations))
				{
					...
					{
						$query->values($id.','.$db->quote('com_simplepoll.poll.item') . ',' . $db->quote($key));
					}
	...
	/**
	 * Method to toggle the featured setting of polls.
	 *
	...
	public function featured($pks, $value = 0)
	{
		...
		if (empty($pks))
		{
			$this->setError(JText::_('COM_SIMPLEPOLL_POLLS_NO_ITEM_SELECTED'));
			return false;
		}
		...
		try
		{
			...
			$query = $db->getQuery(true);
			$query->update($db->quoteName('#__simplepoll_polls'));
	...
	protected function cleanCache($group = null, $client_id = 0)
	{
		...
		parent::cleanCache('mod_simplepoll_polls');
	...
	* @param   array   $data	The data where the original name is stored
	...
	/**
	 * Pre process the form to pick up polls associated by language
	 *
	 ...
	 */
	protected function preprocessForm(JForm $form, $data, $group = 'poll')
	{
		...
		if ($assoc)
		{
			...
			$fieldset->addAttribute('description', 'COM_SIMPLEPOLL_POLLS_ITEM_ASSOCIATIONS_FIELDSET_DESC');
			$add = false;
			foreach ($languages as $tag => $language)
			{
				if (empty($data->language) OR $tag != $data->language)
				{
					...
					$field->addAttribute('type', 'modal_polls');
