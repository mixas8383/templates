 ...
 *
 */
class SimplepollModelAnswers extends JModelAdmin
{
	/**
	 * @var		string	The prefix to use with controller messages.
	 */
	protected $text_prefix = 'COM_SIMPLEPOLL_ANSWERSES';
	/**
	 * @var      string	The type alias for this object (for example, 'com_simplepoll.answers')
	 */
	public $typeAlias = 'com_simplepoll.answers';	
	/**
	 ...
	 */
	protected $event_before_save = 'onAnswersBeforeSave';
	/**
	 * @var		string	The event to trigger after saving the data.
	 */
	protected $event_after_save = 'onAnswersAfterSave';

	/**
	 * @var    string	The event to trigger before deleting the data.
	 */
	protected $event_before_delete = 'onAnswersBeforeDelete';	
	/**
	 * @var    string	The event to trigger after deleting the data.
	 */
	protected $event_after_delete = 'onAnswersAfterDelete';	
	/**
	 * @var    string	The event to trigger after changing the data's state field.
	 */
	protected $event_change_state = 'onAnswersChangeState';	
	...
	protected function canDelete($record)
	{
		...
		{
			return $user->authorise('core.delete', 'com_simplepoll');
		}							
	}
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_simplepoll.answers');
			}	
		}
		...
		if ($assoc)
		{
			...
			if ($item->id != null)
			{
				$associations = JLanguageAssociations::getAssociations('com_simplepoll', '#__simplepoll_answerses', 'com_simplepoll.answers.item', $item->id, 'id', 'alias', null);
	...
	{
		$form = $this->loadForm('com_simplepoll.edit.answers', 'answers', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
		...
		// Prevent messing with Answers language and category when editing existing Answers with associations
	...
	protected function loadFormData()
	{
		...
		$data = $app->getUserState('com_simplepoll.edit.answers.data', array());
	...
	protected function prepareTable($table)
	{
		...
		// Increment the answers version number.
		$table->version++;
	...
	{
		// Include the simplepoll plugins for the onSave events.
		...
		if (parent::save($data))
		{
			...
			if ($assoc)
			{
				...
				{
					JError::raiseNotice(403, JText::_('COM_SIMPLEPOLL_ERROR_ALL_LANGUAGE_ASSOCIATED'));
				}
				...
				try
				{
					...
					$query->where('context='.$db->quote('com_simplepoll.answers.item'));
				...
				if (!$all_language AND count($associations))
				{
					...
					{
						$query->values($id.','.$db->quote('com_simplepoll.answers.item') . ',' . $db->quote($key));
					}
	...
	{
		// Include the simplepoll plugins for the delete events.
		JPluginHelper::importPlugin('simplepoll');	
	...
	/**
	 * Method to toggle the featured setting of answerses.
	 *
	...
	public function featured($pks, $value = 0)
	{
		...
		if (empty($pks))
		{
			$this->setError(JText::_('COM_SIMPLEPOLL_ANSWERSES_NO_ITEM_SELECTED'));
			return false;
		}
		...
		try
		{
			...
			$query = $db->getQuery(true);
			$query->update($db->quoteName('#__simplepoll_answerses'));
	...
	/**
	 * Custom clean the cache of com_simplepoll and simplepoll modules
	 *
	 */
	protected function cleanCache($group = null, $client_id = 0)
	{
		parent::cleanCache('com_simplepoll');
		parent::cleanCache('mod_simplepoll');
		parent::cleanCache('mod_simplepoll_answerses');
	...
	* @param   array   $data	The data where the original name is stored
	...
	/**
	 * Pre process the form to pick up answerses associated by language
	 *
	 ...
	 */
	protected function preprocessForm(JForm $form, $data, $group = 'answers')
	{
		...
		if ($assoc)
		{
			...
			$fieldset->addAttribute('description', 'COM_SIMPLEPOLL_ANSWERSES_ITEM_ASSOCIATIONS_FIELDSET_DESC');
			$add = false;
			foreach ($languages as $tag => $language)
			{
				if (empty($data->language) OR $tag != $data->language)
				{
					...
					$field->addAttribute('type', 'modal_answerses');
