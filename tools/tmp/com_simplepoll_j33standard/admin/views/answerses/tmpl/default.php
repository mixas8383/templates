...
$this->document->addStyleSheet(JUri::root().'media/com_simplepoll/css/admin.css');
...
$can_order	= $user->authorise('core.edit.state', 'com_simplepoll');

$save_order	= ($list_order=='ordering' OR $list_order=='a.ordering');

if ($save_order)
{
	$save_ordering_url = 'index.php?option=com_simplepoll&task=answerses.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'answers-list', 'adminForm', strtolower($list_dirn), $save_ordering_url);
}
...
<form action="<?php echo JRoute::_('index.php?option=com_simplepoll&view=answerses'); ?>" method="post" name="adminForm" id="adminForm">
	...
	<?php else : ?>
		<table class="table table-striped" id="answers-list">
			<thead>
				<tr>
					...
					<th width="10%" class="nowrap center hidden-phone">
						<?php echo JHtml::_('grid.sort',  'COM_SIMPLEPOLL_HEADING_CREATED_BY', 'a.created_by', $list_dirn, $list_order); ?>
					</th>
					<th width="10%" class="nowrap center hidden-phone">
						<?php echo JHtml::_('grid.sort', 'COM_SIMPLEPOLL_HEADING_CREATED', 'a.created', $list_dirn, $list_order); ?>
					</th>		
					<?php if ($assoc) : ?>
						<th width="5%" class="nowrap center hidden-phone">
							<?php echo JHtml::_('grid.sort', 'COM_SIMPLEPOLL_HEADING_ASSOCIATION', 'association', $list_dirn, $list_order); ?>
						</th>
					<?php endif;?>				
			...
			<?php foreach ($this->items as $i => $item) :
				...
				$can_edit	= $user->authorise('core.edit',	'com_simplepoll.answers.'.$item->id);
		
				$can_edit_own	= $user->authorise('core.edit.own',		'com_simplepoll.answers.'.$item->id) 
								AND $item->created_by == $user_id
								;
				$can_change	= $user->authorise('core.edit.state',	'com_simplepoll.answers.'.$item->id) 
								AND $can_checkin
								;
				if ($item->language == '*'):
				...
				<tr class="row<?php echo $i % 2; ?>">
					...
					<td class="center">
						<div class="btn-group">
							<?php echo JHtml::_('jgrid.published', $item->state, $i, 'answerses.', $can_change, 'cb', $item->publish_up, $item->publish_down); ?>
							<?php echo JHtml::_('answersadministrator.featured', $item->featured, $i, $can_change); ?>
							<?php
								if ($archived) :
									JHtml::_('actionsdropdown.unarchive', 'cb' . $i, 'answerses');
								else :
									JHtml::_('actionsdropdown.archive', 'cb' . $i, 'answerses');
								endif;
								if ($trashed) :
									JHtml::_('actionsdropdown.untrash', 'cb' . $i, 'answerses');
								else :
									JHtml::_('actionsdropdown.trash', 'cb' . $i, 'answerses');
								endif;
					...
					<td class="nowrap has-context">
						<div class="pull-left">
							<?php if ($item->checked_out) : ?>
								<?php echo JHtml::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'answerses.', $can_checkin); ?>
							<?php endif; ?>	
							<?php if ($can_edit OR $can_edit_own) : ?>
								<a href="<?php echo JRoute::_('index.php?option=com_simplepoll&task=answers.edit&id='.(int) $item->id); ?>">
					...
					<?php if ($assoc) : ?>
						<td class="small hidden-phone">
							<?php if ($item->association) : ?>
								<?php echo JHtml::_('answersadministrator.association', $item->id); ?>
							<?php endif; ?>
						</td>
					<?php endif;?>				
