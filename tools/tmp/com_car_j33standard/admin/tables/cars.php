 ...
 *
 */
class CarTableCars extends JTable
{
	...
	public function __construct(&$db)
	{
		...
		parent::__construct('#__car_cars', 'id', $db);
		...
		JTableObserverTags::createObserver($this, array('typeAlias' => 'com_car.car'));

		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_car.car'));
	}
	...
	public function check()
	{
		...
		$table = JTable::getInstance('Cars', 'CarTable');
		if ($table->load(array('alias'=>$this->alias)) AND ($table->id != $this->id OR $this->id==0))
		{
			$this->setError(JText::_('COM_CAR_FIELD_ALIAS_DUPLICATED'));
			return false;
		}		
	...
	public function store($update_nulls = false)
	{
		...
		if (empty($this->id))
		{
			// New Car. A created and created_by field can be set by the user,
	...
	protected function _getAssetParentId(JTable $table = null, $id = null)
	{
		...
		if (($this->catid) AND !empty($this->catid))
		{
			...
			if ($asset->loadByName('com_car.category.' . (int) $this->catid))
			{
				return $asset->id;
			}
		}
		
		if ($asset->loadByName('com_car'))
		{
			return $asset->id;
		}
		else
