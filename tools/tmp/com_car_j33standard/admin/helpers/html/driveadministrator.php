...
JLoader::register('ContentHelper', JPATH_ADMINISTRATOR . '/components/com_car/helpers/car.php');
/**
 * Drives component helper.
 *
 */
abstract class JHtmlDriveAdministrator
{
	/**
	 * Render the list of associated items
	 * 
	 * @param	integer $drive_id	The drive item id
	 * 
	 * @return  string  The language HTML
     */
	public static function association($drive_id)
	{
		...
		// Get the associations
		if ($associations = JLanguageAssociations::getAssociations('com_car', '#__car_drives', 'com_car.drive.item',  $drive_id, 'id', 'alias', null))
		{		
			...
			if ($items)
			{
				foreach ($items as &$item)
				{
					...
					$url = JRoute::_('index.php?option=com_car&task=drive.edit&id=' . (int) $item->id);
					$tooltip_parts = array(
	...
	public static function featured($value = 0, $i, $can_change = true)
	{
		...
		$states	= array(
			0	=> array('unfeatured', 'drives.featured', 'COM_CAR_DRIVES_UNFEATURED', 'COM_CAR_DRIVES_TOGGLE_TO_FEATURE'),
			1	=> array('featured', 'drives.unfeatured', 'JFEATURED', 'COM_CAR_DRIVES_TOGGLE_TO_UNFEATURE'),
		);
