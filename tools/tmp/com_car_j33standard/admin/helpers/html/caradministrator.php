 ...
 *
 */
abstract class JHtmlCarAdministrator
{
	/**
	 * Render the list of associated items
	 * 
	 * @param	integer $car_id	The car item id
	 * 
	 * @return  string  The language HTML
     */
	public static function association($car_id)
	{
		...
		// Get the associations
		if ($associations = JLanguageAssociations::getAssociations('com_car', '#__car_cars', 'com_car.car.item',  $car_id, 'id', 'alias', 'catid'))
		{		
			...
			if ($items)
			{
				foreach ($items as &$item)
				{
					...
					$url = JRoute::_('index.php?option=com_car&task=car.edit&id=' . (int) $item->id);
					$tooltip_parts = array(
	...
	public static function featured($value = 0, $i, $can_change = true)
	{
		...
		$states	= array(
			0	=> array('unfeatured', 'cars.featured', 'COM_CAR_CARS_UNFEATURED', 'COM_CAR_CARS_TOGGLE_TO_FEATURE'),
			1	=> array('featured', 'cars.unfeatured', 'JFEATURED', 'COM_CAR_CARS_TOGGLE_TO_UNFEATURE'),
		);
