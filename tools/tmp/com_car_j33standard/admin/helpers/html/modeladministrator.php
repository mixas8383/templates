...
/**
 * Models component helper.
 *
 */
abstract class JHtmlModelAdministrator
{
	/**
	 * Render the list of associated items
	 * 
	 * @param	integer $model_id	The model item id
	 * 
	 * @return  string  The language HTML
     */
	public static function association($model_id)
	{
		...
		// Get the associations
		if ($associations = JLanguageAssociations::getAssociations('com_car', '#__car_models', 'com_car.model.item',  $model_id, 'id', 'alias', null))
		{		
			...
			if ($items)
			{
				foreach ($items as &$item)
				{
					...
					$url = JRoute::_('index.php?option=com_car&task=model.edit&id=' . (int) $item->id);
					$tooltip_parts = array(
	...
	public static function featured($value = 0, $i, $can_change = true)
	{
		...
		$states	= array(
			0	=> array('unfeatured', 'models.featured', 'COM_CAR_MODELS_UNFEATURED', 'COM_CAR_MODELS_TOGGLE_TO_FEATURE'),
			1	=> array('featured', 'models.unfeatured', 'JFEATURED', 'COM_CAR_MODELS_TOGGLE_TO_UNFEATURE'),
		);
