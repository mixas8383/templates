 ...
 *
 */
class CarModelDrive extends JModelAdmin
{
	/**
	 * @var		string	The prefix to use with controller messages.
	 */
	protected $text_prefix = 'COM_CAR_DRIVES';
	/**
	 * @var      string	The type alias for this object (for example, 'com_car.drive')
	 */
	public $typeAlias = 'com_car.drive';	
	/**
	 ...
	 */
	protected $event_before_save = 'onDriveBeforeSave';
	/**
	 * @var		string	The event to trigger after saving the data.
	 */
	protected $event_after_save = 'onDriveAfterSave';

	/**
	 * @var    string	The event to trigger before deleting the data.
	 */
	protected $event_before_delete = 'onDriveBeforeDelete';	
	/**
	 * @var    string	The event to trigger after deleting the data.
	 */
	protected $event_after_delete = 'onDriveAfterDelete';	
	/**
	 * @var    string	The event to trigger after changing the data's state field.
	 */
	protected $event_change_state = 'onDriveChangeState';	
	...
	protected function canDelete($record)
	{
		...
		{
			return $user->authorise('core.delete', 'com_car');
		}							
	}
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_car.drive');
			}	
		}
		...
		if ($assoc)
		{
			...
			if ($item->id != null)
			{
				$associations = JLanguageAssociations::getAssociations('com_car', '#__car_drives', 'com_car.drive.item', $item->id, 'id', 'alias', null);
	...
	{
		$form = $this->loadForm('com_car.edit.drive', 'drive', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
		...
		// Prevent messing with Drive language and category when editing existing Drive with associations
	...
	protected function loadFormData()
	{
		...
		$data = $app->getUserState('com_car.edit.drive.data', array());
	...
	protected function prepareTable($table)
	{
		...
		// Increment the drive version number.
		$table->version++;
	...
	{
		// Include the car plugins for the onSave events.
		...
		if (parent::save($data))
		{
			...
			if ($assoc)
			{
				...
				{
					JError::raiseNotice(403, JText::_('COM_CAR_ERROR_ALL_LANGUAGE_ASSOCIATED'));
				}
				...
				try
				{
					...
					$query->where('context='.$db->quote('com_car.drive.item'));
				...
				if (!$all_language AND count($associations))
				{
					...
					{
						$query->values($id.','.$db->quote('com_car.drive.item') . ',' . $db->quote($key));
					}
	...
	{
		// Include the car plugins for the delete events.
		JPluginHelper::importPlugin('car');	
	...
	/**
	 * Method to toggle the featured setting of drives.
	 *
	...
	public function featured($pks, $value = 0)
	{
		...
		if (empty($pks))
		{
			$this->setError(JText::_('COM_CAR_DRIVES_NO_ITEM_SELECTED'));
			return false;
		}
		...
		try
		{
			...
			$query = $db->getQuery(true);
			$query->update($db->quoteName('#__car_drives'));
	...
	/**
	 * Custom clean the cache of com_car and car modules
	 *
	 */
	protected function cleanCache($group = null, $client_id = 0)
	{
		parent::cleanCache('com_car');
		parent::cleanCache('mod_car');
		parent::cleanCache('mod_car_drives');
	...
	* @param   array   $data	The data where the original name is stored
	...
	/**
	 * Pre process the form to pick up drives associated by language
	 *
	 ...
	 */
	protected function preprocessForm(JForm $form, $data, $group = 'drive')
	{
		...
		if ($assoc)
		{
			...
			$fieldset->addAttribute('description', 'COM_CAR_DRIVES_ITEM_ASSOCIATIONS_FIELDSET_DESC');
			$add = false;
			foreach ($languages as $tag => $language)
			{
				if (empty($data->language) OR $tag != $data->language)
				{
					...
					$field->addAttribute('type', 'modal_drives');
