 ...
 *
 */
class CarModelModel extends JModelAdmin
{
	/**
	 * @var		string	The prefix to use with controller messages.
	 */
	protected $text_prefix = 'COM_CAR_MODELS';
	/**
	 * @var      string	The type alias for this object (for example, 'com_car.model')
	 */
	public $typeAlias = 'com_car.model';	
	/**
	 ...
	 */
	protected $event_before_save = 'onModelBeforeSave';
	/**
	 * @var		string	The event to trigger after saving the data.
	 */
	protected $event_after_save = 'onModelAfterSave';

	/**
	 * @var    string	The event to trigger before deleting the data.
	 */
	protected $event_before_delete = 'onModelBeforeDelete';	
	/**
	 * @var    string	The event to trigger after deleting the data.
	 */
	protected $event_after_delete = 'onModelAfterDelete';	
	/**
	 * @var    string	The event to trigger after changing the data's state field.
	 */
	protected $event_change_state = 'onModelChangeState';	
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_car.model');
			}	
		}
		...
		if ($assoc)
		{
			...
			if ($item->id != null)
			{
				$associations = JLanguageAssociations::getAssociations('com_car', '#__car_models', 'com_car.model.item', $item->id, 'id', 'alias', null);
	...
	{
		$form = $this->loadForm('com_car.edit.model', 'model', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
		...
		// Prevent messing with Model language and category when editing existing Model with associations
	...
	protected function loadFormData()
	{
		...
		$data = $app->getUserState('com_car.edit.model.data', array());
		...
		if (parent::save($data))
		{
			...
			if ($assoc)
			{
				...
				try
				{
					...
					$query->where('context='.$db->quote('com_car.model.item'));
				...
				if (!$all_language AND count($associations))
				{
					...
					{
						$query->values($id.','.$db->quote('com_car.model.item') . ',' . $db->quote($key));
					}
	...
	/**
	 * Method to toggle the featured setting of models.
	 *
	...
	public function featured($pks, $value = 0)
	{
		...
		if (empty($pks))
		{
			$this->setError(JText::_('COM_CAR_MODELS_NO_ITEM_SELECTED'));
			return false;
		}
		...
		try
		{
			...
			$query = $db->getQuery(true);
			$query->update($db->quoteName('#__car_models'));
	...
	protected function cleanCache($group = null, $client_id = 0)
	{
		...
		parent::cleanCache('mod_car_models');
	...
	* @param   array   $data	The data where the original name is stored
	...
	/**
	 * Pre process the form to pick up models associated by language
	 *
	 ...
	 */
	protected function preprocessForm(JForm $form, $data, $group = 'model')
	{
		...
		if ($assoc)
		{
			...
			$fieldset->addAttribute('description', 'COM_CAR_MODELS_ITEM_ASSOCIATIONS_FIELDSET_DESC');
			$add = false;
			foreach ($languages as $tag => $language)
			{
				if (empty($data->language) OR $tag != $data->language)
				{
					...
					$field->addAttribute('type', 'modal_models');
