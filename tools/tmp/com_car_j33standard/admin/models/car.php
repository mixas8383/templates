 ...
 *
 */
class CarModelCar extends JModelAdmin
{
	/**
	 * @var		string	The prefix to use with controller messages.
	 */
	protected $text_prefix = 'COM_CAR_CARS';
	/**
	 * @var      string	The type alias for this object (for example, 'com_car.car')
	 */
	public $typeAlias = 'com_car.car';	
	/**
	 * @var		string	The context for the app call.
	 */
	protected $context = 'com_car.cars';	
	/**
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_car.car');
			}	
		}
		...
		if ($assoc)
		{
			...
			if ($item->id != null)
			{
				$associations = JLanguageAssociations::getAssociations('com_car', '#__car_cars', 'com_car.car.item', $item->id, 'id', 'alias', 'catid');
	...
	{
		$form = $this->loadForm('com_car.edit.car', 'car', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
		...
		// Prevent messing with Car language and category when editing existing Car with associations
	...
	protected function loadFormData()
	{
		...
		$data = $app->getUserState('com_car.edit.car.data', array());
		...
		if (empty($data))
		{
			...
			{
				$filters = (array) $app->getUserState('com_car.cars.filter');
	...
	protected function prepareTable($table)
	{
		...
		// Increment the car version number.
		$table->version++;
		...
		if (parent::save($data))
		{
			...
			if ($assoc)
			{
				...
				try
				{
					...
					$query->where('context='.$db->quote('com_car.car.item'));
				...
				if (!$all_language AND count($associations))
				{
					...
					{
						$query->values($id.','.$db->quote('com_car.car.item') . ',' . $db->quote($key));
					}
	...
	/**
	 * Method to toggle the featured setting of cars.
	 *
	...
	public function featured($pks, $value = 0)
	{
		...
		if (empty($pks))
		{
			$this->setError(JText::_('COM_CAR_CARS_NO_ITEM_SELECTED'));
			return false;
		}
		...
		try
		{
			...
			$query = $db->getQuery(true);
			$query->update($db->quoteName('#__car_cars'));
	...
	protected function cleanCache($group = null, $client_id = 0)
	{
		...
		parent::cleanCache('mod_car_cars');
	...
	* @param   array   $data	The data where the original name is stored
	...
	/**
	 * Pre process the form to pick up cars associated by language
	 *
	 ...
	 */
	protected function preprocessForm(JForm $form, $data, $group = 'car')
	{
		...
		if ($assoc)
		{
			...
			$fieldset->addAttribute('description', 'COM_CAR_CARS_ITEM_ASSOCIATIONS_FIELDSET_DESC');
			$add = false;
			foreach ($languages as $tag => $language)
			{
				if (empty($data->language) OR $tag != $data->language)
				{
					...
					$field->addAttribute('type', 'modal_cars');
