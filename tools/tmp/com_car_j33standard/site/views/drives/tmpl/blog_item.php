...
$layout		= $params->get('drive_layout', 'default');
$info		= $this->item->params->get('drive_info_block_position', 0);
...
<h2 itemprop="name">
		<?php if ($params->get('link_drive_names') ) : ?>
			<a href="<?php echo JRoute::_(CarHelperRoute::getDriveRoute($this->item->slug, 
											$this->item->language,									
											$layout,									
											$params->get('keep_drive_itemid'))); ?>" itemprop="url">
...
<?php if (($params->get('show_drive_image', '0') == '1' AND isset($images['image_url'])  AND $images['image_url'] <> "")): ?>	
...
<?php 
	...
	$use_def_list = (
		...
		($params->get('show_drive_hits')) OR
		$dummy
	);
?>

<?php if ($params->get('access-view') 
		AND (!$params->get('show_drive_readmore')
		OR !$params->get('show_drive_intro'))
		):?>
	<?php
...
<?php else : ?>
	...
	<?php if ($params->get('show_drive_intro')) : ?>
		<?php if ($params->get('access-view') OR (!$params->get('access-view') AND $params->get('show_drive_noauth'))) : ?>
			<span itemprop="description">
	...
	<?php if ($params->get('show_drive_readmore')) :
			if ($params->get('access-view')) :
				$link = JRoute::_(CarHelperRoute::getDriveRoute($this->item->slug, 
																							$this->item->language,									
																							$layout,									
																							$params->get('keep_drive_itemid')));
			else :
				if ($params->get('show_drive_noauth') AND $user->get('guest')) :
					$menu = JFactory::getApplication()->getMenu();
					$active = $menu->getActive();
					$item_id = $active->id;
					$link_1 = JRoute::_('index.php?option=com_users&view=login&Itemid=' . $item_id);
					$return_url = JRoute::_(CarHelperRoute::getDriveRoute($this->item->slug, 
																								$this->item->language,									
																								$layout,									
																								$params->get('keep_drive_itemid')));
			...
			<?php
				...
				if ($item_params->get('drive_alternative_readmore') == null) :
						if ($params->get('access-view')) :
