...
<h2 itemprop="name">
		<?php if ($params->get('link_car_names') ) : ?>
			<a href="<?php echo JRoute::_(CarHelperRoute::getCarRoute($this->item->slug, 
											$this->item->catid, 
											$this->item->language,
											$layout,									
											$params->get('keep_car_itemid'))); ?>" itemprop="url">
...
<?php if ($params->get('access-view') 
		AND (!$params->get('show_car_readmore')
		OR !$params->get('show_car_intro'))
		):?>
	<?php
...
<?php else : ?>
	...
	<?php if ($params->get('show_car_intro')) : ?>
		<?php if ($params->get('access-view') OR (!$params->get('access-view') AND $params->get('show_car_noauth'))) : ?>
			<span itemprop="description">
	...
	<?php if ($params->get('show_car_readmore')) :
			if ($params->get('access-view')) :
				$link = JRoute::_(CarHelperRoute::getCarRoute($this->item->slug, 
																							$this->item->catid, 
																							$this->item->language,
																							$layout,									
																							$params->get('keep_car_itemid')));
			else :
				if ($params->get('show_car_noauth') AND $user->get('guest')) :
					$menu = JFactory::getApplication()->getMenu();
					$active = $menu->getActive();
					$item_id = $active->id;
					$link_1 = JRoute::_('index.php?option=com_users&view=login&Itemid=' . $item_id);
					$return_url = JRoute::_(CarHelperRoute::getCarRoute($this->item->slug, 
																								$this->item->catid, 
																								$this->item->language,									
																								$layout,									
																								$params->get('keep_car_itemid')));
