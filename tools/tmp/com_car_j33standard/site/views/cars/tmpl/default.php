...
$this->document->addStyleSheet(JUri::root().'media/com_car/css/site_cars.css');

if ($lang->isRTL())
{
	$this->document->addStyleSheet(JUri::root().'media/com_car/css/site_car-rtl.css');
	$this->document->addStyleSheet(JUri::root().'media/com_car/css/site_cars-rtl.css');
}
...
<div class="car cars-list<?php echo $this->params->get('pageclass_sfx');?>">
	...
	<form action="<?php echo JFilterOutput::ampReplace(JUri::getInstance()->toString()); ?>" method="post" name="adminForm" id="adminForm">
		<?php if (($this->params->get('show_car_filter_field') != '' AND $this->params->get('show_car_filter_field') != 'hide')) :?>
			<div class="filter-search">
				<?php if ($this->params->get('show_car_filter_field') != '' AND $this->params->get('show_car_filter_field') != 'hide') :?>
					<input type="text" name="filter_search" id="filter_search" value="<?php echo $this->escape($this->state->get('filter.search')); ?>" onchange="document.adminForm.submit();" title="<?php echo JText::_('COM_CAR_FILTER_SEARCH_DESC'); ?>" placeholder="<?php echo JText::_('COM_CAR_'.$this->params->get('show_car_filter_field').'_FILTER_LABEL'); ?>" />
				<?php endif; ?>	
				<select name="filter_category_id" onchange="this.form.submit()">
					<option value=""><?php echo JText::_('JOPTION_SELECT_CATEGORY');?></option>
					<?php echo JHtml::_('select.options', JHtml::_('category.options', 'com_car'), 'value', 'text', $this->state->get('filter.category_id'));?>
				</select>
			</div>
		<?php endif; ?>
		...
		<?php if (empty($this->items)) : ?>
			...
			<p><?php echo JText::_('COM_CAR_CARS_NO_ITEMS'); ?></p>
			<?php endif; ?>
		...
		<?php else : ?>
			...
			<thead>
				<tr>
					...
					<?php if ($this->params->get('list_show_car_created_by',0)) : ?>
			...
			<tbody>
				...
				?>			
					...
					<?php if (in_array($item->access, $user->getAuthorisedViewLevels()) OR $params->get('show_car_noauth')) : ?>
					<td class="list-name">
						<a href="<?php echo JRoute::_(CarHelperRoute::getCarRoute($item->slug, 
																									$item->catid, 
																									$item->language,
																									$layout,									
					...
					<?php if ($this->params->get('list_show_car_date')) : ?>
						<td class="list-date">
							<time datetime="<?php echo JHtml::_('date', $item->display_date, 'c'); ?>">
								<?php echo JHtml::_('date',$item->display_date, $this->escape($this->params->get('car_date_format', JText::_('DATE_FORMAT_LC3')))); ?>
							</time>
						</td>
					<?php endif; ?>
					<?php if ($this->params->get('list_show_car_created_by',0)) : ?>
						<td class="createdby" itemprop="creator" itemscope itemtype="http://schema.org/Person">
							<?php 
								...
								if (!empty($item->created_by )) :
									if ($this->params->get('link_car_created_by') == 1) :
									...
									if ($this->params->get('show_car_headings')) :
										echo $created_by;
									else :
					...
					<?php endif; ?>
					<?php if ($this->params->get('list_show_car_category',0)) : ?>
							...
							<?php if ($can_order) : ?>
								<?php if ($save_order) : ?>
									...
									<?php if ($list_dirn == 'asc') : ?>
										<span><?php echo $this->pagination->orderUpIcon($i,$condition_up,'cars.orderup', 'JLIB_HTML_MOVE_UP', $save_order); ?></span>
										<span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total,$condition_down, 'cars.orderdown', 'JLIB_HTML_MOVE_DOWN', $save_order); ?></span>
									<?php elseif ($list_dirn == 'desc') : ?>
										<span><?php echo $this->pagination->orderUpIcon($i, $condition_up, 'cars.orderdown', 'JLIB_HTML_MOVE_UP', $save_order); ?></span>
										<span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total,$condition_down, 'cars.orderup', 'JLIB_HTML_MOVE_DOWN', $save_order); ?></span>
									<?php endif; ?>
								<?php endif; ?>
