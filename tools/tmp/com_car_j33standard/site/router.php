...
/**
 * Routing class for the Car component
 *
 */
class CarRouter extends JComponentRouterBase
{
	/**
	 * Build the route for the Car component
	 *
	...
	public function build(&$query)
	{
		...
		$menu	= $app->getMenu();
		$params	= JComponentHelper::getParams('com_car');
		...
		// Are we dealing with a car that is attached to a menu item?
	...
	public function parse(&$segments)
	{
		...
		{
			$categories = JCategories::getInstance('Car',$options)->get($id)->getChildren();
		}
		else
		{
			$categories = JCategories::getInstance('Car',$options);	
		...
		foreach($segments as $segment)
		{
			...
			if ($found == 0)
			{
				if ($item->query['view'] == 'drives' OR $item->query['view'] == 'drive'
					OR $view == 'drives' OR $view == 'drive')
				{
					if($advanced)
					{
						$db = JFactory::getDbo();
						...
						$query->from($db->quoteName('#__car_drives'));
				...
				if ($item->query['view'] == 'models' OR $item->query['view'] == 'model'
					OR $view == 'models' OR $view == 'model')
				{
					if($advanced)
					{
						$db = JFactory::getDbo();
						...
						$query = $db->getQuery(true);
						$query->select($db->quoteName('id'));
						$query->from($db->quoteName('#__car_models'));
						$query->where($db->quoteName('alias') . ' = ' . $db->quote($segment));
					...
					OR $view == 'cars' OR $view == 'car')
				{
					if($advanced)
					{
						...
						$query = $db->getQuery(true);
						$query->select($db->quoteName('id'));
						$query->from($db->quoteName('#__car_cars'));
