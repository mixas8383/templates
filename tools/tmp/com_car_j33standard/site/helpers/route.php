...
/**
 * Car Component Route Helper
 *
 */
abstract class CarHelperRoute
{
	...
	/**
	 * @param	integer	The route of the Drive
	 */
	public static function getDriveRoute($id, $language = '*', $layout = 'default', $keep_item_id = false)
	{
		$needles = array(
			'drive'  => array((int) $id)
		);
		...
		{
			//Create the link
			$link = 'index.php?option=com_car&view=drive&id='. $id;
		}
		else
		{
			//Create the link with a layout
			$link = 'index.php?option=com_car&view=drive&layout='.$layout.'&id='. $id;
		}
	...
	/**
	 * @param	integer	The route of the Model
	 */
	public static function getModelRoute($id, $language = '*', $layout = 'default', $keep_item_id = false)
	{
		$needles = array(
			'model'  => array((int) $id)
		);
		...
		{
			//Create the link
			$link = 'index.php?option=com_car&view=model&id='. $id;
		}
		else
		{
			//Create the link with a layout
			$link = 'index.php?option=com_car&view=model&layout='.$layout.'&id='. $id;
		}
	...
	/**
	 * @param	integer	The route of the Car
	 */
	public static function getCarRoute($id, $cat_id = 0, $language = '*', $layout = 'default', $keep_item_id = false)
	{
		...
		{
			//Create the link
			$link = 'index.php?option=com_car&view=car&id='. $id;
		}
		else
		{
			//Create the link with a layout
			$link = 'index.php?option=com_car&view=car&layout='.$layout.'&id='. $id;
		}
	...
	public static function getCategoryRoute($cat_id, $keep_item_id = false, $language = 0)
	{
		...
		else
		{
			...
			$options['countItems'] = false;
			$options['table'] = '';			
			$category = JCategories::getInstance('Car', $options)->get($id);
		}
		...
		else
		{
			...
			$link = 'index.php?option=com_car&view=category&id='.$id;
	...
	protected static function findItem($needles = null, $keep_item_id = false, $layout = 'default')
	{
		...
		if ($active AND $active->component == 'com_car' AND ($language == '*' OR in_array($active->language, array('*', $language)) OR !JLanguageMultilang::isEnabled()))
		{
			return $active->id;
		}
