...
/**
 * This models supports retrieving lists of models.
 *
 */
class CarModelModels extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_car.models';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single model has been specified either as a parameter or in the url Request
		$pk = $params->get('model_id', '') == '' ? $app->input->getInt('id', '') : $params->get('model_id');
		$this->setState('filter.model_id', $pk);
		
		// List state information
		if ($app->input->getString('layout', 'default') == 'blog')
		{
			$limit = $params->def('model_num_leading', 1) + $params->def('model_num_intro', 4) + $params->def('model_num_links', 4);
		}
		else
		{		
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('model_num_per_page'),'integer');
		}
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('model_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('model_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('model_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('model_initial_direction','ASC');
		}
		...
		// process show_model_noauth parameter
		if (!$params->get('show_model_noauth'))
		{
		...
		if ($params->get('filter_model_featured') <> "")
		{
			$this->setState('filter.featured', $params->get('filter_model_featured'));
			
		}
		if ($params->get('filter_model_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_model_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.model_id'));
		$id .= ':'.$this->getState('filter.model_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of models subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->join('LEFT', $db->quoteName('#__car_rating').' AS v ON '.$db->quoteName('a.id').' = '.$db->quoteName('v.content_id').' AND '.$db->quoteName('v.content_type').' = '.$db->quote('models'));
		...
		// Filter by a single or group of models.
		$model_id = $this->getState('filter.model_id');
		if ($model_id != '')
		{
			if (is_numeric($model_id))
			{
				$type = $this->getState('filter.model_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $model_id);
			}
			else
			{
				if (is_array($model_id))
				{
					JArrayHelper::toInteger($model_id);
					$model_id = implode(',', $model_id);
					$type = $this->getState('filter.model_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$model_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_model_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			...
			switch ($params->get('show_model_filter_field'))
			{
				case 'hits':
	...
	/**
	 * Method to get a list of models.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				$model_params = new JRegistry;
				$model_params->loadString($item->params);
				...
				$item->model_alternative_readmore = $model_params->get('model_alternative_readmore');
				$item->layout = $model_params->get('layout');
							
				if (!is_object($this->getState('params')))
				{
					$item->params = $model_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Model params override menu item params only if menu param = 'use_model'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_model' and there is no model param, use global

					// create an array of just the params set to 'use_model'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_model')
						{
							// if the model has a value, use it
							if ($model_params->get($key) != '')
							{
								// get the value from the model
								$model_array[$key] = $model_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$model_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected model params
					if (count($model_array) > 0)
					{
						$model_params = new JRegistry;
						$model_params->loadArray($model_array);
						$item->params->merge($model_params);
					}


					// get display date
					switch ($item->params->get('list_show_model_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an model, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_car.model.'.$item->id;
				...
				if ($access) 
				{
					// If the access filter has been set, we already have only the models this user can view.
				...
				$item->tags->getItemTags('com_car.model', $item->id);
			}
		}
		return $items;
	}
