...
require_once JPATH_ADMINISTRATOR.'/components/com_car/models/drive.php';

class CarModelDriveForm extends CarModelDrive
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_car.edit.drive';
	/**
	...
	*/
	 ...
	 *
	 * @param	integer	$item_id	The id of the drive.
	 *
	 * @return	mixed	Drive item data object on success, false on failure.
	 */
	public function getItem($item_id = null)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('drive.id');
		...
		{
			// New item.
			$item->params->set('access-change', $user->authorise('core.edit.state', 'com_car'));
		}
	...
	protected function setAccessFilters(&$form, $data)
	{
		...
		if (!$user->authorise('core.edit.state', 'com_car'))
		{
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_car.edit.drive.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('drive.id');
		$is_new		= true;
		...
		else
		{
			...
			$actions = JAccess::getActions('com_car');
			$action_array = array();
		...
		// Reorder the drives so the new drive is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onDriveBeforeSave', array('com_car.drive', &$table, $is_new));
		...
		// Clean the cache.
		$cache = JFactory::getCache('com_car');
		$cache->clean();

		$dispatcher->trigger('onDriveAfterSave', array('com_car.drive', $table, $is_new));

		$this->setState('drive.id', $table->id);
