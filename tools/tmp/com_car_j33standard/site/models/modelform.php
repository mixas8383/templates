...
require_once JPATH_ADMINISTRATOR.'/components/com_car/models/model.php';

class CarModelModelForm extends CarModelModel
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_car.edit.model';
	/**
	...
	*/
	 ...
	 *
	 * @param	integer	$item_id	The id of the model.
	 *
	 * @return	mixed	Model item data object on success, false on failure.
	 */
	public function getItem($item_id = null)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('model.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_car.edit.model.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('model.id');
		$is_new		= true;
		...
		// Reorder the models so the new model is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onModelBeforeSave', array('com_car.model', &$table, $is_new));
		...
		$dispatcher->trigger('onModelAfterSave', array('com_car.model', $table, $is_new));

		$this->setState('model.id', $table->id);
