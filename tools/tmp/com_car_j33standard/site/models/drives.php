...
/**
 * This models supports retrieving lists of drives.
 *
 */
class CarModelDrives extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_car.drives';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single drive has been specified either as a parameter or in the url Request
		$pk = $params->get('drive_id', '') == '' ? $app->input->getInt('id', '') : $params->get('drive_id');
		$this->setState('filter.drive_id', $pk);
		
		// List state information
		if ($app->input->getString('layout', 'default') == 'blog')
		{
			$limit = $params->def('drive_num_leading', 1) + $params->def('drive_num_intro', 4) + $params->def('drive_num_links', 4);
		}
		else
		{		
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('drive_num_per_page'),'integer');
		}
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('drive_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('drive_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('drive_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('drive_initial_direction','ASC');
		}
		$this->setState('list.direction', $list_order);
		
				
		if ((!$user->authorise('core.edit.state', 'com_car')) AND  (!$user->authorise('core.edit', 'com_car')))
		{
		...
		// process show_drive_noauth parameter
		if (!$params->get('show_drive_noauth'))
		{
		...
		if ($params->get('filter_drive_featured') <> "")
		{
			$this->setState('filter.featured', $params->get('filter_drive_featured'));
			
		}
		if ($params->get('filter_drive_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_drive_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.drive_id'));
		$id .= ':'.$this->getState('filter.drive_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of drives subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__car_drives').' AS a');
		...
		$query->join('LEFT', $db->quoteName('#__car_rating').' AS v ON '.$db->quoteName('a.id').' = '.$db->quoteName('v.content_id').' AND '.$db->quoteName('v.content_type').' = '.$db->quote('drives'));
		//  Ignore this check publish date if the user can edit state
		$can_publish = $user->authorise('core.edit.state', 'com_car');
		...
		// Filter by a single or group of drives.
		$drive_id = $this->getState('filter.drive_id');
		if ($drive_id != '')
		{
			if (is_numeric($drive_id))
			{
				$type = $this->getState('filter.drive_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $drive_id);
			}
			else
			{
				if (is_array($drive_id))
				{
					JArrayHelper::toInteger($drive_id);
					$drive_id = implode(',', $drive_id);
					$type = $this->getState('filter.drive_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$drive_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_drive_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			...
			switch ($params->get('show_drive_filter_field'))
			{
				case 'hits':
	...
	/**
	 * Method to get a list of drives.
	 *
	...
	public function getItems()
	{
		...
		$global_params = JComponentHelper::getParams('com_car', true);
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				$drive_params = new JRegistry;
				$drive_params->loadString($item->params);
				...
				$item->drive_alternative_readmore = $drive_params->get('drive_alternative_readmore');
				$item->layout = $drive_params->get('layout');
							
				if (!is_object($this->getState('params')))
				{
					$item->params = $drive_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Drive params override menu item params only if menu param = 'use_drive'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_drive' and there is no drive param, use global

					// create an array of just the params set to 'use_drive'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_drive')
						{
							// if the drive has a value, use it
							if ($drive_params->get($key) != '')
							{
								// get the value from the drive
								$drive_array[$key] = $drive_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$drive_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected drive params
					if (count($drive_array) > 0)
					{
						$drive_params = new JRegistry;
						$drive_params->loadArray($drive_array);
						$item->params->merge($drive_params);
					}


					// get display date
					switch ($item->params->get('list_show_drive_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an drive, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_car.drive.'.$item->id;
				...
				if ($access) 
				{
					// If the access filter has been set, we already have only the drives this user can view.
				...
				$item->tags->getItemTags('com_car.drive', $item->id);
			}
		}
		return $items;
	}
