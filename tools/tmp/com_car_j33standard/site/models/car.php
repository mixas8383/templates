 ...
 *
 */
class CarModelCar extends JModelItem
{
	/**
	...
	protected function canDelete($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.delete', 'com_car.car.'.(int) $record->id)
					  OR ($user->authorise('core.delete', 'com_car.car.'.(int) $record->id)
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.edit.state', 'com_car.car.'.(int) $record->id);
		}
		else
		{
			// New car, so check against the category.		
			if (!empty($record->catid))
			{
				return $user->authorise('core.edit.state', 'com_car.category.'.(int) $record->catid);
			}
			else 
			{
	...
	*/
	public function getTable($type = 'Cars', $prefix = 'CarTable', $config = array())
	{
	 ...
	 *
	 * @param	integer	$pk	The id of the car.
	 *
	...
	public function getItem($pk = null)
	{
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				$sub_query .= 'WHERE '.$db->quoteName('parent.extension').' = ' . $db->quote('com_car');
				...
				$query->join('LEFT', $db->quoteName('#__car_rating').' AS v ON '.$db->quoteName('a.id').' = '.$db->quoteName('v.content_id').' AND '.$db->quoteName('v.content_type').' = '.$db->quote('cars'));

				$can_publish = $user->authorise('core.edit.state', 'com_car.car.'.$pk);
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_CAR_CARS_ERROR_ITEM_NOT_FOUND'));
				}
				...
				$item->tags->getItemTags('com_car.car', $item->id);
				...
				$car_params->loadString($item->params);

				// Unpack readmore and layout params
				$item->car_alternative_readmore = $car_params->get('car_alternative_readmore');
				$item->layout = $car_params->get('layout');
				...
				// Technically guest could edit an car, but lets not check that to improve performance a little.
				if (!$user->get('guest')) 
				{
					$user_id	= $user->get('id');
					$asset	= 'com_car.car.'.$item->id;
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onCarChangeState', array('com_car.car', $pks, $value));
	...
	*/
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onCarBeforeDelete', array('com_car.car', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onCarAfterDelete', array('com_car.car', &$table));
				}
				else
				{
					// Prune items that you can't change.
    ...
    public function storeVote($pk = 0, $rate = 0)
    {
        ...
        JError::raiseWarning( 'SOME_ERROR_CODE', JText::sprintf('COM_CAR_CARS_INVALID_RATING', $rate), "CarModelCar::storeVote($rate)");
        return false;
    }
}
