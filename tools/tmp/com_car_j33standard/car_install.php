...
class com_carInstallerScript
{
    ...
    function install($parent) 
    {
		...
		$install_html_file = __DIR__ . '/car_install.html';
		...
		ob_start();            
		?>
		<div id="carinstall-info">
			<h1><?php echo JText::_('COM_CAR_INSTALL_HEADER'); ?></h1>
			<div id="carinstall-intro">
				<?php echo JText::_('COM_CAR_INSTALL_INTRO'); ?>
			</div>
			<table id="carinstall-table" class="adminlist">
				<thead class="carinstall-heading">
					<tr>
						<th colspan="3">
							<?php echo JText::_('COM_CAR_INSTALL_HEADER');?>
						</th>
					</tr>
				</thead>
				<tbody>
					<tr class="carinstall-subheading">
						<th colspan="2">
							<?php echo JText::_('COM_CAR_EXTENSION_HEADER');?>
						</th>
						<th width="50%">
							<?php echo JText::_('COM_CAR_STATUS_HEADER');?>
						</th>					
					</tr>			
					<tr class="carinstall-row">
						<td  colspan="2">
							<?php echo JText::_('COM_CAR');?>
						</td>
						<td class="carinstall-success">
							<?php echo JText::_('COM_CAR_INSTALL_PACKAGE_SUCCESS');?>
						</td>
					</tr>
					<tr>				
						<td colspan="3">
							<?php echo JText::_('COM_CAR_INSTALL_CORE_COMPONENT_SUCCESS');?>
						</td>
					</tr>
		<?php
		$buffer .= ob_get_clean();
		...
		if (count($manifest->plugins->plugin) > 0)
		{
			...
			<tr class="carinstall-subheading">
				<th>
					<?php echo JText::_('COM_CAR_PLUGIN_HEADER');?>
				</th>
				<th>
					<?php echo JText::_('COM_CAR_PLUGIN_GROUP_HEADER');?>
				</th>				
				<th width="50%">
					<?php echo JText::_('COM_CAR_STATUS_HEADER');?>
				</th>					
 		...
 		if (count($manifest->modules->module) > 0)
		{
			...
			<tr class="carinstall-subheading">
				<th>
					<?php echo JText::_('COM_CAR_MODULE_HEADER');?>
				</th>
				<th>
					<?php echo JText::_('COM_CAR_MODULE_GROUP_HEADER');?>
				</th>				
				<th width="50%">
					<?php echo JText::_('COM_CAR_STATUS_HEADER');?>
				</th>					
	...
	?>
				...
				<tfoot>
					<tr>
						<td colspan="3" text-align="center">
							<?php if ($install_error) : ?>
								<div id="carinstall-component-error">
									<?php echo JText::_('COM_CAR_INSTALL_COMPONENT_ERROR'); ?>
								</div>			
							<?php else : ?>
								<div id="carinstall-component-success">
									<?php echo JText::_('COM_CAR_INSTALL_COMPONENT_SUCCESS'); ?>
								</div>			
							<?php endif; ?>
						</td>
					</tr>
					<tr>
						<td colspan="3" text-align="center">
							<?php echo JText::_('COM_CAR_JOOMLA_LOGO_DISCLAIMER'); ?>	
						</td>				
					</tr>					
				</tfoot>
			</table>					
    ...
    function uninstall($parent) 
    {
        ...
        $manifest = $parent->get("manifest");
        $parent = $parent->getParent();
		$uninstall_html_file = __DIR__ . '/car_uninstall.html';
     ...
     ?>
	<div id="carinstall-info">
		<h1><?php echo JText::_('COM_CAR_UNINSTALL_HEADER'); ?></h1>
			<table id="carinstall-table" class="adminlist">
				<thead class="carinstall-heading">
					<tr>
						<th colspan="3">
							<?php echo JText::_('COM_CAR_UNINSTALL_HEADER');?>
						</th>
					</tr>
				</thead>
				<tbody>
					...
					<tr class="carinstall-row">
						...
						<td class="carinstall-success">
							<?php echo JText::_('COM_CAR_UNINSTALL_PACKAGE_SUCCESS');?>
						</td>
					</tr>
	<?php
		$buffer .= ob_get_clean();  
		...
		JFolder::delete(JPATH_SITE.'/images/car'); 
		JFolder::delete(JPATH_SITE.'/plugins/car'); 

		$db->setQuery(
						'DELETE FROM '.$db->quoteName('#__content_types')
						.' WHERE '.$db->quoteName('type_alias').' = '.$db->quote('com_car.drive')
					 );

		$db->execute(); 
		$db->setQuery(
						'DELETE FROM '.$db->quoteName('#__content_types')
						.' WHERE '.$db->quoteName('type_alias').' = '.$db->quote('com_car.model')
					 );

		$db->execute(); 
		$db->setQuery(
						'DELETE FROM '.$db->quoteName('#__content_types')
						.' WHERE '.$db->quoteName('type_alias').' = '.$db->quote('com_car.car')
					 );
		...
		$db->setQuery(
						'DELETE FROM '.$db->quoteName('#__content_types')
						.' WHERE '.$db->quoteName('type_alias').' = '.$db->quote('com_car.category')
					 );
				...
				<tfoot>
					<tr>
						<td colspan="3">
							<?php if ($install_error) : ?>
								<div id="carinstall-component-error">
									<?php echo JText::_('COM_CAR_UNINSTALL_COMPONENT_ERROR'); ?>
								</div>			
							<?php else : ?>
								<div id="carinstall-component-success">
									<?php echo JText::_('COM_CAR_UNINSTALL_COMPONENT_SUCCESS'); ?>
								</div>			
							<?php endif; ?>
						</td>
					</tr>
    ...
    function update($parent) 
    {
		...
		?>
		<div id="carinstall-info">
			<h1><?php echo JText::_('COM_CAR_UPDATE_HEADER'); ?></h1>
			<table id="carinstall-table" class="adminlist">
				<thead class="carinstall-heading">
					<tr>
						<th colspan="3">
							<?php echo JText::_('COM_CAR_UPDATE_HEADER');?>
						</th>
					</tr>
				</thead>
				<tbody>
					...
					<tr class="carinstall-row">
						...
						<td class="carinstall-success">
							<?php echo JText::_('COM_CAR_UPDATE_PACKAGE_SUCCESS');?>
						</td>
					</tr>					
		<?php
		$buffer .= ob_get_clean();  
				...
				<tfoot>
					<tr>
						<td colspan="3">
							<?php if ($install_error) : ?>
								<div id="carinstall-component-error">
									<?php echo JText::_('COM_CAR_UPDATE_COMPONENT_ERROR'); ?>
								</div>			
							<?php else : ?>
								<div id="carinstall-component-success">
									<?php echo JText::_('COM_CAR_UPDATE_COMPONENT_SUCCESS'); ?>
								</div>			
							<?php endif; ?>
						</td>
					</tr>
    ...
    function preflight($type, $parent) 
    {
		...
		if( version_compare( $joomla_version->getShortVersion(), $this->minimum_joomla_release, 'lt' ) ) {
			Jerror::raiseWarning(null, JTEXT::sprintf('COM_CAR_INSTALL_COMPONENT_ERROR_WRONG_JOOMLA_VERSION',$this->minimum_joomla_release));
			return false;
		}
		switch ($type)
		{
			...
			case 'update' :
				...
				{
					Jerror::raiseWarning(null, JTEXT::sprintf('COM_CAR_UPDATE_COMPONENT_ERROR_WRONG_VERSION_SEQUENCE', $rel));
					return false;
				}			
			default :
				break; 
    ...
    function postflight($type, $parent) 
    {
		...
		{
			case 'install' :
				JFolder::create(JPATH_SITE.'/images/car'); 
				JFile::copy('administrator/components/com_car/index.html','images/car/index.html', JPATH_SITE);
				
				// Make sure index.html files in all folders
				if (JFolder::exists(JPATH_SITE.'/plugins/car') AND !JFile::exists(JPATH_SITE.'/plugins/car/index.html'))
				{
					JFile::copy('administrator/components/com_car/index.html','plugins/car/index.html', JPATH_SITE);
				}	
				break;    				
			case 'update' :
				/*
	...
	private function getManifest()
	{
		...
		$query->from($db->quoteName('#__extensions'));	
		$query->where($db->quoteName('name').' = '.$db->quote('com_car'));		
	...
	<tr class="carinstall-row">
		...
		<td class="carinstall-error">
			<div>
				<?php echo JText::_('COM_CAR_'.strtoupper($action).'_PACKAGE_ERROR');?><br />
				<span class="carinstall-errormsg">
					<?php echo $msg; ?>
				</span>	
			</div>		
		</td>
    ...
    private function printSuccess($package, $group, $action)
    {
		...
		<tr class="carinstall-row">
			...
			<td class="carinstall-success">
				<div><?php echo JText::_('COM_CAR_'.strtoupper($action).'_PACKAGE_SUCCESS');?></div>
			</td>
		</tr> 		
		<?php
            $out = ob_get_clean();
    ...
    private function populateUCM()
    {
		...
		$content_type = array();
		$content_type['type_title'] = 'Drive';
		$content_type['type_alias'] = 'com_car.drive';
		$content_type['table'] = '{"special":{"dbtable":"car_drives","key":"id","type":"Drives","prefix":"CarTable","config":"array()"},';
		...
		$content_type['router'] = 'CarHelperRoute::getDriveRoute';

		$content_type['content_history_options'] = '{"formFile":"administrator\/components\/car\/models\/forms\/drive.xml",';
		...
		$db->execute(); 
		$content_type = array();
		$content_type['type_title'] = 'Model';
		$content_type['type_alias'] = 'com_car.model';
		$content_type['table'] = '{"special":{"dbtable":"car_models","key":"id","type":"Models","prefix":"CarTable","config":"array()"},';
		$content_type['table'] .= '"common":{"dbtable":"#__core_content","key":"ucm_id","type":"Corecontent","prefix":"JTable","config":"array()"}}';
		$content_type['rules'] = '';
		$content_type['field_mappings'] = '{"special":{},"common":{"core_content_item_id":"id",';
		$content_type['field_mappings'] .= '"core_title":"name",';
		...
		$content_type['field_mappings'] .= '"asset_id":"asset_id"';
		$content_type['field_mappings'] .= '}}';									
		$content_type['router'] = 'CarHelperRoute::getModelRoute';

		$content_type['content_history_options'] = '{"formFile":"administrator\/components\/car\/models\/forms\/model.xml",';
		$content_type['content_history_options'] .= '"hideFields":[';
		$content_type['content_history_options'] .= '"asset_id",';
		$content_type['content_history_options'] .= '"checked_out","checked_out_time",';
		$content_type['content_history_options'] .= '"version"],"ignoreChanges":[';
		...
		$db->execute(); 
		$content_type = array();
		$content_type['type_title'] = 'Car';
		$content_type['type_alias'] = 'com_car.car';
		$content_type['table'] = '{"special":{"dbtable":"car_cars","key":"id","type":"Cars","prefix":"CarTable","config":"array()"},';
		$content_type['table'] .= '"common":{"dbtable":"#__core_content","key":"ucm_id","type":"Corecontent","prefix":"JTable","config":"array()"}}';
		$content_type['rules'] = '';
		$content_type['field_mappings'] = '{"special":{},"common":{"core_content_item_id":"id",';
		$content_type['field_mappings'] .= '"core_title":"name",';
		...
		$content_type['field_mappings'] .= '"asset_id":"asset_id"';
		$content_type['field_mappings'] .= '}}';									
		$content_type['router'] = 'CarHelperRoute::getCarRoute';

		$content_type['content_history_options'] = '{"formFile":"administrator\/components\/car\/models\/forms\/car.xml",';
		$content_type['content_history_options'] .= '"hideFields":[';
		$content_type['content_history_options'] .= '"asset_id",';
		$content_type['content_history_options'] .= '"checked_out","checked_out_time",';
		$content_type['content_history_options'] .= '"version"],"ignoreChanges":[';
		...
		$content_type = array();
		$content_type['type_title'] = 'Car Category';
		$content_type['type_alias'] = 'com_car.category';
		...
		$content_type['router'] = 'CarHelperRoute::getCategoryRoute';
