...
JModelLegacy::addIncludePath(JPATH_SITE.'/components/com_car/models', 'CarModelDrives');

abstract class ModDrivesHelper
{
	/**
	 * Helper for mod_car
	 *
	...
	public static function getList(&$params)
	{
		...
		// Get an instance of the generic drives model
		$model = JModelLegacy::getInstance('Drives', 'CarModel', array('ignore_request' => true));
		...
		// Access filter
		$access = !JComponentHelper::getParams('com_car')->get('show_drive_noauth');
		...
		if ($params->get('drive_layout') <> '')
		{
			$layout = str_replace('_:','',(string) $params->get('drive_layout'));
		...
		foreach ($items as &$item)
		{
			...
			if ($access OR in_array($item->access, $authorised))
			{
				if ($item_id_str == '')
				{
					// We know that user has the privilege to view the drive
					$item->link = JRoute::_(CarHelperRoute::getDriveRoute($item->slug, $item->language, $layout));
				}
				else
				{
					$item->link = JRoute::_('index.php?option=com_car&view=drive'.$layout_str.$item_id_str.'&id='.$item->id);
				}					
			}
			else
			{
