...
JModelLegacy::addIncludePath(JPATH_SITE.'/components/com_car/models', 'CarModelCars');
...
abstract class ModCarsHelper
{
	...
	public static function getList(&$params)
	{
		...
		$model = JModelLegacy::getInstance('Cars', 'CarModel', array('ignore_request' => true));
		...
		// Access filter
		$access = !JComponentHelper::getParams('com_car')->get('show_car_noauth');
		...
		foreach ($items as &$item)
		{
			...
			if ($access OR in_array($item->access, $authorised))
			{
				if ($item_id_str == '')
				{
					...
					$item->link = JRoute::_(CarHelperRoute::getCarRoute($item->slug, $item->catslug, $item->language, $layout));
				}
				else
				{
					$item->link = JRoute::_('index.php?option=com_car&view=car'.$layout_str.$item_id_str.'&id='.$item->id);
				}					
			}
			else
			{
