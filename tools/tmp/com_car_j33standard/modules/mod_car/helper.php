...
abstract class ModCarHelper
{
	...
	public static function getList(&$params)
	{
		...
		JModelLegacy::addIncludePath(JPATH_SITE.'/components/com_car/models', 'CarModel'.$component_object_names[1]);

		// Get an instance of the model
		$model = JModelLegacy::getInstance($component_object_names[1], 'CarModel', array('ignore_request' => true));
		...
		// Access filter
		$access = !JComponentHelper::getParams('com_car')->get('show_'.JString::strtolower($component_object_names[0]).'_noauth');
		...
		{
			$order_map = array(
				'drives m_dsc' => 'a.modified DESC, a.created',
				'drives c_dsc' => 'a.created',
				'drives p_dsc' => 'a.publish_up',
				'drives h_dsc' => 'a.hits',
				'drives n_asc' => 'a.name',
				'drives n_dsc' => 'a.name',
				'drives o_asc' => 'a.ordering',
				'models m_dsc' => 'a.modified DESC, a.created',
				'models c_dsc' => 'a.created',
				'models p_dsc' => 'a.publish_up',
				'models h_dsc' => 'a.hits',
				'models n_asc' => 'a.name',
				'models n_dsc' => 'a.name',
				'models o_asc' => 'a.ordering',
				'cars m_dsc' => 'a.modified DESC, a.created',
				'cars c_dsc' => 'a.created',
				'cars p_dsc' => 'a.publish_up',
				'cars h_dsc' => 'a.hits',
				'cars n_asc' => 'a.name',
				'cars n_dsc' => 'a.name',
		...
		foreach ($items as &$item)
		{
			...
			if (!isset($item->access) OR $access OR in_array($item->access, $authorised))
			{
				...
				{			
					...
					{
						$item->link = JRoute::_(CarHelperRoute::$route_function($item->slug, $item->catslug, $item->language));
					}
					else
					{
						if (isset($item->catslug))
						{
							$item->link = JRoute::_(CarHelperRoute::$route_function($item->slug, $item->catslug));
						}
						else
						{
							if (isset($item->language))
							{
								$item->link = JRoute::_(CarHelperRoute::$route_function($item->slug, $item->language));
							}
							else
							{
								$item->link = JRoute::_(CarHelperRoute::$route_function($item->slug));
							}
						}
					}
				}
				...
				{
					$item->link = JRoute::_('index.php?option=com_car&view='.JString::strtolower($component_object_names[0]).$item_id_str.'&id='.$item->id);
				}
			}
			else
			{
