 ...
 * @author			 ()
 * @package			com_car
 * @subpackage		com_car.search.car.drives
 * @copyright		
...
/**
 * Drives Search plugin
 *
 */
class PlgSearchDrives extends JPlugin
{
	/**
	...
	{
		static $areas = array(
			'drives' => 'PLG_SEARCH_DRIVES_DRIVES'
			);
			return $areas;
	}

	/**
	 * Drives Search method
	...
	public function onContentSearch($text, $phrase='', $ordering='', $areas=null)
	{
		...
		$sDrive	= $this->params->get('search_drives',1);
		$sDriveArchived		= $this->params->get('search_archived_drives',0);
		...
		$search_section = JText::_('PLG_SEARCH_DRIVES_DRIVES');
		// search drives
		if ($sDrive AND $limit > 0)
		{
			$query->clear();
			...
			if (isset($list))
			{
				...
				{
					$list[$key]->href = JRoute::_(CarHelperRoute::getDriveRoute($item->slug, $item->language, $this->layout, $keep_item_id));
		...
		// search archived drives
		if ($sDriveArchived AND $limit > 0)
		{
			$query->clear();
			...
			$item	= $app->getMenu()->getItems('link', 'index.php?option=com_car&view=drivearchive', true);
			...
			if (isset($list3))
			{
				foreach($list3 as $key => $item)
				{
					...
					$list3[$key]->href	= JRoute::_('index.php?option=com_car&view=drivearchive'.
		...
		if (count($rows))
		{
			...
			{
				$new_row = array();
				foreach($row as $drive)
				{
					if (SearchHelper::checkNoHTML($drive, $search_text, array('title',
																						'description',
																						'metadesc', 
																						'metakey'
																						)))
