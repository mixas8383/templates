 ...
 * @author			 ()
 * @package			com_car
 * @subpackage		com_car.search.car.cars
 * @copyright		
	...
	public function onContentSearch($text, $phrase='', $ordering='', $areas=null)
	{
		...
		if ($sCar AND $limit > 0)
		{
			...
			if (isset($list))
			{
				...
				{
					$list[$key]->href = JRoute::_(CarHelperRoute::getCarRoute($item->slug, $item->catslug, $item->language, $this->layout, $keep_item_id));
		...
		if ($sCarArchived AND $limit > 0)
		{
			...
			$item	= $app->getMenu()->getItems('link', 'index.php?option=com_car&view=cararchive', true);
			...
			if (isset($list3))
			{
				foreach($list3 as $key => $item)
				{
					...
					$list3[$key]->href	= JRoute::_('index.php?option=com_car&view=cararchive'.
