...
/**
 *
 * Example vote plugin class.
 */
class plgExampleVote extends JPlugin
{
	/**
	 * Constructor
	 *
	...
	protected function _OutputRating($context, &$row, &$params, $page=0, $object)
	{
		...
		if ($object <> '')
		{
			...
			$html .= JText::sprintf( 'PLG_EXAMPLE_VOTE_USER_RATING', $img, $rating_count );
			$html .= "</span>\n<br />\n";
			...
			if ( ($view == $object) AND isset($row->state) AND $row->state == 1)
			{
				...
				$html .= JText::_( 'PLG_EXAMPLE_VOTE_POOR' );
				$html .= '<input type="radio" title="'.JText::sprintf('PLG_EXAMPLE_VOTE_VOTE', '1').'" name="user_rating" value="1" />';
				$html .= '<input type="radio" title="'.JText::sprintf('PLG_EXAMPLE_VOTE_VOTE', '2').'" name="user_rating" value="2" />';
				$html .= '<input type="radio" title="'.JText::sprintf('PLG_EXAMPLE_VOTE_VOTE', '3').'" name="user_rating" value="3" />';
				$html .= '<input type="radio" title="'.JText::sprintf('PLG_EXAMPLE_VOTE_VOTE', '4').'" name="user_rating" value="4" />';
				$html .= '<input type="radio" title="'.JText::sprintf('PLG_EXAMPLE_VOTE_VOTE', '5').'" name="user_rating" value="5" checked="checked" />';
				$html .= JText::_( 'PLG_EXAMPLE_VOTE_BEST' );
				$html .= '&#160;<input class="button" type="submit" name="submit_vote" value="'. JText::_( 'PLG_EXAMPLE_VOTE_RATE' ) .'" />';
