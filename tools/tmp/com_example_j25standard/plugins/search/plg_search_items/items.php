 ...
 * @author			 ()
 * @package			com_example
 * @subpackage		com_example.search.example.items
 * @copyright		
...
require_once JPATH_SITE.'/components/com_example/router.php';
	...
	function onContentSearch($text, $phrase='', $ordering='', $areas=null)
	{
		...
		if ($sItem AND $limit > 0)
		{
			...
			if (isset($list))
			{
				...
				{
					$list[$key]->href = JRoute::_(ExampleHelperRoute::getItemRoute($item->slug, $item->catslug, $item->language, $this->layout, $keep_item_id));
		...
		if ($sItemArchived AND $limit > 0)
		{
			...
			$item	= $app->getMenu()->getItems('link', 'index.php?option=com_example&view=itemarchive', true);
			...
			if (isset($list3))
			{
				foreach($list3 as $key => $item)
				{
					...
					$list3[$key]->href	= JRoute::_('index.php?option=com_example&view=itemarchive'.
