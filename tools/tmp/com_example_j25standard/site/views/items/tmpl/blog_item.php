...
<h2>
		<?php if ($params->get('link_item_names') ) : ?>
			<a href="<?php echo JRoute::_(ExampleHelperRoute::getItemRoute($this->item->slug, 
											$this->item->catid, 
											$this->item->language,
											$layout,									
...
<?php 
	...
	<dl class="item-info">
 <dt class="item-info-term"><?php  echo JText::_('COM_EXAMPLE_ITEMS_INFO'); ?></dt>
<?php endif; ?>
<?php if ($params->get('show_item_parent_category') AND $this->item->parent_slug != '1:root') : ?>
		<dd class="parent-category-name">
			<?php $title = $this->escape($this->item->parent_title);
			$url = '<a href="' . JRoute::_(ExampleHelperRoute::getCategoryRoute($this->item->parent_slug, $params->get('keep_item_itemid'))) . '">' . $title . '</a>'; ?>
			<?php if ($params->get('link_item_parent_category') AND $this->item->parent_slug) : ?>
				<?php echo JText::sprintf('COM_EXAMPLE_PARENT_CATEGORY', $url); ?>
				<?php else : ?>
				<?php echo JText::sprintf('COM_EXAMPLE_PARENT_CATEGORY', $title); ?>
			<?php endif; ?>
		</dd>
<?php endif; ?>
<?php if ($params->get('show_item_category')) : ?>
		<dd class="category-name">
			...
			$url = '<a href="'.JRoute::_(ExampleHelperRoute::getCategoryRoute($this->item->catslug, $params->get('keep_item_itemid'))).'">'.$title.'</a>';?>
			<?php if ($params->get('link_item_category') AND $this->item->catslug) : ?>
				<?php echo JText::sprintf('COM_EXAMPLE_CATEGORY', $url); ?>
				<?php else : ?>
				<?php echo JText::sprintf('COM_EXAMPLE_CATEGORY', $title); ?>
			<?php endif; ?>
		</dd>
<?php endif; ?>
<?php if ($params->get('show_item_created')) : ?>
		<dd class="create">
		<?php echo JText::sprintf('COM_EXAMPLE_CREATED_DATE_ON', JHTML::_('date',$this->item->created, JText::_('DATE_FORMAT_LC2'))); ?>
		</dd>
<?php endif; ?>
<?php if ($params->get('show_item_publish_up')) : ?>
<dd class="modified">
		<?php echo JText::sprintf('COM_EXAMPLE_PUBLISH_UP_ON', JHTML::_('date',$this->item->publish_up, JText::_('DATE_FORMAT_LC2'))); ?>
</dd>
<?php endif; ?>
<?php if ($params->get('show_item_modified')) : ?>
		<dd class="modified">
		<?php echo JText::sprintf('COM_EXAMPLE_LAST_UPDATED', JHTML::_('date',$this->item->modified, JText::_('DATE_FORMAT_LC2'))); ?>
		</dd>
<?php endif; ?>
<?php if ($params->get('show_item_creator')) : ?>
	<dd class="createdby"> 
		...
		<?php if (!empty($this->item->created_by ) AND  $this->params->get('link_item_creator') == 1):?>
			<?php echo JText::sprintf('COM_EXAMPLE_CREATED_BY',JHTML::_('link',
...
<?php if ($params->get('show_item_hits')) : ?>
		<dd class="hits">
		<?php echo JText::sprintf('COM_EXAMPLE_HITS', $this->item->hits); ?>
		</dd>
<?php endif; ?>
<?php
	$dummy = false;
...
<?php else : ?>
	...
	<?php if ($params->get('show_item_readmore')) :
			if ($params->get('access-view')) :
				$link = JRoute::_(ExampleHelperRoute::getItemRoute($this->item->slug, 
																							$this->item->catid, 
																							$this->item->language,
																							$layout,									
																							$params->get('keep_item_itemid')));
				...
				if ($params->get('show_item_noauth') AND $user->get('guest')) :
					...
					$return_url = JRoute::_(ExampleHelperRoute::getItemRoute($this->item->slug, 
																								$this->item->catid, 
																								$this->item->language,									
																								$layout,									
																								$params->get('keep_item_itemid')));
				...
				if ($item_params->get('item_alternative_readmore') == null) :
						if ($params->get('access-view')) :
							if ($params->get('show_item_readmore_name') == 0) :
								echo JText::sprintf('COM_EXAMPLE_READ_MORE');
							else :
								echo JText::_('COM_EXAMPLE_READMORE_NAME');
