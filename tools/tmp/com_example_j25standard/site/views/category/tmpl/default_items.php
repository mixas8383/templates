...
<?php if (empty($this->items)) : ?>
	...
	<p><?php echo JText::_('COM_EXAMPLE_ITEMS_CATEGORY_NO_ITEMS'); ?></p>
	<?php endif; ?>
		...
		<fieldset class="filters">
			<div class="filter-search">
				<?php if ($this->params->get('item_filter_field') != '' AND $this->params->get('item_filter_field') != 'hide') :?>
					<label class="filter-search-lbl" for="filter-search"><?php echo JText::_('COM_EXAMPLE_'.$this->params->get('item_filter_field').'_FILTER_LABEL').'&#160;'; ?></label>
					<input type="text" name="filter-search" id="filter-search" value="<?php echo $this->escape($this->state->get('list.filter')); ?>" class="inputbox" onchange="document.adminForm.submit();" title="<?php echo JText::_('COM_EXAMPLE_FILTER_SEARCH_DESC'); ?>" />
				<?php endif; ?>						
			</div>
			...
			<?php endif; ?>
				...
				<?php else : // Show unauth links. ?>
					<td>
						...
						$return_url = JRoute::_(ExampleHelperRoute::getItemRoute($item->slug, 
																								$item->catid, 
																								$item->language,
																								$layout,									
																								$this->params->get('keep_item_itemid')));
						...
						<a href="<?php echo $full_url; ?>" class="register">
							<?php echo JText::_( 'COM_EXAMPLE_REGISTER_TO_READ_MORE' ); ?></a>
					</td>
				<?php endif; ?>
				<?php if ($show_actions) : ?>
