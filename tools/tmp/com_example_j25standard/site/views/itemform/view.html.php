 ...
 *
 */
class ExampleViewItemForm extends JView
{
	protected $state;
	protected $item;
	...
	public function display($tpl = null)
	{
		...
		if (empty($item->id))
		{
			...
			OR (count($user->getAuthorisedCategories('com_example', 'core.create')))
			;
		}
		else
		{
	...
	protected function _prepareDocument()
	{
		...
		{
			$this->params->def('page_heading', JText::_('COM_EXAMPLE_ITEMS_EDIT_ITEM'));
		}

		if (!is_null($this->item->id))
		{
			$title = $this->params->def('page_title', JText::_('COM_EXAMPLE_ITEMS_EDIT_ITEM'));
			
			$pathway->addItem(JText::sprintf('COM_EXAMPLE_EDIT_ITEM', $this->escape($this->item->name)),''); 
		}
		else
		{
			$title = $this->params->def('page_title', JText::_('COM_EXAMPLE_ITEMS_CREATE_ITEM'));
			
			$pathway->addItem(JText::_('COM_EXAMPLE_ITEMS_CREATE_ITEM'),'');
		}
		...
		if (!empty($this->item->page_title))
		{
			...
			$this->document->setTitle($item->page_title.' - '.JText::sprintf('COM_EXAMPLE_PAGEBREAK_PAGE_NUM', $this->state->get('page.offset') + 1));
		}
		...
		$this->document->addScript(JUri::root() ."components/com_example/assets/js/examplevalidate.js");
		
		$this->document->addScript(JUri::root() ."components/com_example/assets/js/formsubmitbutton.js");

		JText::script('COM_EXAMPLE_ERROR_ON_FORM');		// Include Helpers
