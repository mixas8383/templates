 ...
 *
 */
class SliderTableItemses extends JTable
{
	...
	public function __construct(&$db)
	{
		...
		parent::__construct('#__slider_itemses', 'id', $db);
		...
		JTableObserverTags::createObserver($this, array('typeAlias' => 'com_slider.items'));

		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_slider.items'));
	}
	...
	public function check()
	{
		...
		$table = JTable::getInstance('Itemses', 'SliderTable');
		if ($table->load(array('alias'=>$this->alias)) AND ($table->id != $this->id OR $this->id==0))
		{
			$this->setError(JText::_('COM_SLIDER_FIELD_ALIAS_DUPLICATED'));
			return false;
		}		
	...
	public function store($update_nulls = false)
	{
		...
		if (empty($this->id))
		{
			// New Items. A created and created_by field can be set by the user,
	...
	{
		$k = $this->_tbl_key;
		return 'com_slider.items.'.(int) $this->$k;
	}
	...
	protected function _getAssetParentId(JTable $table = null, $id = null)
	{
		...
		if (($this->catid) AND !empty($this->catid))
		{
			...
			if ($asset->loadByName('com_slider.category.' . (int) $this->catid))
			{
				return $asset->id;
			}
		}
		
		if ($asset->loadByName('com_slider'))
		{
			return $asset->id;
		}
		else
