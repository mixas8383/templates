...
abstract class JHtmlItemAdministrator
{
	...
	public static function association($item_id)
	{
		...
		// Get the associations
		if ($associations = JLanguageAssociations::getAssociations('com_slider', '#__slider_items', 'com_slider.item.item',  $item_id, 'id', 'alias', null))
		{		
			...
			if ($items)
			{
				foreach ($items as &$item)
				{
					...
					$url = JRoute::_('index.php?option=com_slider&task=item.edit&id=' . (int) $item->id);
					$tooltip_parts = array(
	...
	public static function featured($value = 0, $i, $can_change = true)
	{
		...
		$states	= array(
			0	=> array('unfeatured', 'items.featured', 'COM_SLIDER_ITEMS_UNFEATURED', 'COM_SLIDER_ITEMS_TOGGLE_TO_FEATURE'),
			1	=> array('featured', 'items.unfeatured', 'JFEATURED', 'COM_SLIDER_ITEMS_TOGGLE_TO_UNFEATURE'),
		);
