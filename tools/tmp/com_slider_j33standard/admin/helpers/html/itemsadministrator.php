...
JLoader::register('ContentHelper', JPATH_ADMINISTRATOR . '/components/com_slider/helpers/slider.php');
/**
...
abstract class JHtmlItemsAdministrator
{
	...
	public static function association($items_id)
	{
		...
		// Get the associations
		if ($associations = JLanguageAssociations::getAssociations('com_slider', '#__slider_itemses', 'com_slider.items.item',  $items_id, 'id', 'alias', 'catid'))
		{		
			...
			if ($items)
			{
				foreach ($items as &$item)
				{
					...
					$url = JRoute::_('index.php?option=com_slider&task=items.edit&id=' . (int) $item->id);
					$tooltip_parts = array(
	...
	public static function featured($value = 0, $i, $can_change = true)
	{
		...
		$states	= array(
			0	=> array('unfeatured', 'itemses.featured', 'COM_SLIDER_ITEMSES_UNFEATURED', 'COM_SLIDER_ITEMSES_TOGGLE_TO_FEATURE'),
			1	=> array('featured', 'itemses.unfeatured', 'JFEATURED', 'COM_SLIDER_ITEMSES_TOGGLE_TO_UNFEATURE'),
		);
