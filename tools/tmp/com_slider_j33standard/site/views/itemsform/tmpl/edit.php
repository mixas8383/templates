...
if ($this->params->get('save_history') AND $this->params->get('items_save_history'))
{
	JHtml::_('behavior.modal', 'a.modal_jform_contenthistory');
}
$this->document->addScript(JUri::root() .'media/com_slider/js/slidervalidate.js');

$this->document->addScript(JUri::root() .'media/com_slider/js/formsubmitbutton.js');

JText::script('COM_SLIDER_ERROR_ON_FORM');
...
<div class="slider items-edit<?php echo $this->escape($params->get('pageclass_sfx')); ?>">
		...
		<h2>
			<?php  
				if (!is_null($this->item->id)) :
					echo JText::sprintf('COM_SLIDER_EDIT_ITEM', $this->escape($this->item->name)); 
				else :
					echo JText::_('COM_SLIDER_ITEMSES_CREATE_ITEM');
				endif;
			?>
		</h2>
		</div>
	...
	<?php endif; ?>
	<form action="<?php echo JRoute::_('index.php?option=com_slider&view=itemsform&id='.(int) $this->item->id); ?>" method="post" name="adminForm" id="items-form" class="form-validate">
		<div class="btn-toolbar">
			<div class="btn-group">
				<button type="button" class="btn btn-primary" onclick="Joomla.submitbutton('items.save')">
					<span class="icon-ok"></span>&#160;<?php echo JText::_('JSAVE') ?>
				</button>
			</div>
			<div class="btn-group">
				<button type="button" class="btn" onclick="Joomla.submitbutton('items.cancel')">
					<span class="icon-cancel"></span>&#160;<?php echo JText::_('JCANCEL') ?>
				</button>
			</div>
			<?php if ($params->get('save_history') AND $params->get('items_save_history')) : ?>
				<div class="btn-group">
		...
		<fieldset>
			<ul class="nav nav-tabs">
				<li class="active"><a href="#basic-details" data-toggle="tab"><?php echo JText::_('COM_SLIDER_ITEMSES_FIELDSET_DETAILS_LABEL');?></a></li>
				<li><a href="#imageslinks" data-toggle="tab"><?php echo JText::_('COM_SLIDER_FIELDSET_IMAGES_URLS_LABEL');?></a></li>
				<?php if ($this->item->params->get('access-change')): ?>
				<li><a href="#publishing" data-toggle="tab"><?php echo JText::_('COM_SLIDER_FIELDSET_PUBLISHING_LABEL');?></a></li>
				<li><a href="#metadata" data-toggle="tab"><?php echo JText::_('COM_SLIDER_FIELDSET_METADATA_LABEL');?></a></li>
				<li><a href="#language" data-toggle="tab"><?php echo JText::_('COM_SLIDER_FIELDSET_LANGUAGE_LABEL');?></a></li>
				<?php endif; ?>				
			</ul>		
			...
			<div class="tab-content">
				...
				<?php if ($this->item->params->get('access-change')): ?>
					<div class="tab-pane" id="publishing">
						...
						<?php else: ?>
							<div class="form-note">
								<p><?php echo JText::_('COM_SLIDER_ITEMSES_ORDERING'); ?></p>
							</div>
						<?php endif; ?>
					</div>	
				...
				<input type="hidden" name="form_id" id="form_id" value="items-form" />
