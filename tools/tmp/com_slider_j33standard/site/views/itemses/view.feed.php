...
class SliderViewItemses extends JViewLegacy
{
	...
	public function display($tpl = null)
	{
		...
		$doc->link = JRoute::_('index.php?option=com_slider&view=itemses');

		// Get some data from the model
		$app->input->set('limit', $app->get('feed_limit'));
		$options['countItems'] = false;
		$options['table'] = '#__slider_itemses';				
		...
		foreach ($rows as $row)
		{
			...
			// url link to items
			$link = JRoute::_(SliderHelperRoute::getItemsRoute($row->slug, 
																						$row->catid, 
																						$row->language,	
																						'default',								
			...
			if ($item->featured == 1)
			{
				$item->category[]	= JText::_('JFEATURED'); // All featured itemses are categorized as "Featured"
			}
