...
class SliderViewItems extends JViewLegacy
{
	...
	public function display($tpl = null)
	{
		...
		foreach ($items as $i => &$item)
		{
			...
			$dispatcher->trigger('onItemPrepare', array ('com_slider.item', &$item, &$item->params,$i));

			$item->event = new stdClass;

			$results = $dispatcher->trigger('onItemAfterName', array('com_slider.item', &$item, &$item->params, $i));
			$item->event->afterDisplayItemName = JString::trim(implode("\n", $results));

			$results = $dispatcher->trigger('onItemBeforeDisplay', array('com_slider.item', &$item, &$item->params, $i));
			$item->event->beforeDisplayItem = JString::trim(implode("\n", $results));

			$results = $dispatcher->trigger('onItemAfterDisplay', array('com_slider.item', &$item, &$item->params,$i));
	...
	protected function prepareDocument()
	{
		...
		{
			$this->params->def('page_heading', JText::_('COM_SLIDER_ITEMS'));
		}
