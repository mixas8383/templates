 ...
 *
 */
class SliderModelItem extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_slider.item';
	...
	protected function canDelete($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.delete', 'com_slider.item.'.(int) $record->id)
					  OR ($user->authorise('core.delete', 'com_slider.item.'.(int) $record->id)
	...
	protected function canEditState($record)
	{
		...
		else
		{
			...
			{
				return $user->authorise('core.edit.state', 'com_slider.category.'.(int) $record->catid);
			}
			else 
			{
			// Default to component settings.			
				return $user->authorise('core.edit.state', 'com_slider');
			}
		}
	}
	/**
	...
	*/
	...
	public function getItem($pk = null)
	{
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				$query->from($db->quoteName('#__slider_items').' AS a');
				...
				$sub_query .= 'WHERE '.$db->quoteName('parent.extension').' = ' . $db->quote('com_slider');
				...
				$can_publish = $user->authorise('core.edit.state', 'com_slider.item.'.$pk);
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_SLIDER_ITEMS_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onItemChangeState', array('com_slider.item', $pks, $value));
	...
	*/
	...
	public function delete(&$pks)
	{
		...
		// Include the slider plugins for the on delete events.
		JPluginHelper::importPlugin('slider');
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onItemBeforeDelete', array('com_slider.item', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onItemAfterDelete', array('com_slider.item', &$table));
				}
				else
				{
					// Prune items that you can't change.
