...
require_once JPATH_ADMINISTRATOR.'/components/com_slider/models/items.php';

class SliderModelItemsForm extends SliderModelItems
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_slider.edit.items';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_slider.items';
	/**
	...
	protected function populateState()
	{
		...
		$this->setState('items.catid', $app->input->getInt('catid'));
	...
	*/
	public function getTable($type = 'Itemses', $prefix = 'SliderTable', $config = array())
	{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_slider.edit.items', 'items', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
		if ($id = (int) $this->getState('items.id'))
		{
			// Existing record. Can only edit in selected categories.
			$form->setFieldAttribute('catid', 'action', 'core.edit');
			// Existing record. Can only edit own itemses in selected categories.
	 ...
	 *
	 * @param	integer	$item_id	The id of the items.
	 *
	 * @return	mixed	Items item data object on success, false on failure.
	 */
	public function getItem($item_id = null)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('items.id');
		...
		{
			// New item.
			$cat_id = (int) $this->getState('items.catid');
			if ($cat_id)
			{
				$item->params->set('access-change', $user->authorise('core.edit.state', 'com_slider.category.'.$cat_id));
				$item->catid = $cat_id;
			}
			else 
			{
				$item->params->set('access-change', $user->authorise('core.edit.state', 'com_slider'));
			}
		}
		...
		if ($item_id)
		{
			...
			$item->tags->getTagIds($item->id, 'com_slider.items');
		}
	...
	protected function setAccessFilters(&$form, $data)
	{
		...
		if (!$user->authorise('core.edit.state', 'com_slider'))
		{
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_slider.edit.items.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('items.id');
		$is_new		= true;
		...
		else
		{
			...
			$actions = JAccess::getActions('com_slider');
			$action_array = array();
		...
		// Reorder the itemses so the new items is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onItemsBeforeSave', array('com_slider.items', &$table, $is_new));
		...
		// Clean the cache.
		$cache = JFactory::getCache('com_slider');
		$cache->clean();

		$dispatcher->trigger('onItemsAfterSave', array('com_slider.items', $table, $is_new));

		$this->setState('items.id', $table->id);
