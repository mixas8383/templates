 ...
 * 
 */
class SliderControllerItem extends JControllerForm
{
	/**
	...
	protected function allowAdd($data = array())
	{
		...
		if ($category_id)
		{
			...
			$allow	= $user->authorise('core.create', 'com_slider.category.'.$category_id);
		}
		if ($allow === null)
		{
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		$user		= JFactory::getUser();
		$asset		= 'com_slider.item.'.$record_id;
	...
	public function save($key = 'id', $url_var = null)
	{
		...
		if ($record_id == 0) 
		{
			$this->setMessage(JText::_('COM_SLIDER_ITEMS_SUBMIT_SAVE_SUCCESS'));
		} 
		else 
		{
			$this->setMessage(JText::_('COM_SLIDER_ITEMS_SAVE_SUCCESS'));
		}
	...
	public function delete()
	{
		...
		if ($id > 0) 
		{
			...
			{
				$this->setMessage(JText::_('COM_SLIDER_ITEMS_DELETE_SUCCESS'));
				
			}
			else
			{
				$this->setMessage(JText::_('COM_SLIDER_ITEMS_DELETE_FAILED'));
			}
		}
