...
/**
 * Routing class for the Slider component
 *
 */
class SliderRouter extends JComponentRouterBase
{
	/**
	 * Build the route for the Slider component
	 *
	...
	public function build(&$query)
	{
		...
		// Are we dealing with a slider that is attached to a menu item?
	...
	public function parse(&$segments)
	{
		...
		{
			$categories = JCategories::getInstance('Slider',$options)->get($id)->getChildren();
		}
		else
		{
			$categories = JCategories::getInstance('Slider',$options);	
			...
			if ($found == 0)
			{
					...
					OR $view == 'items' OR $view == 'item')
				{
					if($advanced)
					{
						...
						$query->from($db->quoteName('#__slider_items'));
