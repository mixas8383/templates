...
/**
 * Game Component Item Model
 *
 */
class GameModelItem extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_game.item';
	...
	protected function canDelete($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.delete', 'com_game.item.'.(int) $record->id)
					  OR ($user->authorise('core.delete', 'com_game.item.'.(int) $record->id)
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.edit.state', 'com_game.item.'.(int) $record->id);
		}
		else
		{
			...
			{
				return $user->authorise('core.edit.state', 'com_game.category.'.(int) $record->catid);
			}
			else 
			{
			// Default to component settings.			
				return $user->authorise('core.edit.state', 'com_game');
			}
		}
	}
	/**
	...
	protected function populateState()
	{
		...
		if ((!$user->authorise('core.edit.state', 'com_game')) AND  (!$user->authorise('core.edit', 'com_game')))
		{
	...
	*/
	...
	public function getItem($pk = null)
	{
		...
		$global_params = JComponentHelper::getParams('com_game', true);
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				$query->from($db->quoteName('#__game_items').' AS a');
				...
				$sub_query .= 'WHERE '.$db->quoteName('parent.extension').' = ' . $db->quote('com_game');
				...
				$query->join('LEFT', $db->quoteName('#__game_rating').' AS v ON '.$db->quoteName('a.id').' = '.$db->quoteName('v.content_id').' AND '.$db->quoteName('v.content_type').' = '.$db->quote('items'));

				$can_publish = $user->authorise('core.edit.state', 'com_game.item.'.$pk);
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_GAME_ITEMS_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		// Include the game plugins for the change of state event.
		...
		$result = $dispatcher->trigger('onItemChangeState', array('com_game.item', $pks, $value));
	...
	*/
	...
	public function delete(&$pks)
	{
		...
		// Include the game plugins for the on delete events.
		JPluginHelper::importPlugin('game');
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onItemBeforeDelete', array('com_game.item', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onItemAfterDelete', array('com_game.item', &$table));
				}
				else
				{
					// Prune items that you can't change.
    ...
    public function storeVote($pk = 0, $rate = 0)
    {
        if ( $rate >= 1 AND $rate <= 5 AND $pk > 0 )
        {
            ...
            $db->setQuery(
                    'SELECT *' .
                    ' FROM '.$db->quoteName('#__game_rating') .
            ...
            if (!$rating)
            {
                ...
                $db->setQuery(
                        'INSERT INTO '.$db->quoteName('#__game_rating').' ( '.$db->quoteName('content_type').', '.$db->quoteName('content_id').', '.$db->quoteName('lastip').', '.$db->quoteName('rating_sum').', '.$db->quoteName('rating_count').' )' .
            ...
            else
            {
                ...
                {
                    $db->setQuery(
                            'UPDATE '.$db->quoteName('#__game_rating') .
        ...
        JError::raiseWarning( 'SOME_ERROR_CODE', JText::sprintf('COM_GAME_ITEMS_INVALID_RATING', $rate), "GameModelItem::storeVote($rate)");
        return false;
    }
}
