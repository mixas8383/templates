...
class GameViewItems extends JViewLegacy
{
	...
	public function display($tpl = null)
	{
		...
		foreach ($items as $i => &$item)
		{
			...
			$dispatcher->trigger('onItemPrepare', array ('com_game.item', &$item, &$item->params,$i));

			$item->event = new stdClass;

			$results = $dispatcher->trigger('onItemAfterName', array('com_game.item', &$item, &$item->params, $i));
			$item->event->afterDisplayItemName = JString::trim(implode("\n", $results));

			$results = $dispatcher->trigger('onItemBeforeDisplay', array('com_game.item', &$item, &$item->params, $i));
			$item->event->beforeDisplayItem = JString::trim(implode("\n", $results));

			$results = $dispatcher->trigger('onItemAfterDisplay', array('com_game.item', &$item, &$item->params,$i));
		...
		{			
			...
			if ($order == 0 AND $this->columns > 1)
			{
				...
				$this->intro_items = GameHelperQuery::orderDownColumns($this->intro_items, $this->columns);
			}
	...
	protected function prepareDocument()
	{
		...
		{
			$this->params->def('page_heading', JText::_('COM_GAME_ITEMS'));
		}
