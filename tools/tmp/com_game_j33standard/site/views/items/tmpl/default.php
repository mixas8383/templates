...
$this->document->addStyleSheet(JUri::root().'media/com_game/css/site_items.css');

if ($lang->isRTL())
{
	$this->document->addStyleSheet(JUri::root().'media/com_game/css/site_game-rtl.css');
	$this->document->addStyleSheet(JUri::root().'media/com_game/css/site_items-rtl.css');
}
...
<div class="game items-list<?php echo $this->params->get('pageclass_sfx');?>">
	...
	<form action="<?php echo JFilterOutput::ampReplace(JUri::getInstance()->toString()); ?>" method="post" name="adminForm" id="adminForm">
		<?php if (($this->params->get('show_item_filter_field') != '' AND $this->params->get('show_item_filter_field') != 'hide')) :?>
			<div class="filter-search">
				<?php if ($this->params->get('show_item_filter_field') != '' AND $this->params->get('show_item_filter_field') != 'hide') :?>
					<input type="text" name="filter_search" id="filter_search" value="<?php echo $this->escape($this->state->get('filter.search')); ?>" onchange="document.adminForm.submit();" title="<?php echo JText::_('COM_GAME_FILTER_SEARCH_DESC'); ?>" placeholder="<?php echo JText::_('COM_GAME_'.$this->params->get('show_item_filter_field').'_FILTER_LABEL'); ?>" />
				<?php endif; ?>	
				<select name="filter_category_id" onchange="this.form.submit()">
					<option value=""><?php echo JText::_('JOPTION_SELECT_CATEGORY');?></option>
					<?php echo JHtml::_('select.options', JHtml::_('category.options', 'com_game'), 'value', 'text', $this->state->get('filter.category_id'));?>
				</select>
			</div>
		<?php endif; ?>
		...
		<?php if (empty($this->items)) : ?>
			...
			<p><?php echo JText::_('COM_GAME_ITEMS_NO_ITEMS'); ?></p>
			<?php endif; ?>
			...
			<thead>
				<tr>
					...
					<?php if ($this->params->get('list_show_item_ordering',0)) : ?>
						<th width="10%">
							<?php echo JHtml::_('grid.sort',  'COM_GAME_HEADING_ORDERING', 'a.ordering', $list_dirn, $list_order); ?>
						</th>
					<?php endif; ?>	
					<?php if ($show_actions) : ?>
							...
							<?php 
								...
								if (!empty($item->created_by )) :
									...
									if ($this->params->get('show_item_headings')) :
										...
										echo JText::sprintf('COM_GAME_CREATED_BY', $created_by);
									endif;
								else:
									echo $empty;
								endif;								
