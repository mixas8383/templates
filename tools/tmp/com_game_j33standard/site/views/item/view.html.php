 ...
 *
 */
class GameViewItem extends JViewLegacy
{
	protected $item;
	protected $params;
	protected $print;
	...
	public function display($tpl = null)
	{
		...
		$item->readmore_link = JRoute::_(GameHelperRoute::getItemRoute($item->slug, 
										$item->catid, 
										$item->language,
										$this->getLayout(),									
		...
		if ($item->params->get('access-view') != true)
		{
			...
			if (!$item->params->get('show_item_noauth'))
			{
				...
				if( $user->get('guest'))
				{
					...
					$app->redirect('index.php?option=com_users&view=login&return=' . base64_encode($uri), JText::_('COM_GAME_ITEMS_ERROR_LOGIN_TO_VIEW_ITEM'));
		...
		//
		// Process the game plugins.
		//
		JPluginHelper::importPlugin('game');
		$results = $dispatcher->trigger('onItemPrepare', array ('com_game.item', &$item, &$this->params, $offset));

		$item->event = new stdClass;
		$results = $dispatcher->trigger('onItemAfterName', array('com_game.item', &$item, &$this->params, $offset));
		$item->event->afterDisplayItemName = JString::trim(implode("\n", $results));
		
		$results = $dispatcher->trigger('onItemBeforeDisplay', array('com_game.item', &$item, &$this->params, $offset));
		$item->event->beforeDisplayItem = JString::trim(implode("\n", $results));

		$results = $dispatcher->trigger('onItemAfterDisplay', array('com_game.item', &$item, &$this->params, $offset));
	...
	protected function prepareDocument()
	{
		...
		if ($menu AND ($menu->query['option'] != 'com_game' OR $menu->query['view'] != 'item' OR $id != $this->item->id))
		{
			...
			{			
				...
				$options['table'] = '#__game_items';					
				$category = JCategories::getInstance('Game', $options)->get($this->item->catid);
				while ($category AND ($menu->query['option'] != 'com_game' OR $menu->query['view'] == 'item' OR $id != $category->id) AND $category->id > 1)
				{
		...
		if (!empty($this->item->page_title))
		{
			...
			$this->document->setTitle($this->item->page_title . ' - ' . JText::sprintf('COM_GAME_PAGEBREAK_PAGE_NUM', $this->state->get('list.offset') + 1));
		}
