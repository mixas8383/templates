...
JModelLegacy::addIncludePath(JPATH_SITE.'/components/com_game/models', 'GameModelItems');
...
abstract class ModItemsHelper
{
	...
	public static function getList(&$params)
	{
		...
		$model = JModelLegacy::getInstance('Items', 'GameModel', array('ignore_request' => true));
		...
		// Access filter
		$access = !JComponentHelper::getParams('com_game')->get('show_item_noauth');
		...
		foreach ($items as &$item)
		{
			...
			if ($access OR in_array($item->access, $authorised))
			{
				if ($item_id_str == '')
				{
					...
					$item->link = JRoute::_(GameHelperRoute::getItemRoute($item->slug, $item->catslug, $item->language, $layout));
				}
				else
				{
					$item->link = JRoute::_('index.php?option=com_game&view=item'.$layout_str.$item_id_str.'&id='.$item->id);
				}					
			}
			else
			{
