...
require_once JPATH_SITE.'/components/com_game/helpers/route.php';

abstract class ModGameHelper
{
	/**
	 * Helper for mod_game
	 *
	...
	public static function getList(&$params)
	{
		...
		JModelLegacy::addIncludePath(JPATH_SITE.'/components/com_game/models', 'GameModel'.$component_object_names[1]);

		// Get an instance of the model
		$model = JModelLegacy::getInstance($component_object_names[1], 'GameModel', array('ignore_request' => true));
		...
		// Access filter
		$access = !JComponentHelper::getParams('com_game')->get('show_'.JString::strtolower($component_object_names[0]).'_noauth');
		...
		foreach ($items as &$item)
		{
			...
			if (!isset($item->access) OR $access OR in_array($item->access, $authorised))
			{
				...
				{			
					...
					{
						$item->link = JRoute::_(GameHelperRoute::$route_function($item->slug, $item->catslug, $item->language));
					}
					else
					{
						if (isset($item->catslug))
						{
							$item->link = JRoute::_(GameHelperRoute::$route_function($item->slug, $item->catslug));
						}
						else
						{
							if (isset($item->language))
							{
								$item->link = JRoute::_(GameHelperRoute::$route_function($item->slug, $item->language));
							}
							else
							{
								$item->link = JRoute::_(GameHelperRoute::$route_function($item->slug));
							}
						}
					}
				}
				...
				{
					$item->link = JRoute::_('index.php?option=com_game&view='.JString::strtolower($component_object_names[0]).$item_id_str.'&id='.$item->id);
				}
			}
			else
			{
