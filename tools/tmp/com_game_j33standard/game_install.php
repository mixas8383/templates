...
class com_gameInstallerScript
{
    ...
    function install($parent) 
    {
		...
		$install_html_file = __DIR__ . '/game_install.html';
		...
		ob_start();            
		?>
		<div id="gameinstall-info">
			<h1><?php echo JText::_('COM_GAME_INSTALL_HEADER'); ?></h1>
			<div id="gameinstall-intro">
				<?php echo JText::_('COM_GAME_INSTALL_INTRO'); ?>
			</div>
			<table id="gameinstall-table" class="adminlist">
				<thead class="gameinstall-heading">
					<tr>
						<th colspan="3">
							<?php echo JText::_('COM_GAME_INSTALL_HEADER');?>
						</th>
					</tr>
				</thead>
				<tbody>
					<tr class="gameinstall-subheading">
						<th colspan="2">
							<?php echo JText::_('COM_GAME_EXTENSION_HEADER');?>
						</th>
						<th width="50%">
							<?php echo JText::_('COM_GAME_STATUS_HEADER');?>
						</th>					
					</tr>			
					<tr class="gameinstall-row">
						<td  colspan="2">
							<?php echo JText::_('COM_GAME');?>
						</td>
						<td class="gameinstall-success">
							<?php echo JText::_('COM_GAME_INSTALL_PACKAGE_SUCCESS');?>
						</td>
					</tr>
					<tr>				
						<td colspan="3">
							<?php echo JText::_('COM_GAME_INSTALL_CORE_COMPONENT_SUCCESS');?>
						</td>
					</tr>
		<?php
		$buffer .= ob_get_clean();
		...
		if (count($manifest->plugins->plugin) > 0)
		{
			...
			<tr class="gameinstall-subheading">
				<th>
					<?php echo JText::_('COM_GAME_PLUGIN_HEADER');?>
				</th>
				<th>
					<?php echo JText::_('COM_GAME_PLUGIN_GROUP_HEADER');?>
				</th>				
				<th width="50%">
					<?php echo JText::_('COM_GAME_STATUS_HEADER');?>
				</th>					
 		...
 		if (count($manifest->modules->module) > 0)
		{
			...
			<tr class="gameinstall-subheading">
				<th>
					<?php echo JText::_('COM_GAME_MODULE_HEADER');?>
				</th>
				<th>
					<?php echo JText::_('COM_GAME_MODULE_GROUP_HEADER');?>
				</th>				
				<th width="50%">
					<?php echo JText::_('COM_GAME_STATUS_HEADER');?>
				</th>					
	...
	?>
				...
				<tfoot>
					<tr>
						<td colspan="3" text-align="center">
							<?php if ($install_error) : ?>
								<div id="gameinstall-component-error">
									<?php echo JText::_('COM_GAME_INSTALL_COMPONENT_ERROR'); ?>
								</div>			
							<?php else : ?>
								<div id="gameinstall-component-success">
									<?php echo JText::_('COM_GAME_INSTALL_COMPONENT_SUCCESS'); ?>
								</div>			
							<?php endif; ?>
						</td>
					</tr>
					<tr>
						<td colspan="3" text-align="center">
							<?php echo JText::_('COM_GAME_JOOMLA_LOGO_DISCLAIMER'); ?>	
						</td>				
					</tr>					
				</tfoot>
			</table>					
    ...
    function uninstall($parent) 
    {
        ...
        $manifest = $parent->get("manifest");
        $parent = $parent->getParent();
		$uninstall_html_file = __DIR__ . '/game_uninstall.html';
     ...
     ?>
	<div id="gameinstall-info">
		<h1><?php echo JText::_('COM_GAME_UNINSTALL_HEADER'); ?></h1>
			<table id="gameinstall-table" class="adminlist">
				<thead class="gameinstall-heading">
					<tr>
						<th colspan="3">
							<?php echo JText::_('COM_GAME_UNINSTALL_HEADER');?>
						</th>
					</tr>
				</thead>
				<tbody>
					...
					<tr class="gameinstall-row">
						...
						<td class="gameinstall-success">
							<?php echo JText::_('COM_GAME_UNINSTALL_PACKAGE_SUCCESS');?>
						</td>
					</tr>
	<?php
		$buffer .= ob_get_clean();  
		...
		JFolder::delete(JPATH_SITE.'/images/game'); 
		JFolder::delete(JPATH_SITE.'/plugins/game'); 

		$db->setQuery(
						'DELETE FROM '.$db->quoteName('#__content_types')
						.' WHERE '.$db->quoteName('type_alias').' = '.$db->quote('com_game.item')
					 );
		...
		$db->setQuery(
						'DELETE FROM '.$db->quoteName('#__content_types')
						.' WHERE '.$db->quoteName('type_alias').' = '.$db->quote('com_game.category')
					 );
				...
				<tfoot>
					<tr>
						<td colspan="3">
							<?php if ($install_error) : ?>
								<div id="gameinstall-component-error">
									<?php echo JText::_('COM_GAME_UNINSTALL_COMPONENT_ERROR'); ?>
								</div>			
							<?php else : ?>
								<div id="gameinstall-component-success">
									<?php echo JText::_('COM_GAME_UNINSTALL_COMPONENT_SUCCESS'); ?>
								</div>			
							<?php endif; ?>
						</td>
					</tr>
    ...
    function update($parent) 
    {
		...
		?>
		<div id="gameinstall-info">
			<h1><?php echo JText::_('COM_GAME_UPDATE_HEADER'); ?></h1>
			<table id="gameinstall-table" class="adminlist">
				<thead class="gameinstall-heading">
					<tr>
						<th colspan="3">
							<?php echo JText::_('COM_GAME_UPDATE_HEADER');?>
						</th>
					</tr>
				</thead>
				<tbody>
					...
					<tr class="gameinstall-row">
						...
						<td class="gameinstall-success">
							<?php echo JText::_('COM_GAME_UPDATE_PACKAGE_SUCCESS');?>
						</td>
					</tr>					
		<?php
		$buffer .= ob_get_clean();  
				...
				<tfoot>
					<tr>
						<td colspan="3">
							<?php if ($install_error) : ?>
								<div id="gameinstall-component-error">
									<?php echo JText::_('COM_GAME_UPDATE_COMPONENT_ERROR'); ?>
								</div>			
							<?php else : ?>
								<div id="gameinstall-component-success">
									<?php echo JText::_('COM_GAME_UPDATE_COMPONENT_SUCCESS'); ?>
								</div>			
							<?php endif; ?>
						</td>
					</tr>
    ...
    function preflight($type, $parent) 
    {
		...
		if( version_compare( $joomla_version->getShortVersion(), $this->minimum_joomla_release, 'lt' ) ) {
			Jerror::raiseWarning(null, JTEXT::sprintf('COM_GAME_INSTALL_COMPONENT_ERROR_WRONG_JOOMLA_VERSION',$this->minimum_joomla_release));
			return false;
		}
		switch ($type)
		{
			...
			case 'update' :
				...
				{
					Jerror::raiseWarning(null, JTEXT::sprintf('COM_GAME_UPDATE_COMPONENT_ERROR_WRONG_VERSION_SEQUENCE', $rel));
					return false;
				}			
			default :
				break; 
    ...
    function postflight($type, $parent) 
    {
		...
		{
			case 'install' :
				JFolder::create(JPATH_SITE.'/images/game'); 
				JFile::copy('administrator/components/com_game/index.html','images/game/index.html', JPATH_SITE);
				
				// Make sure index.html files in all folders
				if (JFolder::exists(JPATH_SITE.'/plugins/game') AND !JFile::exists(JPATH_SITE.'/plugins/game/index.html'))
				{
					JFile::copy('administrator/components/com_game/index.html','plugins/game/index.html', JPATH_SITE);
				}	
				break;    				
			case 'update' :
				/*
	...
	private function getManifest()
	{
		...
		$query->from($db->quoteName('#__extensions'));	
		$query->where($db->quoteName('name').' = '.$db->quote('com_game'));		
	...
	<tr class="gameinstall-row">
		...
		<td class="gameinstall-error">
			<div>
				<?php echo JText::_('COM_GAME_'.strtoupper($action).'_PACKAGE_ERROR');?><br />
				<span class="gameinstall-errormsg">
					<?php echo $msg; ?>
				</span>	
			</div>		
		</td>
    ...
    private function printSuccess($package, $group, $action)
    {
		...
		<tr class="gameinstall-row">
			...
			<td class="gameinstall-success">
				<div><?php echo JText::_('COM_GAME_'.strtoupper($action).'_PACKAGE_SUCCESS');?></div>
			</td>
		</tr> 		
		<?php
            $out = ob_get_clean();
    ...
    private function populateUCM()
    {
		...
		$content_type['type_alias'] = 'com_game.item';
		$content_type['table'] = '{"special":{"dbtable":"game_items","key":"id","type":"Items","prefix":"GameTable","config":"array()"},';
		...
		$content_type['router'] = 'GameHelperRoute::getItemRoute';

		$content_type['content_history_options'] = '{"formFile":"administrator\/components\/game\/models\/forms\/item.xml",';
		...
		$content_type = array();
		$content_type['type_title'] = 'Game Category';
		$content_type['type_alias'] = 'com_game.category';
		...
		$content_type['router'] = 'GameHelperRoute::getCategoryRoute';
