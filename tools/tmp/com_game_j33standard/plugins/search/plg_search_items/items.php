 ...
 * @author			 ()
 * @package			com_game
 * @subpackage		com_game.search.game.items
 * @copyright		
...
require_once JPATH_SITE.'/components/com_game/router.php';
	...
	public function onContentSearch($text, $phrase='', $ordering='', $areas=null)
	{
		...
		if ($sItem AND $limit > 0)
		{
			...
			if (isset($list))
			{
				...
				{
					$list[$key]->href = JRoute::_(GameHelperRoute::getItemRoute($item->slug, $item->catslug, $item->language, $this->layout, $keep_item_id));
		...
		if ($sItemArchived AND $limit > 0)
		{
			...
			$item	= $app->getMenu()->getItems('link', 'index.php?option=com_game&view=itemarchive', true);
			...
			if (isset($list3))
			{
				foreach($list3 as $key => $item)
				{
					...
					$list3[$key]->href	= JRoute::_('index.php?option=com_game&view=itemarchive'.
