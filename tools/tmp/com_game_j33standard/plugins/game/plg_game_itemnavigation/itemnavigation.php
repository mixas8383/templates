 ...
 * @author			 ()
 * @package			com_game
 * @subpackage		com_game.itemnavigation
 * @copyright		
...
/**
 * Game navigation plugin class.
 *
 */
class PlgGameItemnavigation extends JPlugin
{
	/**
	...
	public function onItemBeforeDisplay ($context, &$row, &$params, $page=0)
	{
			...
			($view == 'item')) 
		{
			...
			if ($row->prev) 
			{
				$row->prev = JRoute::_(GameHelperRoute::getItemRoute($row->prev->slug,$row->prev->catslug, $row->prev->language, $layout, $keep_item_id));
			} 
			else 
			{
				$row->prev = '';
			...
			if ($row->next) 
			{
				$row->next = JRoute::_(GameHelperRoute::getItemRoute($row->next->slug,$row->next->catslug, $row->next->language, $layout, $keep_item_id));
