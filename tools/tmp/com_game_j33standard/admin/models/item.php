 ...
 *
 */
class GameModelItem extends JModelAdmin
{
	/**
	 * @var		string	The prefix to use with controller messages.
	 */
	protected $text_prefix = 'COM_GAME_ITEMS';
	/**
	 * @var      string	The type alias for this object (for example, 'com_game.item')
	 */
	public $typeAlias = 'com_game.item';	
	/**
	 * @var		string	The context for the app call.
	 */
	protected $context = 'com_game.items';	
	/**
	...
	protected function canDelete($record)
	{
		...
		{
			return $user->authorise('core.delete', 'com_game');
		}							
	}
	...
	public function getItem($pk = null)
	{
		...
		if ($assoc)
		{
			...
			if ($item->id != null)
			{
				$associations = JLanguageAssociations::getAssociations('com_game', '#__game_items', 'com_game.item.item', $item->id, 'id', 'alias', 'catid');
	...
	protected function loadFormData()
	{
		...
		$data = $app->getUserState('com_game.edit.item.data', array());
		...
		if (empty($data))
		{
			...
			{
				$filters = (array) $app->getUserState('com_game.items.filter');
	...
	public function save($data)
	{
		...
		if (parent::save($data))
		{
			...
			if ($assoc)
			{
				...
				{
					JError::raiseNotice(403, JText::_('COM_GAME_ERROR_ALL_LANGUAGE_ASSOCIATED'));
				}
				...
				try
				{
					...
					$query->where('context='.$db->quote('com_game.item.item'));
				...
				if (!$all_language AND count($associations))
				{
					...
					{
						$query->values($id.','.$db->quote('com_game.item.item') . ',' . $db->quote($key));
					}
	...
	{
		// Include the game plugins for the delete events.
		JPluginHelper::importPlugin('game');	
	...
	public function featured($pks, $value = 0)
	{
		...
		if (empty($pks))
		{
			$this->setError(JText::_('COM_GAME_ITEMS_NO_ITEM_SELECTED'));
			return false;
		}
		...
		try
		{
			...
			$query = $db->getQuery(true);
			$query->update($db->quoteName('#__game_items'));
	...
	/**
	 * Custom clean the cache of com_game and game modules
	 *
	 */
	protected function cleanCache($group = null, $client_id = 0)
	{
		parent::cleanCache('com_game');
		parent::cleanCache('mod_game');
		parent::cleanCache('mod_game_items');
	...
	* @param   array   $data	The data where the original name is stored
	...
	protected function preprocessForm(JForm $form, $data, $group = 'item')
	{
		...
		if ($assoc)
		{
			...
			$fieldset->addAttribute('description', 'COM_GAME_ITEMS_ITEM_ASSOCIATIONS_FIELDSET_DESC');
			$add = false;
