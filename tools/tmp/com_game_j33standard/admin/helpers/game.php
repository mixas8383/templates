 ...
 *
 */
class GameHelper extends JHelperContent
{
	...
	{
		$config = JComponentHelper::getParams(JText::_('COM_GAME_FIELD_CATEGORY_COMPONENT_DEFAULT'));
		$category_component = $config->get('category_component', JText::_('COM_GAME_FIELD_CATEGORY_COMPONENT_DEFAULT'));	

		$active = $view_name == 'items'? true : false;
		JHtmlSidebar::addEntry(
			JText::_('COM_GAME_ITEMS_SUBMENU'),
			'index.php?option=com_game&view=items',
			$view_name == 'items',
			$active
		);
	
		if ($category_component != JText::_('COM_GAME_FIELD_CATEGORY_COMPONENT_NO_CATEGORIES'))
		{	
			$active = $view_name == 'categories'? true : false;
			
			JHtmlSidebar::addEntry(
				JText::_('COM_GAME_CATEGORIES_SUBMENU'),
