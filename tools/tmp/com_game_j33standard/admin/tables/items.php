 ...
 *
 */
class GameTableItems extends JTable
{
	...
	public function __construct(&$db)
	{
		...
		parent::__construct('#__game_items', 'id', $db);
		...
		JTableObserverTags::createObserver($this, array('typeAlias' => 'com_game.item'));

		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_game.item'));
	}
	...
	public function check()
	{
		...
		$table = JTable::getInstance('Items', 'GameTable');
		if ($table->load(array('alias'=>$this->alias)) AND ($table->id != $this->id OR $this->id==0))
		{
			$this->setError(JText::_('COM_GAME_FIELD_ALIAS_DUPLICATED'));
			return false;
		}		
	...
	{
		$k = $this->_tbl_key;
		return 'com_game.item.'.(int) $this->$k;
	}
	...
	protected function _getAssetParentId(JTable $table = null, $id = null)
	{
		...
		if (($this->catid) AND !empty($this->catid))
		{
			...
			if ($asset->loadByName('com_game.category.' . (int) $this->catid))
			{
				return $asset->id;
			}
		}
		
		if ($asset->loadByName('com_game'))
		{
			return $asset->id;
		}
		else
